"""
sim_s48_backtest.py — Backtest S48 (MACD crossover entry)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S47, ไม่ wire เข้า live
"""
...
import config
from strategy48 import S48_DEFAULTS, detect_s48
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S48_DEFAULTS[key]


def replay48(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    slow = int(_cfg_get(cfg, "MACD_SLOW"))
    signal = int(_cfg_get(cfg, "MACD_SIGNAL"))
    win_size = slow + signal + 60
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s48(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay48(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    slow = int(cfg["MACD_SLOW"]); signal = int(cfg["MACD_SIGNAL"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=slow + signal + 100)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s48_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "macd_fast", "macd_slow", "macd_signal",
              "sl_atr_mult", "tp_rr", "risk_pct", "trades", "wr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "macd_fast": cfg["MACD_FAST"], "macd_slow": cfg["MACD_SLOW"],
            "macd_signal": cfg["MACD_SIGNAL"], "sl_atr_mult": cfg["SL_ATR_MULT"],
