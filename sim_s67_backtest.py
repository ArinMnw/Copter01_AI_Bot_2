"""
sim_s67_backtest.py - Backtest S67 All-in-4S Clear Candle Reversal.
...
import config
from strategy67 import S67_DEFAULTS, _detect_closed, _in_session
...
def replay67(bars, spread, cfg):
    min_gap_bars = int(cfg.get("MIN_GAP_BARS", 4))
    min_start = int(cfg["TREND_LOOKBACK"]) + 80
...
def run_single(entry_bars, cfg, days, spread):
    return replay67(entry_bars, spread, cfg)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s67_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "mode", "cover", "min_body_atr",
              "min_range_atr", "wick_body_mult", "wick_range_min", "trend_lookback",
              "trend_min_atr", "sl_atr_mult", "tp_rr", "trades", "wr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "mode": cfg["MODE"], "cover": cfg["CLOSE_COVER"],
            "min_body_atr": cfg["MIN_BODY_ATR"], "min_range_atr": cfg["MIN_RANGE_ATR"],
            "wick_body_mult": cfg["WICK_BODY_MULT"], "wick_range_min": cfg["WICK_RANGE_MIN"],
            "trend_lookback": cfg["TREND_LOOKBACK"], "trend_min_atr": cfg["TREND_MIN_ATR"],
...
def main():
    ...
    args = ap.parse_args()
    cfg = dict(S67_DEFAULTS)
    if args.tf: cfg["ENTRY_TF"] = args.tf
    if args.mode: cfg["MODE"] = args.mode
    if args.cover: cfg["CLOSE_COVER"] = args.cover
    if args.minbody is not None: cfg["MIN_BODY_ATR"] = args.minbody
    if args.minrange is not None: cfg["MIN_RANGE_ATR"] = args.minrange
    if args.wickbody is not None: cfg["WICK_BODY_MULT"] = args.wickbody
    if args.wickrange is not None: cfg["WICK_RANGE_MIN"] = args.wickrange
    if args.trendlookback is not None: cfg["TREND_LOOKBACK"] = args.trendlookback
    if args.trendmin is not None: cfg["TREND_MIN_ATR"] = args.trendmin
