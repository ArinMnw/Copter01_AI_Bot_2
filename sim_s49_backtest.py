"""
sim_s49_backtest.py — Backtest S49 (Session VWAP Bounce)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S48, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S49_DEFAULTS[key]


def replay49(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    win_size = 320  # ครอบคลุม > 1 วันเต็มของ M5 (288 แท่ง/วัน) กัน VWAP reset ตกขอบ window
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s49(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx,
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay49(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=400)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s49_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "std_mult", "touch_atr_mult", "reject_atr_mult",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "std_mult": cfg["STD_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S49_DEFAULTS["RISK_PCT"])
    ...
    print(f"S49 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | stdmult={cfg['STD_MULT']} "
          f"touch={cfg['TOUCH_ATR_MULT']} reject={cfg['REJECT_ATR_MULT']} sl={cfg['SL_ATR_MULT']} "
