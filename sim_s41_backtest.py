"""
sim_s41_backtest.py — Backtest S41 (RSI Divergence)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S40, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S41_DEFAULTS[key]


def replay41(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    max_age = int(_cfg_get(cfg, "MAX_LEVEL_AGE_BARS"))
    win_size = max_age + 60
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s41(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay41(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=max_age + 100)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s41_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "pivot_wing", "max_level_age_bars",
              "min_price_diff_atr", "min_rsi_diff", "max_confirm_age_bars", "sl_atr_mult", "tp_rr",
              "confirmation_type", "risk_pct", "trades", "wr", "avg_per_day_span", "avg_per_month",
              "max_dd_pct", "profit_factor", "pct_pos_days", "max_losing_day_streak", "sharpe_like",
              "final_equity"]
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "pivot_wing": cfg["PIVOT_WING"],
            "max_level_age_bars": cfg["MAX_LEVEL_AGE_BARS"], "min_price_diff_atr": cfg["MIN_PRICE_DIFF_ATR"],
            "min_rsi_diff": cfg["MIN_RSI_DIFF"], "max_confirm_age_bars": cfg["MAX_CONFIRM_AGE_BARS"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S41_DEFAULTS["RISK_PCT"])
    ap.add_argument("--wing", type=int, default=None)
    ap.add_argument("--maxage", type=int, default=None)
    ap.add_argument("--pricediff", type=float, default=None)
    ap.add_argument("--rsidiff", type=float, default=None)
    ap.add_argument("--confirmage", type=int, default=None)
    ...
    cfg = dict(S41_DEFAULTS)
    cfg["RISK_PCT"] = args.risk
    if args.wing is not None:
        cfg["PIVOT_WING"] = args.wing
    if args.maxage is not None:
        cfg["MAX_LEVEL_AGE_BARS"] = args.maxage
    if args.pricediff is not None:
        cfg["MIN_PRICE_DIFF_ATR"] = args.pricediff
    if args.rsidiff is not None:
        cfg["MIN_RSI_DIFF"] = args.rsidiff
    if args.confirmage is not None:
        cfg["MAX_CONFIRM_AGE_BARS"] = args.confirmage
    ...
    print(f"S41 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | wing={cfg['PIVOT_WING']} "
          f"maxage={cfg['MAX_LEVEL_AGE_BARS']} pricediff={cfg['MIN_PRICE_DIFF_ATR']} rsidiff={cfg['MIN_RSI_DIFF']} "
          f"confirmage={cfg['MAX_CONFIRM_AGE_BARS']} sl={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']} conf={cfg['CONFIRMATION_TYPE']}")
