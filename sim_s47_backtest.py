"""
sim_s47_backtest.py — Backtest S47 (SuperTrend flip entry)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S46, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S47_DEFAULTS[key]


def replay47(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    period = int(_cfg_get(cfg, "ST_ATR_PERIOD"))
    win_size = period + 60
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s47(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay47(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    period = int(cfg["ST_ATR_PERIOD"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=period + 100)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s47_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "st_atr_period", "st_atr_mult",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "st_atr_period": cfg["ST_ATR_PERIOD"],
            "st_atr_mult": cfg["ST_ATR_MULT"], "sl_atr_mult": cfg["SL_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S47_DEFAULTS["RISK_PCT"])
    ...
    if args.period is not None:
        cfg["ST_ATR_PERIOD"] = args.period
    if args.mult is not None:
        cfg["ST_ATR_MULT"] = args.mult
    if args.slmult is not None:
        cfg["SL_ATR_MULT"] = args.slmult
    if args.rr is not None:
        cfg["TP_RR"] = args.rr

    print(f"S47 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | period={cfg['ST_ATR_PERIOD']} "
          f"mult={cfg['ST_ATR_MULT']} sl={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']}")
