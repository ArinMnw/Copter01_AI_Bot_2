# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S191."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy191 import detect_s191
from sweep_s161_payoff import _summary


CASES = ((7.0, 1.00), (12.0, 1.00), (14.0, 1.00), (15.0, 1.00),
         (16.0, 1.00), (17.0, 1.00), (18.0, 1.00), (19.0, 1.00),
         (20.0, 1.00))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s191(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 191)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(191, args.months, "M5", 0.20, 0.01, end, lookback,
