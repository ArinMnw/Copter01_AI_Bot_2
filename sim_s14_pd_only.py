"""
sim_s14_pd_only.py — จำลอง S14 พร้อม PD Zone เท่านั้น (ไม่มี Trend Recheck, ไม่มี RSI Recheck)
...
# ─── STEP 1: Build log timeline (swing H/L เท่านั้น — ไม่ต้องการ trend/HHLL) ──
print("Loading log timeline (swing H/L for PD Zone)...")
_log_lines = open(LOG_PATH, encoding='utf-8-sig').readlines()

re_ts_line  = re.compile(r'^\[(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\]')
re_tf_hdr   = re.compile(r'┌─\s*\S+\s+(\w+)')
...
_tl_data = []   # corresponding {tf: {swing_h, swing_l, eq}}
...
for _ln in _log_lines:
    ...
    for sec in sections:
        ...
        if eq > 0:
            tf_data[tf] = {'swing_h': swing_h, 'swing_l': swing_l, 'eq': eq}

    if tf_data:
        _tl_keys.append(dt_bkk)
        _tl_data.append(tf_data)

print(f"Timeline: {len(_tl_keys)} snapshots  "
      f"[{_tl_keys[0] if _tl_keys else '-'}  →  {_tl_keys[-1] if _tl_keys else '-'}]")


def lookup_eq(signal_dt: datetime, tf: str) -> float:
    """หา EQ ล่าสุด ≤ signal_dt สำหรับ TF นี้ — คืน 0 ถ้าไม่มีข้อมูล"""
    ...
    for j in range(idx, max(idx - 20, -1), -1):
        if j < 0:
            break
        if tf in _tl_data[j]:
            return _tl_data[j][tf]['eq']
    return 0.0


# ─── STEP 2: PD Zone filter only ─────────────────────────────────────────────
def passes_pd_zone(signal_dt: datetime, tf: str, signal: str, entry: float) -> tuple[bool, str]:
    """
    เฉพาะ PD Zone:
    SELL → entry ต้องอยู่เหนือ EQ (Premium zone)
    BUY  → entry ต้องอยู่ใต้  EQ (Discount zone)
    """
    eq = lookup_eq(signal_dt, tf)
    if eq <= 0:
        return True, ''   # ไม่มีข้อมูล → ปล่อยผ่าน
...
def backtest_tf(tf_name: str, tf_val: int) -> tuple[list, list]:
    ...
    for i in range(start_idx, len(bars)):
        ...
        for ord_ in orders:
            ...
            passed, reason = passes_pd_zone(bt, tf_name, s, e)
            if not passed:
                blocked_log.append({
...
def main():
    ...
    print(f'Filters: Trend Recheck ✗ | PD Zone ✓ | RSI Recheck ✗')
    print('=' * 70)
    ...
    for tf_name, tf_val in TF_MAP.items():
        ...
        for t in trades:
            ...
            print(f'   {dt} {t["signal"]:<4} E={t["entry"]:.2f} SL={t["sl"]:.2f}'
                  f' TP={t["tp"]:.2f}'
                  f' → {t["close_type"]:<4} @ {t.get("close_price",0):.2f}'
                  f' [{ct}]  {pnl_s} USD  [{t["pattern"]}]')

        if blocked_log:
            print(f'   --- Blocked by PD Zone ({len(blocked_log)}) ---')
            for b in blocked_log:
                dt = b['time'].strftime('%d-%m %H:%M')
                print(f'   {dt} {b["signal"]:<4} E={b["entry"]:.2f}  ✗ {b["reason"]}'
                      f'  [{b["pattern"]}]')
    ...
    for row in summary_rows:
        tf_name, n, tp, sl, op, wr, pnl, blk = row
