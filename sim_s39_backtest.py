"""
sim_s39_backtest.py — Backtest S39 (Demand/Supply zone)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S38, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S39_DEFAULTS[key]


def replay39(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    max_age = int(_cfg_get(cfg, "MAX_ZONE_AGE_BARS"))
    base_n = int(_cfg_get(cfg, "BASE_BARS"))
    win_size = max_age + base_n + 30
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s39(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay39(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    max_age = int(cfg["MAX_ZONE_AGE_BARS"])
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s39_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "base_bars", "base_atr_mult", "impulse_atr_mult",
              "max_zone_age_bars", "sl_atr_mult", "tp_rr", "risk_pct", "trades", "wr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "base_bars": cfg["BASE_BARS"],
            "base_atr_mult": cfg["BASE_ATR_MULT"], "impulse_atr_mult": cfg["IMPULSE_ATR_MULT"],
            "max_zone_age_bars": cfg["MAX_ZONE_AGE_BARS"], "sl_atr_mult": cfg["SL_ATR_MULT"],
            "tp_rr": cfg["TP_RR"], "risk_pct": cfg["RISK_PCT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S39_DEFAULTS["RISK_PCT"])
    ap.add_argument("--basebars", type=int, default=None)
    ap.add_argument("--baseatr", type=float, default=None)
    ap.add_argument("--impulseatr", type=float, default=None)
    ...
    if args.basebars is not None:
        cfg["BASE_BARS"] = args.basebars
    if args.baseatr is not None:
        cfg["BASE_ATR_MULT"] = args.baseatr
    if args.impulseatr is not None:
        cfg["IMPULSE_ATR_MULT"] = args.impulseatr
    if args.maxage is not None:
        cfg["MAX_ZONE_AGE_BARS"] = args.maxage
    if args.slmult is not None:
        cfg["SL_ATR_MULT"] = args.slmult
    if args.rr is not None:
        cfg["TP_RR"] = args.rr

    print(f"S39 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | basebars={cfg['BASE_BARS']} "
          f"baseatr={cfg['BASE_ATR_MULT']} impulseatr={cfg['IMPULSE_ATR_MULT']} maxage={cfg['MAX_ZONE_AGE_BARS']} "
          f"sl={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']}")
