# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S184."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy184 import detect_s184
from sweep_s161_payoff import _summary


CASES = tuple((rr, be)
              for rr in (8.5, 8.6, 8.7, 8.8, 8.9)
              for be in (0.45, 0.50, 0.52, 0.54, 0.56)) + ((7.0, 1.0),)
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s184(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 184)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(184, args.months, "M5", 0.20, 0.01, end, lookback,
