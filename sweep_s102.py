...
def run(cfg):
    ...
    for i in range(LOOKBACK, len(all_bars) - 2):
        ...
        sig = strategy102.detect_s102(rates_slice, tf=TF, dt_bkk=dt_bkk, cfg=cfg)
...
variants = [
    ("v10", dict(V)),
    ("v10+retr15", dict(V, ENTRY_RETRACE_ATR=0.15)),
    ("v10+range24", dict(V, RANGE_BARS=24)),
    ("v10+retr15+range24", dict(V, ENTRY_RETRACE_ATR=0.15, RANGE_BARS=24)),
    ("v10+rmax3.5", dict(V, RANGE_MAX_ATR=3.5)),
    ("v10+retr15+rmax3.5", dict(V, ENTRY_RETRACE_ATR=0.15, RANGE_MAX_ATR=3.5)),
    ("v10+retr15+body0.9", dict(V, ENTRY_RETRACE_ATR=0.15, BREAK_BODY_ATR=0.9)),
    ("v10+retr15+h13-16", dict(V, ENTRY_RETRACE_ATR=0.15,
                               ALLOW_HOURS=(13, 14, 15, 16, 20, 21, 22))),
    ("v10+retr15+cd0", dict(V, ENTRY_RETRACE_ATR=0.15, _cooldown=0)),
]
