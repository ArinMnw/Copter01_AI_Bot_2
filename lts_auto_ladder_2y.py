...
def main():
    ...
    log_file = "lts_auto_ladder_log.md"
    if not os.path.exists(log_file):
        with open(log_file, "w", encoding="utf-8") as f:
            f.write("# LTS Auto Ladder Log\n\n")
    ...
    while curr_idx <= args.max_idx:
        ...
        # Run sweep
        sweep_cmd = [sys.executable, "ambfix_sweep_2y.py", "--base", base_csv, "--family", fam, "--cfg-idx", str(cfg_idx), "--w-step", "10.0"]
        ...
        with open("ambfix_sweep_results.csv", "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                # Require >= 5 trades at 550d
                if counts.get(550, 0) >= 5:
        ...
        out_prefix = f"lts{curr_idx}_ambfix_{fam}c{cfg_idx}_{mode[:3]}_{band}_h{hour}"
        
        print(f"[{curr_idx}] Building {out_prefix}...")
        build_cmd = [
            sys.executable, "ambfix_build_2y.py", 
            "--base", base_csv, 
            "--family", fam, 
            "--cfg-idx", str(cfg_idx), 
            "--mode", mode, 
