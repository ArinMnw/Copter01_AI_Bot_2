# -*- coding: utf-8 -*-
"""S199 - Excess-kurtosis fat-tail structural-sweep reclaim, 16.9R."""
...
def detect_s199(rates, tf, dt_bkk, cfg):
    """Fade a structural sweep only inside a fat-tailed return regime."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
    ...
    return {
        ...
        "tp": tp,
        "order_type": "limit",
        "pattern": f"S199 {signal} Fat-Tail Sweep {rr:g}R",
        "reason": (f"Excess kurtosis={kurtosis:.2f} fat-tail regime; "
