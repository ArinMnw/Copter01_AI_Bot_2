# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S247."""
...
from strategy247 import detect_s247
from sweep_s235_payoff import _summary_market


RR_VALUES = tuple(round(38.0 + 0.1 * index, 1) for index in range(71))
BE_VALUES = tuple(round(0.05 + 0.01 * index, 2) for index in range(46))
CASES = (
    tuple((rr, 0.25) for rr in RR_VALUES)
    + tuple((42.0, be) for be in BE_VALUES)
    + ((10.0, 1.0),)
)
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s247(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 247)
        cached[index] = signal
    rows = [
