...
def detect_s132(rates, tf, dt_bkk, cfg):
    """Return S128 carry when Asia leaves capacity versus recent daily range."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    ...
    capacity_ratio = asia_range / reference
    if capacity_ratio > float(c["ASIA_TO_MEDIAN_RANGE_MAX"]):
        return _wait(f"Asia consumed too much median daily range ({capacity_ratio:.0%})")

    result = dict(result)
    result["pattern"] = f"S132 {result['signal']} Asia Carry Median Capacity"
    result["reason"] = (f"{result.get('reason', '')}; Asia/median daily range="
                        f"{capacity_ratio:.0%} across {len(ranges)} days")
    return result
