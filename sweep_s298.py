# -*- coding: utf-8 -*-
"""Exact cached payoff/BE sweep for relaxed S298 SELL market signals."""
...
from strategy298 import detect_s298
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s298(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 298)
        cached[index] = signal
    rr_values = RR_VALUES
    be_values = BE_VALUES
    if args.rr_values:
