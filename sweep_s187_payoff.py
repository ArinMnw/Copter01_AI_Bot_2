# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S187."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy187 import detect_s187
from sweep_s161_payoff import _summary


CASES = tuple((rr, be)
              for rr in (8.0, 8.1, 8.2, 8.3, 8.4, 8.5)
              for be in (0.70, 0.74, 0.75, 0.76, 0.78)) + ((7.0, 1.0),)
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s187(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 187)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(187, args.months, "M5", 0.20, 0.01, end, lookback,
