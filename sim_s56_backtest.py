"""
sim_s56_backtest.py — Backtest S56 (Previous-Week High/Low Bounce)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S55, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S56_DEFAULTS[key]


def _build_week_levels(days):
...
def _prev_week_hl(ts, week_starts, week_hl):
    ...
    prev_start = week_starts[idx - 1]
    return week_hl[prev_start]


def replay56(bars, htf_series, spread, cfg, week_starts, week_hl):
    ...
    for j in range(start_j, n - 1):
        ...
        pwhl = _prev_week_hl(ts, week_starts, week_hl)

        res = detect_s56(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx,
                          prev_week_hl=pwhl)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    week_starts, week_hl = _build_week_levels(days)
    if week_starts is None:
        return []
    return replay56(entry_bars, htf_series, spread, cfg, week_starts, week_hl)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s56_backtest_summary.csv")
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S56_DEFAULTS["RISK_PCT"])
    ...
    print(f"S56 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | touch={cfg['TOUCH_ATR_MULT']} "
