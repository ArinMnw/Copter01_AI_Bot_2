...
Recent closed returns are summarized with sample probability-weighted moments.
L-skewness tau3 = L3/L2 measures whole-distribution tail asymmetry using linear
order-statistic weights, making it more robust than conventional third moments
and richer than a three-quantile Bowley statistic.  Its magnitude must expand
from disjoint baseline blocks while path, displacement, participation, and a
closed release candle agree.  Execution is next-open market with an
event-extreme plus ATR stop and at least 7R reward.
"""
...
def _l_moment_profile(values):
    ordered = sorted(values)
    count = len(ordered)
    if count < 6:
        return None, 0.0
    b0 = statistics.fmean(ordered)
    denominator1 = count - 1
    denominator2 = (count - 1) * (count - 2)
    b1 = sum(index * value for index, value in enumerate(ordered))
    b1 /= count * denominator1
    b2 = sum(
        index * (index - 1) * value
        for index, value in enumerate(ordered)
        if index >= 2
    )
    b2 /= count * denominator2
    l2 = 2.0 * b1 - b0
    l3 = 6.0 * b2 - 6.0 * b1 + b0
...
def detect_s398(rates, tf="", dt_bkk=None, cfg=None, **kwargs):
    """Return a complete S398 market payload from fully closed bars."""
    del tf, kwargs
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
        ...
        skew_min = float(c["L_SKEW_MIN"])
        rise_min = float(c["L_SKEW_RISE_MIN"])
    ...
    if baseline_count < recent_count or baseline_count // recent_count < 2:
        return _wait("Invalid config: L-moment windows are inconsistent")
    gates = (skew_min, rise_min, scale_min, path_min, net_move_min)
    if not all(math.isfinite(value) and value >= 0.0 for value in gates):
        return _wait("Invalid config: L-moment gates are invalid")
    ...
    try:
        ...
        profiles = [
            _l_moment_profile(_returns(baseline[index:index + recent_count]))
            for index in range(0, len(baseline), recent_count)
        ]
        recent_returns = _returns(recent)
        recent_skew, recent_scale = _l_moment_profile(recent_returns)
        baseline_abs_skew = statistics.median(abs(item[0]) for item in profiles)
    ...
    if recent_skew is None or atr <= 0.0:
        return _wait("L-skewness or ATR is unavailable")
    side = 1 if recent_skew >= 0.0 else -1
    skew_strength = abs(recent_skew)
    skew_rise = skew_strength - baseline_abs_skew
    if skew_strength < skew_min:
        return _wait(f"Recent L-skewness is weak ({skew_strength:.3f})")
    if skew_rise < rise_min:
        return _wait(f"L-skewness has not expanded ({skew_rise:.3f})")
    ...
    if side * net_move <= 0.0 or path_efficiency < path_min:
        return _wait(f"Auction path does not confirm L-skew ({path_efficiency:.3f})")
    ...
    if body == 0.0 or candle_range <= 0.0 or side * body <= 0.0:
        return _wait("Event does not confirm L-skew direction")
    ...
    return {
        ...
        "tp": tp,
        "order_type": "market",
        "pattern": f"S398 {signal} L-Moment Tail-Asymmetry {rr:g}R",
        "reason": (
            f"l_skew={recent_skew:.4f}, baseline_abs={baseline_abs_skew:.4f}, "
            f"rise={skew_rise:.4f}, l_scale={recent_scale:.4f}, "
