...
mt5.initialize()
days = 60
cfg0 = dict(S45_DEFAULTS)
entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg0["ENTRY_TF"], days, extra_bars=cfg0["MAX_OB_AGE_BARS"] + 100)
...
combos = list(itertools.product(
    [0.6, 1.0, 1.5],         # IMPULSE_ATR_MULT
    [25, 40, 60],            # MAX_OB_AGE_BARS
    [0.05, 0.1, 0.2],        # MAX_VIOLATION_WICK_ATR
    [0.8, 1.0],              # SL_ATR_MULT
    [1.0, 1.5],              # TP_RR
))
...
t0 = time.time()
results = []
for i, (ia, ma, vw, sl, rr) in enumerate(combos):
    cfg = dict(S45_DEFAULTS)
    cfg.update(IMPULSE_ATR_MULT=ia, MAX_OB_AGE_BARS=ma, MAX_VIOLATION_WICK_ATR=vw, SL_ATR_MULT=sl, TP_RR=rr)
    ...
    results.append((ia, ma, vw, sl, rr, s, c))
print(f"done in {time.time()-t0:.1f}s, {len(results)} valid")
mt5.shutdown()

results.sort(key=lambda r: r[6]["sharpe_like"], reverse=True)
print(f"\n{'ia':>4} {'ma':>4} {'vw':>5} {'sl':>4} {'rr':>4} {'n':>4} {'WR%':>5} "
      f"{'$/mo':>8} {'DD%':>6} {'PF':>5} {'posDay%':>8} {'streak':>7} {'sharpe':>7}")
for ia, ma, vw, sl, rr, s, c in results[:15]:
    print(f"{ia:>4} {ma:>4} {vw:>5} {sl:>4} {rr:>4} {s['trades']:>4} {s['wr']:>5.1f} "
