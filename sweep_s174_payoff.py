# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S174."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy174 import detect_s174
from sweep_s161_payoff import _summary


CASES = ((7.0, 1.0), (9.0, 0.35), (9.0, 0.36), (9.0, 0.37),
         (9.0, 0.38), (9.0, 0.39), (9.0, 0.40), (9.1, 0.40))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s174(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 174)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(174, args.months, "M5", 0.20, 0.01, end, lookback,
