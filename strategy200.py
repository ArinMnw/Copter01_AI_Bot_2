# -*- coding: utf-8 -*-
"""S200 - Absolute-return autocorrelation clustering structural-sweep reclaim, 16.9R."""
...
def _abs_return_acf1(returns):
    """Lag-1 autocorrelation of absolute returns (volatility clustering)."""
    values = [abs(value) for value in returns]
    count = len(values)
    if count < 8:
        return 0.0
    ...
    if variance <= 0.0:
        return 0.0
    covariance = sum((values[index] - mean) * (values[index - 1] - mean)
                     for index in range(1, count)) / (count - 1)
    return covariance / variance


def detect_s200(rates, tf, dt_bkk, cfg):
    """Fade a structural sweep gated by volatility-clustering strength."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
        period = max(1, int(c["ATR_PERIOD"]))
        window = max(40, int(c["ACF_WINDOW"]))
    ...
    clustering = _abs_return_acf1(returns)
    if not float(c["ACF_MIN"]) <= clustering <= float(c["ACF_MAX"]):
        return _wait(f"Volatility clustering outside band (acf={clustering:.2f})")
    ...
    if (exhaustion_range <= 0.0
        ...
        return _wait("Clustering shock lacks directional body")
    ...
    if exhaustion["tick_volume"] < volume_floor:
        return _wait("Clustering shock lacks volume")
    ...
    if not confirmed or reclaim["tick_volume"] < reclaim_volume_floor:
        return _wait("Clustering shock lacks confirmed reclaim")
    ...
    if risk <= 0.0 or risk > atr * float(c["MAX_RISK_ATR"]):
        return _wait(f"Clustering-sweep risk outside range ({risk / atr:.2f} ATR)")
    risk_pct = risk / entry * 100.0
    if risk_pct > float(c["MAX_RISK_PRICE_PCT"]):
        return _wait(f"Clustering-sweep risk too large versus price ({risk_pct:.2f}%)")
    ...
    return {
        ...
        "tp": tp,
        "order_type": "limit",
        "pattern": f"S200 {signal} Vol-Clustering Sweep {rr:g}R",
        "reason": (f"Volatility clustering acf={clustering:.2f} in band; "
