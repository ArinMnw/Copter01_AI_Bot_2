...
import config
from strategy35 import S35_DEFAULTS
import sim_s30_backtest as s30sim
import sim_s31_backtest as s31sim
import sim_s35_backtest as sim
...
combos = list(itertools.product(
    [14, 20],                 # SMA_PERIOD
    [1.5, 2.0, 2.5],          # DEV_STDEV_MULT
    [70],                     # RSI_OVERBOUGHT (oversold = 100-ob)
    [0.0, 25.0],              # ADX_MAX_THRESHOLD
    [1.0],                    # SL_ATR_MULT
    [0.8, 1.0, 1.5],          # TP_RR
))
...
t0 = time.time()
results = []
for i, (sma, dev, ob, adxmax, sl, rr) in enumerate(combos):
    cfg = dict(S35_DEFAULTS)
    cfg.update(SMA_PERIOD=sma, DEV_STDEV_MULT=dev, RSI_OVERBOUGHT=ob, RSI_OVERSOLD=100 - ob,
               ADX_MAX_THRESHOLD=adxmax, SL_ATR_MULT=sl, TP_RR=rr)
    raw = sim.run_single(entry_bars, cfg, days, 0.20)
