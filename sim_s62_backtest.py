"""
sim_s62_backtest.py - Backtest S62 All-in-4S First-Wave Close-Cover Reversal.
...
import config
from strategy62 import S62_DEFAULTS, _detect_closed, _in_session
...
def replay62(bars, spread, cfg):
    min_gap_bars = int(cfg.get("MIN_GAP_BARS", 3))
    min_start = max(int(cfg["TREND_LOOKBACK"]), int(cfg["LEVEL_LOOKBACK"])) + 80
    all_dt = cfg.get("_DT_BKK") or [config.mt5_ts_to_bkk(int(b["time"])) for b in bars]
    closes = cfg.get("_CLOSES") or [float(b["close"]) for b in bars]
    ...
    for j in range(min_start, n - 1):
        ...
        if not _in_session(dt_bkk, cfg):
            continue
        sig = _detect_closed(bars, j, cfg, closes=closes, atr_value=atr14[j])
        if sig is None:
            continue
        last_fire_idx = j
...
def run_single(entry_bars, cfg, days, spread):
    return replay62(entry_bars, spread, cfg)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s62_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = [
        "timestamp", "label", "entry_tf", "trend_lookback", "pivot_lookback",
        "level_lookback", "level_mode", "cover_mode", "min_body_atr",
        "max_body_atr", "min_body_ratio", "first_wave_bars", "sl_atr_mult",
        "tp_rr", "trades", "wr", "avg_per_day_span", "max_dd_pct",
        "profit_factor", "fixed_per_day", "fixed_per_month", "fixed_pf",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "max_body_atr": cfg["MAX_BODY_ATR"],
            "min_body_ratio": cfg["MIN_BODY_RATIO"],
            "first_wave_bars": cfg["FIRST_WAVE_BARS"],
...
def print_examples(raw, limit=8):
    ...
    for t in raw[:limit]:
        ...
        print(
            ...
            f"sl={t['sl']:.2f} tp={t['tp']:.2f} diff={t['diff_usd_per_001lot']:.2f} "
            f"{t['reason']}"
        )
...
def main():
    ...
    ap.add_argument("--tf")
    ap.add_argument("--levelmode")
    ap.add_argument("--cover")
    ap.add_argument("--trend", type=int)
    ap.add_argument("--pivot", type=int)
    ap.add_argument("--levellookback", type=int)
    ap.add_argument("--minbody", type=float)
    ap.add_argument("--maxbody", type=float)
    ap.add_argument("--bodyratio", type=float)
    ap.add_argument("--firstwave", type=int)
    ...
    if args.levelmode:
        cfg["LEVEL_MODE"] = args.levelmode
    if args.cover:
        cfg["COVER_MODE"] = args.cover
    if args.trend is not None:
        cfg["TREND_LOOKBACK"] = args.trend
    if args.pivot is not None:
        cfg["PIVOT_LOOKBACK"] = args.pivot
    if args.levellookback is not None:
        cfg["LEVEL_LOOKBACK"] = args.levellookback
    if args.minbody is not None:
        cfg["MIN_BODY_ATR"] = args.minbody
    if args.maxbody is not None:
        cfg["MAX_BODY_ATR"] = args.maxbody
    if args.bodyratio is not None:
        cfg["MIN_BODY_RATIO"] = args.bodyratio
    if args.firstwave is not None:
        cfg["FIRST_WAVE_BARS"] = args.firstwave
