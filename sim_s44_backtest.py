"""
sim_s44_backtest.py — Backtest S44 (Volume Profile POC/VAH/VAL bounce)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S43, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S44_DEFAULTS[key]


def replay44(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    lb = int(_cfg_get(cfg, "LOOKBACK_BARS"))
    win_size = lb + 30
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s44(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay44(entry_bars, htf_series, spread, cfg)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s44_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "lookback_bars", "bucket_atr_mult", "value_area_pct",
              "touch_atr_mult", "reject_atr_mult", "sl_atr_mult", "tp_rr", "confirmation_type",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "lookback_bars": cfg["LOOKBACK_BARS"],
            "bucket_atr_mult": cfg["BUCKET_ATR_MULT"], "value_area_pct": cfg["VALUE_AREA_PCT"],
            "touch_atr_mult": cfg["TOUCH_ATR_MULT"], "reject_atr_mult": cfg["REJECT_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S44_DEFAULTS["RISK_PCT"])
    ap.add_argument("--lookback", type=int, default=None)
    ap.add_argument("--bucket", type=float, default=None)
    ap.add_argument("--vapct", type=float, default=None)
    ...
    if args.bucket is not None:
        cfg["BUCKET_ATR_MULT"] = args.bucket
    if args.vapct is not None:
        cfg["VALUE_AREA_PCT"] = args.vapct
    ...
    print(f"S44 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | lookback={cfg['LOOKBACK_BARS']} "
          f"bucket={cfg['BUCKET_ATR_MULT']} vapct={cfg['VALUE_AREA_PCT']} touch={cfg['TOUCH_ATR_MULT']} "
          f"reject={cfg['REJECT_ATR_MULT']} sl={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']}")
