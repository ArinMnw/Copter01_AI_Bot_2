"""
sim_s42_backtest.py — Backtest S42 (CRT — Candle Range Theory sweep+reversal)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S41, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S42_DEFAULTS[key]


def replay42(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    range_bars = int(_cfg_get(cfg, "RANGE_BARS"))
    win_size = range_bars + 40
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s42(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay42(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    range_bars = int(cfg["RANGE_BARS"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=range_bars + 80)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s42_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "range_bars", "sweep_atr_mult", "min_range_atr",
              "max_setup_age_bars", "sl_atr_mult", "tp_rr", "confirmation_type", "risk_pct", "trades",
              "wr", "avg_per_day_span", "avg_per_month", "max_dd_pct", "profit_factor", "pct_pos_days",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "range_bars": cfg["RANGE_BARS"],
            "sweep_atr_mult": cfg["SWEEP_ATR_MULT"], "min_range_atr": cfg["MIN_RANGE_ATR"],
            "max_setup_age_bars": cfg["MAX_SETUP_AGE_BARS"], "sl_atr_mult": cfg["SL_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S42_DEFAULTS["RISK_PCT"])
    ap.add_argument("--rangebars", type=int, default=None)
    ...
    if args.rangebars is not None:
        cfg["RANGE_BARS"] = args.rangebars
    if args.sweep is not None:
        cfg["SWEEP_ATR_MULT"] = args.sweep
    ...
    print(f"S42 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | rangebars={cfg['RANGE_BARS']} "
          f"sweep={cfg['SWEEP_ATR_MULT']} minrange={cfg['MIN_RANGE_ATR']} sl={cfg['SL_ATR_MULT']} "
