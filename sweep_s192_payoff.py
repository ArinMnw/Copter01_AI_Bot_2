# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S192."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy192 import detect_s192
from sweep_s161_payoff import _summary


CASES = tuple((rr, be)
              for rr in (16.8, 16.9, 17.0, 17.1, 17.2)
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s192(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 192)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(192, args.months, "M5", 0.20, 0.01, end, lookback,
