...
def s5_runtime_feature_coverage() -> list[dict]:
    return [
        {
            "name": "S5 scalping detect",
            "config_on": bool(config.active_strategies.get(5, False)),
            "runtime": "apply",
            "replay": "apply",
            "note": "Replay calls shared strategy5.strategy_5()",
        },
        {
            "name": "S5 internal filters",
            "config_on": True,
            "runtime": "apply",
            "replay": "partial",
            "note": "Strategy filters are reused, but time filter uses current wall-clock hour like runtime code",
        },
        {
        ...
        },
        {
            ...
            "runtime": "apply",
            "replay": "gap",
            "note": "Runtime applies PD Fibo Plus to S5 pending/fill",
        },
        {
            ...
            "runtime": "apply",
            "replay": "gap",
            "note": "Runtime applies trend recheck to S5 pending/fill",
        },
        {
            ...
            "runtime": "apply",
            "replay": "gap",
            "note": "Runtime can close S5 after fill when RSI recheck fails",
        },
        {
            ...
            "runtime": "apply",
            "replay": "gap",
            "note": "Shared lifecycle features are not included in S5 baseline yet",
        },
    ]


def s5_unreplayed_active_features() -> list[dict]:
    return [
        item for item in s5_runtime_feature_coverage()
...
def _pending_from_result(result: dict, tf_name: str, detect_bar: dict) -> dict:
    ...
    return {
        ...
        "signal": signal,
        "side": signal,
        "pattern": result.get("pattern", "S5 Scalping"),
...
def backtest_tf(tf_name: str, tf_val: int) -> list[dict]:
    ...
    for i in range(start_idx, len(bars)):
        ...
        result = strategy_5(scan_rates, tf=tf_name)
