...
def s5_runtime_feature_coverage() -> list[dict]:
    return [
        ...
        },
        {
            ...
            "runtime": "apply",
            "replay": "apply",
            "note": "Replay injects historical signal time and HHLL cache for time/zone filters",
        },
        {
...
def _strategy_5_historical(
    ...
    try:
        return strategy_5(scan_rates, tf=tf_name if hist_data else "", signal_time=signal_time)
    finally:
        if old_data is None:
...
def _strategy_5_with_hhll(scan_rates: list[dict], hist_data: dict, tf_name: str, signal_time: datetime) -> dict:
    old_data = hhll_swing._hhll_data.get(tf_name)
    if hist_data:
        hhll_swing._hhll_data[tf_name] = hist_data
    try:
...
def backtest_tf(tf_name: str, tf_val: int, range_end_utc: datetime | None = None) -> list[dict]:
    ...
    for i in range(start_idx, len(bars)):
        ...
        result = _strategy_5_with_hhll(scan_rates, hist_data, tf_name, bt)
