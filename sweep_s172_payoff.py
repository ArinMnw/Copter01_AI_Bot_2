# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S172."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy172 import detect_s172
from sweep_s161_payoff import _summary


CASES = ((7.0, 1.0), (10.5, 0.75), (10.6, 0.75),
         (10.7, 0.75), (10.8, 0.75), (10.9, 0.75), (11.0, 0.75))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s172(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 172)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(172, args.months, "M5", 0.20, 0.01, end, lookback,
