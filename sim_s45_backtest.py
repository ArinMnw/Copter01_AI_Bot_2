"""
sim_s45_backtest.py — Backtest S45 (Order Block, SMC)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S44, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S45_DEFAULTS[key]


def replay45(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    max_age = int(_cfg_get(cfg, "MAX_OB_AGE_BARS"))
    win_size = max_age + 40
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s45(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay45(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    max_age = int(cfg["MAX_OB_AGE_BARS"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=max_age + 80)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s45_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "impulse_atr_mult", "max_ob_age_bars",
              "max_violation_wick_atr", "sl_atr_mult", "tp_rr", "confirmation_type", "risk_pct",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "impulse_atr_mult": cfg["IMPULSE_ATR_MULT"],
            "max_ob_age_bars": cfg["MAX_OB_AGE_BARS"], "max_violation_wick_atr": cfg["MAX_VIOLATION_WICK_ATR"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S45_DEFAULTS["RISK_PCT"])
    ap.add_argument("--impulseatr", type=float, default=None)
    ap.add_argument("--maxage", type=int, default=None)
    ap.add_argument("--violation", type=float, default=None)
    ...
    if args.maxage is not None:
        cfg["MAX_OB_AGE_BARS"] = args.maxage
    if args.violation is not None:
        cfg["MAX_VIOLATION_WICK_ATR"] = args.violation
    ...
    print(f"S45 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | impulseatr={cfg['IMPULSE_ATR_MULT']} "
          f"maxage={cfg['MAX_OB_AGE_BARS']} violation={cfg['MAX_VIOLATION_WICK_ATR']} sl={cfg['SL_ATR_MULT']} "
          f"rr={cfg['TP_RR']} conf={cfg['CONFIRMATION_TYPE']}")
