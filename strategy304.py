# -*- coding: utf-8 -*-
"""S304 - Rollover rolling-range drive gated by higher-timeframe bias, 10R.

Tests whether the session's newest finding generalizes across break structures.
S303 showed that filtering the *anchored* opening-range break (S224) by an ~8h
trend bias cuts drawdown hard (2026-H1 DD 19.78 -> 10.32, ratio 12.9 -> 26.9)
while keeping nearly all the profit. S304 applies the identical filter to the
*rolling* micro-range break (S206), which is a different structure with roughly
three times the signal count, so there is room to filter. If the bias filter is
a property of the rollover edge rather than of one break form, this should
improve S206 the same way.
"""
...
DEFAULT_CFG = {
    ...
    "RANGE_MAX_ATR": 0.0,
    # Same filter as S303: close must be on the correct side of the SMA of the
    # last N M5 closes (96 = ~8h, the value the walk-forward window chose there).
    "HTF_BIAS_BARS": 96,
    "SL_BUFFER_ATR": 0.08,
    "MIN_RISK_ABS": 0.60,
    "MAX_RISK_ATR": 2.00,
...
def detect_s304(rates, tf="", dt_bkk=None, cfg=None, **kwargs):
    ...
    return {
        ...
        "tp": tp,
        "order_type": "market",
        "pattern": f"S304 {signal} Rollover Drive+HTF {rr:g}R",
