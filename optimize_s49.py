...
import sim_s49_backtest as sim

mt5.initialize()
days = 90
cfg0 = dict(S49_DEFAULTS)
entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg0["ENTRY_TF"], days, extra_bars=400)
htf_bars = s30sim.fetch_bars(config.SYMBOL, cfg0["HTF_TF"], days, extra_bars=200)
print(f"entry_bars={len(entry_bars)}")

combos = list(itertools.product(
    [1.0, 1.5, 2.0],         # STD_MULT
    [0.2, 0.3, 0.5],         # TOUCH_ATR_MULT
    [0.1, 0.15, 0.25],       # REJECT_ATR_MULT
    [0.8, 1.0],              # SL_ATR_MULT
    [1.0, 1.5, 2.0],         # TP_RR
))
...
t0 = time.time()
results = []
for i, (stdmult, touch, reject, sl, rr) in enumerate(combos):
    cfg = dict(S49_DEFAULTS)
    cfg.update(STD_MULT=stdmult, TOUCH_ATR_MULT=touch, REJECT_ATR_MULT=reject, SL_ATR_MULT=sl, TP_RR=rr)
    ...
    results.append((stdmult, touch, reject, sl, rr, s, c))
print(f"done in {time.time()-t0:.1f}s, {len(results)} valid")
mt5.shutdown()

results.sort(key=lambda r: r[6]["sharpe_like"], reverse=True)
print(f"\n{'std':>5} {'touch':>6} {'reject':>6} {'sl':>4} {'rr':>4} {'n':>4} {'WR%':>5} "
      f"{'$/mo':>8} {'DD%':>6} {'PF':>5} {'posDay%':>8} {'streak':>7} {'sharpe':>7}")
for stdmult, touch, reject, sl, rr, s, c in results[:15]:
    print(f"{stdmult:>5} {touch:>6} {reject:>6} {sl:>4} {rr:>4} {s['trades']:>4} {s['wr']:>5.1f} "
