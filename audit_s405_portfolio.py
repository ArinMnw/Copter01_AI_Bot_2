# -*- coding: utf-8 -*-
"""Portfolio audit for S405, with a reusable frozen event cache."""
...
def _json_events(events):
    return [[stamp.isoformat(), profit] for stamp, profit in events]


def main():
    by_sid = {sid: [] for sid in (*BASE_IDS, 405)}
    h1_trades = {}
    risks = []
    ...
    target_times = {trade["signal_time"] for trade in h1_trades[405]}
    overlap = {
    ...
    after = before + by_sid[405]
    CACHE_PATH.parent.mkdir(parents=True, exist_ok=True)
    CACHE_PATH.write_text(
        json.dumps({
            "base_ids": list(BASE_IDS),
            "windows": list(WINDOWS),
            "baseline": _json_events(before),
            "s405": _json_events(by_sid[405]),
        }, ensure_ascii=False, separators=(",", ":")),
        encoding="utf-8",
        newline="\n",
    )
    print("cache", str(CACHE_PATH), flush=True)
    print("overlap", overlap)
    print("risk", {"min": min(risks), "median": median(risks), "max": max(risks)})
    print("s405_wf_h1", _stats(by_sid[405]))
    print("portfolio_before", _stats(before))
    for weight in (0.25, 0.50, 0.75):
        weighted = before + [
            (stamp, profit * weight) for stamp, profit in by_sid[405]
        ]
