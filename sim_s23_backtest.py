"""
sim_s23_backtest.py — Backtest S23 Trend-Following ADX/EMA Pullback จากข้อมูล MT5 จริง
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — strategy23.py ไม่ถูก wire เข้า scanner.py/trailing.py/
   main.py ใดๆ ทั้งสิ้น ไฟล์นี้ไม่แก้ S1-S22, ไม่แก้ bot_state.json, ไม่แตะ live trading

เรียก strategy23.detect_s23() ตรง (pure function — logic เดียวกับที่จะใช้ถ้า wire จริง)

กัน look-ahead bias (เหมือน sim_s21/sim_s22_backtest.py):
  - แท่ง "กำลังวิ่ง" (rates[-1]) ใช้ open ของแท่งถัดจากแท่ง pullback-rejection
...
Position sizing (risk-based, ทุนเริ่มต้น $1000, compounding ตาม equity ปัจจุบัน):
  เหมือน sim_s21/sim_s22 — lot = risk_usd / risk_distance, margin guard กัน
  margin usage เกิน MAX_MARGIN_USAGE_PCT ของ equity ที่ leverage ASSUMED_LEVERAGE

ตัวอย่าง:
  python sim_s23_backtest.py --days 60 --tf M15,H1
  python sim_s23_backtest.py --days 60 --tf H1 --risk 1.0 --rr 2.5 --csv
"""
...
import config
from strategy23 import S23_DEFAULTS, detect_s23
...
def replay_tf(bars, tf_name, spread, cfg):
    ...
    swing_lb = int(cfg["SWING_LOOKBACK"])
    win_size = max(ema_slow, adx_period * 2) + swing_lb + 10
    ...
    for j in range(start_j, n - 1):
        ...
        lo = max(0, j + 1 - (win_size + 30))
        window = list(bars[lo:j + 1]) + [live]

        dt_bkk = config.mt5_ts_to_bkk(int(sig_bar["time"]))
        res = detect_s23(window, tf=tf_name, dt_bkk=dt_bkk, cfg=cfg)
        sig = res.get("signal")
...
def append_summary_csv(label, s, cfg, risk_pct):
    path = os.path.join(os.path.dirname(__file__), "s23_backtest_summary.csv")
    ...
    fields = [
        ...
        "ema_fast", "ema_slow", "adx_min", "tp_rr", "sl_atr_mult",
        "session_filter",
    ]
...
def main():
    ...
    ap.add_argument("--tf", default="M15,H1")
    ap.add_argument("--spread", type=float, default=DEFAULT_SPREAD)
    ap.add_argument("--risk", type=float, default=S23_DEFAULTS["RISK_PCT"], help="risk %% per trade")
    ...
    ap.add_argument("--adxmin", type=float, default=None, help="override ADX_MIN")
    ap.add_argument("--pullback", type=float, default=None, help="override PULLBACK_ATR_MULT")
    ap.add_argument("--nosession", action="store_true", help="ปิด session filter (เทรดได้ทั้งวัน)")
    ap.add_argument("--label", default="baseline", help="label สำหรับ s23_backtest_summary.csv")
    ap.add_argument("--csv", action="store_true", help="เซฟ trade-level CSV ด้วย")
    args = ap.parse_args()

    cfg = dict(S23_DEFAULTS)
    ...
    if args.adxmin is not None:
        cfg["ADX_MIN"] = args.adxmin
    if args.pullback is not None:
        cfg["PULLBACK_ATR_MULT"] = args.pullback
    if args.nosession:
    ...
    if not mt5.initialize():
        ...
        return
    symbol = config.SYMBOL
    print(f"S23 backtest | Symbol={symbol} | days={args.days} | spread=${args.spread:.2f} | "
          f"risk={cfg['RISK_PCT']}%/trade | start_equity=${START_EQUITY:.0f}")
    print(f"cfg: ema_fast={cfg['EMA_FAST']} ema_slow={cfg['EMA_SLOW']} adx_min={cfg['ADX_MIN']} "
    ...
    if args.csv and trades_with_pnl:
        out_dir = os.path.join("excel_reports", "backtest_compare", "s23")
        os.makedirs(out_dir, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M")
        path = os.path.join(out_dir, f"sim_s23_{args.label}_{stamp}.csv")
