"""
sim_s14_flip_pd.py — S14 + Flip Logic + PD Zone เท่านั้น (ไม่มี Trend Recheck)
...
# ── Build EQ timeline from bot.log ───────────────────────────────────────────
print("Loading log timeline (EQ for PD Zone)...")
...
def _px(rx, txt):
    mx=rx.search(txt); return (float(mx.group(1)),_pt(mx.group(2))) if mx else (None,None)
...
def backtest_tf(tf_name, tf_val):
    ...
    for i in range(start_idx, len(bars)):
        ...
        for ord_ in orders:
            ...
            passed, reason = passes_pd(bt, tf_name, s, e)
            if not passed:
...
def main():
    ...
    for tf_name, tf_val in TF_MAP.items():
        ...
        print(f'\n── {tf_name} ─────────────────────────────────────────────')
        print(f'   trades={len(trades)}  TP={tp_cnt}  SL={sl_cnt}  FLIP={flip_cnt}'
              f'  OPEN={op_cnt}  WR={wr:.0f}%   blocked={len(blocked_log)}')
    ...
    print(f'GRAND TOTAL: {"+" if grand_total>=0 else ""}{grand_total:.2f} USD  '
          f'(volume={VOLUME} lot)   blocked={all_blocked}')
    ...
    print('\n── เปรียบเทียบทุก mode (ทุกแบบมี Flip) ────────────────────────────')
    print(f'  Flip only:           79 trades  +137.04 USD')
    print(f'  Flip + PD Zone:  {sum(r[1] for r in summary_rows):>3} trades  {"+" if grand_total>=0 else ""}{grand_total:.2f} USD  ← นี้')
    print(f'  Flip + Trend+PD:     30 trades   +53.98 USD')
