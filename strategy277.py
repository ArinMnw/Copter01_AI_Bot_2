...
from strategy276 import DEFAULT_CFG as S276_DEFAULT_CFG
from strategy276 import detect_s276
...
def detect_s277(rates, tf="", dt_bkk=None, cfg=None, **kwargs):
    """Fade a significant volume-led structural break after S276 failed."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    original = detect_s276(rates, tf, dt_bkk, c, **kwargs)
    ...
    return {
        ...
        "tp": tp,
        "order_type": "market",
        "pattern": f"S277 {signal} Volume Lead Break Fade {rr:g}R",
        "reason": (
            f"Fade significant volume-led break; original={original['signal']}"
        ),
        "be_rr": float(c["BE_RR"]),
