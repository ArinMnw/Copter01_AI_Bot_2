"""
optimize_s67_clear_candle.py - Search S67 Clear Candle configs.
...
from sim_s67_backtest import DEFAULT_SPREAD, START_EQUITY, run_single
from strategy67 import S67_DEFAULTS
...
def _cfg_label(cfg):
    return (
        f"{cfg['ENTRY_TF']}_{cfg['MODE']}_{cfg['CLOSE_COVER']}"
        f"_mb{cfg['MIN_BODY_ATR']:g}_mr{cfg['MIN_RANGE_ATR']:g}"
        f"_wb{cfg['WICK_BODY_MULT']:g}_wr{cfg['WICK_RANGE_MIN']:g}"
        f"_tl{cfg['TREND_LOOKBACK']}_tm{cfg['TREND_MIN_ATR']:g}"
        f"_sl{cfg['SL_ATR_MULT']:g}_rr{cfg['TP_RR']:g}"
    )
...
def main():
    ...
    ap.add_argument("--out", default="s67_clear_candle_optimize.csv")
    ...
    bars_by_tf_days = {}
    tfs = sorted(set(_grid(args.preset)[0]))
    for tf in tfs:
        for days in windows:
            bars_by_tf_days[(tf, days)] = s30sim.fetch_bars(config.SYMBOL, tf, days, extra_bars=500)
    mt5.shutdown()

    configs = []
    for vals in itertools.product(*_grid(args.preset)):
        tf, mode, cover, minbody, minrange, wickbody, wickrange, tlb, tmin, slmult, rr = vals
    ...
    for cfg in configs:
        ...
        for days in windows:
            ...
            row = _row_exact(raw, cfg, days) if args.mode == "exact" else _row_fixed(raw, days, DEFAULT_SPREAD)
            rows.append(row)
