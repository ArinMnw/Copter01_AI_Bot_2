...
def detect_s178(rates, tf, dt_bkk, cfg):
    """Fade a volume-weighted price outlier after a closed fair-value reclaim."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
        period = max(1, int(c["ATR_PERIOD"]))
        window = max(24, int(c["FAIR_VALUE_WINDOW"]))
    ...
    total_weight = sum(max(float(bar["tick_volume"]), 1.0) for bar in history)
    weighted_price = sum(
        ((bar["high"] + bar["low"] + bar["close"]) / 3.0)
        * max(float(bar["tick_volume"]), 1.0)
        for bar in history
    ) / total_weight
    weighted_variance = sum(
        max(float(bar["tick_volume"]), 1.0)
        * (((bar["high"] + bar["low"] + bar["close"]) / 3.0) - weighted_price) ** 2
        for bar in history
    ) / total_weight
    weighted_scale = max(math.sqrt(weighted_variance), atr * 0.10)

    exhaustion = bars[-2]
    reclaim = bars[-1]
    weighted_z = (exhaustion["close"] - weighted_price) / weighted_scale
    if abs(weighted_z) < float(c["WEIGHTED_Z_MIN"]):
        return _wait(f"Price is not a weighted fair-value outlier (z={weighted_z:.2f})")
    exhaustion_body = exhaustion["close"] - exhaustion["open"]
    exhaustion_range = exhaustion["high"] - exhaustion["low"]
    if (exhaustion_range <= 0.0
            or weighted_z * exhaustion_body <= 0.0
            or abs(exhaustion_body) < atr * float(c["EXHAUSTION_BODY_MIN_ATR"])):
        return _wait("Fair-value outlier lacks aligned exhaustion body")
    ...
    if exhaustion["tick_volume"] < exhaustion_volume_min:
        return _wait("Fair-value outlier lacks institutional volume")
    ...
    if not confirmed or reclaim["tick_volume"] < reclaim_volume_min:
        return _wait("Outlier lacks confirmed fair-value reclaim")
    ...
    if risk <= 0.0 or risk > atr * float(c["MAX_RISK_ATR"]):
        return _wait(f"Fair-value-reclaim risk outside range ({risk / atr:.2f} ATR)")
    risk_pct = risk / entry * 100.0
    if risk_pct > float(c["MAX_RISK_PRICE_PCT"]):
        return _wait(f"Fair-value-reclaim risk too large versus price ({risk_pct:.2f}%)")
    ...
    return {
        ...
        "tp": tp,
        "order_type": "limit",
        "pattern": f"S178 {signal} Weighted Fair-Value {rr:g}R",
        "reason": (f"Volume-weighted z={weighted_z:.2f}; institutional-price anomaly "
                   "closed a fair-value reclaim"),
        "be_rr": float(c["BE_RR"]),
