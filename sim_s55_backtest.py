"""
sim_s55_backtest.py — Backtest S55 (Round-Number Psychological Level Bounce)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S54, ไม่ wire เข้า live
"""
...
import config
from strategy55 import S55_DEFAULTS, detect_s55
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S55_DEFAULTS[key]


def replay55(bars, htf_series, spread, cfg):
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s55(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay55(entry_bars, htf_series, spread, cfg)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s55_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "round_step", "touch_atr_mult",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "round_step": cfg["ROUND_STEP"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S55_DEFAULTS["RISK_PCT"])
    ...
    cfg = dict(S55_DEFAULTS)
    cfg["RISK_PCT"] = args.risk
    if args.step is not None:
        cfg["ROUND_STEP"] = args.step
    ...
    print(f"S55 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | step={cfg['ROUND_STEP']} "
