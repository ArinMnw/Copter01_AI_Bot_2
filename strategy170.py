# -*- coding: utf-8 -*-
"""S170 - Volatility-compression bullish expansion retrace with a 7R target."""
...
def detect_s170(rates, tf, dt_bkk, cfg):
    """Buy the retrace of a closed bullish expansion out of volatility compression."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
    ...
    breakout_body = breakout["close"] - breakout["open"]
    if breakout_range <= 0.0 or breakout_body < atr * float(c["BREAKOUT_BODY_MIN_ATR"]):
        return _wait("No bullish range expansion")
    close_location = (breakout["close"] - breakout["low"]) / breakout_range
    prior_high = max(bar["high"] for bar in bars[-breakout_lookback - 1:-1])
    if (breakout["close"] <= prior_high
            or close_location < float(c["BREAKOUT_CLOSE_LOCATION_MIN"])):
        return _wait("Expansion did not close above prior structure")
    ...
    sl = breakout["low"] - atr * float(c["SL_BUFFER_ATR"])
    entry = round(entry, 2)
    ...
    return {
        ...
        "tp": tp,
        "order_type": "limit",
        "pattern": f"S170 BUY Compression Expansion {rr:g}R",
        "reason": (f"Compression={compression_ratio:.2f}; bullish expansion closed above "
