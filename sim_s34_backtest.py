"""
sim_s34_backtest.py — Backtest S34 (volume-confirmed breakout)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S33, ไม่ wire เข้า live

กัน look-ahead เหมือน sim_s30/s31 (entry-TF MARKET ที่ open แท่งถัดไป + HTF bisect บนเวลาปิดแท่ง)
reuse fetch_bars/build_htf_series/htf_lookup จาก sim_s30_backtest
"""
...
import sim_s31_backtest as s31sim  # reuse consistency_metrics / daily_series_from_trades

START_EQUITY = 1000.0


def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S34_DEFAULTS[key]


def replay34(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    lookback = int(_cfg_get(cfg, "BREAKOUT_LOOKBACK"))
    vol_period = int(_cfg_get(cfg, "VOLUME_SMA_PERIOD"))
    win_size = max(lookback, vol_period) + 20
    ...
    for j in range(start_j, n - 1):
        ...
        live = {"time": int(entry_bar["time"]), "open": float(entry_bar["open"]),
                ...
                "close": float(entry_bar["open"]), "tick_volume": 0}
        ...
        res = detect_s34(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay34(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    lookback = int(cfg["BREAKOUT_LOOKBACK"]); vol_period = int(cfg["VOLUME_SMA_PERIOD"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=max(lookback, vol_period) + 80)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s34_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "breakout_lookback", "volume_surge_mult",
              "volume_sma_period", "min_breakout_atr", "sl_atr_mult", "tp_rr", "risk_pct",
              "trades", "wr", "avg_per_day_span", "avg_per_month", "max_dd_pct", "profit_factor",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "breakout_lookback": cfg["BREAKOUT_LOOKBACK"],
            "volume_surge_mult": cfg["VOLUME_SURGE_MULT"], "volume_sma_period": cfg["VOLUME_SMA_PERIOD"],
            "min_breakout_atr": cfg["MIN_BREAKOUT_ATR"], "sl_atr_mult": cfg["SL_ATR_MULT"],
            "tp_rr": cfg["TP_RR"], "risk_pct": cfg["RISK_PCT"], "trades": s["trades"], "wr": s["wr"],
            "avg_per_day_span": s["avg_per_day_span"], "avg_per_month": round(s["avg_per_day_span"] * 30, 2),
            ...
            "sharpe_like": s["sharpe_like"], "final_equity": s["final_equity"],
        })
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S34_DEFAULTS["RISK_PCT"])
    ap.add_argument("--lookback", type=int, default=None)
    ap.add_argument("--volmult", type=float, default=None)
    ap.add_argument("--volperiod", type=int, default=None)
    ap.add_argument("--minbreakout", type=float, default=None)
    ...
    if args.lookback is not None:
        cfg["BREAKOUT_LOOKBACK"] = args.lookback
    if args.volmult is not None:
        cfg["VOLUME_SURGE_MULT"] = args.volmult
    if args.volperiod is not None:
        cfg["VOLUME_SMA_PERIOD"] = args.volperiod
    if args.minbreakout is not None:
        cfg["MIN_BREAKOUT_ATR"] = args.minbreakout
    ...
    print(f"S34 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | lookback={cfg['BREAKOUT_LOOKBACK']} "
          f"volmult={cfg['VOLUME_SURGE_MULT']} sl={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']}")
