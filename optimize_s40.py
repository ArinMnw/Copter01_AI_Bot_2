...
import sim_s40_backtest as sim

mt5.initialize()
days = 90
cfg0 = dict(S40_DEFAULTS)
entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg0["ENTRY_TF"], days, extra_bars=cfg0["ZIGZAG_LOOKBACK_BARS"] + 100)
...
combos = list(itertools.product(
    [0.8, 1.2, 1.5],        # ZIGZAG_MIN_ATR
    [100, 150, 200],        # ZIGZAG_LOOKBACK_BARS
    [15, 25],               # MAX_WAVE4_AGE_BARS
    [0.05, 0.1],            # ENTRY_BREAK_ATR_MULT
    [0.8, 1.0],             # SL_ATR_MULT
    [1.0, 1.5],             # TP_RR
))
...
t0 = time.time()
results = []
for i, (zm, lb, ma, eb, sl, rr) in enumerate(combos):
    cfg = dict(S40_DEFAULTS)
    cfg.update(ZIGZAG_MIN_ATR=zm, ZIGZAG_LOOKBACK_BARS=lb, MAX_WAVE4_AGE_BARS=ma,
               ENTRY_BREAK_ATR_MULT=eb, SL_ATR_MULT=sl, TP_RR=rr)
    ...
    results.append((zm, lb, ma, eb, sl, rr, s, c))
print(f"done in {time.time()-t0:.1f}s, {len(results)} valid")
mt5.shutdown()

results.sort(key=lambda r: r[7]["sharpe_like"], reverse=True)
print(f"\n{'zm':>4} {'lb':>4} {'ma':>4} {'eb':>5} {'sl':>4} {'rr':>4} {'n':>4} {'WR%':>5} "
      f"{'$/mo':>8} {'DD%':>6} {'PF':>5} {'posDay%':>8} {'streak':>7} {'sharpe':>7}")
for zm, lb, ma, eb, sl, rr, s, c in results[:15]:
    print(f"{zm:>4} {lb:>4} {ma:>4} {eb:>5} {sl:>4} {rr:>4} {s['trades']:>4} {s['wr']:>5.1f} "
