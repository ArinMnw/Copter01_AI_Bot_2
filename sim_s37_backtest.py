"""
sim_s37_backtest.py — Backtest S37 (Horizontal Support/Resistance pivot bounce)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S36, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S37_DEFAULTS[key]


def replay37(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    max_level_age = int(_cfg_get(cfg, "MAX_LEVEL_AGE_BARS"))
    wing = int(_cfg_get(cfg, "PIVOT_WING"))
    win_size = max_level_age + wing * 2 + 30
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay37(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    max_level_age = int(cfg["MAX_LEVEL_AGE_BARS"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=max_level_age + 100)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s37_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "pivot_wing", "max_level_age_bars",
              "touch_atr_mult", "reject_atr_mult", "sl_atr_mult", "tp_rr", "risk_pct", "trades", "wr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "max_level_age_bars": cfg["MAX_LEVEL_AGE_BARS"], "touch_atr_mult": cfg["TOUCH_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S37_DEFAULTS["RISK_PCT"])
    ...
    print(f"S37 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | wing={cfg['PIVOT_WING']} "
          f"maxage={cfg['MAX_LEVEL_AGE_BARS']} touch={cfg['TOUCH_ATR_MULT']} reject={cfg['REJECT_ATR_MULT']} "
