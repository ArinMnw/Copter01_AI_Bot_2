...
DEFAULT_CFG = {
    ...
    "RECLAIM_CLOSE_LOCATION_MIN": 0.65,
...
def detect_s165(rates, tf, dt_bkk, cfg):
    """Buy a closed bullish reclaim after downside-semivariance capitulation."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
    ...
    if (exhaustion_body < atr * float(c["EXHAUSTION_BODY_MIN_ATR"])
        ...
        return _wait("Prior bar was not bearish capitulation")
    ...
    if (reclaim["close"] <= reclaim["open"]
        ...
        return _wait("No bullish reclaim after capitulation")
    ...
    return {
        ...
        "tp": tp,
        "order_type": "limit",
        "pattern": f"S165 BUY Semivariance Capitulation {rr:g}R",
