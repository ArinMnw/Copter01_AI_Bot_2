...
*   **Historical PD drift:** PD pre-create/pending/fill skip list ปัจจุบันอ่านจาก `config.PDFIBOPLUS_SKIP_SIDS` และข้าม `S1/S2/S3/S9/S10/S11/S13/S14/S15/S16/S17/S18/S19` ให้ตรงกันทุกจุด; ถ้าเทียบ MT5 history เก่าก่อน skip-list fix อาจยังเห็น live close reason `PD Zone...` ในท่าเหล่านี้ เป็น expected drift จาก runtime คนละเวอร์ชัน
    *   Fix 2026-06-19: `config.PDFIBOPLUS_SKIP_SIDS` ถูก sync ให้มี S2/S3 จริงแล้ว หลังพบว่า S3 replay ยังออก `PD_FILL_FAIL` แม้ coverage table ระบุว่า skip
...
*   **S3 baseline replay:** S3 replay ใช้ range-based fetch + `strategy3.strategy_3()` จริง, จำลอง normal confirm lookback, swing fallback, Marubozu/No Engulf pending ยืนยันแท่งถัดไป, pending limit fill, fixed SL/TP, PD Fibo Plus ตาม `config.PDFIBOPLUS_SKIP_SIDS` ปัจจุบัน, adjacent same-sid block, first-pass sweep scan block และ current-TF Trend Filter scan block เมื่อ config เปิด; runtime ปัจจุบันยัง skip Trend recheck สำหรับ S3 และ replay มี Limit Guard, Opposite Order, Trail SL, RSI modes, SL Guard baseline แล้ว gap ที่ยังเหลือคือ global runtime state, higher-TF/exported trend scan state, exact sweep expiry/state และ timing/shared-state drift
    *   S3 compare update: report ล่าสุด tag old PD close เป็น `LIVE_HISTORICAL_PD_SKIP_DRIFT`; หลังตัดกลุ่มนี้ออก งานที่เหลือคือ high backtest-only count, adjacent/sweep/global-state timing และ Trail/Guard shared state
