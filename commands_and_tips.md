...
*   **S3 baseline replay:** S3 replay ใช้ range-based fetch + `strategy3.strategy_3()` จริง, จำลอง normal confirm lookback, swing fallback, Marubozu/No Engulf pending ยืนยันแท่งถัดไป, pending limit fill, fixed SL/TP, adjacent same-sid block และ first-pass sweep scan block; runtime ปัจจุบัน skip PD/Trend recheck สำหรับ S3 แล้ว และ replay มี Limit Guard, Opposite Order, Trail SL, RSI modes, SL Guard baseline แล้ว gap ที่ยังเหลือคือ global runtime state, trend scan block, exact sweep expiry/state และ timing/shared-state drift
