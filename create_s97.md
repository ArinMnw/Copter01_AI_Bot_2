# S97 Champion - Inverse S84 RD 1.3-2.0 Late-Day Overlay

สถานะ: research/backtest-only, ยังไม่ wire เข้า live bot

## Baseline

Champion ล่าสุดก่อนหน้า:

```text
S96 = S95 + INV_S84_M15_OLDWICK_FOLLOW_RD1.3_2.0_H10_13x6.628
```

| Metric | S96 |
|---|---:|
| Avg $/day | 622.0345 |
| Min $/day | 602.8985 |
| Min PF | 5.38154 |
| Max losing-day streak | 3 |
| Worst day | -999.90826 |
| Max lot | 0.19 |
| Max leg DD | 55.01% |
| Skipped by circuit breaker | 10391 |

## New Leg

Winning overlay:

```text
INV_S84_M15_lb48_rw0.25_wb0.8_eat0.06_fail0.03_op1_mb0.06_mr0.35_rr_follow_sl0.2_rr0.9_Frdmin1.3_rd2_hfrom17_hbefore18
```

Meaning:

- Base generator: S84 old-wick follow
- Mode: inverse raw
- TF: M15
- Lookback: 48
- Old wick min: 0.25 ATR
- Wick/body min: 0.8
- Eat tolerance: 0.06 ATR
- Close-fail threshold: 0.03 ATR
- Require opposite close: on
- Min body: 0.06 ATR
- Min range: 0.35 ATR
- Target mode: RR
- SL buffer: 0.2 ATR
- RR: 0.9
- Post-filter: `1.3 <= risk_distance <= 2.0`
- Time filter: `17 <= fill_hour < 18` BKK

## New Champion

```text
S97 = S96 + INV_S84_M15_OLDWICK_FOLLOW_RD1.3_2.0_H17_18x29.986
```

| Metric | S97 |
|---|---:|
| Avg $/day | 629.2131 |
| Min $/day | 606.8866 |
| Min PF | 5.38924 |
| Max losing-day streak | 3 |
| Worst day | -999.90520 |
| Max lot | 0.19 |
| Max leg DD | 55.01% |
| Skipped by circuit breaker | 10391 |

Per-window exact from `s97_s84_inv_rdmin13_rd20_h17_18_daily.csv`:

| Window | $/day | PF | Streak | Worst day | Raw trades |
|---:|---:|---:|---:|---:|---:|
| 90 | 606.8866 | 5.38924 | 3 | -978.3384 | 7 |
| 120 | 667.0051 | 5.93626 | 3 | -976.22864 | 13 |
| 150 | 632.1434 | 5.86224 | 3 | -986.25864 | 18 |
| 180 | 610.8173 | 5.44400 | 3 | -999.90520 | 22 |

## Weight Threshold

`s97_s84_inv_rdmin13_rd20_h17_18_target28_ultrafine.csv`:

| Weight | Result |
|---:|---|
| 29.986 | highest 0.001-step weight that passes `-999.91` |
| 29.987 | fails `-999.91` |
| 30.000 | passes only `-1000`, fails `-999.91` |

## No-Blow Guard

| Floor | Result |
|---|---|
| -700 | fail because the champion ladder already has near -1000 days |
| -900 | fail because the champion ladder already has near -1000 days |
| -973.16 | fail because the champion ladder already has near -1000 days |
| -999.91 | pass |
| -1000 | pass |

## Evidence

- `optimize_s88_allin4s_fast.py`
- `s97_s84_inv_rdmin13_rd20_h17_18_target28_probe.csv`
- `s97_s84_inv_rdmin13_rd20_h17_18_target28_wide.csv`
- `s97_s84_inv_rdmin13_rd20_h17_18_target28_ultrafine.csv`
- `s97_s84_inv_rdmin13_rd20_h17_18_target28_ultrafine_worst_day.csv`
- `s97_s84_inv_rdmin13_rd20_h17_18_daily.csv`

## Look-Ahead Bias Audit

- Research/backtest-only; no live bot wiring.
- S97 uses `s96_s84_inv_rdmin13_rd20_h10_13_daily.csv` as the base, so it is
  compared against the current champion S96.
- The new leg still uses `simulate_equity_substream(raw, cfg, START_EQUITY=1000)`
  for per-leg sizing.
- S84 detection uses closed bar `j`.
- Fill is simulated from `j+1` via the existing `sim_s84_backtest.run_single`
  replay.
- Inverse mode swaps signal/TP/SL on the same raw event and negates
  `diff_usd_per_001lot`.
- `risk_distance` is entry/SL distance known at signal construction time.
- `17 <= fill_hour < 18` uses the simulated fill timestamp available at entry,
  not any post-entry outcome.
- No future candle, same-bar close fill, or result filter is used.

## Verdict

Found new champion:

```text
S97 = S96 + INV_S84_M15_OLDWICK_FOLLOW_RD1.3_2.0_H17_18x29.986
```

This improves both avg $/day and min $/day while keeping max losing-day streak
at 3 and passing the `-999.91` / `-1000` floors. Continue with S98 search using
S97 as the new baseline.
