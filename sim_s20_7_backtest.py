...
config.active_strategies[20.7] = True
...
def replay_tf(bars, tf_name, spread):
    ...
    for j in range(5, n - 1):
        entry_bar = bars[j]
        # rates สำหรับให้ strategy_20_7 มองเห็น ต้องดึงมาให้พอสำหรับคำนวณ ATR และ Liquidity Sweep (60+)
        ...
        res = strategy_20_7(rates_slice, tf=tf_name, dt_bkk=bar_dt_bkk)
        sig = res.get("signal")
        ...
        trades.append({
            "tf": tf_name, 
            "signal": sig,
            "sub_pattern": res.get("pattern", "S20.7"),
            "outcome": outcome,
