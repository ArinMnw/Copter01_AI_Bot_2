...
# ============================================================
#  order_send timeout guard
#  ครอบ mt5.order_send ให้ค้างไม่เกิน timeout (default 30s)
#  ─ ต้นเหตุ STALL: MT5 call เดี่ยว ๆ (โดยเฉพาะ order_send) ค้างยาว 180s+
#    ตอน broker/terminal มีปัญหา → freeze single event loop → heartbeat
#    เขียนไม่ได้ → supervisor kill (STALL). การ cap เวลาแต่ละ call ไว้ที่ timeout
#    ทำให้ไม่มี call ไหนค้างถึง 180s อีก
#  ─ ทำที่จุดเดียว: mt5 เป็น module object เดียวกันทุกไฟล์
#    (config: import MetaTrader5 as mt5) → ครอบทุก call site อัตโนมัติ
#    โดยไม่ต้องแก้ signature ฟังก์ชันใด ๆ
#  ─ run บน worker thread แยก เพื่อให้ .result(timeout) ตัดได้จริง
#    (order_send เป็น synchronous foreign call — cancel กลางคันไม่ได้
#     แต่ result(timeout) คืน None ให้ caller มองเป็น order ไม่สำเร็จได้)
# ============================================================
import concurrent.futures as _futures

_order_send_executor = _futures.ThreadPoolExecutor(
    max_workers=4, thread_name_prefix="mt5_order_send"
)
_order_send_patched = False


def _install_order_send_guard():
    """ติดตั้ง timeout guard ครอบ mt5.order_send (idempotent)"""
    global _order_send_patched
    if _order_send_patched:
        return
    _real_order_send = mt5.order_send

    def _guarded_order_send(request, *args, **kwargs):
        timeout = float(getattr(config, "ORDER_SEND_TIMEOUT", 30.0) or 30.0)
        fut = _order_send_executor.submit(_real_order_send, request, *args, **kwargs)
        try:
            ...
            try:
                ...
                log_event(
                    "ORDER_SEND_TIMEOUT",
                    f"mt5.order_send ค้าง > {timeout:.0f}s — คืน None (กัน freeze event loop)",
                    timeout=timeout,
                )
            except Exception:
                pass
            return None   # caller มองเป็น order ไม่สำเร็จ (เหมือน order_send คืน None ปกติ)

    mt5.order_send = _guarded_order_send
    _order_send_patched = True


_install_order_send_guard()
