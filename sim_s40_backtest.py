"""
sim_s40_backtest.py — Backtest S40 (Elliott Wave simplified 5-wave impulse)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S39, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S40_DEFAULTS[key]


def replay40(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    lb = int(_cfg_get(cfg, "ZIGZAG_LOOKBACK_BARS"))
    win_size = lb + 30
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s40(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay40(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    lb = int(cfg["ZIGZAG_LOOKBACK_BARS"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=lb + 100)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s40_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "zigzag_min_atr", "zigzag_lookback_bars",
              "max_wave4_age_bars", "entry_break_atr_mult", "sl_atr_mult", "tp_rr", "risk_pct",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "zigzag_min_atr": cfg["ZIGZAG_MIN_ATR"],
            "zigzag_lookback_bars": cfg["ZIGZAG_LOOKBACK_BARS"], "max_wave4_age_bars": cfg["MAX_WAVE4_AGE_BARS"],
            "entry_break_atr_mult": cfg["ENTRY_BREAK_ATR_MULT"], "sl_atr_mult": cfg["SL_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S40_DEFAULTS["RISK_PCT"])
    ap.add_argument("--zigzagmin", type=float, default=None)
    ...
    cfg = dict(S40_DEFAULTS)
    cfg["RISK_PCT"] = args.risk
    if args.zigzagmin is not None:
        cfg["ZIGZAG_MIN_ATR"] = args.zigzagmin
    if args.lookback is not None:
        cfg["ZIGZAG_LOOKBACK_BARS"] = args.lookback
    if args.maxage is not None:
        cfg["MAX_WAVE4_AGE_BARS"] = args.maxage
    if args.slmult is not None:
        cfg["SL_ATR_MULT"] = args.slmult
    if args.rr is not None:
        cfg["TP_RR"] = args.rr

    print(f"S40 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | zigzagmin={cfg['ZIGZAG_MIN_ATR']} "
          f"lookback={cfg['ZIGZAG_LOOKBACK_BARS']} maxage={cfg['MAX_WAVE4_AGE_BARS']} sl={cfg['SL_ATR_MULT']} "
          f"rr={cfg['TP_RR']}")
