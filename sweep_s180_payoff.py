# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S180."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy180 import detect_s180
from sweep_s161_payoff import _summary


CASES = ((7.0, 1.0), (14.0, 1.0), (15.0, 1.0),
         (16.0, 0.50), (16.0, 0.75), (16.0, 0.90), (16.0, 1.0),
         (16.0, 1.10), (16.0, 1.18), (16.0, 1.25), (16.0, 1.50),
         (17.0, 1.0), (18.0, 1.0), (19.0, 1.0), (20.0, 1.0))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s180(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 180)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(180, args.months, "M5", 0.20, 0.01, end, lookback,
