# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S233."""
...
from strategy233 import detect_s233


CASES = tuple(
    (rr, be)
    for rr in (27.0, 27.1, 27.2, 27.3, 27.4, 27.5)
    for be in (0.82, 0.83, 0.84, 0.85, 0.86, 0.87, 0.88, 0.89)
) + ((10.0, 1.0),)
...
def _summary_market(
    ...
    """Replay S233 with the generic backtester's market-fill semantics."""
    trades = []
    signals = invalid = 0
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s233(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 233)
        cached[index] = signal
    rows = [
    ...
    if not args.skip_guard:
        ...
        base = next(
            row for row in rows if row["rr"] == 10.0 and row["be"] == 1.0
        )
        if (
