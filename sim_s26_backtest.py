"""
sim_s26_backtest.py — Backtest S26 High-Frequency M1 Scalping จากข้อมูล MT5 จริง
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — strategy26.py ไม่ถูก wire เข้า scanner.py/trailing.py/
   main.py ใดๆ ทั้งสิ้น ไฟล์นี้ไม่แก้ S1-S25, ไม่แก้ bot_state.json, ไม่แตะ live trading

เรียก strategy26.detect_s26() ตรง (pure function)

กัน look-ahead bias (เหมือน sim_s21-25_backtest.py):
  - แท่ง "กำลังวิ่ง" (rates[-1]) ใช้ close = open ของแท่งถัดจากแท่ง signal
  - entry เป็น MARKET ที่ open ของแท่งถัดจากแท่ง signal
  - แท่งเดียวกันแตะทั้ง TP และ SL → นับ SL (conservative)
  - spread หักจาก P/L ทุกไม้ (default 0.20 USD ต่อ "lot unit" 0.01) — M1 ความถี่สูงมาก
    ดังนั้น spread cost ต่อวันมีนัยสำคัญกว่า TF อื่นมาก ต้องรายงานให้ชัด

ตัวอย่าง:
  python sim_s26_backtest.py --days 30 --setup ema_pullback
  python sim_s26_backtest.py --days 30 --setup momentum_pullback --risk 0.5 --csv
"""
...
import config
from strategy26 import S26_DEFAULTS, detect_s26
...
def fetch_bars(symbol, days):
    count = min(days * PER_DAY + 300, 95000)
...
def replay(bars, spread, cfg):
    ema_trend = int(cfg["EMA_TREND"])
    range_lb = int(cfg["RANGE_LOOKBACK"])
    win_size = max(ema_trend, range_lb) + 60

    trades = []
    last_fire_idx = -10
    min_gap_bars = 2  # ห้ามยิงสัญญาณซ้ำติดกัน <2 แท่ง (กันสัญญาณซ้อนทับโดยตรง)
    n = len(bars)
    start_j = win_size + 5
    for j in range(start_j, n - 1):
        ...
        res = detect_s26(window, tf="M1", dt_bkk=dt_bkk, cfg=cfg)
        sig = res.get("signal")
...
def append_summary_csv(label, s, cfg, risk_pct):
    path = os.path.join(os.path.dirname(__file__), "s26_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = [
        "timestamp", "label", "setup_type", "trades", "wr", "total_pnl",
        "avg_per_day_span", "avg_per_day_active", "trades_per_active_day",
        ...
        "ema_fast", "ema_trend", "pullback_touch_atr", "momentum_body_atr",
        "pullback_max_atr", "range_lookback", "range_edge_pct", "sl_atr_mult", "tp_rr",
    ]
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        row = {
            ...
            **{k: s[k] for k in (
            ...
            "momentum_body_atr": cfg["MOMENTUM_BODY_ATR"],
            "pullback_max_atr": cfg["PULLBACK_MAX_ATR"],
            "range_lookback": cfg["RANGE_LOOKBACK"],
            "range_edge_pct": cfg["RANGE_EDGE_PCT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S26_DEFAULTS["RISK_PCT"])
    ap.add_argument("--setup", default=None, choices=["ema_pullback", "momentum_pullback", "range_scalp"])
    ap.add_argument("--emafast", type=int, default=None)
    ap.add_argument("--ematrend", type=int, default=None)
    ap.add_argument("--touchatr", type=float, default=None)
    ap.add_argument("--mombody", type=float, default=None)
    ap.add_argument("--pbmax", type=float, default=None)
    ap.add_argument("--rangelb", type=int, default=None)
    ap.add_argument("--rangeedge", type=float, default=None)
    ...
    ap.add_argument("--session", default=None, help="override session window เช่น '19:00-23:00'")
    ap.add_argument("--atrregime", type=float, default=None, help="เปิด ATR_REGIME_FILTER ด้วย mult ที่กำหนด")
    ...
    if args.setup is not None:
        cfg["SETUP_TYPE"] = args.setup
    if args.emafast is not None:
        cfg["EMA_FAST"] = args.emafast
    if args.ematrend is not None:
        cfg["EMA_TREND"] = args.ematrend
    if args.touchatr is not None:
        cfg["PULLBACK_TOUCH_ATR"] = args.touchatr
    if args.mombody is not None:
        cfg["MOMENTUM_BODY_ATR"] = args.mombody
    if args.pbmax is not None:
        cfg["PULLBACK_MAX_ATR"] = args.pbmax
    if args.rangelb is not None:
        cfg["RANGE_LOOKBACK"] = args.rangelb
    if args.rangeedge is not None:
        cfg["RANGE_EDGE_PCT"] = args.rangeedge
    ...
    if args.atrregime is not None:
        cfg["ATR_REGIME_FILTER"] = True
        cfg["ATR_REGIME_MULT"] = args.atrregime

    if not mt5.initialize():
        print(f"MT5 initialize ล้มเหลว: {mt5.last_error()}")
        return
    symbol = config.SYMBOL
    print(f"S26 backtest | Symbol={symbol} | TF=M1 | days={args.days} | spread=${args.spread:.2f} | "
          f"risk={cfg['RISK_PCT']}%/trade | setup={cfg['SETUP_TYPE']} | start_equity=${START_EQUITY:.0f}")
    print(f"cfg: ema_fast={cfg['EMA_FAST']} ema_trend={cfg['EMA_TREND']} touch_atr={cfg['PULLBACK_TOUCH_ATR']} "
          f"mom_body_atr={cfg['MOMENTUM_BODY_ATR']} pb_max_atr={cfg['PULLBACK_MAX_ATR']} "
          f"range_lb={cfg['RANGE_LOOKBACK']} range_edge={cfg['RANGE_EDGE_PCT']} "
          f"sl_mult={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']} session_filter={cfg['SESSION_FILTER']}")

    bars = fetch_bars(symbol, args.days)
    if bars is None:
    ...
    print(f"  M1: {len(bars)} bars ({t0} -> {t1} BKK)")
    raw = replay(bars, args.spread, cfg)
    ...
    if args.csv and trades_with_pnl:
        out_dir = os.path.join("excel_reports", "backtest_compare", "s26")
        os.makedirs(out_dir, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M")
        path = os.path.join(out_dir, f"sim_s26_{args.label}_{stamp}.csv")
