...
# I will find the ML scoring block and fix the indentation
...
# I want everything from "from datetime..." to the end of the if observable... to be indented inside "if sid_num >= 80:"

pattern = r'''        if getattr\(config, "ML_SCORING_ENABLED", False\):
            try:
                sid_num = int\(str\(sid\)\.replace\("S", ""\)\)
            except:
                sid_num = 0
...
rep = '''        if getattr(config, "ML_SCORING_ENABLED", False):
            try:
                sid_num = int(str(sid).replace("S", ""))
            except:
                sid_num = 0
...
with open('mt5_utils.py', 'w', encoding='utf-8') as f:
    f.write(content)
print('mt5_utils.py patched for ML indentation')
