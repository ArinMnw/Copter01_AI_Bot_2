"""
sim_s52_backtest.py — Backtest S52 (Quantified Pin Bar Reversal)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S51, ไม่ wire เข้า live
"""
...
import config
from strategy52 import S52_DEFAULTS, detect_s52
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S52_DEFAULTS[key]


def replay52(bars, htf_series, spread, cfg):
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s52(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay52(entry_bars, htf_series, spread, cfg)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s52_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "min_wick_body_ratio", "max_opposite_wick_ratio",
              "min_range_atr", "sl_atr_mult", "tp_rr", "confirmation_type", "risk_pct", "trades", "wr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "min_wick_body_ratio": cfg["MIN_WICK_BODY_RATIO"],
            "max_opposite_wick_ratio": cfg["MAX_OPPOSITE_WICK_RATIO"], "min_range_atr": cfg["MIN_RANGE_ATR"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S52_DEFAULTS["RISK_PCT"])
    ap.add_argument("--wickbody", type=float, default=None)
    ap.add_argument("--oppwick", type=float, default=None)
    ...
    cfg = dict(S52_DEFAULTS)
    cfg["RISK_PCT"] = args.risk
    if args.wickbody is not None:
        cfg["MIN_WICK_BODY_RATIO"] = args.wickbody
    if args.oppwick is not None:
        cfg["MAX_OPPOSITE_WICK_RATIO"] = args.oppwick
    ...
    print(f"S52 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | wickbody={cfg['MIN_WICK_BODY_RATIO']} "
          f"oppwick={cfg['MAX_OPPOSITE_WICK_RATIO']} minrange={cfg['MIN_RANGE_ATR']} sl={cfg['SL_ATR_MULT']} "
