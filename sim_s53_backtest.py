"""
sim_s53_backtest.py — Backtest S53 (Linear Regression Channel Bounce)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S52, ไม่ wire เข้า live
"""
...
import config
from strategy53 import S53_DEFAULTS, detect_s53
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S53_DEFAULTS[key]


def replay53(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    lookback = int(_cfg_get(cfg, "LOOKBACK_BARS"))
    win_size = lookback + 20
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s53(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay53(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=lookback + 100)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s53_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "lookback_bars", "std_mult", "touch_atr_mult",
              "reject_atr_mult", "sl_atr_mult", "tp_rr", "confirmation_type", "risk_pct", "trades", "wr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "lookback_bars": cfg["LOOKBACK_BARS"], "std_mult": cfg["STD_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S53_DEFAULTS["RISK_PCT"])
    ap.add_argument("--lookback", type=int, default=None)
    ap.add_argument("--stdmult", type=float, default=None)
    ...
    cfg = dict(S53_DEFAULTS)
    cfg["RISK_PCT"] = args.risk
    if args.lookback is not None:
        cfg["LOOKBACK_BARS"] = args.lookback
    if args.stdmult is not None:
        cfg["STD_MULT"] = args.stdmult
    ...
    print(f"S53 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | lookback={cfg['LOOKBACK_BARS']} "
          f"stdmult={cfg['STD_MULT']} touch={cfg['TOUCH_ATR_MULT']} reject={cfg['REJECT_ATR_MULT']} "
