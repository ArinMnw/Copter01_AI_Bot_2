...
from strategy263 import DEFAULT_CFG as S263_DEFAULT_CFG
from strategy263 import detect_s263
...
def detect_s264(rates, tf="", dt_bkk=None, cfg=None, **kwargs):
    """Fade the ordinal-predicted structural break that failed in S263."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    original = detect_s263(rates, tf, dt_bkk, c, **kwargs)
    ...
    return {
        ...
        "tp": tp,
        "order_type": "market",
        "pattern": f"S264 {signal} Bayesian Ordinal Break Fade {rr:g}R",
        "reason": (
            f"Fade ordinal-predicted break; original={original['signal']}"
        ),
        "be_rr": float(c["BE_RR"]),
