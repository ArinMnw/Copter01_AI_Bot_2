...
def run(cfg):
    ...
    for i in range(LOOKBACK, len(all_bars) - 2):
        ...
        sig = strategy109.detect_s109(rates_slice, tf=TF, dt_bkk=dt_bkk, cfg=cfg)
...
variants = [
    ("bf (ref)", {"PATTERNS_ENABLED": ("Butterfly",)}),
    ("bf+p4", {"PATTERNS_ENABLED": ("Butterfly",), "PIVOT_LEFT": 4, "PIVOT_RIGHT": 4}),
    ("bf+sl0.10", {"PATTERNS_ENABLED": ("Butterfly",), "SL_XA_PCT": 0.10}),
    ("bf+p4+sl0.10", {"PATTERNS_ENABLED": ("Butterfly",), "PIVOT_LEFT": 4, "PIVOT_RIGHT": 4, "SL_XA_PCT": 0.10}),
    ("bf+xa5", {"PATTERNS_ENABLED": ("Butterfly",), "MIN_XA_ATR": 5.0}),
    ("bf+p4+xa5", {"PATTERNS_ENABLED": ("Butterfly",), "PIVOT_LEFT": 4, "PIVOT_RIGHT": 4, "MIN_XA_ATR": 5.0}),
    ("bf+p4+sl0.10+xa5", {"PATTERNS_ENABLED": ("Butterfly",), "PIVOT_LEFT": 4, "PIVOT_RIGHT": 4, "SL_XA_PCT": 0.10, "MIN_XA_ATR": 5.0}),
    ("p4+sl0.10 (all)", {"PIVOT_LEFT": 4, "PIVOT_RIGHT": 4, "SL_XA_PCT": 0.10}),
    ("bf+tp0.5", {"PATTERNS_ENABLED": ("Butterfly",), "TP_AD_PCT": 0.5}),
]
