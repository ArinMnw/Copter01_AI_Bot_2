"""
sim_s46_backtest.py — Backtest S46 (Opening Range Breakout, ORB)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S45, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S46_DEFAULTS[key]


def replay46(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    win_size = 100  # ~8h ของ M5 ครอบคลุม OR + breakout window สบายๆ
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s46(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx,
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay46(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=150)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s46_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "or_session_start", "or_minutes",
              "max_breakout_age_min", "min_break_atr", "sl_atr_mult", "tp_rr", "confirmation_type",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "or_session_start": cfg["OR_SESSION_START"],
            "or_minutes": cfg["OR_MINUTES"], "max_breakout_age_min": cfg["MAX_BREAKOUT_AGE_MIN"],
            "min_break_atr": cfg["MIN_BREAK_ATR"], "sl_atr_mult": cfg["SL_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S46_DEFAULTS["RISK_PCT"])
    ap.add_argument("--orstart", default=None)
    ap.add_argument("--orminutes", type=int, default=None)
    ap.add_argument("--maxage", type=int, default=None)
    ap.add_argument("--minbreak", type=float, default=None)
    ...
    if args.orstart is not None:
        cfg["OR_SESSION_START"] = args.orstart
    if args.orminutes is not None:
        cfg["OR_MINUTES"] = args.orminutes
    if args.maxage is not None:
        cfg["MAX_BREAKOUT_AGE_MIN"] = args.maxage
    if args.minbreak is not None:
        cfg["MIN_BREAK_ATR"] = args.minbreak
    ...
    print(f"S46 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | orstart={cfg['OR_SESSION_START']} "
          f"orminutes={cfg['OR_MINUTES']} maxage={cfg['MAX_BREAKOUT_AGE_MIN']} minbreak={cfg['MIN_BREAK_ATR']} "
