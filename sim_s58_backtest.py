"""
sim_s58_backtest.py — Backtest S58 (Weekly-Open Reaction)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S57, ไม่ wire เข้า live
"""
...
import config
from strategy58 import S58_DEFAULTS, detect_s58
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S58_DEFAULTS[key]


def _build_week_opens(days):
    """fetch W1 bars แล้วคืน (sorted list ของ week_start_ts, dict {week_start_ts: open})"""
    weeks = days // 7 + 12
    w1 = mt5.copy_rates_from_pos(config.SYMBOL, mt5.TIMEFRAME_W1, 0, weeks)
    ...
    for b in w1:
        ts = int(b["time"])
        starts.append(ts)
        opens[ts] = float(b["open"])
    starts.sort()
    return starts, opens


def _cur_week_open(ts, week_starts, week_opens):
    """หา open ของสัปดาห์ปัจจุบัน ณ เวลา ts (W1 bar ที่ครอบ ts = start ล่าสุดที่ <= ts)"""
    idx = bisect.bisect_right(week_starts, ts) - 1
    if idx < 0:
        return None
    return week_opens[week_starts[idx]]


def replay58(bars, htf_series, spread, cfg, week_starts, week_hl):
    ...
    for j in range(start_j, n - 1):
        ...
        pwhl = _cur_week_open(ts, week_starts, week_hl)

        res = detect_s58(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx,
                          week_open=pwhl)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    week_starts, week_hl = _build_week_opens(days)
    if week_starts is None:
        return []
    return replay58(entry_bars, htf_series, spread, cfg, week_starts, week_hl)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s58_backtest_summary.csv")
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S58_DEFAULTS["RISK_PCT"])
    ...
    print(f"S58 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | touch={cfg['TOUCH_ATR_MULT']} "
