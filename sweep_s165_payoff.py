# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S165."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy165 import detect_s165
from sweep_s161_payoff import _summary


CASES = ((7.0, 0.75), (7.0, 1.0), (7.0, 1.25),
         (8.0, 1.0), (9.0, 1.0), (10.0, 1.0), (12.0, 1.0),
         (16.0, 1.0), (20.0, 1.0), (30.0, 1.0),
         (40.0, 1.0), (50.0, 1.0), (70.0, 1.0))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s165(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 165)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(165, args.months, "M5", 0.20, 0.01, end, lookback,
