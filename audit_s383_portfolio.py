# -*- coding: utf-8 -*-
"""Overlap, risk-distance, and incremental portfolio audit for S383."""
...
BASE_IDS = (346, 349, 351, 355, 358, 359, 361, 362, 363, 365, 366, 367,
            369, 370, 371, 372, 373, 374, 375, 376, 378, 379, 380, 382)
WINDOWS = (
...
def main():
    by_sid = {sid: [] for sid in (*BASE_IDS, 383)}
    h1_trades = {}
    risks = []
    with ProcessPoolExecutor(max_workers=len(WINDOWS)) as executor:
        window_results = executor.map(_run_window, WINDOWS)
        for name, result in window_results:
    ...
    target_times = {trade["signal_time"] for trade in h1_trades[383]}
    overlap = {
