# -*- coding: utf-8 -*-
"""S167 - Directional-efficiency volume hand-off SELL with a 7R target."""
...
def detect_s167(rates, tf, dt_bkk, cfg):
    """Sell a high-volume rejection after a low-volume pullback in an efficient decline."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
    ...
    if net_move > -atr * float(c["NET_MOVE_MIN_ATR"]):
        return _wait("Regime displacement is not bearish enough")
    if efficiency < float(c["EFFICIENCY_MIN"]):
        return _wait(f"Bearish path is too noisy (ER={efficiency:.2f})")
    ...
    if (rejection["close"] >= rejection["open"]
        ...
        return _wait("Pullback did not hand off to sellers")
    if (rejection["tick_volume"] < rejection_volume_min
        ...
        return _wait("Rejection volume does not confirm seller hand-off")
    ...
    return {
        ...
        "tp": tp,
        "order_type": "limit",
        "pattern": f"S167 SELL Efficiency Hand-off {rr:g}R",
        "reason": (f"Bearish ER={efficiency:.2f}, net={net_move / atr:.2f}ATR; "
