"""
sim_s50_backtest.py — Backtest S50 (Asian Range Liquidity Sweep / Judas Swing)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S49, ไม่ wire เข้า live
"""
...
import config
from strategy50 import S50_DEFAULTS, detect_s50
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S50_DEFAULTS[key]


def replay50(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    win_size = 150  # ~12.5h ของ M5 ครอบคลุม Asian range + Judas Swing window สบายๆ
    ...
    for j in range(start_j, n - 1):
        ...
        bar_dt_list = all_dt[lo:j + 1]
        dt_bkk = all_dt[j + 1]
        htf_ctx = s30sim.htf_lookup(htf_series, int(entry_bar["time"])) if conf_type != "none" else None

        res = detect_s50(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx,
                          bar_dt_list=bar_dt_list)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay50(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=200)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s50_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "asian_range_start", "asian_range_end",
              "sweep_session_start", "max_sweep_age_min", "sweep_atr_mult", "min_range_atr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "asian_range_start": cfg["ASIAN_RANGE_START"],
            "asian_range_end": cfg["ASIAN_RANGE_END"], "sweep_session_start": cfg["SWEEP_SESSION_START"],
            "max_sweep_age_min": cfg["MAX_SWEEP_AGE_MIN"], "sweep_atr_mult": cfg["SWEEP_ATR_MULT"],
            "min_range_atr": cfg["MIN_RANGE_ATR"], "sl_atr_mult": cfg["SL_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S50_DEFAULTS["RISK_PCT"])
    ap.add_argument("--astart", default=None)
    ap.add_argument("--aend", default=None)
    ap.add_argument("--sweepstart", default=None)
    ap.add_argument("--maxage", type=int, default=None)
    ap.add_argument("--sweepmult", type=float, default=None)
    ...
    cfg = dict(S50_DEFAULTS)
    cfg["RISK_PCT"] = args.risk
    if args.astart is not None:
        cfg["ASIAN_RANGE_START"] = args.astart
    if args.aend is not None:
        cfg["ASIAN_RANGE_END"] = args.aend
    if args.sweepstart is not None:
        cfg["SWEEP_SESSION_START"] = args.sweepstart
    if args.maxage is not None:
        cfg["MAX_SWEEP_AGE_MIN"] = args.maxage
    if args.sweepmult is not None:
        cfg["SWEEP_ATR_MULT"] = args.sweepmult
    ...
    print(f"S50 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | "
          f"asian=[{cfg['ASIAN_RANGE_START']},{cfg['ASIAN_RANGE_END']}] sweepstart={cfg['SWEEP_SESSION_START']} "
          f"maxage={cfg['MAX_SWEEP_AGE_MIN']} sweepmult={cfg['SWEEP_ATR_MULT']} minrange={cfg['MIN_RANGE_ATR']} "
