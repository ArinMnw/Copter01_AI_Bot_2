# -*- coding: utf-8 -*-
"""S217 - Rollover opening drive through a round institutional level, 10R."""
...
def detect_s217(rates, tf, dt_bkk, cfg):
    ...
    grid = float(c["ROUND_GRID"])
    if grid > 0.0:
        low_end = min(breakout["open"], breakout["close"])
        high_end = max(breakout["open"], breakout["close"])
        if math.floor(high_end / grid) * grid < low_end:
            return _wait("Drive body does not cross a round level")
    ...
    return {
        ...
        "tp": tp,
        "order_type": "market",
        "pattern": f"S217 {signal} Round-Level Drive {rr:g}R",
