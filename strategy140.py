# -*- coding: utf-8 -*-
"""S140 — Asymmetric downside RV continuation with a short 7R stop."""
...
def detect_s140(rates, tf, dt_bkk, cfg):
    """Keep only S119 downside expansion and compress risk to its signal bar."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
        ...
        max_risk = float(c["MAX_SHORT_RISK_ATR"])
    ...
    if result.get("signal") == "BUY":
        return _wait("S140 accepts only downside-volatility continuation")
    ...
    risk = sl - entry
    if risk <= 0.0 or risk > atr * max_risk:
    ...
    output.update({
        "sl": sl, "tp": tp,
        "pattern": f"S140 SELL Downside RV Continuation {rr:g}R",
        "reason": (f"Asymmetric downside clustering; {result.get('reason', '')}; "
