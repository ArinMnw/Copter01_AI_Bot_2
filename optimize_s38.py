...
import sim_s38_backtest as sim

mt5.initialize()
days = 60
cfg0 = dict(S38_DEFAULTS)
entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg0["ENTRY_TF"], days, extra_bars=cfg0["SWING_LOOKBACK_BARS"] + 100)
...
combos = list(itertools.product(
    [25, 40, 60],         # SWING_LOOKBACK_BARS
    [2.0, 3.0, 4.0],       # MIN_SWING_ATR
    [15, 20],              # MAX_RETRACE_AGE_BARS
    [0.8, 1.0],            # SL_ATR_MULT
    [1.0, 1.5, 2.0],       # TP_RR
))
...
t0 = time.time()
results = []
for i, (lb, ms, ma, sl, rr) in enumerate(combos):
    cfg = dict(S38_DEFAULTS)
    cfg.update(SWING_LOOKBACK_BARS=lb, MIN_SWING_ATR=ms, MAX_RETRACE_AGE_BARS=ma, SL_ATR_MULT=sl, TP_RR=rr)
    ...
    results.append((lb, ms, ma, sl, rr, s, c))
print(f"done in {time.time()-t0:.1f}s, {len(results)} valid")
mt5.shutdown()

results.sort(key=lambda r: r[6]["sharpe_like"], reverse=True)
print(f"\n{'lb':>4} {'minsw':>5} {'maxage':>6} {'sl':>4} {'rr':>4} {'n':>4} {'WR%':>5} "
      f"{'$/mo':>8} {'DD%':>6} {'PF':>5} {'posDay%':>8} {'streak':>7} {'sharpe':>7}")
for lb, ms, ma, sl, rr, s, c in results[:15]:
    print(f"{lb:>4} {ms:>5} {ma:>6} {sl:>4} {rr:>4} {s['trades']:>4} {s['wr']:>5.1f} "
