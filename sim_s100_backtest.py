...
parser = argparse.ArgumentParser(description="Backtest S100 MTF Trend + Micro-Sweep Pullback")
parser.add_argument("--days", type=int, default=90)
parser.add_argument("--start", type=str, help="YYYY-MM-DD")
parser.add_argument("--end", type=str, help="YYYY-MM-DD")
parser.add_argument("--fill-bars", type=int, default=12, help="Cancel limit if not filled within N bars")
parser.add_argument("--cooldown", type=int, default=10, help="Bars between trades")
parser.add_argument("--tf", type=str, default="M5")
parser.add_argument("--out-prefix", type=str, default="s100")
...
if args.disp_atr is not None:
    cfg_s100["DISP_BODY_ATR"] = args.disp_atr
if args.tp_rr is not None:
    cfg_s100["TP_RR"] = args.tp_rr
if args.retrace is not None:
    cfg_s100["ENTRY_RETRACE"] = args.retrace
if args.sl_buf is not None:
    cfg_s100["SL_BUF_ATR"] = args.sl_buf
if args.rsi_buy_min is not None:
    cfg_s100["RSI_BUY_MIN"] = args.rsi_buy_min
if args.rsi_sell_max is not None:
    cfg_s100["RSI_SELL_MAX"] = args.rsi_sell_max
if args.htf_span is not None:
    cfg_s100["HTF_EMA_SPAN"] = args.htf_span
if args.mtf_span is not None:
    cfg_s100["MTF_EMA_SPAN"] = args.mtf_span
if args.macd:
    cfg_s100["MACD_CONFIRM"] = True
if args.no_rsi_slope:
    cfg_s100["RSI_SLOPE_CONFIRM"] = False
if args.no_time_filter:
    cfg_s100["TIME_FILTER_ENABLED"] = False
...
for i in range(LOOKBACK, len(all_bars) - 2):
    ...
    sig = strategy100.detect_s100(rates_slice, tf=TF, dt_bkk=dt_bkk, cfg=cfg_s100)
