"""
sim_s29_backtest.py — Backtest S29 (entry-quality patterns + DD control บนฐาน S27 locked config)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — strategy29.py ไม่ถูก wire เข้า scanner.py/trailing.py/
   main.py ใดๆ ทั้งสิ้น ไฟล์นี้ไม่แก้ S1-S28, ไม่แก้ bot_state.json, ไม่แตะ live trading

กัน look-ahead bias 2 ชั้น (เหมือน sim_s27_backtest.py):
  1) ชั้น entry-TF: แท่งกำลังวิ่งใช้ close=open ของแท่งถัดไป, entry เป็น MARKET ที่ open ของ
     แท่งถัดจากแท่ง signal, แท่งเดียวกันแตะทั้ง TP/SL นับ SL (conservative)
  2) ชั้นข้าม timeframe: HTF context ใช้เฉพาะ "แท่ง HTF ที่ปิดสมบูรณ์แล้วก่อนเวลา entry-bar"
     เท่านั้น (bisect บน close_time ของ HTF series)

S29 เพิ่ม DD_CONTROL (ลำดับเวลาจริงของ trade stream ใน simulate_equity_v2):
  - "none"           : risk% คงที่
  - "dynamic_risk"    : ลด risk% เป็น REDUCED_RISK_PCT เมื่อแพ้ติดกัน >= CONSEC_LOSS_TRIGGER
                        ไม้ คืนเป็น RISK_PCT เดิมทันทีที่ชนะ 1 ไม้
  - "circuit_breaker" : ข้าม COOLDOWN_TRADES ไม้ถัดไป (ไม่เข้าเทรด) เมื่อแพ้ติดกัน
                        >= CONSEC_LOSS_TRIGGER ไม้

ตัวอย่าง:
  python sim_s29_backtest.py --days 30 --pattern engulfing --dd none
  python sim_s29_backtest.py --days 90 --pattern confluence --dd dynamic_risk --csv
"""
...
import config
from strategy29 import S29_DEFAULTS, detect_s29
...
def build_htf_series(htf_bars, cfg):
    """Precompute indicator บน HTF bars ทั้งชุด (กัน lookahead จริงทำใน htf_lookup ด้านล่าง)"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S29_DEFAULTS[key]
...
def replay(bars, htf_series, spread, cfg):
    ...
    confluence_bars = int(cfg.get("CONFLUENCE_BARS", S29_DEFAULTS["CONFLUENCE_BARS"]))
    win_size = ema_fast + 30 + confluence_bars
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s29(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def simulate_equity_v2(all_trades, cfg):
    """
    เดินตามลำดับเวลาจริง (sorted by fill_time) + DD_CONTROL:
      - none           : risk% = RISK_PCT คงที่ตลอด
      - dynamic_risk    : risk% ลดเป็น REDUCED_RISK_PCT เมื่อ consec_loss >= CONSEC_LOSS_TRIGGER
                          คืนเป็น RISK_PCT ทันทีที่ชนะ 1 ไม้ (reset streak)
      - circuit_breaker : ข้าม (ไม่เข้าเทรด ไม่เสีย risk) COOLDOWN_TRADES ไม้ถัดไป เมื่อ
                          consec_loss >= CONSEC_LOSS_TRIGGER แล้ว reset streak หลัง cooldown
    """
    dd_control = cfg.get("DD_CONTROL", "none")
    base_risk_pct = float(cfg["RISK_PCT"])
    trigger = int(cfg.get("CONSEC_LOSS_TRIGGER", S29_DEFAULTS["CONSEC_LOSS_TRIGGER"]))
    reduced_risk_pct = float(cfg.get("REDUCED_RISK_PCT", S29_DEFAULTS["REDUCED_RISK_PCT"]))
    cooldown_trades = int(cfg.get("COOLDOWN_TRADES", S29_DEFAULTS["COOLDOWN_TRADES"]))
    ...
    for t in ordered:
        ...
        row = dict(t)
        row["lot"] = lot
        row["risk_pct_used"] = round(risk_pct, 3)
...
def append_summary_csv(label, s, cfg, risk_pct):
    path = os.path.join(os.path.dirname(__file__), "s29_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = [
        "timestamp", "label", "entry_tf", "entry_pattern", "confirmation_type", "htf_tf",
        "dd_control", "trades", "wr",
        ...
        "avg_r_multiple", "profit_factor", "skipped_by_circuit_breaker",
        "ema_fast", "pullback_touch_atr", "engulf_min_ratio", "pinbar_wick_ratio",
        "confluence_bars", "sl_atr_mult", "tp_rr",
        "htf_ema_period", "htf_slope_bars", "adx_min_threshold",
        "consec_loss_trigger", "reduced_risk_pct", "cooldown_trades",
    ]
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        row = {
            ...
            "entry_pattern": cfg["ENTRY_PATTERN"],
            ...
            **{k: s[k] for k in (
            ...
            "engulf_min_ratio": cfg["ENGULF_MIN_RATIO"],
            "pinbar_wick_ratio": cfg["PINBAR_WICK_RATIO"],
            "confluence_bars": cfg["CONFLUENCE_BARS"],
            "sl_atr_mult": cfg["SL_ATR_MULT"],
            "tp_rr": cfg["TP_RR"],
            "htf_ema_period": cfg["HTF_EMA_PERIOD"],
            "htf_slope_bars": cfg["HTF_SLOPE_BARS"],
            "adx_min_threshold": cfg["ADX_MIN_THRESHOLD"],
            "consec_loss_trigger": cfg["CONSEC_LOSS_TRIGGER"],
            "reduced_risk_pct": cfg["REDUCED_RISK_PCT"],
            "cooldown_trades": cfg["COOLDOWN_TRADES"],
        }
        w.writerow(row)
...
def run_backtest(cfg, days, spread, label, save_csv=False, verbose=True, mt5_already_init=False):
    ...
    symbol = config.SYMBOL
    confluence_bars = int(cfg.get("CONFLUENCE_BARS", S29_DEFAULTS["CONFLUENCE_BARS"]))
    entry_bars = fetch_bars(symbol, cfg["ENTRY_TF"], days,
                             extra_bars=max(300, int(cfg["EMA_FAST"]) + 60 + confluence_bars))
    ...
    if verbose:
        ...
        print(f"  {cfg['ENTRY_TF']}: {len(entry_bars)} bars ({t0} -> {t1} BKK) | "
              f"pattern={cfg['ENTRY_PATTERN']} conf={cfg['CONFIRMATION_TYPE']} htf={cfg['HTF_TF']} "
              f"dd={cfg['DD_CONTROL']}")
    ...
    if save_csv and trades_with_pnl:
        out_dir = os.path.join("excel_reports", "backtest_compare", "s29")
        os.makedirs(out_dir, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M")
        path = os.path.join(out_dir, f"sim_s29_{label}_{stamp}.csv")
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S29_DEFAULTS["RISK_PCT"])
    ap.add_argument("--pattern", default=None,
                     choices=["ema_bounce", "engulfing", "pinbar", "confluence"])
    ...
    ap.add_argument("--pinbarratio", type=float, default=None)
    ap.add_argument("--confluencebars", type=int, default=None)
    ...
    ap.add_argument("--reducedrisk", type=float, default=None)
    ...
    if args.pinbarratio is not None:
        cfg["PINBAR_WICK_RATIO"] = args.pinbarratio
    if args.confluencebars is not None:
        cfg["CONFLUENCE_BARS"] = args.confluencebars
    ...
    if args.reducedrisk is not None:
        cfg["REDUCED_RISK_PCT"] = args.reducedrisk
    if args.cooldown is not None:
        cfg["COOLDOWN_TRADES"] = args.cooldown
    if args.nosession:
        cfg["SESSION_FILTER"] = False

    print(f"S29 backtest | Symbol={config.SYMBOL} | days={args.days} | spread=${args.spread:.2f} | "
          f"risk={cfg['RISK_PCT']}%/trade | entry_tf={cfg['ENTRY_TF']} pattern={cfg['ENTRY_PATTERN']} "
          f"conf={cfg['CONFIRMATION_TYPE']} htf={cfg['HTF_TF']} dd={cfg['DD_CONTROL']} | "
          f"start_equity=${START_EQUITY:.0f}")
