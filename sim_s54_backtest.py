"""
sim_s54_backtest.py — Backtest S54 (Floor Trader Pivot Points bounce)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S53, ไม่ wire เข้า live
"""
...
import config
from strategy54 import S54_DEFAULTS, detect_s54
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S54_DEFAULTS[key]


def replay54(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    win_size = 320  # >1 วันเต็มของ M5 (288 แท่ง/วัน) กันแท่งของ "เมื่อวาน" ตกขอบ window
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s54(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx,
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay54(entry_bars, htf_series, spread, cfg)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s54_backtest_summary.csv")
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S54_DEFAULTS["RISK_PCT"])
    ...
    print(f"S54 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | touch={cfg['TOUCH_ATR_MULT']} "
