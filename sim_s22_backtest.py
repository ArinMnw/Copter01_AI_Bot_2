"""
sim_s22_backtest.py — Backtest S22 Session-VWAP Mean-Reversion จากข้อมูล MT5 จริง
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — strategy22.py ไม่ถูก wire เข้า scanner.py/trailing.py/
   main.py ใดๆ ทั้งสิ้น ไฟล์นี้ไม่แก้ S1-S21, ไม่แก้ bot_state.json, ไม่แตะ live trading

เรียก strategy22.detect_s22() ตรง (pure function — logic เดียวกับที่จะใช้ถ้า wire จริง)

กัน look-ahead bias (เหมือน sim_s21_backtest.py):
  - แท่ง "กำลังวิ่ง" (rates[-1]) ใช้ open ของแท่งถัดจากแท่งสัญญาณ (ไม่รู้ high/low/close)
  - entry เป็น MARKET (fill ทันทีที่ open ของแท่งถัดไป — ไม่มี limit/retest)
...
Position sizing (risk-based, ทุนเริ่มต้น $1000, compounding ตาม equity ปัจจุบัน):
  - risk_usd ต่อไม้ = equity_ปัจจุบัน × S22_RISK_PCT / 100
  - lot = risk_usd / risk_distance(entry,sl) × 0.01   (XAUUSD: 0.01 lot = 1 oz
    → $1 เคลื่อนไหวราคา = $1 ต่อ 0.01 lot)
  - lot ถูก cap ด้วย margin guard (ไม่ให้ margin ที่ใช้เกิน S22_MAX_MARGIN_USAGE_PCT
    ของ equity ที่ leverage S22_ASSUMED_LEVERAGE) — กัน lot ใหญ่จน margin call/blow
    account แม้ risk per trade ดูเล็ก
  - ทุก trade ที่เปลี่ยน lot จะ log risk_usd และ risk_pct ไว้ใน CSV/summary เสมอ

ตัวอย่าง:
  python sim_s22_backtest.py --days 60 --tf M1,M5
  python sim_s22_backtest.py --days 60 --tf M5 --risk 1.0 --rr 0.8 --csv
"""
...
import config
from strategy22 import S22_DEFAULTS, detect_s22
...
def replay_tf(bars, tf_name, spread, cfg):
    ...
    lookback = int(cfg["VWAP_LOOKBACK_BARS"])
    atr_period = int(cfg["ATR_PERIOD"])
    win_size = lookback + atr_period + 30
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s22(window, tf=tf_name, dt_bkk=dt_bkk, cfg=cfg)
        sig = res.get("signal")
        if sig not in ("BUY", "SELL"):
            continue

        bar_time = int(res.get("signal_bar_time", 0))
        key = sig
        lt = last_fire.get(key, 0)
        if lt and (bar_time - lt) < cooldown_bars * _tf_secs(tf_name):
            continue
...
def append_summary_csv(label, s, cfg, risk_pct):
    path = os.path.join(os.path.dirname(__file__), "s22_backtest_summary.csv")
    ...
    fields = [
        ...
        "vwap_lookback", "dev_stdev_mult", "tp_rr", "sl_atr_mult",
        "session_filter",
    ]
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        row = {
            ...
            **{k: s[k] for k in (
            ...
            "vwap_lookback": cfg["VWAP_LOOKBACK_BARS"],
            "dev_stdev_mult": cfg["DEV_STDEV_MULT"],
            "tp_rr": cfg["TP_RR"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S22_DEFAULTS["RISK_PCT"], help="risk %% per trade")
    ...
    ap.add_argument("--dev", type=float, default=None, help="override DEV_STDEV_MULT")
    ap.add_argument("--lookback", type=int, default=None, help="override VWAP_LOOKBACK_BARS")
    ap.add_argument("--nosession", action="store_true", help="ปิด session filter (เทรดได้ทั้งวัน)")
    ap.add_argument("--label", default="baseline", help="label สำหรับ s22_backtest_summary.csv")
    ap.add_argument("--csv", action="store_true", help="เซฟ trade-level CSV ด้วย")
    args = ap.parse_args()

    cfg = dict(S22_DEFAULTS)
    ...
    if args.dev is not None:
        cfg["DEV_STDEV_MULT"] = args.dev
    if args.lookback is not None:
        cfg["VWAP_LOOKBACK_BARS"] = args.lookback
    if args.nosession:
    ...
    if not mt5.initialize():
        ...
        return
    symbol = config.SYMBOL
    print(f"S22 backtest | Symbol={symbol} | days={args.days} | spread=${args.spread:.2f} | "
          f"risk={cfg['RISK_PCT']}%/trade | start_equity=${START_EQUITY:.0f}")
    print(f"cfg: vwap_lookback={cfg['VWAP_LOOKBACK_BARS']} dev_mult={cfg['DEV_STDEV_MULT']} "
    ...
    if args.csv and trades_with_pnl:
        out_dir = os.path.join("excel_reports", "backtest_compare", "s22")
        os.makedirs(out_dir, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M")
        path = os.path.join(out_dir, f"sim_s22_{args.label}_{stamp}.csv")
