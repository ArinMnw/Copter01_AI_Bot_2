# -*- coding: utf-8 -*-
"""Parallel overlap, risk-distance, and portfolio audit for S388."""
...
def main():
    by_sid = {sid: [] for sid in (*BASE_IDS, 388)}
    h1_trades = {}
    risks = []
    ...
    target_times = {trade["signal_time"] for trade in h1_trades[388]}
    overlap = {
    ...
    after = before + by_sid[388]
    print("overlap", overlap)
    print("risk", {"min": min(risks), "median": median(risks), "max": max(risks)})
    print("s388_wf_h1", _stats(by_sid[388]))
    print("portfolio_before", _stats(before))
    for weight in (0.25, 0.50, 0.75):
        weighted = before + [
            (stamp, profit * weight) for stamp, profit in by_sid[388]
        ]
