"""
sim_s43_backtest.py — Backtest S43 (Turtle Trading — Donchian channel breakout)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S42, ไม่ wire เข้า live
"""
...
import config
from strategy43 import S43_DEFAULTS, detect_s43
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S43_DEFAULTS[key]


def replay43(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    n_entry = int(_cfg_get(cfg, "DONCHIAN_ENTRY_BARS"))
    win_size = n_entry + 30
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s43(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay43(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    n_entry = int(cfg["DONCHIAN_ENTRY_BARS"])
    entry_bars = s30sim.fetch_bars(symbol, cfg["ENTRY_TF"], days, extra_bars=n_entry + 80)
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s43_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "donchian_entry_bars", "min_break_atr",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "donchian_entry_bars": cfg["DONCHIAN_ENTRY_BARS"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S43_DEFAULTS["RISK_PCT"])
    ...
    if args.donchian is not None:
        cfg["DONCHIAN_ENTRY_BARS"] = args.donchian
    ...
    print(f"S43 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | donchian={cfg['DONCHIAN_ENTRY_BARS']} "
          f"minbreak={cfg['MIN_BREAK_ATR']} sl={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']} conf={cfg['CONFIRMATION_TYPE']}")
