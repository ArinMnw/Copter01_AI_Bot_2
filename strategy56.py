"""
strategy56.py — S56 Previous-Week High/Low Bounce, RESEARCH/BACKTEST-ONLY
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ standalone 100% — ไม่ถูก import โดย scanner.py/trailing.py/main.py, ไม่มี wiring เข้า live

แนวคิด: high/low ของ "สัปดาห์ก่อนหน้า" (previous completed week, W1 bar) เป็นแนวรับ/ต้านระดับ
...
weekly H/L เป็น endogenous level ที่ยังไม่เคยลอง เข้า BUY เมื่อราคาแตะ prev-week low แล้ว reject
กลับขึ้น, SELL เมื่อแตะ prev-week high แล้ว reject กลับลง ต่อทิศ htf_trend
"""
...
def detect_s56(rates, tf: str = "", dt_bkk=None, cfg: dict | None = None, htf_ctx: dict | None = None,
               prev_week_hl=None):
    """
    prev_week_hl: tuple (pwh, pwl) = high/low ของสัปดาห์ก่อนหน้า (ต้องส่งมาจาก replay เพราะคำนวณจาก
    W1 bars ซึ่งอยู่นอกไฟล์นี้)
    """
    if rates is None or len(rates) < 30:
        return {"signal": "WAIT", "reason": "S56: ข้อมูลไม่พอ"}
    if prev_week_hl is None:
        return {"signal": "WAIT", "reason": "S56: ไม่มีระดับสัปดาห์ก่อน"}
    if not _in_session(dt_bkk, cfg):
        return {"signal": "WAIT", "reason": "S56: นอก session"}

    closed = rates[:-1]
    atr = _calc_atr(closed, 14)
    if not atr or atr <= 0:
        return {"signal": "WAIT", "reason": "S56: ATR ไม่ได้"}

    pwh, pwl = prev_week_hl
    if pwh is None or pwl is None or pwh <= pwl:
        return {"signal": "WAIT", "reason": "S56: ระดับสัปดาห์ผิด"}
    ...
    if direction == "BUY":
        ...
        if not (0 < risk <= max_risk_mult * atr and tp > entry):
            return {"signal": "WAIT", "reason": "S56: risk ผิดปกติ"}
    else:
        risk = sl - entry
        tp = round(entry - rr * risk, 2)
        if not (0 < risk <= max_risk_mult * atr and tp < entry):
    ...
    return {
        ...
        "pattern": f"ท่าที่ 56 PrevWeekHL+{conf_type} {'🟢 BUY' if direction == 'BUY' else '🔴 SELL'}",
        "reason": f"PrevWeek H/L=[{pwl:.2f},{pwh:.2f}] level={level_hit:.2f}\n"
