...
LEAN = list("bcdfghikmnpqr")  # 13-way legs
...
d1 = mt5.copy_rates_from_pos(config.SYMBOL, mt5.TIMEFRAME_D1, 0, days+10)
day_trend = {}
for b in d1:
    rng = float(b['high'])-float(b['low'])
    if rng>0:
        key = config.mt5_ts_to_bkk(int(b['time'])).date().isoformat()
        day_trend[key] = abs(float(b['close'])-float(b['open']))/rng
vals = sorted(day_trend.values())
lo_th = np.percentile(vals,33); hi_th = np.percentile(vals,67)
def regime(d):
    t = day_trend.get(d)
    if t is None: return None
    return "TREND" if t>=hi_th else ("RANGE" if t<=lo_th else "MID")
...
nR=sum(1 for v in day_trend.values() if v<=lo_th); nT=sum(1 for v in day_trend.values() if v>=hi_th)
print(f"regime days: RANGE~{nR} TREND~{nT} (thresholds lo={lo_th:.2f} hi={hi_th:.2f})")
print(f"{'leg':<12} {'RANGE$/d':>9} {'TREND$/d':>9}  bias")
for kk in LEAN:
    ds = days_series[kk]
    R=[]; T=[]
    ...
    bias = "range-fav" if mr>mt_ else "trend-fav"
    print(f"{names[kk]:<12} {mr:>9.2f} {mt_:>9.2f}  {bias} (R-T={mr-mt_:+.1f})")
mt5.shutdown()
