...
t_l = {t["signal_time_ts"] for t in raws["l"]}
print(f"L(OrderBlock)={len(t_l)}")
for kk in "abcdefghik":
    t_kk = {t["signal_time_ts"] for t in raws[kk]}
    ov = len(t_kk & t_l)
    pct = 100*ov/len(t_l) if t_l else 0
    print(f"overlap {kk.upper()}&L: {ov} ({pct:.1f}% of L)")
...
for t in raws["l"]:
    ...
    if not ok:
        bad += 1
print(f"\nbad trades total: {bad}/{len(raws['l'])}")
mt5.shutdown()
