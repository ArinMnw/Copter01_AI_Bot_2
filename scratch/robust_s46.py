...
cfg.update(OR_SESSION_START="14:00", OR_MINUTES=30, MAX_BREAKOUT_AGE_MIN=90, MIN_BREAK_ATR=0.1,
           SL_ATR_MULT=0.8, TP_RR=1.5)

print(f"config: ost=14:00 om=30 ma=90 mb=0.1 sl=0.8 rr=1.5")
print(f"{'days':>5} {'n':>5} {'WR%':>5} {'$/mo':>8} {'DD%':>6} {'PF':>5} {'posDay%':>8} {'streak':>7} {'sharpe':>7}")
for days in [30, 45, 60, 90, 120, 150, 180]:
    entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg["ENTRY_TF"], days, extra_bars=150)
