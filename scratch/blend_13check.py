...
FULL = list("abcdefghiklmnpqr")
LEAN = [x for x in FULL if x not in "ela"]  # 13-way: drop S38(e), S45(l), S31(a)
for days in [60, 90, 120, 150, 180]:
    ...
    fmo, fc = combine(FULL); lmo, lc = combine(LEAN)
    print(f"--- {days}d ---")
    print(f"  16-way: $/mo={fmo:7.0f} posDay={fc['pct_pos_days']:5.1f}% maxStreak={fc['max_losing_day_streak']:2}d sharpe={fc['sharpe_like']:.3f}")
    print(f"  13-way: $/mo={lmo:7.0f} posDay={lc['pct_pos_days']:5.1f}% maxStreak={lc['max_losing_day_streak']:2}d sharpe={lc['sharpe_like']:.3f}")
mt5.shutdown()
