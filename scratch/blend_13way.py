...
sims = {"a": s31sim, "b": s34sim, "c": s36sim, "d": s37sim, "e": s38sim, "f": s39sim, "g": s40sim,
        "h": s41sim, "i": s42sim, "k": s44sim, "l": s45sim, "m": s46sim, "n": s47sim}
cfgs = {"a": cfg_a, "b": cfg_b, "c": cfg_c, "d": cfg_d, "e": cfg_e, "f": cfg_f, "g": cfg_g,
        "h": cfg_h, "i": cfg_i, "k": cfg_k, "l": cfg_l, "m": cfg_m, "n": cfg_n}
...
for days in [60, 90, 120, 150, 180]:
    ...
    total_12, c_12 = combine(list("abcdefghiklm"))
    total_13, c_13 = combine(list("abcdefghiklmn"))

    print(f"--- {days}d ---")
    print(f"  12-way เดิม: $/mo={total_12/days*30:7.1f} posDay={c_12['pct_pos_days']:5.1f}% "
          f"maxStreak={c_12['max_losing_day_streak']:2}d sharpe={c_12['sharpe_like']:6.3f}")
    print(f"  13-way (+N): $/mo={total_13/days*30:7.1f} posDay={c_13['pct_pos_days']:5.1f}% "
