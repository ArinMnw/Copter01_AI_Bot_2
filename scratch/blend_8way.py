...
sims = {"a": s31sim, "b": s34sim, "c": s36sim, "d": s37sim, "e": s38sim, "f": s39sim, "g": s40sim, "h": s41sim}
cfgs = {"a": cfg_a, "b": cfg_b, "c": cfg_c, "d": cfg_d, "e": cfg_e, "f": cfg_f, "g": cfg_g, "h": cfg_h}
...
for days in [60, 90, 120, 150, 180]:
    ...
    for k, s in sims.items():
        raw = s.run_single(entry_bars, htf_bars, cfgs[k], days, 0.20)
    ...
    total_7, c_7 = combine(list("abcdefg"))
    total_8, c_8 = combine(list("abcdefgh"))

    print(f"--- {days}d ---")
    print(f"  7-way เดิม: $/mo={total_7/days*30:7.1f} posDay={c_7['pct_pos_days']:5.1f}% "
          f"maxStreak={c_7['max_losing_day_streak']:2}d sharpe={c_7['sharpe_like']:6.3f}")
    print(f"  8-way ใหม่: $/mo={total_8/days*30:7.1f} posDay={c_8['pct_pos_days']:5.1f}% "
