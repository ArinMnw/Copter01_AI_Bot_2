...
variants = {
  "16-way FULL":      ALL,
  "drop e(S38fib)":   [x for x in ALL if x not in "e"],
  "drop e,l(45OB)":   [x for x in ALL if x not in "el"],
  "drop e,l,a(31)":   [x for x in ALL if x not in "ela"],
  "drop e,l,a,g(40)": [x for x in ALL if x not in "elag"],
  "drop 5drags+m(46)":[x for x in ALL if x not in "elagm"],
}
import statistics as st
res = {v: {"mo":[], "sh":[], "pd":[], "ms":[]} for v in variants}
for days in [90, 150, 180]:
    ...
    def combine(keys):
        ...
        return sum(comb.values())/days*30, c
    for v, keys in variants.items():
        mo, c = combine(keys)
        res[v]["mo"].append(mo); res[v]["sh"].append(c["sharpe_like"])
        res[v]["pd"].append(c["pct_pos_days"]); res[v]["ms"].append(c["max_losing_day_streak"])

print(f"{'variant':<20} {'n_legs':>6} {'avg$/mo':>9} {'avgSharpe':>10} {'avgPosDay':>10} {'avgMaxStreak':>12}")
for v, keys in variants.items():
    r = res[v]
    print(f"{v:<20} {len(keys):>6} {st.mean(r['mo']):>9.0f} {st.mean(r['sh']):>10.4f} {st.mean(r['pd']):>9.1f}% {st.mean(r['ms']):>12.1f}")
mt5.shutdown()
