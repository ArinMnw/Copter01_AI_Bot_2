...
cfgs = {"a": cfg_a, "b": cfg_b, "c": cfg_c, "d": cfg_d, "e": cfg_e, "f": cfg_f, "g": cfg_g,
        ...
        "r": cfg_r}
import numpy as np
LEAN = list("bcdfghikmnpqr")
names={"b":"S34volbrk","c":"S36FVG","d":"S37 S/R","f":"S39zone","g":"S40elliott","h":"S41rsi","i":"S42crt","k":"S44volprof","m":"S46 ORB","n":"S47supertr","p":"S49vwap","q":"S51pdhl","r":"S56 wkHL"}
days=180
...
# CAUSAL vol regime: prior 5-day avg range, known at day open (vol clusters -> persists)
causal={}
for i in range(6,len(d1)):
    causal[dates[i]]=np.mean(ranges[i-5:i])
cv=sorted(causal.values()); med=np.percentile(cv,50)
# autocorrelation check: does prior-5d vol predict today's range?
rng_by_date={dates[i]:ranges[i] for i in range(len(d1))}
xs=[];ys=[]
for d,pv in causal.items():
    if d in rng_by_date: xs.append(pv); ys.append(rng_by_date[d])
print(f"vol persistence: corr(prior5d_vol, today_range)={np.corrcoef(xs,ys)[0,1]:.3f}")
def vreg(d):
    t=causal.get(d); return None if t is None else ("HIVOL" if t>=med else "LOVOL")
ds_all={}
for kk in sims:
    raw=sims[kk].run_single(eb,hb,cfgs[kk],days,0.20); twp,eq=s31sim.simulate_equity_substream(raw,cfgs[kk],s31sim.START_EQUITY)
    ds_all[kk]=s31sim.daily_series_from_trades(twp)
print(f"{'leg':<12} {'LOVOL$/d':>9} {'HIVOL$/d':>9}  bias")
for kk in LEAN:
    L=[];H=[]
    for d,pnl in ds_all[kk].items():
        r=vreg(d)
        if r=="LOVOL":L.append(pnl)
        elif r=="HIVOL":H.append(pnl)
    ml=np.mean(L) if L else 0; mh=np.mean(H) if H else 0
    print(f"{names[kk]:<12} {ml:>9.2f} {mh:>9.2f}  {'lovol-fav' if ml>mh else 'hivol-fav'} ({ml-mh:+.1f})")
mt5.shutdown()
