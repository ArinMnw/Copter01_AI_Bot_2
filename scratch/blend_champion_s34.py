...
for days in [60, 90, 120, 150, 180]:
    ...
    twp_c, eq_c = s31sim.simulate_equity_substream(raw_champ, cfg_champ, s31sim.START_EQUITY / 2)
    twp_s, eq_s = s31sim.simulate_equity_substream(raw_s34, cfg_s34, s31sim.START_EQUITY / 2)
    ...
    # baseline เทียบ: champion เต็มทุน $1000 (ไม่แบ่ง)
    twp_full, eq_full = s31sim.simulate_equity_substream(raw_champ, cfg_champ, s31sim.START_EQUITY)
    day_full = s31sim.daily_series_from_trades(twp_full)
    c_full = s31sim.consistency_metrics(day_full)
    total_full = sum(day_full.values())

    print(f"--- {days}d ---")
    print(f"  BLEND (champ$500+S34$500): $/mo={total/days*30:7.1f} posDay={c['pct_pos_days']:5.1f}% "
          f"maxStreak={c['max_losing_day_streak']:2}d sharpe={c['sharpe_like']:6.3f} final=${final_eq:.2f}")
    print(f"  CHAMPION-only ($1000)    : $/mo={total_full/days*30:7.1f} posDay={c_full['pct_pos_days']:5.1f}% "
          f"maxStreak={c_full['max_losing_day_streak']:2}d sharpe={c_full['sharpe_like']:6.3f} final=${eq_full['final_equity']:.2f}")
mt5.shutdown()
