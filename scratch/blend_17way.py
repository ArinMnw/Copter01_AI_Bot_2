...
for days in [60, 90, 120, 150, 180]:
    ...
    total_15, c_15 = combine(list("abcdefghiklmnpq"))
    total_16, c_16 = combine(list("abcdefghiklmnpqr"))

    print(f"--- {days}d ---")
    print(f"  15-way เดิม: $/mo={total_15/days*30:7.1f} posDay={c_15['pct_pos_days']:5.1f}% "
          f"maxStreak={c_15['max_losing_day_streak']:2}d sharpe={c_15['sharpe_like']:6.3f}")
    print(f"  16-way (+R): $/mo={total_16/days*30:7.1f} posDay={c_16['pct_pos_days']:5.1f}% "
          f"maxStreak={c_16['max_losing_day_streak']:2}d sharpe={c_16['sharpe_like']:6.3f}")
mt5.shutdown()
