...
raws = {k: sims[k].run_single(entry_bars, htf_bars, cfgs[k], days, 0.20) for k in sims}
t_i = {t["signal_time_ts"] for t in raws["i"]}
print(f"I(CRT)={len(t_i)}")
for k in "abcdefgh":
    t_k = {t["signal_time_ts"] for t in raws[k]}
    ov = len(t_k & t_i)
    pct = 100*ov/len(t_i) if t_i else 0
    print(f"overlap {k.upper()}&I: {ov} ({pct:.1f}% of I)")
...
for t in raws["i"]:
    ...
    if not ok:
        bad += 1
print(f"\nbad trades total: {bad}/{len(raws['i'])}")
mt5.shutdown()
