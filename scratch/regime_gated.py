...
BASE=list("bcdfghikmnpqr")  # 13-way
# regime-favorability from causal vol test:
DROP_IN_HIVOL=["k","d"]   # S44, S37 (strong lovol-fav) -> turn OFF on high-vol days
DROP_IN_LOVOL=["m","n"]   # S46 ORB, S47 supertr (strong hivol-fav) -> turn OFF on low-vol days
for days in [60,90,120,150,180]:
    eb=s30sim.fetch_bars(config.SYMBOL,"M5",days,extra_bars=600); hb=s30sim.fetch_bars(config.SYMBOL,"M15",days,extra_bars=200)
    d1=mt5.copy_rates_from_pos(config.SYMBOL,mt5.TIMEFRAME_D1,0,days+40)
    dates=[config.mt5_ts_to_bkk(int(b['time'])).date().isoformat() for b in d1]
    ranges=[float(b['high'])-float(b['low']) for b in d1]
    causal={}
    for i in range(6,len(d1)): causal[dates[i]]=np.mean(ranges[i-5:i])
    med=np.percentile(sorted(causal.values()),50)
    def hi(d):
        t=causal.get(d); return None if t is None else t>=med
    ds={}
    for kk in sims:
        raw=sims[kk].run_single(eb,hb,cfgs[kk],days,0.20); twp,eq=s31sim.simulate_equity_substream(raw,cfgs[kk],s31sim.START_EQUITY)
        ds[kk]=s31sim.daily_series_from_trades(twp)
    alldays=set()
    for kk in BASE: alldays|=set(ds[kk])
    # always-on 13-way
    base_daily={d:sum(ds[kk].get(d,0.0) for kk in BASE) for d in alldays}
    # gated
    gated_daily={}
    for d in alldays:
        h=hi(d)
        active=list(BASE)
        if h is True:
            active=[k for k in active if k not in DROP_IN_HIVOL]
        elif h is False:
            active=[k for k in active if k not in DROP_IN_LOVOL]
        gated_daily[d]=sum(ds[kk].get(d,0.0) for kk in active)
    cb=s31sim.consistency_metrics(base_daily); cg=s31sim.consistency_metrics(gated_daily)
    print(f"--- {days}d ---")
    print(f"  13-way always-on: $/mo={sum(base_daily.values())/days*30:7.0f} sharpe={cb['sharpe_like']:.3f} maxStreak={cb['max_losing_day_streak']}d posDay={cb['pct_pos_days']:.1f}%")
    print(f"  13-way vol-gated : $/mo={sum(gated_daily.values())/days*30:7.0f} sharpe={cg['sharpe_like']:.3f} maxStreak={cg['max_losing_day_streak']}d posDay={cg['pct_pos_days']:.1f}%")
mt5.shutdown()
