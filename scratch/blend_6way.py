...
for days in [60, 90, 120, 150, 180]:
    ...
    twp_e, eq_e = s31sim.simulate_equity_substream(raw_e, cfg_e, s31sim.START_EQUITY)
    twp_f, eq_f = s31sim.simulate_equity_substream(raw_f, cfg_f, s31sim.START_EQUITY)
    ...
    day_e = s31sim.daily_series_from_trades(twp_e)
    day_f = s31sim.daily_series_from_trades(twp_f)

    days_5 = set(day_a) | set(day_b) | set(day_c) | set(day_d) | set(day_e)
    comb_5 = {d: day_a.get(d, 0.0) + day_b.get(d, 0.0) + day_c.get(d, 0.0) + day_d.get(d, 0.0) + day_e.get(d, 0.0) for d in days_5}
    c_5 = s31sim.consistency_metrics(comb_5)
    total_5 = sum(comb_5.values())

    days_6 = days_5 | set(day_f)
    comb_6 = {d: comb_5.get(d, 0.0) + day_f.get(d, 0.0) for d in days_6}
    c_6 = s31sim.consistency_metrics(comb_6)
    total_6 = sum(comb_6.values())

    print(f"--- {days}d ---")
    print(f"  A+B+C+D+E (5-way เดิม): $/mo={total_5/days*30:7.1f} posDay={c_5['pct_pos_days']:5.1f}% "
          f"maxStreak={c_5['max_losing_day_streak']:2}d sharpe={c_5['sharpe_like']:6.3f}")
    print(f"  A+B+C+D+E+F (6-way)    : $/mo={total_6/days*30:7.1f} posDay={c_6['pct_pos_days']:5.1f}% "
