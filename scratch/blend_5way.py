...
for days in [60, 90, 120, 150, 180]:
    ...
    days_abcde = set(day_a) | set(day_b) | set(day_c) | set(day_d) | set(day_e)
    comb_abcde = {d: day_a.get(d, 0.0) + day_b.get(d, 0.0) + day_c.get(d, 0.0) + day_d.get(d, 0.0) + day_e.get(d, 0.0) for d in days_abcde}
    c_abcde = s31sim.consistency_metrics(comb_abcde)
    total_abcde = sum(comb_abcde.values())

    print(f"--- {days}d ---")
    print(f"  A+B+C+D (4-way เดิม)  : $/mo={total_abcd/days*30:7.1f} posDay={c_abcd['pct_pos_days']:5.1f}% "
          f"maxStreak={c_abcd['max_losing_day_streak']:2}d sharpe={c_abcd['sharpe_like']:6.3f}")
    print(f"  A+B+C+D+E (5-way)     : $/mo={total_abcde/days*30:7.1f} posDay={c_abcde['pct_pos_days']:5.1f}% "
          f"maxStreak={c_abcde['max_losing_day_streak']:2}d sharpe={c_abcde['sharpe_like']:6.3f}")
mt5.shutdown()
