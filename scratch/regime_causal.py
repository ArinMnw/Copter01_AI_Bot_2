...
names = {"b":"S34volbrk","c":"S36FVG","d":"S37 S/R","f":"S39zone","g":"S40elliott","h":"S41rsi",
         "i":"S42crt","k":"S44volprof","m":"S46 ORB","n":"S47supertr","p":"S49vwap","q":"S51pdhl","r":"S56 wkHL"}
days=180
...
# CAUSAL regime: daily ADX-like (|EMA20 slope|) + dist from EMA20, known at day START (use prior day close)
d1=mt5.copy_rates_from_pos(config.SYMBOL,mt5.TIMEFRAME_D1,0,days+40)
closes=[float(b['close']) for b in d1]; dates=[config.mt5_ts_to_bkk(int(b['time'])).date().isoformat() for b in d1]
# EMA20 of daily close
ema=[]; k=2/21
e=closes[0]
for c in closes: e=c*k+e*(1-k); ema.append(e)
# causal trend strength at day i = |close[i-1]-ema[i-1]|/ATR-ish(prior 14d range), known before day i
ranges=[float(b['high'])-float(b['low']) for b in d1]
causal={}
for i in range(20,len(d1)):
    atr14=np.mean(ranges[i-14:i])
    if atr14>0:
        causal[dates[i]]=abs(closes[i-1]-ema[i-1])/atr14  # prior-day dist from EMA in ATR units
cv=sorted(causal.values()); lo=np.percentile(cv,50)
def creg(d):
    t=causal.get(d)
    return None if t is None else ("TREND" if t>=lo else "RANGE")
# also test persistence: does prior-day trendiness predict?
days_series={}
for kk in sims:
...
nT=sum(1 for v in causal.values() if v>=lo); nR=len(causal)-nT
print(f"CAUSAL regime (prior-day dist-from-EMA20, known at day open): TREND~{nT} RANGE~{nR}")
print(f"{'leg':<12} {'cRANGE$/d':>10} {'cTREND$/d':>10}  bias")
for kk in LEAN:
    R=[];T=[]
    for d,pnl in days_series[kk].items():
        r=creg(d)
        if r=="RANGE":R.append(pnl)
        elif r=="TREND":T.append(pnl)
    mr=np.mean(R) if R else 0; mt_=np.mean(T) if T else 0
    print(f"{names[kk]:<12} {mr:>10.2f} {mt_:>10.2f}  {'range-fav' if mr>mt_ else 'trend-fav'} ({mr-mt_:+.1f})")
mt5.shutdown()
