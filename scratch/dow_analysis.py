...
blend_daily={d: sum(ds[kk].get(d,0.0) for kk in LEAN) for d in alldays}
by_dow = {i: [] for i in range(7)}
for d, pnl in blend_daily.items():
    dow = datetime.strptime(d, "%Y-%m-%d").weekday()
    by_dow[dow].append(pnl)
print(f"13-way blend PnL by day-of-week ({len(blend_daily)} trading days over {days}d):")
print(f"{'day':<5} {'n':>4} {'sum$':>9} {'mean$':>8} {'posDay%':>8}")
for i in range(7):
    v = by_dow[i]
    if not v: continue
    pos = 100*sum(1 for x in v if x>0)/len(v)
    print(f"{DOW_NAMES[i]:<5} {len(v):>4} {sum(v):>9.1f} {np.mean(v):>8.2f} {pos:>7.1f}%")
mt5.shutdown()
