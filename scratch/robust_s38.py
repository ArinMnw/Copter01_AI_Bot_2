...
cfg.update(SWING_LOOKBACK_BARS=25, MIN_SWING_ATR=3.0, MAX_RETRACE_AGE_BARS=20, SL_ATR_MULT=1.0, TP_RR=1.0)

print(f"config: lb=25 minsw=3.0 maxage=20 sl=1.0 rr=1.0")
print(f"{'days':>5} {'n':>5} {'WR%':>5} {'$/mo':>8} {'DD%':>6} {'PF':>5} {'posDay%':>8} {'streak':>7} {'sharpe':>7}")
for days in [30, 45, 60, 90, 120, 150, 180]:
    entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg["ENTRY_TF"], days, extra_bars=cfg["SWING_LOOKBACK_BARS"] + 100)
