...
cfgs = {"a": cfg_a, "b": cfg_b, "c": cfg_c, "d": cfg_d, "e": cfg_e, "f": cfg_f, "g": cfg_g,
        ...
        "r": cfg_r}
import numpy as np
BASE=list("bcdfghikmnpqr")
DROP_IN_HIVOL=["k","d"]; DROP_IN_LOVOL=["m","n"]
for days in [90,150,180]:
    ...
    causal={}
    for i in range(6,len(d1)): causal[dates[i]]=float(np.mean(ranges[i-5:i]))
    med=float(np.percentile(sorted(causal.values()),50))
    ds={}
    for kk in sims:
    ...
    base_daily={}; gated_daily={}; nhi=0;nlo=0;nnone=0
    for d in alldays:
        base_daily[d]=sum(ds[kk].get(d,0.0) for kk in BASE)
        t=causal.get(d)
        if t is None: nnone+=1; active=BASE
        elif t>=med: nhi+=1; active=[k for k in BASE if k not in DROP_IN_HIVOL]
        else: nlo+=1; active=[k for k in BASE if k not in DROP_IN_LOVOL]
    ...
    print(f"--- {days}d (gated: hi={nhi} lo={nlo} none={nnone}) ---")
    print(f"  always-on: $/mo={sum(base_daily.values())/days*30:7.0f} sharpe={cb['sharpe_like']:.3f}")
    print(f"  vol-gated: $/mo={sum(gated_daily.values())/days*30:7.0f} sharpe={cg['sharpe_like']:.3f}")
mt5.shutdown()
