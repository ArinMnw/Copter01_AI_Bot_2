...
import sim_s43_backtest as s43sim
...
cfg_j = dict(S43_DEFAULTS); cfg_j.update(DONCHIAN_ENTRY_BARS=80, MIN_BREAK_ATR=0.25, SL_ATR_MULT=2.0,
                                          TP_RR=2.0, CONFIRMATION_TYPE="htf_trend")

sims = {"a": s31sim, "b": s34sim, "c": s36sim, "d": s37sim, "e": s38sim, "f": s39sim, "g": s40sim,
        "h": s41sim, "i": s42sim, "j": s43sim}
cfgs = {"a": cfg_a, "b": cfg_b, "c": cfg_c, "d": cfg_d, "e": cfg_e, "f": cfg_f, "g": cfg_g,
        "h": cfg_h, "i": cfg_i, "j": cfg_j}
...
for days in [60, 90, 120, 150, 180]:
    ...
    total_10, c_10 = combine(list("abcdefghij"))
    ...
    print(f"  10-way (+J): $/mo={total_10/days*30:7.1f} posDay={c_10['pct_pos_days']:5.1f}% "
