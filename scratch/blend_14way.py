...
from strategy48 import S48_DEFAULTS
...
import sim_s48_backtest as s48sim
...
cfg_o = dict(S48_DEFAULTS); cfg_o.update(MACD_FAST=8, MACD_SLOW=21, MACD_SIGNAL=5, SL_ATR_MULT=1.5,
                                          TP_RR=1.0, CONFIRMATION_TYPE="none", MIN_HIST_ATR=0.1,
                                          SESSION_FILTER=False)

sims = {"a": s31sim, "b": s34sim, "c": s36sim, "d": s37sim, "e": s38sim, "f": s39sim, "g": s40sim,
        "h": s41sim, "i": s42sim, "k": s44sim, "l": s45sim, "m": s46sim, "n": s47sim, "o": s48sim}
cfgs = {"a": cfg_a, "b": cfg_b, "c": cfg_c, "d": cfg_d, "e": cfg_e, "f": cfg_f, "g": cfg_g,
        "h": cfg_h, "i": cfg_i, "k": cfg_k, "l": cfg_l, "m": cfg_m, "n": cfg_n, "o": cfg_o}
...
for days in [60, 90, 120, 150, 180]:
    ...
    total_13, c_13 = combine(list("abcdefghiklmn"))
    total_14, c_14 = combine(list("abcdefghiklmno"))

    print(f"--- {days}d ---")
    print(f"  13-way เดิม: $/mo={total_13/days*30:7.1f} posDay={c_13['pct_pos_days']:5.1f}% "
          f"maxStreak={c_13['max_losing_day_streak']:2}d sharpe={c_13['sharpe_like']:6.3f}")
    print(f"  14-way (+O): $/mo={total_14/days*30:7.1f} posDay={c_14['pct_pos_days']:5.1f}% "
          f"maxStreak={c_14['max_losing_day_streak']:2}d sharpe={c_14['sharpe_like']:6.3f}")
mt5.shutdown()
