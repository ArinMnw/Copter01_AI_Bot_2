...
active_strategies = {
    ...
    12: True,  # ท่าที่ 12: Range Trading (M5 only, standalone)
    13: True,  # Strategy 13: EzAlgo V5 Supertrend
}
...
def compute_tso_effective_steps(tp_orig_dist: float) -> list:
    """
    คำนวณ effective TP steps (หน่วยราคา) สำหรับ TSO ตาม TP เดิมของ order
    Logic:
      - filter SCALE_OUT_TP_POINTS ที่ <= tp_orig_dist (= valid steps)
      - append tp_orig_dist เป็น step สุดท้าย (ถ้ายังไม่ match exact)
      - ถ้า tp_orig_dist < TP1 (= 300pt × scale) → ใช้ TP1 step เดียว (override TP)
    Return: list of step distances (price units)
    """
    try:
        info = mt5.symbol_info(SYMBOL)
        point = float(getattr(info, "point", 0.01) or 0.01)
        ...
        point = 0.01
    scale = points_scale()
    tso_steps = [float(pts) * point * scale for pts in SCALE_OUT_TP_POINTS]
    tp_orig_dist = float(tp_orig_dist or 0.0)
    if tp_orig_dist <= 0:
        return []
    tp1 = tso_steps[0] if tso_steps else 0.0
    # ถ้า TP เดิม < TP1 → ใช้ TP1 step เดียว (override)
    if tp_orig_dist < tp1:
        return [tp1]
    # filter steps ที่ <= tp_orig
    valid = []
    tol = point * scale * 0.5   # tolerance ครึ่ง point
    for s in tso_steps:
        if s < tp_orig_dist - tol:
            valid.append(s)
        elif abs(s - tp_orig_dist) <= tol:
            valid.append(tp_orig_dist)   # exact match → ใช้ tp_orig เป็น step นั้น
            return valid                  # stop — TP_orig matches a step
        else:
            break   # s > tp_orig → ไม่มี step ใหญ่กว่าใน TSO_POINTS
    # append TP_orig เป็น step สุดท้าย (ถ้ายังไม่ใช่)
    if not valid or abs(valid[-1] - tp_orig_dist) > tol:
        valid.append(tp_orig_dist)
    return valid
