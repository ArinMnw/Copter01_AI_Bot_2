...
# audit 03/07/2026 (live 11/06-03/07: 26 ไม้ WR 69.2% -11.21 vs sim ช่วงเดียวกัน 50 ไม้ WR 90% +22.70):
#   gap หลักมาจาก (1) bot downtime W24 = 0 ไม้ทั้งสัปดาห์ (2) sid=None leak ให้
#   Fill Trend Recheck/SL Guard ปิดก่อนเวลา (แก้แล้ว _resolve_pos_sid ใน trailing.py)
# sim bid/ask model 60d (03/07): M1 +86.20 WR89.4% n=226 | M30 +28.78 n=12 | H1 +42.02 n=7
#   M5 -16.83 (ลบซ้ำ 2 รอบ backtest) | M15 -4.67 + live ลบ → ตัดออก
# WR tune 03/07 (SLB 1.5): M1 60d WR92.5% +92.24 streak=1 | 30d 93.5% | 22d 96.0%
#   | spread0.35 90.3% +70.62 | M30+H1 19/19 +94.00 — ทางเลือก WR สูงสุด: TP=0.2 → 94.7% (+56.70)
S17_ALLOWED_TFS         = ["M1", "M30", "H1"]  # ตัด M5/M15 (ลบ) + H4 (ไม่เคยทดสอบ/ไม่มี trade)
...
S17_SL_ATR_BUFFER       = 1.5   # SL = ไส้ sweep ∓ ATR × นี้ (1.0→1.5 03/07/2026: sim 60d WR 89.4→92.5% P/L 86→92, ทุก validation window ดีขึ้น; แลก avgL -3.91→-5.50)
...
# ── S17 Compounding (แบบเดียวกับ S20.12 — lot = balance × risk% / (ระยะ SL × contract)) ──
S17_COMPOUNDING_ENABLED = False  # default OFF — เปิดผ่าน Telegram (ท่า 17 → Compounding)
S17_RISK_PCT            = 2.0    # % ของ balance ที่เสี่ยงต่อไม้ (ระยะ SL เต็ม)
S17_MAX_LOT             = 50.0   # เพดาน lot ต่อไม้
...
S20_SYMBOL_FILTER       = ["XAUUSD"]  # S20 family (S20–S20.12) เทรดเฉพาะ symbol เหล่านี้; [] = ไม่จำกัด
...
def restore_runtime_state():
    ...
    try:
        ...
        global S17_COMPOUNDING_ENABLED, S17_RISK_PCT, S17_MAX_LOT
        S17_COMPOUNDING_ENABLED = bool(state.get("s17_compounding_enabled", S17_COMPOUNDING_ENABLED))
        saved_s17_risk = state.get("s17_risk_pct")
        if isinstance(saved_s17_risk, (int, float)) and saved_s17_risk > 0:
            S17_RISK_PCT = float(saved_s17_risk)
        saved_s17_max = state.get("s17_max_lot")
        if isinstance(saved_s17_max, (int, float)) and saved_s17_max > 0:
            S17_MAX_LOT = float(saved_s17_max)
