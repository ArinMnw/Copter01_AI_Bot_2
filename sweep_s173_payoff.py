# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S173."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy173 import detect_s173
from sweep_s161_payoff import _summary


CASES = ((7.0, 1.0), (16.0, 0.75), (16.1, 0.75), (16.2, 0.75),
         (16.3, 0.75), (16.4, 0.75), (16.5, 0.75), (16.6, 0.75),
         (16.7, 0.75), (16.8, 0.75), (16.9, 0.75), (17.0, 0.75))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s173(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 173)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(173, args.months, "M5", 0.20, 0.01, end, lookback,
