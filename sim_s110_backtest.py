...
parser = argparse.ArgumentParser(description="Backtest S110 Fractal Alignment")
...
parser.add_argument("--out-prefix", type=str, default="s110")
...
if args.disp_atr is not None:
    cfg_s110["DISP_BODY_ATR"] = args.disp_atr
if args.tp_rr is not None:
    cfg_s110["TP_RR"] = args.tp_rr
if args.retrace is not None:
    cfg_s110["ENTRY_RETRACE"] = args.retrace
if args.sl_buf is not None:
    cfg_s110["SL_BUF_ATR"] = args.sl_buf
if args.rsi_buy_min is not None:
    cfg_s110["RSI_BUY_MIN"] = args.rsi_buy_min
if args.rsi_sell_max is not None:
    cfg_s110["RSI_SELL_MAX"] = args.rsi_sell_max
if args.htf_span is not None:
    cfg_s110["HTF_EMA_SPAN"] = args.htf_span
if args.mtf_span is not None:
    cfg_s110["MTF_EMA_SPAN"] = args.mtf_span
if args.macd:
    cfg_s110["MACD_CONFIRM"] = True
if args.no_rsi_slope:
    cfg_s110["RSI_SLOPE_CONFIRM"] = False
if args.no_time_filter:
    cfg_s110["TIME_FILTER_ENABLED"] = False
...
for i in range(LOOKBACK, len(all_bars) - 2):
    ...
    sig = strategy110.detect_s110(rates_slice, tf=TF, dt_bkk=dt_bkk, cfg=cfg_s110)
