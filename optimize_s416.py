# -*- coding: utf-8 -*-
"""Cross-window falsification and optimization probes for S416."""

import argparse

from sim_strategy_backtest import backtest, parse_bkk, prepare_rates


WINDOWS = {
    "recent": (2, "2026-07-20T00:00:00+07:00"),
    "h1": (6, "2026-07-01T00:00:00+07:00"),
    "wf": (6, "2026-01-01T00:00:00+07:00"),
    "latest": (2, "2026-07-30T00:00:00+07:00"),
}
GROUPS = {
    "structure": (
        ("base", {}),
        ("fade", {"FADE_JUMP": True}),
        ("jump090", {"JUMP_RATIO_MIN": 0.90}),
        ("jump130", {"JUMP_RATIO_MIN": 1.30}),
        ("expand100", {"JUMP_EXPANSION_RATIO_MIN": 1.00,
                       "JUMP_RISE_MIN": 0.00}),
        ("expand125", {"JUMP_EXPANSION_RATIO_MIN": 1.25,
                       "JUMP_RISE_MIN": 0.15}),
        ("energy000", {"SIGNED_ENERGY_MIN": 0.00}),
        ("energy015", {"SIGNED_ENERGY_MIN": 0.15}),
        ("recent020", {"BASELINE_BARS": 60, "RECENT_BARS": 20}),
        ("recent028", {"BASELINE_BARS": 84, "RECENT_BARS": 28}),
        ("session0007", {"SESSION_START_HOUR": 0, "SESSION_END_HOUR": 7}),
        ("session0715", {"SESSION_START_HOUR": 7, "SESSION_END_HOUR": 15}),
        ("session1523", {"SESSION_START_HOUR": 15, "SESSION_END_HOUR": 23}),
        ("buy_only", {"ALLOW_SELL": False}),
        ("sell_only", {"ALLOW_BUY": False}),
    ),
    "finalists": (
        ("jump090", {"JUMP_RATIO_MIN": 0.90}),
        ("recent020", {"BASELINE_BARS": 60, "RECENT_BARS": 20}),
        ("session0007", {"SESSION_START_HOUR": 0, "SESSION_END_HOUR": 7}),
        ("session0715", {"SESSION_START_HOUR": 7, "SESSION_END_HOUR": 15}),
        ("session0715_jump090", {"SESSION_START_HOUR": 7,
                                 "SESSION_END_HOUR": 15,
                                 "JUMP_RATIO_MIN": 0.90}),
        ("session0715_recent020", {"SESSION_START_HOUR": 7,
                                   "SESSION_END_HOUR": 15,
                                   "BASELINE_BARS": 60,
                                   "RECENT_BARS": 20}),
        ("session0715_buy", {"SESSION_START_HOUR": 7,
                             "SESSION_END_HOUR": 15,
                             "ALLOW_SELL": False}),
        ("session0715_sell", {"SESSION_START_HOUR": 7,
                              "SESSION_END_HOUR": 15,
                              "ALLOW_BUY": False}),
    ),
    "direction": (
        ("jump090_buy", {"JUMP_RATIO_MIN": 0.90, "ALLOW_SELL": False}),
        ("jump090_sell", {"JUMP_RATIO_MIN": 0.90, "ALLOW_BUY": False}),
        ("recent020_buy", {"BASELINE_BARS": 60, "RECENT_BARS": 20,
                           "ALLOW_SELL": False}),
        ("recent020_sell", {"BASELINE_BARS": 60, "RECENT_BARS": 20,
                            "ALLOW_BUY": False}),
    ),
}


def _view(summary):
    return {key: summary[key] for key in (
        "closed", "wins", "win_rate", "net_profit", "pnl_per_day",
        "pnl_per_month", "profit_factor", "max_drawdown",
    )}


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--window", choices=tuple(WINDOWS), required=True)
    parser.add_argument("--group", choices=tuple(GROUPS), required=True)
    args = parser.parse_args()
    months, end_text = WINDOWS[args.window]
    end = parse_bkk(end_text)
    prepared = prepare_rates(months, "M5", end, 300)
    for name, cfg in GROUPS[args.group]:
        summary, _ = backtest(
            416, months, "M5", 0.20, 0.01, end, 300, cfg, prepared
        )
        print(name, _view(summary), flush=True)


if __name__ == "__main__":
    main()
