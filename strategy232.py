# -*- coding: utf-8 -*-
"""S232 - Rogers-Satchell volatility-compression efficiency breakout, 10R.

S212 identifies compression from rolling high-low range.  S232 instead uses the
Rogers-Satchell OHLC estimator, which measures intrabar diffusion without being
biased by directional drift.  A short RS-volatility collapse versus its longer
baseline must be followed by a body-efficient break of the compressed range.
"""

from __future__ import annotations

import math

from strategy119 import _atr, _bars
from strategy197 import _wait


DEFAULT_CFG = {
    "ATR_PERIOD": 14,
    "RANGE_BARS": 12,
    "RS_SHORT_WINDOW": 12,
    "RS_LONG_WINDOW": 72,
    "RS_COMPRESSION_MAX": 0.65,
    "BREAK_BODY_MIN_FRACTION": 0.55,
    "BREAK_BODY_MIN_ATR": 0.30,
    "SL_BUFFER_ATR": 0.08,
    "MIN_RISK_ABS": 0.60,
    "MAX_RISK_ATR": 1.75,
    "MAX_RISK_PRICE_PCT": 0.34,
    "TP_RR": 10.00,
    "BE_RR": 1.00,
    "CANCEL_BARS": 3,
}


def _rs_variance(bar):
    open_price = float(bar["open"])
    high = float(bar["high"])
    low = float(bar["low"])
    close = float(bar["close"])
    if min(open_price, high, low, close) <= 0.0:
        return 0.0
    value = (
        math.log(high / open_price) * math.log(high / close)
        + math.log(low / open_price) * math.log(low / close)
    )
    return max(0.0, value)


def detect_s232(rates, tf="", dt_bkk=None, cfg=None, **kwargs):
    """Trade an efficient range break after RS volatility compresses."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
        period = max(1, int(c["ATR_PERIOD"]))
        range_bars = max(4, int(c["RANGE_BARS"]))
        short_window = max(4, int(c["RS_SHORT_WINDOW"]))
        long_window = max(short_window + 4, int(c["RS_LONG_WINDOW"]))
    except (KeyError, TypeError, ValueError, OverflowError) as exc:
        return _wait(f"Invalid config: {exc}")
    required = max(period + range_bars + 5, long_window + 3)
    if rates is None or len(rates) < required or dt_bkk is None:
        return _wait("Not enough data or dt_bkk missing")
    try:
        bars = _bars(rates)
        atr = _atr(bars[:-1], period)
    except (KeyError, TypeError, ValueError, OverflowError, AttributeError) as exc:
        return _wait(f"Invalid rates: {exc}")
    if atr <= 0.0:
        return _wait("ATR is zero")

    history = bars[-long_window - 1:-1]
    rs_values = [_rs_variance(bar) for bar in history]
    long_rs = sum(rs_values) / len(rs_values)
    short_rs = sum(rs_values[-short_window:]) / short_window
    if long_rs <= 0.0:
        return _wait("Long Rogers-Satchell variance is zero")
    compression_ratio = short_rs / long_rs
    if compression_ratio > float(c["RS_COMPRESSION_MAX"]):
        return _wait(
            f"RS volatility is not compressed (ratio={compression_ratio:.2f})"
        )

    compressed = bars[-range_bars - 1:-1]
    range_high = max(bar["high"] for bar in compressed)
    range_low = min(bar["low"] for bar in compressed)
    if range_high <= range_low:
        return _wait("Compressed range is degenerate")
    breakout = bars[-1]
    body = breakout["close"] - breakout["open"]
    bar_range = breakout["high"] - breakout["low"]
    if breakout["close"] > range_high and body > 0.0:
        side = 1
    elif breakout["close"] < range_low and body < 0.0:
        side = -1
    else:
        return _wait("No directional break of the compressed range")
    if bar_range <= 0.0:
        return _wait("Breakout bar range is zero")
    if abs(body) < bar_range * float(c["BREAK_BODY_MIN_FRACTION"]):
        return _wait("Breakout bar lacks directional efficiency")
    if abs(body) < atr * float(c["BREAK_BODY_MIN_ATR"]):
        return _wait("Breakout body is too small versus ATR")

    buffer = atr * float(c["SL_BUFFER_ATR"])
    entry = round(breakout["close"], 2)
    if side > 0:
        sl = math.floor((breakout["low"] - buffer + 1e-12) * 100.0) / 100.0
    else:
        sl = math.ceil((breakout["high"] + buffer - 1e-12) * 100.0) / 100.0
    risk = side * (entry - sl)
    if risk < float(c["MIN_RISK_ABS"]):
        return _wait(f"Risk below spread-honesty floor ({risk:.2f})")
    if risk > atr * float(c["MAX_RISK_ATR"]):
        return _wait(f"RS-break risk outside range ({risk / atr:.2f} ATR)")
    if risk / entry * 100.0 > float(c["MAX_RISK_PRICE_PCT"]):
        return _wait("RS-break risk too large versus price")

    rr = max(7.0, float(c["TP_RR"]))
    raw_tp = entry + side * rr * risk
    if side > 0:
        signal = "BUY"
        tp = math.ceil((raw_tp - 1e-12) * 100.0) / 100.0
    else:
        signal = "SELL"
        tp = math.floor((raw_tp + 1e-12) * 100.0) / 100.0
    return {
        "signal": signal,
        "entry": entry,
        "sl": sl,
        "tp": tp,
        "order_type": "market",
        "pattern": f"S232 {signal} RS Compression Break {rr:g}R",
        "reason": (
            f"Efficient range break after RS compression "
            f"(short/long={compression_ratio:.2f})"
        ),
        "be_rr": float(c["BE_RR"]),
        "cancel_bars": int(c["CANCEL_BARS"]),
    }
