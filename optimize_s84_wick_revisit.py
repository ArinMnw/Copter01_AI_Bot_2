"""
optimize_s84_wick_revisit.py - Search S84 Old Wick Close-Fail configs.
...
from sim_s84_backtest import DEFAULT_SPREAD, START_EQUITY, run_single
...
def _cfg_label(cfg):
    return (
        f"{cfg['ENTRY_TF']}_lb{cfg['LOOKBACK']}_rw{cfg['REF_MIN_WICK_ATR']:g}"
        f"_wb{cfg['REF_WICK_BODY_MULT']:g}_et{cfg['EAT_TOL_ATR']:g}"
        f"_cf{cfg['CLOSE_FAIL_ATR']:g}_op{int(cfg['REQUIRE_OPPOSITE_CLOSE'])}"
        f"_mb{cfg['MIN_BODY_ATR']:g}_mr{cfg['MIN_RANGE_ATR']:g}"
        f"_{cfg['MODE']}_{cfg['TARGET_MODE']}_sl{cfg['SL_ATR_MULT']:g}_rr{cfg['TP_RR']:g}"
    )
...
def main():
    ...
    ap.add_argument("--out", default="s84_wick_revisit_optimize.csv")
    ...
    for vals in itertools.product(*_grid(args.preset)):
        tf, lb, refwick, wickbody, eattol, fail, opp, minbody, minrange, target, mode, slmult, rr = vals
