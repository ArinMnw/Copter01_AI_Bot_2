//+------------------------------------------------------------------+
//|                                              ichimoku_EA_v3.mq5   |
//|  Ichimoku Trend EA v3 — เข้า/ปิด order ตามความสัมพันธ์ของเส้น Ichimoku |
...
//|                                                                     |
//|  เงื่อนไข SELL แบบเต็ม (ใช้เปิด order, ที่แท่งปิดล่าสุด):            |
//|    Lagging Span < Conversion Line AND Lagging Span < Base Line     |
//|    AND Conversion Line < Base Line AND Leading Span A < Span B     |
//|  เงื่อนไข SELL แบบย่อ (ใช้ปิด order BUY เท่านั้น):                   |
//|    Lagging Span < Conversion Line AND Lagging Span < Base Line     |
//|    AND Conversion Line < Base Line  (ไม่เช็ค Lead A/B)              |
//|  เงื่อนไข BUY = กลับด้านทุกเครื่องหมายจาก SELL (ทั้งแบบเต็ม/ย่อ)      |
//|                                                                     |
...
//|  TP    : ไม่มี                                                      |
//|  Close : ปิดออเดอร์เมื่อเกิดสัญญาณฝั่งตรงข้าม (แบบย่อ) แล้วเช็คเปิด  |
//|          ฝั่งใหม่ต่อ (แบบเต็ม) ในติ๊กเดียวกัน                        |
//+------------------------------------------------------------------+
...
void OnTick()
  {
   ...
   bool relaxSell = (lag < conv && lag < base && conv < base);
   bool relaxBuy  = (lag > conv && lag > base && conv > base);
   bool strictSell = relaxSell && (leadA < leadB);
   bool strictBuy  = relaxBuy  && (leadA > leadB);
   ...
   if(hasPos)
     {
      // v3: ปิดด้วยเงื่อนไขแบบย่อ (ไม่ต้องรอ Lead A/B)
      if((posType == POSITION_TYPE_SELL && relaxBuy) ||
         (posType == POSITION_TYPE_BUY  && relaxSell))
        {
         ClosePositions();
         hasPos = false;
        }
   ...
   // v3: เปิด position ใหม่ด้วยเงื่อนไขแบบเต็ม (ต้องมี Lead A/B) เท่านั้น
   if(strictBuy)
     {
      double sl = SwingLow(InpSwingLookback);
      if(sl != EMPTY_VALUE)
