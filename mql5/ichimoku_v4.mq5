//+------------------------------------------------------------------+
//|                                                 ichimoku_v4.mq5   |
//|  Ichimoku Cloud v4 (แปลงจาก Pine Script v6 "Ichimoku Cloud")      |
//|  ⚠️ v4: เงื่อนไขเดียว — Conversion vs Base เท่านั้น (ไม่ใช้ Lag,     |
//|         ไม่ใช้ Lead A/B) เปิด/ปิด order ด้วยเงื่อนไขเดียวกัน (mirror) |
...
bool SellSignalAt(const int i)
  {
   if(ConversionBuffer[i] == EMPTY_VALUE || BaseBuffer[i] == EMPTY_VALUE)
      return(false);
   return(ConversionBuffer[i] < BaseBuffer[i]);
  }
