//+------------------------------------------------------------------+
//|                                              ichimoku_EA_v2.mq5   |
//|  Ichimoku Trend EA v2 — เข้า/ปิด order ตามความสัมพันธ์ของเส้น Ichimoku |
//|  ⚠️ v2: ตัดเงื่อนไข Lead A/Lead B ออก เหลือแค่ Lag/Conversion/Base   |
//|                                                                     |
...
void OnTick()
  {
   ...
   bool sellSignal = (lag < conv && lag < base && conv < base);
   bool buySignal  = (lag > conv && lag > base && conv > base);
