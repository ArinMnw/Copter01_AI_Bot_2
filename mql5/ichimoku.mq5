//+------------------------------------------------------------------+
//|                                                    ichimoku.mq5   |
//|  Ichimoku Cloud (แปลงจาก Pine Script v6 "Ichimoku Cloud")         |
//|                                                                     |
...
//+------------------------------------------------------------------+
// เงื่อนไข Sell/Buy — ใช้ค่าที่คำนวณแล้วในแต่ละ index i (ไม่ปรับ offset ตาม displacement)
bool SellSignalAt(const int i)
  {
   if(ConversionBuffer[i] == EMPTY_VALUE || BaseBuffer[i] == EMPTY_VALUE || LeadSpanBBuffer[i] == EMPTY_VALUE)
      return(false);
   return(LaggingBuffer[i] < ConversionBuffer[i] && LaggingBuffer[i] < BaseBuffer[i] &&
          ConversionBuffer[i] < BaseBuffer[i] && LeadSpanABuffer[i] < LeadSpanBBuffer[i]);
  }

bool BuySignalAt(const int i)
  {
   if(ConversionBuffer[i] == EMPTY_VALUE || BaseBuffer[i] == EMPTY_VALUE || LeadSpanBBuffer[i] == EMPTY_VALUE)
      return(false);
   return(LaggingBuffer[i] > ConversionBuffer[i] && LaggingBuffer[i] > BaseBuffer[i] &&
   ...
   if(InpShowSignals)
     {
      ...
      // จำลอง state machine เดียวกับ EA: ปิดฝั่งเดิมเมื่อเจอสัญญาณตรงข้าม แล้วค่อยเปิดฝั่งใหม่ถ้าว่าง
      ...
      for(int i = sigStart; i < rates_total; i++)
        {
         bool sellNow = SellSignalAt(i);
         bool buyNow  = BuySignalAt(i);
         ...
         if(g_OpenDir == 1 && sellNow) // ปิด BUY ด้วย TP = close ของแท่งสัญญาณ Sell (เปลี่ยนฝั่งจริง)
           {
            if(draw)
               DrawExit(g_OpenEntryTime, g_OpenEntryPrice, time[i], close[i], true);
            g_OpenDir = 0;
           }
         else if(g_OpenDir == -1 && buyNow) // ปิด SELL ด้วย TP = close ของแท่งสัญญาณ Buy (เปลี่ยนฝั่งจริง)
           {
            if(draw)
