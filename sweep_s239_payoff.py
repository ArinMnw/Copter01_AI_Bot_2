# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S239."""
...
from strategy239 import detect_s239
from sweep_s235_payoff import _summary_market


RR_VALUES = tuple(round(43.0 + 0.1 * index, 1) for index in range(51))
BE_VALUES = tuple(round(0.95 + 0.01 * index, 2) for index in range(21))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s239(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 239)
        cached[index] = signal
    rows = [
