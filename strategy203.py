# -*- coding: utf-8 -*-
"""S203 - Bipower jump and variance-ratio confluence structural-sweep reclaim, 16.9R."""
...
from strategy193 import _bipower_variation
...
def detect_s203(rates, tf, dt_bkk, cfg):
    """Fade a structural sweep on a discontinuous jump in an anti-persistent regime."""
    c = dict(DEFAULT_CFG)
    if cfg:
        c.update(cfg)
    try:
        period = max(1, int(c["ATR_PERIOD"]))
        window = max(40, int(c["JUMP_WINDOW"]))
    ...
    jump_score = shock_return * shock_return / continuous_variance
    if jump_score < float(c["JUMP_SCORE_MIN"]):
        return _wait(f"Shock is not a discontinuous jump (score={jump_score:.2f})")
    ...
    if (exhaustion_range <= 0.0
        ...
        return _wait("Jump shock lacks directional body")
    ...
    if exhaustion["tick_volume"] < volume_floor:
        return _wait("Jump shock lacks volume")
    ...
    if not confirmed or reclaim["tick_volume"] < reclaim_volume_floor:
        return _wait("Jump shock lacks confirmed reclaim")
    ...
    return {
        ...
        "tp": tp,
        "order_type": "limit",
        "pattern": f"S203 {signal} Jump-VR Confluence Sweep {rr:g}R",
        "reason": (f"Jump score={jump_score:.2f}, VR={variance_ratio:.2f}; "
