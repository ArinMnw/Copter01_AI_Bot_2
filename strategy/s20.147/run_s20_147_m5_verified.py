# -*- coding: utf-8 -*-
"""run_s20_147_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.147 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.147 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Volume Profile POC Migration & Value Area Rejection", flush=True)
    print("=" * 115, flush=True)
    ...
    pd.DataFrame(monthly_rows).to_csv(os.path.join(os.path.dirname(__file__), "S20_147_monthly.csv"), index=False)
    print(f"Monthly breakdown saved to S20_147_monthly.csv", flush=True)
