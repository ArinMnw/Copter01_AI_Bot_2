# -*- coding: utf-8 -*-
"""s20_38_research.py
Explore quantum breakthroughs to beat S20.37 (+$9,431.72) on STRICT LOT 0.01:
Hard Constraints:
...
1. Adding Previous Month High/Low (PMH/PML) to macro liquidity pools.
2. Extended Session Window: 06:00 to 22:00 UTC (capturing London pre-market & NY late sweep).
3. Hexa-Stage Quantum Ratchet Lock with extended TP (4.2R, 4.5R, 4.8R).
4. Retest Depth Precision (0.12, 0.13, 0.14, 0.15).
5. Volume Climax fine tuning (1.18, 1.20, 1.22).
"""
...
def compute_indicators(rates):
    ...
    # Previous Month High / Low (PMH/PML shifted 1 month)
...
def extract_setups(df, tf_label, min_vol=1.20, min_wick=0.38, retest_depth=0.150, hours=(6, 22), use_pmh=True):
    setups = []
    h_start, h_end = hours
    ...
    for s in setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if lock5_trig > 0 and not l5_active and b['high'] >= l5_target:
                    l5_active = True
                    current_sl = l5_sl
                ...
                if lock5_trig > 0 and not l5_active and b['low'] <= l5_target:
                    l5_active = True
                    current_sl = l5_sl
...
def main():
    ...
    print(" S20.38 QUANT RESEARCH (STRICT LOT 0.01 | M5 SL-FIRST | TARGET > $9,431.72)", flush=True)
    print(" Benchmark S20.37: +$9,431.72 | MaxDD $14.34 | WR 87.4%", flush=True)
    print("=" * 115, flush=True)

    # 1. Test Session Windows: (7, 21) vs (6, 21) vs (6, 22) vs (5, 22)
    print("--- 1. Testing Session Windows (London Early + NY Late) ---", flush=True)
    for win in [(7, 21), (6, 21), (6, 22), (5, 22)]:
        s0 = extract_setups(df_h4, "H4", min_vol=1.20, retest_depth=0.150, hours=win)
        s1 = extract_setups(df_h1, "H1", min_vol=1.20, retest_depth=0.150, hours=win)
        s2 = extract_setups(df_m30, "M30", min_vol=1.20, retest_depth=0.150, hours=win)
        s3 = extract_setups(df_m15, "M15", min_vol=1.20, retest_depth=0.150, hours=win)
        ...
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
                             ...
                             lock4_trig=3.5, lock4_amt=3.3, lock5_trig=0.0, lock5_amt=0.0,
                             tp_r=4.0)
    ...
    # 2. Test Retest Depth: 0.12, 0.13, 0.14, 0.15
    print("\n--- 2. Testing Retest Depth ---", flush=True)
    for depth in [0.12, 0.13, 0.14, 0.15]:
        s0 = extract_setups(df_h4, "H4", min_vol=1.20, retest_depth=depth, hours=(6, 22))
        s1 = extract_setups(df_h1, "H1", min_vol=1.20, retest_depth=depth, hours=(6, 22))
        s2 = extract_setups(df_m30, "M30", min_vol=1.20, retest_depth=depth, hours=(6, 22))
        s3 = extract_setups(df_m15, "M15", min_vol=1.20, retest_depth=depth, hours=(6, 22))
        comb = sorted(s0 + s1 + s2 + s3, key=lambda x: x['time'])
        ...
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
                             ...
                             lock4_trig=3.5, lock4_amt=3.3, lock5_trig=0.0, lock5_amt=0.0,
                             tp_r=4.0)
        print(f"Depth {depth:.2f}: Trades: {res['trades']:4d} | W/BE/L: {res['wins']:3d}/{res['bes']:3d}/{res['losses']:2d} | WR: {res['wr']:4.1f}% | Net: ${res['pnl']:8.2f} | PF: {res['pf']:5.2f} | MaxDD: ${res['max_dd']:5.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)

    # 3. Test Hexa-Stage Trailing Lock with Extended Runners (TP 4.2R, 4.5R, 4.8R)
    print("\n--- 3. Testing Hexa-Stage Ratchet Trailing ---", flush=True)
    s0 = extract_setups(df_h4, "H4", min_vol=1.20, retest_depth=0.15, hours=(6, 22))
    s1 = extract_setups(df_h1, "H1", min_vol=1.20, retest_depth=0.15, hours=(6, 22))
    s2 = extract_setups(df_m30, "M30", min_vol=1.20, retest_depth=0.15, hours=(6, 22))
    s3 = extract_setups(df_m15, "M15", min_vol=1.20, retest_depth=0.15, hours=(6, 22))
    comb = sorted(s0 + s1 + s2 + s3, key=lambda x: x['time'])

    trailers = [
        ("Base Penta TP 4.0R", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 0.0, 0.0, 4.0),
        ("Hexa-A: TP 4.2R (L5@3.8->3.5)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2),
        ("Hexa-B: TP 4.5R (L5@4.0->3.6)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 4.0, 3.6, 4.5),
        ("Hexa-C: TP 4.5R (L5@4.0->3.8)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 4.0, 3.8, 4.5),
        ("Hexa-D: TP 4.8R (L5@4.2->3.8)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 4.2, 3.8, 4.8),
    ]

    for name, be, l1_t, l1_a, l2_t, l2_a, l3_t, l3_a, l4_t, l4_a, l5_t, l5_a, tp in trailers:
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
