# -*- coding: utf-8 -*-
"""run_s20_51_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.51 on Gold (XAUUSD.iux)
...
5. Multi-Strategy Single-Candle Cross-Breed Confluence:
   ...
   - Strategy 11: Fibonacci Golden Zone & Premium/Discount Equilibrium Confluence (Discount 38.2% for BUY / Premium 61.8% for SELL)
   - Wyckoff VSA: Volume Climax Ratio >= 1.17x of 20 MA
   - Price Action Rejection Wick: lower_wick_pct / upper_wick_pct >= 32% or wick >= 1.1x body
   - Candle Range Theory (CRT - Strategy 10): Closed Momentum >= 45% of total range
   - RSI Momentum Guard (Strategy 9): BUY <= 68, SELL >= 32
6. Quindeca-Stage Quantum Ratchet Lock Engine (15 Stages):
   ...
   - Stage 15: Lock +6.30R @ +6.50R (Quindeca-Stage Lock เพิ่มใหม่!)
   - Full Target: TP @ +6.75R
"""
...
from strategy20_51 import compute_indicators, extract_setups
...
def main():
    ...
    print(" S20.51 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    ...
    print("Extracting omni-horizon confluences (H4, H3, H2, H1, M30, M20, M15, M12 + Multi-Bar FVG + Strategy 11 Fibo Equilibrium)...", flush=True)
    base_tfs = [
    ...
    for tf_label, df_tf in base_tfs:
        all_setups.extend(extract_setups(
            df_tf, tf_label,
            min_vol=1.17, min_wick=0.32, retest_depth=0.124,
            hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True, use_ny_pm=True, use_fvg=True, use_fibo=True
        ))
    ...
    lock14_trig = 6.35; lock14_amt = 6.15
    lock15_trig = 6.50; lock15_amt = 6.30
    tp_r = 6.75
    ...
    print("=" * 115, flush=True)
    print(" S20.51 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "S20_51_monthly.csv")
