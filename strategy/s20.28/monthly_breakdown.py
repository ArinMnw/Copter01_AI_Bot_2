# -*- coding: utf-8 -*-
"""monthly_breakdown.py — Monthly tear sheet for S20.28 Profit-Lock Step Matrix."""
...
import backtest_s20_28_runner


def run_breakdown():
    if not backtest_s20_28_runner.init_mt5():
    ...
    for tf_str, tf_mt5 in [("M30", mt5.TIMEFRAME_M30), ("M15", mt5.TIMEFRAME_M15)]:
        print(f"\n{'='*75}")
        print(f" S20.28 PROFIT-LOCK STEP — MONTHLY TEAR SHEET ({tf_str})")
        print(f"{'='*75}")
        ...
        df = strategy20_28.compute_indicators_df(g_rates, s_rates)
        res = backtest_s20_28_runner.simulate_s20_28(g_rates, df, mode="mode_profit_lock", total_lot=0.02, tf_name=tf_str)
        ...
        out_csv = os.path.join(current_dir, f"S20_28_{tf_str}_monthly.csv")
