# -*- coding: utf-8 -*-
"""run_s20_42_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.42 on Gold (XAUUSD.iux)
...
5. Target: Verifiably beat S20.41 (+$13,456.19) and achieve over $16,700+ on strict 0.01 lot!
"""
...
from strategy20_42 import compute_indicators, extract_setups
...
def main():
    ...
    print(" S20.42 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print("=" * 115, flush=True)

    print("Fetching omni-horizon rates from MT5 (H4, H3, H2, H1, M30, M20, M15)...", flush=True)
    ...
    print("Extracting omni-horizon confluences (H4, H3, H2, H1, M30, M20, M15)...", flush=True)
    s0 = extract_setups(df_h4, "H4", min_vol=1.20, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True)
    s_h3 = extract_setups(df_h3, "H3", min_vol=1.20, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True)
    s_h2 = extract_setups(df_h2, "H2", min_vol=1.20, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True)
    s1 = extract_setups(df_h1, "H1", min_vol=1.20, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True)
    s2 = extract_setups(df_m30, "M30", min_vol=1.20, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True)
    s_m20 = extract_setups(df_m20, "M20", min_vol=1.20, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True)
    s3 = extract_setups(df_m15, "M15", min_vol=1.20, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True)
    all_setups = sorted(s0 + s_h3 + s_h2 + s1 + s2 + s_m20 + s3, key=lambda x: x['time'])
    print(f"Total raw setups detected across all horizons: {len(all_setups)}", flush=True)

    # Simulation parameters (Nona-Stage Ratchet Trailing)
    be_trigger = 0.8
    lock1_trig = 1.5
    lock1_amt = 1.0
    lock2_trig = 2.4
    ...
    for s in all_setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if not l8_active and b['high'] >= l8_target:
                    l8_active = True
                    current_sl = l8_sl
                ...
                if not l8_active and b['low'] <= l8_target:
                    l8_active = True
                    current_sl = l8_sl
    ...
    print("\n" + "=" * 115, flush=True)
    print(" 🏆 FINAL VERIFIED PERFORMANCE — STRATEGY S20.42 (STRICT LOT 0.01)", flush=True)
    print("=" * 115, flush=True)
