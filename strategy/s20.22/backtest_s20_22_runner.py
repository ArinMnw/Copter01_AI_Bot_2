# -*- coding: utf-8 -*-
"""backtest_s20_22_runner.py — 365-Day Backtest & Monthly Analysis for S20.22:
1. VWAP ±2.0/2.5σ Reversion
2. Asian Range Fibonacci Expansion (1.618x Extreme)
3. Institutional Opening Range Breakout (ORB 30m)
"""
...
def simulate_engine(rates, df, engine_name, lot=0.01, point_val=1.0):
    ...
    n = len(rates)
    while i < n - 5:
        if engine_name == "VWAP_Reversion":
            res = strategy20_22.evaluate_vwap_reversion(df, i)
        elif engine_name == "Asian_Expansion":
            res = strategy20_22.evaluate_asian_expansion(df, i)
        elif engine_name == "Institutional_ORB":
            res = strategy20_22.evaluate_institutional_orb(df, i)
        else:
            res = None
...
def main():
    ...
    print("=========================================================================")
    print(f"  🏛️ S20.22 Quant Tier-1 Matrix 365-Day Backtest")
    print(f"  Symbol: {symbol} | TF: {tf_name} | Lot: {lot} | Days: {days}")
    print("=========================================================================\n")
    ...
    print(f"Loaded {len(rates):,} bars for Gold")
    df = strategy20_22.compute_indicators_df(rates)

    engines = ["VWAP_Reversion", "Asian_Expansion", "Institutional_ORB"]
    summary_list = []
    monthly_all = {}

    for eng in engines:
        print(f"Running Engine: {eng:20s} ... ", end="", flush=True)
    ...
    print("\n" + "=" * 90)
    print("🏆 S20.22 QUANT TIER-1 ENGINES 365-DAY OVERALL SUMMARY (Lot 0.01)")
    print("=" * 90)
    ...
    if m_rows:
        ...
        csv_path = os.path.join(current_dir, "S20_22_monthly_breakdown.csv")
        m_comp_df.to_csv(csv_path, index=False)
        print(f"\n📅 S20.22 MONTH-BY-MONTH PnL COMPARISON (365 Days):")
        print("=" * 90)
