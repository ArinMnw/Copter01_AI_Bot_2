# -*- coding: utf-8 -*-
"""run_s20_33_m5_verified.py — 100% Verifiable M5 Sequential Execution:
...
4. Triple-Horizon: Evaluates H1 + M30 + M15 confluence setups.
5. Tri-Stage Profit Lock: BE at +0.8R, Lock +1.0R at +1.5R, Lock +1.8R at +2.2R, Target 2.8R.
"""
...
def main():
    ...
    df_h1 = strategy20_33.compute_indicators_df(h1_gold)
    df_m30 = strategy20_33.compute_indicators_df(m30_gold)
    df_m15 = strategy20_33.compute_indicators_df(m15_gold)
    ...
    # Evaluate setups
    setups_h1 = []
    for i in range(45, len(df_h1)):
        s = strategy20_33.evaluate_setup(df_h1, i, tf_label="H1", retest_depth=0.300)
        if s:
            setups_h1.append(s)

    setups_m30 = []
    for i in range(45, len(df_m30)):
        s = strategy20_33.evaluate_setup(df_m30, i, tf_label="M30", retest_depth=0.300)
        if s:
            setups_m30.append(s)

    setups_m15 = []
    for i in range(45, len(df_m15)):
        s = strategy20_33.evaluate_setup(df_m15, i, tf_label="M15", retest_depth=0.300)
        if s:
            setups_m15.append(s)

    combined_setups = sorted(setups_h1 + setups_m30 + setups_m15, key=lambda x: x['time'])
    ...
    print("=" * 105)
    print(" S20.33 100% VERIFIABLE M5 INTRABAR SEQUENTIAL EXECUTION (STRICT LOT 0.01)")
    print(" Triple-Horizon (H1+M30+M15) | Precision Retest 0.30 | SL-First on Every Bar")
    print("=" * 105)
    ...
    print(f"S20.33 Triple-Horizon Champion | Trades: {trades:3d} | W/BE/L: {wins:3d}/{bes:3d}/{losses:2d} | WR: {wr:4.1f}% | NonLoss: {non_loss:4.1f}% | NetPnL: ${pnl:8.2f} | PF: {pf:6.2f} | MaxDD: ${max_dd:5.2f} | Mos: {pos_m}/{tot_m}")
