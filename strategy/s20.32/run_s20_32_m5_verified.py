# -*- coding: utf-8 -*-
"""run_s20_32_m5_verified.py — 100% Verifiable M5 Sequential Execution:
...
4. Dual-Horizon: Evaluates M15 + M30 confluence setups.
5. Stepped Profit Lock: BE at +0.8R, Lock +1.0R at +1.6R, Target 2.5R.
"""
...
def main():
    ...
    print("=" * 105)
    print(" S20.32 100% VERIFIABLE M5 INTRABAR SEQUENTIAL EXECUTION (STRICT LOT 0.01)")
    print(" Dual-Horizon (M15+M30) | Zero Lookahead | SL-First on Every 5-Min Bar")
    print("=" * 105)

    scenarios = [
        ("S20.32-A: Pure BE (TP 1.8R)", combined_setups, 1.8, 0.8, 0.0, 0.0),
        ("S20.32-B: Pure BE (TP 2.0R)", combined_setups, 2.0, 0.8, 0.0, 0.0),
        ("S20.32-C: Profit-Lock (Lock +0.8R @1.4R, TP 2.2R)", combined_setups, 2.2, 0.8, 1.4, 0.8),
        ("S20.32-Champion: Profit-Lock (Lock +1.0R @1.6R, TP 2.5R)", combined_setups, 2.5, 0.8, 1.6, 1.0),
    ]

    for name, setups, tp_r, be_r, lock_trig, lock_amt in scenarios:
        trades = 0
        wins = 0
        losses = 0
        bes = 0
        ...
        for s in setups:
            ...
            for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
                ...
                if sig == "BUY":
                    ...
                    # TP check only if SL not touched
                    if b['high'] >= tp:
                        outcome = "WIN"
                        trade_pnl = abs(tp - entry) * point_val
                        break

                    # Profit Lock / BE trigger
                    if lock_trig > 0 and not lock_active and b['high'] >= lock_target:
                        lock_active = True
                    ...
                    if lock_trig > 0 and not lock_active and b['low'] <= lock_target:
                        lock_active = True
                        current_sl = locked_sl
        ...
        print(f"{name:55s} | Trades: {trades:3d} | W/BE/L: {wins:3d}/{bes:3d}/{losses:2d} | WR: {wr:4.1f}% | NonLoss: {non_loss:4.1f}% | NetPnL: ${pnl:8.2f} | PF: {pf:6.2f} | MaxDD: ${max_dd:5.2f} | Mos: {pos_m}/{tot_m}")
