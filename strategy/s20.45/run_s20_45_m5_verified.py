# -*- coding: utf-8 -*-
"""run_s20_45_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.45 on Gold (XAUUSD.iux)
...
5. Target: Verifiably beat S20.44 (+$19,049.30) and achieve over $19,800+ on strict 0.01 lot!
"""
...
from strategy20_45 import compute_indicators, extract_setups
...
def main():
    ...
    print(" S20.45 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    ...
    print("Extracting omni-horizon confluences (H4, H3, H2, H1, M30, M20, M15, M12 + Dual-Session Sweeps)...", flush=True)
    retest_depth = 0.121
    s0 = extract_setups(df_h4, "H4", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    s_h3 = extract_setups(df_h3, "H3", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    s_h2 = extract_setups(df_h2, "H2", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    s1 = extract_setups(df_h1, "H1", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    s2 = extract_setups(df_m30, "M30", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    s_m20 = extract_setups(df_m20, "M20", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    s3 = extract_setups(df_m15, "M15", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    s_m12 = extract_setups(df_m12, "M12", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True)
    ...
    # Simulation parameters (Deca-Plus Ratchet Trailing)
    be_trigger = 0.8
    lock1_trig = 1.5
    lock1_amt = 1.0
    lock2_trig = 2.4
    ...
    for s in all_setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if not l10_active and b['high'] >= l10_target:
                    l10_active = True
                    current_sl = l10_sl
                elif not l9_active and b['high'] >= l9_target:
                    l9_active = True
                    current_sl = l9_sl
                ...
                if not l10_active and b['low'] <= l10_target:
                    l10_active = True
                    current_sl = l10_sl
                elif not l9_active and b['low'] <= l9_target:
                    l9_active = True
                    current_sl = l9_sl
    ...
    print("\n" + "=" * 115, flush=True)
    print(" 🏆 FINAL VERIFIED PERFORMANCE — STRATEGY S20.45 (STRICT LOT 0.01)", flush=True)
    print("=" * 115, flush=True)
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "S20_45_monthly.csv")
