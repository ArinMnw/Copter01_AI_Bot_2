# -*- coding: utf-8 -*-
"""run_s20_135_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.135 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.135 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Paradigm 5: Auction Market Theory (AMT) Value Area Absorption Climax Confluence", flush=True)
    print("=" * 115, flush=True)
    ...
    for tf, df_tf in dfs.items():
        for cur in df_tf.to_dict('records'):
            ...
            sig = "BUY" if ((swept_low or ob_mitigated_bull or bpr_tap_bull or is_abs_buy) and has_wick_buy and closed_high) else                   ("SELL" if ((swept_high or ob_mitigated_bear or bpr_tap_bear or is_abs_sell) and has_wick_sell and closed_low) else None)

            if sig:
                is_hc = (ob_mitigated_bull if sig == "BUY" else ob_mitigated_bear) or                         (bpr_tap_bull if sig == "BUY" else bpr_tap_bear) or                         (is_abs_buy if sig == "BUY" else is_abs_sell)
    ...
    stages = [(1.5, 1.0), (2.4, 2.0), (3.0, 2.8), (3.5, 3.3), (3.8, 3.5), (4.2, 3.9), (4.6, 4.3), (5.2, 4.9), (5.5, 5.2), (5.6, 5.4), (5.8, 5.6), (6.0, 5.8), (6.15, 5.95), (6.35, 6.15), (6.5, 6.3), (6.7, 6.5), (6.85, 6.65), (7.0, 6.8), (7.15, 6.95), (7.3, 7.1), (7.45, 7.25), (7.6, 7.4), (7.75, 7.55), (7.9, 7.7), (8.05, 7.85), (8.2, 8.0), (8.35, 8.15), (8.5, 8.3), (8.65, 8.45), (8.8, 8.6), (8.95, 8.75), (9.1, 8.9), (9.25, 9.05), (9.4, 9.2), (9.55, 9.35), (9.7, 9.5), (9.85, 9.65), (10.0, 9.8)]
    res = run_simulation(m5_gold, m5_times, setups, tp_r=11.60, stages=stages)

    print("=" * 115, flush=True)
    print(" S20.135 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    pd.DataFrame(monthly_rows).to_csv(os.path.join(os.path.dirname(__file__), "S20_135_monthly.csv"), index=False)
    print(f"Monthly breakdown saved to S20_135_monthly.csv", flush=True)
