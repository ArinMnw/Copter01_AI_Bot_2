# -*- coding: utf-8 -*-
"""tune_multi_paradigm.py
Testing three genuine synergistic combinations for:
- S20.101: Apex + Cumulative Volume Delta (CVD) Absorption Matrix
- S20.102: Apex + Multi-Session Judas Timing & Order Flow Engine
...
def main():
    ...
    # S20.100 Ratchet stages (29 stages, extending to 30 stages)
    stg_30 = [
    ...
    print("\n--- 1. Testing S20.101 (Apex + Order Flow CVD Delta Exhaustion Confluence) ---")
    for wick in [0.10, 0.11, 0.12]:
            ...
            for tp in [8.90, 8.95]:
                ...
                for tf, df_tf in dfs.items():
                    ...
                    for idx in range(45, len(recs)):
                        ...
                        # Order Flow Confluence: CVD Delta Confirmation
                        cvd_buy = cur['cvd_20'] > cur['cvd_min_12']
                        cvd_sell = cur['cvd_20'] < cur['cvd_max_12']

                        sig = None
                        if swept_low and has_wick_buy and closed_high and cvd_buy:
                            sig = "BUY"
                        elif swept_high and has_wick_sell and closed_low and cvd_sell:
                            sig = "SELL"
                ...
                res = run_simulation(m5_gold, m5_times, setups, tp_r=tp, stages=stg_30)
                print(f"[S20.101 CVD] wick={wick} depth={depth} TP={tp} -> PnL: ${res['pnl']:,.2f} | Trades: {res['trades']} | WR: {res['wr']:.1f}% | DD: ${res['max_dd']:.2f}")

    print("\n--- 2. Testing S20.102 (Apex + Session Judas Sweep Killzone Confluence) ---")
    for wick in [0.10, 0.11, 0.12]:
        for depth in [0.124, 0.125]:
            for tp in [8.90, 8.95]:
                setups = []
                for tf, df_tf in dfs.items():
                    ...
                    for idx in range(45, len(recs)):
                        ...
                        # Session Judas Confluence: London / NY opening liquidity traps
                        is_judas = cur['is_judas_window']

                        sig = None
                        # Allow normal trades, but if in Judas window require slightly less wick or vice versa
                        ...
                        if sig:
                            # During Judas Killzone, liquidity traps offer deeper pullbacks (depth 0.120)
                ...
                setups = sorted(setups, key=lambda x: x['time'])
                res = run_simulation(m5_gold, m5_times, setups, tp_r=tp, stages=stg_30)
                print(f"[S20.102 Judas] wick={wick} depth={depth} TP={tp} -> PnL: ${res['pnl']:,.2f} | Trades: {res['trades']} | WR: {res['wr']:.1f}% | DD: ${res['max_dd']:.2f}")

    print("\n--- 3. Testing S20.103 (Apex + Volatility Regime Expansion Adaptive Confluence) ---")
    for wick in [0.10, 0.11, 0.12]:
        for depth in [0.124, 0.125]:
            for tp in [8.90, 8.95]:
                setups = []
                for tf, df_tf in dfs.items():
                    ...
                    for idx in range(45, len(recs)):
                        ...
                        # Volatility Expansion: BB Width expanding above 50-period average
                        ...
                        if sig:
                            # If expanding, SL can be tighter by 5% because momentum protects direction
                ...
                setups = sorted(setups, key=lambda x: x['time'])
                res = run_simulation(m5_gold, m5_times, setups, tp_r=tp, stages=stg_30)
                print(f"[S20.103 VolRegime] wick={wick} depth={depth} TP={tp} -> PnL: ${res['pnl']:,.2f} | Trades: {res['trades']} | WR: {res['wr']:.1f}% | DD: ${res['max_dd']:.2f}")
