# -*- coding: utf-8 -*-
"""run_s20_136_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.136 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.136 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Paradigm 6: Institutional Inducement Theorem (LIT) + 42-Stage Dual-Engine Quantum Ratchet", flush=True)
    print("=" * 115, flush=True)
    ...
    res = run_simulation(m5_gold, m5_times, setups, tp_r=11.90, stages=stages)

    print("=" * 115, flush=True)
    print(" S20.136 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    pd.DataFrame(monthly_rows).to_csv(os.path.join(os.path.dirname(__file__), "S20_136_monthly.csv"), index=False)
    print(f"Monthly breakdown saved to S20_136_monthly.csv", flush=True)
