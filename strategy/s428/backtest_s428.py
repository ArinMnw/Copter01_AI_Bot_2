# -*- coding: utf-8 -*-
"""Standalone backtest runner สำหรับ S428 Flawless Victory Strategy —
long-only, ไม่มี security() เลยจึงไม่มีความเสี่ยง repaint (ต่างจาก S420/S427)
โครงสร้างคล้าย backtest_s424.py แต่เป็น "เข้า long ตอน buy trigger, ปิด flat
ตอน sell trigger หรือ SL/TP" ไม่ใช่ reversal (ไม่มี short เลยในต้นฉบับ)

Usage:
    python backtest_s428.py --start 2026-06-01 --end 2026-08-13 --tf M15
    python backtest_s428.py --days 60 --tf all --cfg-json '{"VERSION":2}'
"""
...
from strategy428 import DEFAULT_CFG, _bb_series, _mfi_series, _rsi_series
...
def _trigger_series(bars_norm, cfg):
    version = int(cfg["VERSION"])
    closes = [b["close"] for b in bars_norm]
    rsi = _rsi_series(closes, int(cfg["RSI_LEN"]))
    ...
    mfi = _mfi_series(bars_norm, int(cfg["MFI_LEN"])) if version == 3 else None
    ...
    for i in range(n):
        ...
        close_i = closes[i]
        if version == 1:
            buy[i] = close_i < lower[i] and rsi[i] > 42.0
            sell[i] = close_i > upper[i] and rsi[i] > 70.0
        elif version == 2:
            buy[i] = close_i < lower[i] and rsi[i] > 42.0
            sell[i] = close_i > upper[i] and rsi[i] > 76.0
        else:
            if mfi[i] is None:
                continue
            buy[i] = close_i < lower[i] and mfi[i] < 60.0
            sell[i] = close_i > upper[i] and rsi[i] > 65.0 and mfi[i] > 64.0
    return buy, sell, sl_pct, tp_pct
...
def backtest(tf_name, spread, lot, start_bkk, end_bkk, cfg=None,
    ...
    S420-S427 ของโปรเจกต์). ถ้าใส่ค่า -> โหมด "TV mode" (percent_of_equity
    ตาม Pine default_qty_value=100 — เข้าเต็ม equity ทุกไม้ ตรงต้นฉบับเป๊ะ)"""
    ...
    buy_series, sell_series, sl_pct, tp_pct = _trigger_series(bars_norm, detector_cfg)
    ...
    for index in range(max(start_index, 1), len(bars_norm)):
        ...
        if position is not None:
            if position["sl"] is not None and bar["low"] <= position["sl"]:
                ...
                position = None
            elif position["tp"] is not None and bar["high"] >= position["tp"]:
                close_position(index, position["tp"], "TP")
                position = None
            elif sell_series[index - 1]:
                fill_price = float(bar["open"])
                close_position(index, fill_price, "SIGNAL")
                position = None

        if position is None and buy_series[index - 1]:
            fill_price = float(bar["open"])
            entry_time = bar_time_to_bkk(bar["time"])
            signals += 1
            sl = round(fill_price * (1.0 - sl_pct / 100.0), 2) if sl_pct is not None else None
            tp = round(fill_price * (1.0 + tp_pct / 100.0), 2) if tp_pct is not None else None
            qty = (equity * pct_equity / 100.0 / fill_price) if tv_mode else None
            position = {
                "entry": fill_price, "sl": sl, "tp": tp, "qty": qty,
                "entry_time": entry_time,
                "pattern": f"S428 BUY Flawless Victory v{detector_cfg['VERSION']}",
...
def main():
    ...
    parser.add_argument("--cfg-json", default="{}",
                         help='JSON object ส่งเข้า cfg (เช่น {"VERSION":2})')
    parser.add_argument("--tv-mode", action="store_true",
                         help="เทียบ exact-match กับ TradingView: percent_of_equity 100%% ตาม Pine ต้นฉบับ")
    parser.add_argument("--pct-equity", type=float, default=100.0,
                         help="%% ของ equity ต่อไม้ (เฉพาะ --tv-mode, default 100 ตาม Pine default_qty_value)")
    ...
    parser.add_argument("--csv", default=os.path.join(_HERE, "s428_trades.csv"),
                         help="path ไฟล์ CSV ผลลัพธ์ (default = s428_trades.csv ในโฟลเดอร์นี้)")
    ...
    parser.add_argument("--compare-comment-prefix", default="S428",
                         help="กรอง deal ที่มี comment ขึ้นต้น/มีคำนี้เท่านั้น (default S428)")
