# -*- coding: utf-8 -*-
"""s20_48_research.py
Explore Cross-Breed Multi-Strategy Confluence on the EXACT SAME CANDLE to beat S20.47 (+$25,579.91):
Candidate Confluence Strategies to Integrate:
1. Strategy 11 (Fibonacci Golden Pocket 61.8% - 78.6% Zone Confluence)
2. Strategy 1 / 2 (Fair Value Gap - FVG Imbalance Mitigated / Rebalance)
3. Strategy 9 (RSI Structural Divergence & Dynamic Momentum Filter)
4. Strategy 4 / Market Structure Shift (MSS / Swing Structure Alignment)
5. Strategy 15 (Volume Profile / Multi-Session Value Area Extension)
...
def compute_indicators(rates, use_london=True, use_ny=True):
    ...
    # Fibonacci Retracement Levels based on 24-bar swing (Causal shifted by 1)
    # Range of recent swing = swing_high_24 - swing_low_24
    sw_range = df['swing_high_24'] - df['swing_low_24']
    df['fib_618_buy'] = df['swing_low_24'] + (0.382 * sw_range)   # Deep discount zone for BUY
    df['fib_618_sell'] = df['swing_low_24'] + (0.618 * sw_range)  # Deep premium zone for SELL
    df['fib_500'] = df['swing_low_24'] + (0.500 * sw_range)       # Equilibrium

    # Fair Value Gap (FVG) detection (Shifted by 1 - Causal)
    # Bullish FVG: Low of candle 0 > High of candle 2 (two candles ago)
    # Bearish FVG: High of candle 0 < Low of candle 2
    df['fvg_bull_zone'] = np.where(df['low'].shift(1) > df['high'].shift(3), df['high'].shift(3), np.nan)
    df['fvg_bear_zone'] = np.where(df['high'].shift(1) < df['low'].shift(3), df['low'].shift(3), np.nan)
...
def extract_setups(df, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, sl_mult=0.20, hours=(0, 22),
                   use_pyh=True, use_ldn=True, use_ny=True,
                   use_fib_zone=False, use_fvg=False, use_dynamic_rsi=True):
    setups = []
    h_start, h_end = hours
    ...
    for idx in range(45, len(records)):
        ...
        # Strategy 1 / 2 FVG Infiltration (Optional Confluence)
        if use_fvg:
        ...
        # Strategy 11 Fibonacci Golden Pocket / Equilibrium Check (Optional Confluence)
        fib_buy_ok = True
        fib_sell_ok = True
        if use_fib_zone:
            fib_b = cur.get('fib_618_buy', np.nan)
            fib_s = cur.get('fib_618_sell', np.nan)
            if not pd.isna(fib_b):
                fib_buy_ok = cur['low'] <= fib_b  # Must reach discount Fibonacci zone
            if not pd.isna(fib_s):
                fib_sell_ok = cur['high'] >= fib_s # Must reach premium Fibonacci zone

        has_wick_buy = (cur['lower_wick_pct'] >= min_wick) or (cur['lower_wick'] >= 1.1 * cur['body'])
        closed_high = cur['close'] >= (cur['low'] + 0.45 * cur['range'])
        rsi_ok_buy = cur['rsi'] <= 68.0 if use_dynamic_rsi else True

        has_wick_sell = (cur['upper_wick_pct'] >= min_wick) or (cur['upper_wick'] >= 1.1 * cur['body'])
        closed_low = cur['close'] <= (cur['high'] - 0.45 * cur['range'])
        rsi_ok_sell = cur['rsi'] >= 32.0 if use_dynamic_rsi else True

        sig = None
        if has_wick_buy and closed_high and rsi_ok_buy and swept_low and fib_buy_ok:
            sig = "BUY"
        elif has_wick_sell and closed_low and rsi_ok_sell and swept_high and fib_sell_ok:
            sig = "SELL"
...
def run_simulation(setups, m5_gold, m5_times, m5_len,
                   ...
                   lock10_trig=5.6, lock10_amt=5.4, lock11_trig=5.8, lock11_amt=5.6,
                   lock12_trig=None, lock12_amt=None, tp_r=6.05):
    ...
    for s in setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if lock12_trig and b['high'] >= l12_target:
                    l12_active = True
                ...
                if lock12_trig and b['low'] <= l12_target:
                    l12_active = True
...
def main():
    ...
    print(" S20.48 MULTI-STRATEGY CROSS-BREED CONFLUENCE RESEARCH (TARGET > $25,579.91)", flush=True)
    print(" Benchmark S20.47: +$25,579.91 | MaxDD $12.96 | WR 87.4% | Trades 3,035", flush=True)
    print("=" * 115, flush=True)

    # Experiment 1: Integrate Strategy 11 (Fibonacci Golden Pocket 61.8% Zone Confluence)
    print("\n--- 1. Testing Strategy 11 (Fibonacci 61.8% Zone Confluence) ---", flush=True)
    s_list_fib = []
    for tf_label, df_tf in base_tfs:
        s_list_fib.extend(extract_setups(df_tf, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True, use_fib_zone=True))
    comb_fib = sorted(s_list_fib, key=lambda x: x['time'])
    res_fib = run_simulation(comb_fib, m5_gold, m5_times, m5_len, tp_r=6.05)
    print(f"S11 Fib Confluence | Trades: {res_fib['trades']} | W/BE/L: {res_fib['wins']}/{res_fib['bes']}/{res_fib['losses']} | WR: {res_fib['wr']:.1f}% | Net: ${res_fib['pnl']:.2f} | MaxDD: ${res_fib['max_dd']:.2f}", flush=True)

    # Experiment 2: Integrate Strategy 1 / 2 (Fair Value Gap - FVG Imbalance Mitigated)
    print("\n--- 2. Testing Strategy 1/2 (Fair Value Gap - FVG Imbalance Confluence) ---", flush=True)
    s_list_fvg = []
    for tf_label, df_tf in base_tfs:
        s_list_fvg.extend(extract_setups(df_tf, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True, use_fvg=True))
    comb_fvg = sorted(s_list_fvg, key=lambda x: x['time'])
    res_fvg = run_simulation(comb_fvg, m5_gold, m5_times, m5_len, tp_r=6.05)
    print(f"S1/S2 FVG Confluence | Trades: {res_fvg['trades']} | W/BE/L: {res_fvg['wins']}/{res_fvg['bes']}/{res_fvg['losses']} | WR: {res_fvg['wr']:.1f}% | Net: ${res_fvg['pnl']:.2f} | MaxDD: ${res_fvg['max_dd']:.2f}", flush=True)

    # Experiment 3: Both FVG + Retest depth + Dodeca-Stage Trailing (Lock 12 @ +6.0R -> +5.8R, TP 6.20R)
    print("\n--- 3. Testing FVG + Dodeca-Stage Trailing (TP 6.10R - 6.30R) ---", flush=True)
    for tp in [6.10, 6.15, 6.20, 6.25]:
        res_fvg_tp = run_simulation(comb_fvg, m5_gold, m5_times, m5_len,
                                    lock12_trig=6.0, lock12_amt=5.8, tp_r=tp)
        print(f"FVG + TP={tp:.2f}R | Trades: {res_fvg_tp['trades']} | W/BE/L: {res_fvg_tp['wins']}/{res_fvg_tp['bes']}/{res_fvg_tp['losses']} | WR: {res_fvg_tp['wr']:.1f}% | Net: ${res_fvg_tp['pnl']:.2f} | MaxDD: ${res_fvg_tp['max_dd']:.2f}", flush=True)

    # Experiment 4: Dodeca-Stage Trailing on Base (TP 6.10R - 6.30R)
    print("\n--- 4. Testing Dodeca-Stage Trailing on Base S20.47 ---", flush=True)
    s_list_base = []
    for tf_label, df_tf in base_tfs:
        s_list_base.extend(extract_setups(df_tf, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True))
    comb_base = sorted(s_list_base, key=lambda x: x['time'])
    for tp in [6.10, 6.15, 6.20, 6.25]:
        res_base_tp = run_simulation(comb_base, m5_gold, m5_times, m5_len,
                                     lock12_trig=6.0, lock12_amt=5.8, tp_r=tp)
        print(f"Base + TP={tp:.2f}R | Trades: {res_base_tp['trades']} | W/BE/L: {res_base_tp['wins']}/{res_base_tp['bes']}/{res_base_tp['losses']} | WR: {res_base_tp['wr']:.1f}% | Net: ${res_base_tp['pnl']:.2f} | MaxDD: ${res_base_tp['max_dd']:.2f}", flush=True)
