...
# S20.127 had 4,047 trades!
# Let's find which parameters produce exactly 4,047 trades:
for hrs in [(0,23), (0,22)]:
    for vsa in [1.11, 1.12, 1.13]:
        for wick in [0.08, 0.09, 0.10, 0.11, 0.12, 0.14]:
            for depth in [0.124, 0.125]:
                for judas_b in [True, False]:
                    for vol_g in [True, False]:
                        for cvd_m in ['none', 'soft_gate']:
                            setups = []
                            for tf, df_tf in dfs.items():
                                for cur in df_tf.to_dict('records'):
                                    if pd.isna(cur['atr']) or cur['atr'] <= 0.05 or cur['range'] < 0.45 * cur['atr'] or cur['vol_ratio'] < vsa: continue
                                    if not (hrs[0] <= cur['hour'] <= hrs[1]): continue
                                    ...
                                    cvd_ok_buy = (bull_cvd or is_judas) if cvd_m == 'soft_gate' else True
                                    cvd_ok_sell = (bear_cvd or is_judas) if cvd_m == 'soft_gate' else True
                                    sig = "BUY" if (swept_low and has_wick_buy and closed_high and cvd_ok_buy) else ("SELL" if (swept_high and has_wick_sell and closed_low and cvd_ok_sell) else None)
                                    if sig:
                            ...
                            stg34 = [(1.5, 1.0), (2.4, 2.0), (3.0, 2.8), (3.5, 3.3), (3.8, 3.5), (4.2, 3.9), (4.6, 4.3), (5.2, 4.9), (5.5, 5.2), (5.6, 5.4), (5.8, 5.6), (6.0, 5.8), (6.15, 5.95), (6.35, 6.15), (6.5, 6.3), (6.7, 6.5), (6.85, 6.65), (7.0, 6.8), (7.15, 6.95), (7.3, 7.1), (7.45, 7.25), (7.6, 7.4), (7.75, 7.55), (7.9, 7.7), (8.05, 7.85), (8.2, 8.0), (8.35, 8.15), (8.5, 8.3), (8.65, 8.45), (8.80, 8.60), (8.95, 8.75), (9.10, 8.90), (9.25, 9.05), (9.40, 9.20)]
                            res = run_simulation(m5_gold, m5_times, setups, tp_r=11.25, stages=stg34)
                            if res['trades'] == 4047:
                                print(f"MATCH S20.127! hrs={hrs} vsa={vsa} wick={wick} depth={depth} judas={judas_b} vol={vol_g} cvd={cvd_m} -> PnL: ${res['pnl']:,.2f} Trades: {res['trades']}")
                                sys.exit(0)
