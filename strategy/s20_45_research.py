# -*- coding: utf-8 -*-
"""s20_45_research.py
Explore quantum breakthroughs to beat S20.44 (+$19,049.30) on STRICT LOT 0.01:
Hard Constraints:
...
1. Addition of New York Pre-Close Liquidity Pool (NY AM High/Low sweeps after 17:00 UTC).
2. Asian Early Ignition: Session Window starting at 00:00 vs 00:30 vs 01:00 UTC.
3. Duo-Deca (11/12-Stage) Ratchet Lock Tuning (TP 5.8R, 5.9R, 6.0R, 6.1R).
4. Retest Depth Precision (0.119, 0.120, 0.121, 0.122).
"""
...
def extract_setups(df, tf_label, min_vol=1.20, min_wick=0.38, retest_depth=0.121, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True, use_ny=False):
    setups = []
    h_start, h_end = hours
...
def main():
    ...
    print(" S20.45 QUANT RESEARCH (TARGET > $19,049.30 ON STRICT LOT 0.01)", flush=True)
    print(" Benchmark S20.44: +$19,049.30 | MaxDD $12.94 | WR 88.5% | PF 46.17", flush=True)
    print("=" * 115, flush=True)
    ...
    # Test 1: New York Session High/Low (PNYH/PNYL) Sweeps
    print("--- 1. Testing New York Pre-Close Liquidity Sweeps ---", flush=True)
    for use_ny in [False, True]:
        s_list = []
        for tf_label, df_tf in base_tfs:
            s_list.extend(extract_setups(df_tf, tf_label, retest_depth=0.121, hours=(1, 22), use_pyh=True, use_ldn=True, use_ny=use_ny))
        ...
        lbl = "+New York PM Sweep" if use_ny else "Base S20.44"
    ...
    # Test 2: Asian Session Start Window (00:00 vs 00:30 vs 01:00 UTC)
    print("\n--- 2. Testing Asian Session Ignition Hours ---", flush=True)
    for win in [(0, 22), (1, 22)]:
        s_list = []
        for tf_label, df_tf in base_tfs:
            s_list.extend(extract_setups(df_tf, tf_label, retest_depth=0.121, hours=win, use_pyh=True, use_ldn=True, use_ny=False))
        comb = sorted(s_list, key=lambda x: x['time'])
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
                             be_trigger=0.8, lock1_trig=1.5, lock1_amt=1.0,
                             lock2_trig=2.4, lock2_amt=2.0, lock3_trig=3.0, lock3_amt=2.8,
    ...
    # Test 3: Trailing Fine-Tuning around TP 5.8R - 6.0R
    print("\n--- 3. Testing Duo-Deca Trailing Configurations ---", flush=True)
    s_list = []
    for tf_label, df_tf in base_tfs:
        s_list.extend(extract_setups(df_tf, tf_label, retest_depth=0.121, hours=(1, 22), use_pyh=True, use_ldn=True, use_ny=False))
    comb = sorted(s_list, key=lambda x: x['time'])

    configs = [
        ("Base S20.44 TP 5.8R", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.5, 5.2, 0.0, 0.0, 5.8),
        ("Tune-A: TP 5.85R (L9@5.5->5.2)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.5, 5.2, 0.0, 0.0, 5.85),
        ("Tune-B: TP 5.90R (L9@5.5->5.2)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.5, 5.2, 0.0, 0.0, 5.90),
        ("DuoDeca-A: TP 6.0R (L10@5.7->5.4)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.5, 5.2, 5.7, 5.4, 6.0),
        ("DuoDeca-B: TP 6.0R (L10@5.8->5.5)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.5, 5.2, 5.8, 5.5, 6.0),
    ]
    for name, be, l1_t, l1_a, l2_t, l2_a, l3_t, l3_a, l4_t, l4_a, l5_t, l5_a, l6_t, l6_a, l7_t, l7_a, l8_t, l8_a, l9_t, l9_a, l10_t, l10_a, tp in configs:
        ...
        print(f"{name:34s} | Trades: {res['trades']:4d} | W/BE/L: {res['wins']:3d}/{res['bes']:3d}/{res['losses']:2d} | WR: {res['wr']:4.1f}% | Net: ${res['pnl']:8.2f} | PF: {res['pf']:5.2f} | MaxDD: ${res['max_dd']:5.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)
