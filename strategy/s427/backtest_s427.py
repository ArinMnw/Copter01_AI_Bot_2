# -*- coding: utf-8 -*-
"""Standalone backtest runner สำหรับ S427 Open Close Cross Strategy —
โครงสร้างเหมือน backtest_s425.py (precompute signal ทั้งชุดครั้งเดียว,
reversal-only ไม่มี SL/TP ตาม default, market fill ที่ open ของแท่งถัดไป)
ต่างตรงสัญญาณมาจาก SMMA(close) x SMMA(open) ครอสกันบน Alt-TF (⚠️ non-repaint
เท่านั้น — ดู docstring บนสุดของ strategy427.py สำหรับคำเตือนเรื่อง repaint
ของต้นฉบับ Pine)

รองรับ --tv-mode (percent_of_equity sizing ตรงตาม Pine default_qty_value=10,
ไม่มี commission ใน Pine ต้นฉบับตัวนี้ — default_qty_type=percent_of_equity
เท่านั้น ไม่มี commission_type= ระบุไว้ = commission เริ่มต้น 0% ของ TradingView)

Usage:
    python backtest_s427.py --start 2026-07-20 --end 2026-08-13 --tf M5
    python backtest_s427.py --days 60 --tf all --tv-mode
"""
...
from strategy427 import DEFAULT_CFG, TF_MINUTES, _aggregate_alt, _cross_series
...
def _side_lookup(bars_norm, base_minutes, mult, ma_len):
    """คืน list ยาวเท่า bars_norm ของ ("BUY"/"SELL"/None) — สัญญาณล่าสุดที่มา
    จาก cross บน Alt-TF ที่ปิดสมบูรณ์แล้วก่อนแท่งนี้ (activation ตอน alt-bar
    ปิด = non-repaint)"""
    n = len(bars_norm)
    out = [None] * n
    alt_bars = _aggregate_alt(bars_norm, base_minutes, mult)
    crosses = _cross_series(alt_bars, ma_len)
    alt_seconds = base_minutes * mult * 60

    activations = []
    for alt_bar, value in zip(alt_bars, crosses):
        if value == 0:
            continue
        activation_time = alt_bar["time"] + alt_seconds
        activations.append((activation_time, "BUY" if value == 1 else "SELL"))
...
def backtest(tf_name, spread, lot, start_bkk, end_bkk, cfg=None,
    ...
    """pct_equity=None -> โหมดเดิม (fixed lot + flat spread, ตรง convention
    S420-S426 ของโปรเจกต์). ถ้าใส่ค่า -> โหมด "TV mode" (percent_of_equity
    sizing ตาม Pine default_qty_value=10, commission_pct default 0 เพราะ
    ต้นฉบับไม่ได้ตั้ง commission_type= เลย)"""
    detector_cfg = dict(DEFAULT_CFG)
    detector_cfg.update(cfg or {})
    mult = max(1, int(detector_cfg["ALT_MULT"])) if bool(detector_cfg["USE_ALT_RES"]) else 1
    ma_len = max(1, int(detector_cfg["MA_LEN"]))
    ...
    base_minutes = TF_MINUTES[tf_name]
    sides = _side_lookup(bars_norm, base_minutes, mult, ma_len)
    ...
    for index in range(start_index, len(bars_norm) - 1):
        ...
        if side_str is None:
            continue
        side_wanted = 1 if side_str == "BUY" else -1
        ...
        position = {
            ...
            "entry_time": entry_time, "pattern": f"S427 {side_str} Open Close Cross",
            "reason": f"OCC SMMA{ma_len} altTFx{mult} ATR={atr_str}",
        }
...
def main():
    ...
    parser.add_argument("--cfg-json", default="{}",
                         help="JSON object ส่งเข้า cfg (เช่น MA_LEN/ALT_MULT/SL_ATR_MULT)")
    ...
    parser.add_argument("--tv-mode", action="store_true",
                         help="เทียบ exact-match กับ TradingView: percent_of_equity sizing ตาม "
                              "Pine ต้นฉบับ (แทน fixed lot + flat spread)")
    parser.add_argument("--pct-equity", type=float, default=10.0,
                         help="%% ของ equity ต่อไม้ (เฉพาะ --tv-mode, default 10 ตาม Pine default_qty_value)")
    parser.add_argument("--commission-pct", type=float, default=0.0,
                         help="%% commission ต่อ order (เฉพาะ --tv-mode, default 0 — ต้นฉบับไม่ได้ตั้ง commission_type)")
    parser.add_argument("--tv-balance", type=float, default=100000.0,
                         help="ยอดเงินตั้งต้น (เฉพาะ --tv-mode, default 100000 ตาม TV)")
    parser.add_argument("--csv", default=os.path.join(_HERE, "s427_trades.csv"),
                         help="path ไฟล์ CSV ผลลัพธ์ (default = s427_trades.csv ในโฟลเดอร์นี้)")
    ...
    parser.add_argument("--compare-comment-prefix", default="S427",
                         help="กรอง deal ที่มี comment ขึ้นต้น/มีคำนี้เท่านั้น (default S427)")
