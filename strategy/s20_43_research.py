# -*- coding: utf-8 -*-
"""s20_43_research.py
Explore quantum breakthroughs to beat S20.42 (+$16,758.10) on STRICT LOT 0.01:
Hard Constraints:
...
1. Addition of MT5 Native Timeframes: M10 (10-Min), M12 (12-Min), H6, H8, H12.
2. Trailing Ratchet Tuning: Deca-Stage (10-Stage) Trailing (TP 5.8R, 6.0R, 6.2R).
3. Precision Retest Depth: 0.120, 0.122, 0.124, 0.126, 0.128.
4. Confluence Enhancement: Fair Value Gap (FVG) / Imbalance validation or Session Open Price Sweep.
"""
...
def extract_setups(df, tf_label, min_vol=1.20, min_wick=0.38, retest_depth=0.128, sl_mult=0.20, hours=(1, 22), use_pyh=True):
    setups = []
    h_start, h_end = hours
...
def run_simulation(setups, m5_gold, m5_times, m5_len,
                   ...
                   lock9_trig=0.0, lock9_amt=0.0,
                   tp_r=5.6, dedup_sec=900):
    lot = 0.01
    contract_size = 100.0
    ...
    for s in setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if lock9_trig > 0 and not l9_active and b['high'] >= l9_target:
                    l9_active = True
                    current_sl = l9_sl
                elif lock8_trig > 0 and not l8_active and b['high'] >= l8_target:
                    l8_active = True
                    current_sl = l8_sl
                ...
                if lock9_trig > 0 and not l9_active and b['low'] <= l9_target:
                    l9_active = True
                    current_sl = l9_sl
                elif lock8_trig > 0 and not l8_active and b['low'] <= l8_target:
                    l8_active = True
                    current_sl = l8_sl
...
def main():
    ...
    m10_gold = mt5.copy_rates_range(symbol_gold, mt5.TIMEFRAME_M10, start_dt, now)
    ...
    df_h8 = compute_indicators(h8_gold) if h8_gold is not None and len(h8_gold) > 50 else None
    df_h6 = compute_indicators(h6_gold) if h6_gold is not None and len(h6_gold) > 50 else None
    ...
    df_m12 = compute_indicators(m12_gold) if m12_gold is not None and len(m12_gold) > 50 else None
    df_m10 = compute_indicators(m10_gold) if m10_gold is not None and len(m10_gold) > 50 else None

    m5_times = [int(r['time']) for r in m5_gold]
    m5_len = len(m5_gold)

    print("=" * 115, flush=True)
    print(" S20.43 QUANT RESEARCH (TARGET > $16,758.10 ON STRICT LOT 0.01)", flush=True)
    print(" Benchmark S20.42: +$16,758.10 | MaxDD $13.10 | WR 87.7% | PF 46.37", flush=True)
    print("=" * 115, flush=True)

    # Base S20.42 setups
    s_h4 = extract_setups(df_h4, "H4", retest_depth=0.128, hours=(1, 22), use_pyh=True)
    s_h3 = extract_setups(df_h3, "H3", retest_depth=0.128, hours=(1, 22), use_pyh=True)
    s_h2 = extract_setups(df_h2, "H2", retest_depth=0.128, hours=(1, 22), use_pyh=True)
    s_h1 = extract_setups(df_h1, "H1", retest_depth=0.128, hours=(1, 22), use_pyh=True)
    s_m30 = extract_setups(df_m30, "M30", retest_depth=0.128, hours=(1, 22), use_pyh=True)
    s_m20 = extract_setups(df_m20, "M20", retest_depth=0.128, hours=(1, 22), use_pyh=True)
    s_m15 = extract_setups(df_m15, "M15", retest_depth=0.128, hours=(1, 22), use_pyh=True)
    base_s20_42 = s_h4 + s_h3 + s_h2 + s_h1 + s_m30 + s_m20 + s_m15

    # Test 1: Additional Horizons (+M12, +M10, +H6, +H8)
    print("--- 1. Testing Extra Horizons on S20.42 Base ---", flush=True)
    extras = [
        ("Base S20.42", []),
        ("+M12", extract_setups(df_m12, "M12", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_m12 is not None else []),
        ("+M10", extract_setups(df_m10, "M10", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_m10 is not None else []),
        ("+H6", extract_setups(df_h6, "H6", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_h6 is not None else []),
        ("+H8", extract_setups(df_h8, "H8", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_h8 is not None else []),
        ("+M12 +H6", (extract_setups(df_m12, "M12", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_m12 is not None else []) + (extract_setups(df_h6, "H6", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_h6 is not None else [])),
        ("+M10 +H6", (extract_setups(df_m10, "M10", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_m10 is not None else []) + (extract_setups(df_h6, "H6", retest_depth=0.128, hours=(1, 22), use_pyh=True) if df_h6 is not None else [])),
    ]

    for label, extra_list in extras:
        comb = sorted(base_s20_42 + extra_list, key=lambda x: x['time'])
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
                             ...
                             lock8_trig=5.2, lock8_amt=4.9, tp_r=5.6)
        print(f"{label:24s} | Trades: {res['trades']:4d} | W/BE/L: {res['wins']:3d}/{res['bes']:3d}/{res['losses']:2d} | WR: {res['wr']:4.1f}% | Net: ${res['pnl']:8.2f} | PF: {res['pf']:5.2f} | MaxDD: ${res['max_dd']:5.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)

    # Test 2: Retest Depth fine tuning on S20.42
    print("\n--- 2. Testing Retest Depth Tuning (0.120 to 0.128) ---", flush=True)
    for d in [0.120, 0.122, 0.124, 0.125, 0.126, 0.128]:
        s = (extract_setups(df_h4, "H4", retest_depth=d, hours=(1, 22), use_pyh=True) +
             extract_setups(df_h3, "H3", retest_depth=d, hours=(1, 22), use_pyh=True) +
             extract_setups(df_h2, "H2", retest_depth=d, hours=(1, 22), use_pyh=True) +
             extract_setups(df_h1, "H1", retest_depth=d, hours=(1, 22), use_pyh=True) +
             extract_setups(df_m30, "M30", retest_depth=d, hours=(1, 22), use_pyh=True) +
             extract_setups(df_m20, "M20", retest_depth=d, hours=(1, 22), use_pyh=True) +
             extract_setups(df_m15, "M15", retest_depth=d, hours=(1, 22), use_pyh=True))
        comb = sorted(s, key=lambda x: x['time'])
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
                             be_trigger=0.8, lock1_trig=1.5, lock1_amt=1.0,
                             lock2_trig=2.4, lock2_amt=2.0, lock3_trig=3.0, lock3_amt=2.8,
                             lock4_trig=3.5, lock4_amt=3.3, lock5_trig=3.8, lock5_amt=3.5,
    ...
    # Test 3: Trailing Ratchet Engine - Deca-Stage (10-Stage) Trailing (TP 5.8R, 6.0R, 6.2R)
    print("\n--- 3. Testing Deca-Stage Ratchet Trailing on S20.42 Base ---", flush=True)
    comb_s = sorted(base_s20_42, key=lambda x: x['time'])
    configs = [
        ("Base Nona TP 5.6R", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 0.0, 0.0, 5.6),
        ("Deca-A: TP 5.8R (L9@5.5->5.2)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.5, 5.2, 5.8),
        ("Deca-B: TP 6.0R (L9@5.6->5.3)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.6, 5.3, 6.0),
        ("Deca-C: TP 6.2R (L9@5.7->5.4)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 5.2, 4.9, 5.7, 5.4, 6.2),
    ]

    for name, be, l1_t, l1_a, l2_t, l2_a, l3_t, l3_a, l4_t, l4_a, l5_t, l5_a, l6_t, l6_a, l7_t, l7_a, l8_t, l8_a, l9_t, l9_a, tp in configs:
        res = run_simulation(comb_s, m5_gold, m5_times, m5_len,
                             ...
                             lock8_trig=l8_t, lock8_amt=l8_a, lock9_trig=l9_t, lock9_amt=l9_a, tp_r=tp)
