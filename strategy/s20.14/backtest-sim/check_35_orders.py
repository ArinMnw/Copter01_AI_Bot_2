...
for o in orders:
    ...
    if matches:
        ...
        if exact_matches:
            ...
            o['match_info'] = f"EXACT MATCH in {m_t['file']} (Bot E:{m_t['entry']} SL:{m_t['sl']} TP:{m_t['tp']})"
        else:
            o['matched_time_only'] = True
            m_t = matches[0][0]
            o['match_info'] = f"TIME MATCH ONLY in {m_t['file']} (Bot E:{m_t['entry']} SL:{m_t['sl']} TP:{m_t['tp']})"
