# -*- coding: utf-8 -*-
"""s20_37_research.py
Explore quantitative breakthroughs to beat S20.36 (+$7,663.53) on STRICT LOT 0.01:
Rules strictly maintained:
1. Strict 0.01 lot single position (no scaling, no splits).
2. Zero look-ahead bias (backward-looking indicators, shifted swings).
3. Zero "ชน SL แล้วชน TP" (executed on 75,000+ real M5 bars, SL-first on every bar).
4. Zero magic fill (realistic M5 limit retest fill within 2 hours).
5. Zero repaint.

Test Dimensions:
1. Retest Depths: 0.12, 0.14, 0.15, 0.18
2. Trailing Engine Options:
   - Base S20.36: BE@0.8R, +1.0R@1.5R, +2.0R@2.4R, +2.8R@3.0R, TP 3.5R
   - Extended TP 3.8R / 4.0R with 5-stage lock
3. M5 Sniper Setup addition during London/NY peak volume hours (08-11h, 13-17h)
"""
...
def extract_setups_fast(df, tf_label, min_vol=1.30, min_wick=0.38, retest_depth=0.180, hours=(7, 21)):
    setups = []
    h_start, h_end = hours
...
def run_simulation(setups, m5_gold, m5_times, m5_len,
                   ...
                   lock4_trig=0.0, lock4_amt=0.0,
                   tp_r=3.5, dedup_sec=900):
    lot = 0.01
    contract_size = 100.0
...
def main():
    ...
    print(" S20.37 QUANT RESEARCH (STRICT LOT 0.01 | M5 SL-FIRST)", flush=True)
    print(" Benchmark to beat: S20.36 (+$7,663.53 | MaxDD $16.46 | WR 86.5%)", flush=True)
    print("=" * 115, flush=True)

    # 1. Test Retest Depth: 0.14, 0.15, 0.16, 0.18
    for depth in [0.14, 0.15, 0.16, 0.18]:
        s0 = extract_setups_fast(df_h4, "H4", retest_depth=depth, hours=(7, 21))
        s1 = extract_setups_fast(df_h1, "H1", retest_depth=depth, hours=(7, 21))
        s2 = extract_setups_fast(df_m30, "M30", retest_depth=depth, hours=(7, 21))
        s3 = extract_setups_fast(df_m15, "M15", retest_depth=depth, hours=(7, 21))
        comb = sorted(s0 + s1 + s2 + s3, key=lambda x: x['time'])

        for tp in [3.2, 3.5, 3.8]:
            res = run_simulation(comb, m5_gold, m5_times, m5_len, be_trigger=0.8, lock1_trig=1.5, lock1_amt=1.0, lock2_trig=2.4, lock2_amt=2.0, lock3_trig=3.0, lock3_amt=2.8, tp_r=tp)
            print(f"Depth: {depth:.2f} | TP: {tp:.1f}R | Trades: {res['trades']:3d} | W/BE/L: {res['wins']:3d}/{res['bes']:3d}/{res['losses']:2d} | WR: {res['wr']:4.1f}% | NetPnL: ${res['pnl']:8.2f} | PF: {res['pf']:5.2f} | MaxDD: ${res['max_dd']:5.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)

    # 2. Test 5-Stage Ratchet Trailing (Extend to 3.8R and 4.0R with Stage 4 and 5)
    print("-" * 115, flush=True)
    s0 = extract_setups_fast(df_h4, "H4", retest_depth=0.15, hours=(7, 21))
    s1 = extract_setups_fast(df_h1, "H1", retest_depth=0.15, hours=(7, 21))
    s2 = extract_setups_fast(df_m30, "M30", retest_depth=0.15, hours=(7, 21))
    s3 = extract_setups_fast(df_m15, "M15", retest_depth=0.15, hours=(7, 21))
    comb_15 = sorted(s0 + s1 + s2 + s3, key=lambda x: x['time'])

    trailers = [
        ("Base S20.36 Trailing (TP 3.5R)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 0.0, 0.0, 3.5),
        ("S20.37-A: Quad-Stage (BE@0.8, +1.0@1.5, +2.0@2.4, +2.8@3.0, TP 3.8R)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 0.0, 0.0, 3.8),
        ("S20.37-B: Penta-Stage (BE@0.8, +1.0@1.5, +2.0@2.4, +2.8@3.0, +3.3@3.5, TP 4.0R)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 4.0),
        ("S20.37-C: Penta-Stage (BE@0.75, +1.0@1.4, +2.0@2.3, +2.8@2.9, +3.3@3.5, TP 4.0R)", 0.75, 1.4, 1.0, 2.3, 2.0, 2.9, 2.8, 3.5, 3.3, 4.0),
    ]

    for name, be, l1_t, l1_a, l2_t, l2_a, l3_t, l3_a, l4_t, l4_a, tp in trailers:
        res = run_simulation(comb_15, m5_gold, m5_times, m5_len, be_trigger=be, lock1_trig=l1_t, lock1_amt=l1_a, lock2_trig=l2_t, lock2_amt=l2_a, lock3_trig=l3_t, lock3_amt=l3_a, lock4_trig=l4_t, lock4_amt=l4_a, tp_r=tp)
        print(f"{name:75s} | Trades: {res['trades']:3d} | W/BE/L: {res['wins']:3d}/{res['bes']:3d}/{res['losses']:2d} | WR: {res['wr']:4.1f}% | NetPnL: ${res['pnl']:8.2f} | PF: {res['pf']:5.2f} | MaxDD: ${res['max_dd']:5.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)
