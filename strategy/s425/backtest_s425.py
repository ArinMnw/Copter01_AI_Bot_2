# -*- coding: utf-8 -*-
"""Standalone backtest runner สำหรับ S425 D1 Candle Color Reversal — โครงสร้าง
เหมือน backtest_s424.py ทุกจุด (precompute daily bucket ครั้งเดียว, reversal-
only ไม่มี SL/TP ตาม default, rollover offset เดียวกัน) ต่างแค่สัญญาณ (สีแท่ง
D1 แทนการเทียบ close ข้ามวัน) — เคยลองเปลี่ยนเป็นแบบ "แท่งแรกของวัน" แล้ว
backtest ยืนยันว่าแย่กว่าทุก TF (ดู docstring ของ strategy425.py) จึงกลับมาใช้
D1 เต็มวันเป็นหลัก

Usage:
    python backtest_s425.py --start 2026-06-01 --end 2026-08-11 --tf M15
    python backtest_s425.py --days 60 --tf M15
    python backtest_s425.py --days 60 --tf all   (M1/M5/M15/M30/H1)
"""
...
for _p in (_ROOT, _S424_DIR, _HERE):
...
from strategy425 import DEFAULT_CFG
...
def _color_lookup(bars_norm, daily):
    """คืน list ความยาวเท่า bars_norm ของ (buying: True/False หรือ None) — สีของ
    แท่ง D1 ล่าสุดที่ปิดสมบูรณ์แล้วก่อนแท่งนี้ (ค่าคงที่ทั้งวัน อัปเดตตอน rollover)"""
    n = len(bars_norm)
    out = [None] * n
    if len(daily) < 1:
        return out
    activations = []
    for d in daily:
        activation_time = d["time"] + DAY_SECONDS
        if d["close"] > d["open"]:
            buying = True
        elif d["close"] < d["open"]:
            buying = False
        else:
            continue  # doji เมื่อวาน — ข้าม ให้คงค่าก่อนหน้า
        activations.append((activation_time, buying))
...
def backtest(tf_name, spread, lot, start_bkk, end_bkk, cfg=None):
    ...
    colors = _color_lookup(bars_norm, daily)
    ...
    for index in range(start_index, len(bars_norm) - 1):
        ...
        if buying is None:
            continue
        side_wanted = 1 if buying else -1
        ...
        position = {
            ...
            "entry_time": entry_time, "pattern": f"S425 {signal_name} D1 Candle Color",
            "reason": f"D1 color flip ATR={atr_str}",
        }
...
def main():
    ...
    parser.add_argument("--cfg-json", default="{}",
                         help="JSON object ส่งเข้า cfg (เช่น SL_ATR_MULT/TP_RR)")
    parser.add_argument("--use-atr-stop", action="store_true",
                         help="เปิด ATR-based SL/TP (default ปิด — reversal-only ตามไอเดียเดิม)")
    parser.add_argument("--csv", default=os.path.join(_HERE, "s425_trades.csv"),
                         help="path ไฟล์ CSV ผลลัพธ์ (default = s425_trades.csv ในโฟลเดอร์นี้)")
    ...
    parser.add_argument("--compare-comment-prefix", default="S425",
                         help="กรอง deal ที่มี comment ขึ้นต้น/มีคำนี้เท่านั้น (default S425)")
