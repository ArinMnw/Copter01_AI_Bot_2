# -*- coding: utf-8 -*-
"""s20_42_research.py
Explore quantum breakthroughs to beat S20.41 (+$13,456.19) on STRICT LOT 0.01:
Hard Constraints:
...
1. Window (1, 22) vs (1.5, 22) vs (2, 22) with H3 + H2 + Yearly Liquidity.
2. Addition of M20 (20-Minute) and H6 (6-Hour) native horizons.
3. Nona-Stage and Deca-Stage (10-Stage) Ratchet Trailing (TP 5.2R, 5.4R, 5.5R).
4. Retest Depth Precision (0.125, 0.128, 0.130).
"""
...
def extract_setups(df, tf_label, min_vol=1.20, min_wick=0.38, retest_depth=0.130, sl_mult=0.20, hours=(1, 22), use_pyh=True):
    setups = []
    h_start, h_end = hours
...
def main():
    ...
    df_h6 = compute_indicators(h6_gold)
    ...
    print(" S20.42 QUANT RESEARCH (TARGET > $13,456.19 ON STRICT LOT 0.01)", flush=True)
    print(" Benchmark S20.41: +$13,456.19 | MaxDD $13.77 | WR 86.7% | PF 45.78", flush=True)
    print("=" * 115, flush=True)

    # Test 1: Window (1, 22) vs (2, 22) on S20.41 Core (H4, H3, H2, H1, M30, M15)
    print("--- 1. Testing Session Window (01:00 vs 02:00) with H3 ---", flush=True)
    for win in [(2, 22), (1, 22)]:
    ...
    # Test 2: Testing Extra Horizons: +M20 and +H6 on Window (1, 22)
    print("\n--- 2. Testing Additional Horizons (+M20, +H6) on Window (1, 22) ---", flush=True)
    base_s = (extract_setups(df_h4, "H4", hours=(1, 22), use_pyh=True) +
              extract_setups(df_h3, "H3", hours=(1, 22), use_pyh=True) +
              extract_setups(df_h2, "H2", hours=(1, 22), use_pyh=True) +
              extract_setups(df_h1, "H1", hours=(1, 22), use_pyh=True) +
              extract_setups(df_m30, "M30", hours=(1, 22), use_pyh=True) +
              extract_setups(df_m15, "M15", hours=(1, 22), use_pyh=True))

    for add_label, extra_s in [("Base (H4-H3-H2-H1-M30-M15)", []),
                               ("+M20", extract_setups(df_m20, "M20", hours=(1, 22), use_pyh=True)),
                               ("+H6", extract_setups(df_h6, "H6", hours=(1, 22), use_pyh=True)),
                               ("+M20 +H6", extract_setups(df_m20, "M20", hours=(1, 22), use_pyh=True) + extract_setups(df_h6, "H6", hours=(1, 22), use_pyh=True))]:
        comb = sorted(base_s + extra_s, key=lambda x: x['time'])
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
                             be_trigger=0.8, lock1_trig=1.5, lock1_amt=1.0,
                             lock2_trig=2.4, lock2_amt=2.0, lock3_trig=3.0, lock3_amt=2.8,
                             lock4_trig=3.5, lock4_amt=3.3, lock5_trig=3.8, lock5_amt=3.5,
                             lock6_trig=4.2, lock6_amt=3.9, lock7_trig=4.6, lock7_amt=4.3,
                             lock8_trig=0.0, lock8_amt=0.0, tp_r=5.0)
        print(f"{add_label:32s}: Trades: {res['trades']:4d} | W/BE/L: {res['wins']:3d}/{res['bes']:3d}/{res['losses']:2d} | WR: {res['wr']:4.1f}% | Net: ${res['pnl']:8.2f} | PF: {res['pf']:5.2f} | MaxDD: ${res['max_dd']:5.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)

    # Test 3: Trailing Engine Tuning (TP 5.0R vs 5.2R vs 5.4R) on Window (1, 22)
    print("\n--- 3. Testing Trailing Lock Tuning on Window (1, 22) ---", flush=True)
    comb_1 = sorted(base_s, key=lambda x: x['time'])
    configs = [
        ("Base Octa TP 5.0R", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 0.0, 0.0, 5.0),
        ("Nona-A: TP 5.2R (L8@4.9->4.6)", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 4.6, 4.3, 4.9, 4.6, 5.2),
    ...
    for name, be, l1_t, l1_a, l2_t, l2_a, l3_t, l3_a, l4_t, l4_a, l5_t, l5_a, l6_t, l6_a, l7_t, l7_a, l8_t, l8_a, tp in configs:
        res = run_simulation(comb_1, m5_gold, m5_times, m5_len,
