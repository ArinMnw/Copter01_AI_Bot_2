...
if __name__ == '__main__':
    ...
    group_id = 2
    target_orders = [2]
    target_tfs = {"Naiya": [], "Inst_Gap": ["H1"], "Fibo": [], "FVG": [], "GapSweep": [], "Doji": [], "MA12": [], "Div": [], "ATR": []}
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            if tf_str in target_tfs.get("Inst_Gap", ["H1", "M30", "M15"]):
                if getattr(row, 'inst_gap_buy', False) == True:
                    ...
                    sl = row.prev_low_2 if not pd.isna(row.prev_low_2) else (limit_price - (row.atr * 2.0))
                ...
                if getattr(row, 'inst_gap_sell', False) == True:
                    ...
                    limit_p = row.prev_open_2 if not pd.isna(row.prev_open_2) else row.prev_close
                    sl = row.prev_high_2 if not pd.isna(row.prev_high_2) else (limit_price + (row.atr * 2.0))
