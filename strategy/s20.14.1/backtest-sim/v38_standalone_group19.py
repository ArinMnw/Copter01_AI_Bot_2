    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            if patterns_buy:
                ...
                if "Fibo" in patterns_buy:
                    limit_price = fibo_buy_krh3
                    sl = s11_sl_buy if s11_sl_buy else sl
                    tp = s11_tp_buy if s11_tp_buy else tp
                elif "Group26" in patterns_buy:
                    limit_price = group26_limit
                    sl = group26_sl
                    
                dyn_tp = get_dynamic_tp(row.time_dt, True, tf_str, patterns_buy)
                if dyn_tp is not None and dyn_tp > limit_price:
                      if group_id == 19 or "Fibo" in patterns_buy:
                          tp = dyn_tp
                      else:
            ...
            # Group 19 Pattern 26 (MA12 H1 Group)
            if tf_str == "H1":
                if row.close < row.sma12:
                    patterns_sell.append("Group26")
                    group26_limit = row.sma12
                    group26_sl = row.recent_high
            ...
            if patterns_sell:
                ...
                elif "Group26" in patterns_sell:
                    limit_price = group26_limit
                    sl = group26_sl
                    
                dyn_tp = get_dynamic_tp(row.time_dt, False, tf_str, patterns_sell)
                if dyn_tp is not None and dyn_tp < limit_price:
                      if group_id == 19 or "Fibo" in patterns_sell:
                          tp = dyn_tp
                      else:
