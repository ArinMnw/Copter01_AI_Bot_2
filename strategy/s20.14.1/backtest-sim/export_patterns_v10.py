...
def backtest_patterns_to_csv(days, compound):
    ...
    for tf_str in all_tfs:
        ...
        df['sma12_slope'] = df['sma12'] - df['sma12'].shift(1)
        ...
        for row in valid.itertuples():
            ...
            if tf_str in target_tfs["ATR"] and row.close < row.sma20 - 2.0 * row.atr:
            ...
            if tf_str in target_tfs["MA12"] and row.low <= row.sma12 and row.close > row.sma12 and row.sma12_slope > 0:
            ...
            if tf_str in target_tfs["MA12"] and row.high >= row.sma12 and row.close < row.sma12 and row.sma12_slope < 0:
