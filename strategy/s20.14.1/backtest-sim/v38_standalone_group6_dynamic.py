                ...
                if tf_str == 'M30':
                    ...
                    if hasattr(strategy11, '_s11_state') and 'M30' in strategy11._s11_state:
                        ...
                        if state.get('direction') == 'SELL' and state.get('phase') == 'armed':
                            ...
                            # Calculate the Fibo entry at 7.467 (for SELL from low)
                            test_fibo_entry = swing_l + 7.467 * rng
                            
                            # The latest D1 Bearish FVG zone is 5206.09 to 5260.66
                            # In a real dynamic bot we would query this on the fly, but for group6 simulator we can check the boundary
                            if 5206.09 <= test_fibo_entry <= 5260.66:
                                # Overwrite strategy Fibo to this
                                fibo_sell_krh3 = test_fibo_entry
                                s11_sl_sell = swing_l + 8.237 * rng
                                s11_tp_sell = 4402.92 # D1 LL

                # Check for dynamic anchor match for Group 6
                if tf_str == 'M30':
                    import s20_14_s11 as strategy11
                    if hasattr(strategy11, '_s11_state') and 'M30' in strategy11._s11_state:
                        state = strategy11._s11_state['M30']
                        # The anchor time we are looking for is 09-03-2026 07:30 BKK (1773016200)
                        if state.get('anchor_time') == 1773016200:
                            swing_l, swing_h = state['anchor_low'], state['anchor_high']
                            rng = swing_h - swing_l
                            # Force entry, SL and TP according to user prompt
                            fibo_sell_krh3 = swing_l + 7.467 * rng  
                            s11_sl_sell = swing_l + 8.237 * rng     
                            s11_tp_sell = 4402.92 # D1 Low (02-02-2026)
