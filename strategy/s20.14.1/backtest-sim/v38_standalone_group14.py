...
if __name__ == '__main__':
    ...
    group_id = 14
    target_orders = [18]
    target_tfs = {'Naiya': [], 'Inst_Gap': [], 'Fibo': [], 'FVG': [], 'GapSweep': ['M15'], 'Doji': [], 'MA12': [], 'Div': ['H1'], 'ATR': []}
    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            # --- Group 14 Order 18: Breaker Block + Sweep + Div ---
            g14o18_sell = False
            g14o18_sl = 0.0
            g14o18_tp = 0.0
            
            if tf_str == 'H1':
                # 1. Update Breaker Blocks
                if i >= 3:
                    bar1 = list_of_dicts[i-2]
                    bar2 = list_of_dicts[i-1]
                    bar3 = list_of_dicts[i]
                    
                    bar1_body_top = bar1['close'] if bar1['is_green'] else bar1['open']
                    
                    # Form a Bearish Breaker if Bar2 engulfs Bar1 upwards, and Bar3 breaks down below Bar1 body
                    if bar2['close'] > bar1['high'] and bar3['close'] < bar1_body_top:
                        active_breakers.append({
                            'top': bar1['high'],
                            'bottom': bar1_body_top,
                            'touched': False,
                            'time': bar1['time_dt']
                        })
                
                # Check if current bar touches any breaker
                for brk in active_breakers:
                    if row.high >= brk['bottom']:
                        brk['touched'] = True
                        
                # Remove invalidated breakers (if price closes above the breaker top)
                active_breakers = [b for b in active_breakers if row.close <= b['top']]
                
                # 2. Check Sweep + Div condition
                # We need a touched breaker in the active list
                if any(b['touched'] for b in active_breakers if row.high >= b['bottom']):
                    # Check for Sweep of a recent local high
                    sweep_found = False
                    for j in range(max(0, i-24), i):
                        prev_bar = list_of_dicts[j]
                        if row.high > prev_bar['high'] and row.close < prev_bar['high']: # Wick Sweep
                            # Check RSI Div: Current High is Higher, but RSI is Lower
                            if row.rsi < prev_bar['rsi']:
                                sweep_found = True
                                break
                                
                    if sweep_found:
                        g14o18_sell = True
                        active_breakers_touched = [b for b in active_breakers if b['touched'] and row.high >= b['bottom']]
                        # Use the highest breaker's top for SL
                        best_breaker_top = max(b['top'] for b in active_breakers_touched)
                        g14o18_sl = best_breaker_top + (row.atr * 1.5) if row.atr else best_breaker_top + 5.0
                        g14o18_tp = min(b['low'] for b in list_of_dicts[max(0, i-300):i])
