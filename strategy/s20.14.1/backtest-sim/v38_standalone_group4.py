...
if __name__ == '__main__':
    ...
    group_id = 4
    target_orders = [4]
    target_tfs = {"Naiya": [], "Inst_Gap": [], "Fibo": ["H1"], "FVG": [], "GapSweep": [], "Doji": [], "MA12": [], "Div": [], "ATR": []}
    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            s11_sl_buy = None
            s11_tp_buy = None
            krh3_tp_override = False  # True = TP set from KRH3 Fibo level, skip dyn_tp
                ...
                for b in range(2, 250):
                    ...
                    if b_bar['close'] < b_bar['open']:
                        ...
                        if rng > 0:
                            ...
                            # Entry: KRH3_down = anchor_h - rng * 5.165
                            # SL: anchor_h - rng * 5.6  (just below KRH3 zone)
                            # TP: anchor_l + rng * 7.467  (RUN level)
                            
                            if abs(row.low - krh3_down) < 6.0 and row.close > row.open:
                                if tf_str in target_tfs.get("Fibo", []): patterns_buy.append("Fibo")
                                fibo_buy_krh3 = krh3_down
                                # Always override with Fibo-derived SL/TP (more accurate than S11 estimate)
                                # SL = anchor_h - rng * 5.6  (just below KRH3 zone)
                                # TP = anchor_l + rng * 7.467  (RUN level)
                                s11_sl_buy = b_bar['high'] - (rng * 5.6)
                                s11_tp_buy = b_bar['low'] + (rng * 7.467)
                                krh3_tp_override = True  # Do NOT let dyn_tp override this
                                break

                                
                            # Pattern 3 removed (O(N^2) - too slow)
                    ...
                    # BUY Reversal Pattern 3 (3/1) BUY Anchor: Look for GREEN anchor, KRH3 goes UP
                        ...
                        if rng > 0:
                            ...
                            # Pattern 3 (GREEN anchor) removed (O(N^2) - too slow)
            ...
            if patterns_buy:
                ...
                if dyn_tp is not None and dyn_tp > limit_price and not krh3_tp_override:
