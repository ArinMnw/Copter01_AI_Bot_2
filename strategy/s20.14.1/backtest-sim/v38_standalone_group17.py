...
if __name__ == '__main__':
    ...
    for tf_str in all_tfs:
        ...
        open_trades = []
        d1_fvg_sm = {'state': 0, 'disp_idx': -1, 'disp_candle': None, 'sl_price': 0.0}
        pending_orders = []
        pending_naiya_buy = []
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            # [INJECTED] D1 Naiya FVG State Machine for Order 22
            if tf_str == "D1":
                lookback_chunk = list_of_dicts[max(0, i-20):i]
                if lookback_chunk:
                    naiya_low = min([x['low'] for x in lookback_chunk])
                else:
                    naiya_low = row['low']

                if d1_fvg_sm['state'] == 0:
                    if row['close'] < naiya_low and row['open'] > row['close']:
                        d1_fvg_sm['disp_idx'] = i
                        d1_fvg_sm['disp_candle'] = row
                        d1_fvg_sm['sl_price'] = naiya_low
                        d1_fvg_sm['state'] = 1
                
                elif d1_fvg_sm['state'] == 1:
                    disp_idx = d1_fvg_sm['disp_idx']
                    if i >= disp_idx + 2:
                        pre_disp = list_of_dicts[disp_idx - 1] if disp_idx > 0 else list_of_dicts[disp_idx]
                        
                        if row['high'] < pre_disp['low']:
                            limit_p = row['high']
                            sl = d1_fvg_sm['sl_price']
                            lowest_low = min([list_of_dicts[j]['low'] for j in range(max(0, i-40), i)])
                            tp = lowest_low
                            ...
                            pending_orders.append({
                                'Pattern': 'Naiya FVG', 'TF': tf_str, 'Time (BKK)': row['time_dt'].strftime('%Y-%m-%d %H:%M'),
                                'Type': 'SELL', 'Limit_Price': round(limit_p, 2), 'Entry': 0.0, 'SL': round(sl, 2), 'TP': round(tp, 2),
                                'be_trig': limit_p - (limit_p - tp) * 0.4, 'be_act': False,
                            ...
                            d1_fvg_sm['state'] = 0  # <---- RESET TO SEARCH AGAIN
                        else:
                            d1_fvg_sm['state'] = 0
            ...
            if tf_str in target_tfs.get("Naiya", ["H1", "H12", "D1"]):
                ...
                if found_doji_buy: # default fallback
                    ...
                    if _tpsl_mode and _tpsl_target:
                        ...
                        # Guess entry
                        _entry_p = limit_price if 'limit_price' in locals() else (limit_p if 'limit_p' in locals() else row.close)
                        if 'weekly_open' in str('row.close'): _entry_p = row.weekly_open
                        _tpsl_res = tpsl_engine.get_tpsl(mode=_tpsl_mode, signal="SELL", entry_price=_entry_p, current_bar=row, recent_3=_recent_3, df=df, idx=i, tf=tf_str)
                        if _tpsl_target in ["SL_ONLY", "BOTH"] and _tpsl_res["sl"] != 0:
                            sl = _tpsl_res["sl"]
                        if _tpsl_target in ["TP_ONLY", "BOTH"] and _tpsl_res["tp"] != 0:
            ...
            # ==== NEW DOJI LOGIC ====
            if tf_str == "H1":
                if row.body > 8 and row.is_red:
                    for b in range(1, 15):
                        if i - b < 0: continue
                        b_bar = list_of_dicts[i-b]
                        if b_bar['body'] <= 3.0: 
                            if row.open > max(b_bar['open'], b_bar['close']) and row.close < min(b_bar['open'], b_bar['close']):
                                doji_state_machine.append({
                                    'state': 1,
                                    'doji_idx': i-b,
                                    'engulf_idx': i,
                                    'zone_high': row.open,
                ...
                for sm in doji_state_machine[:]:
                    if sm['state'] == 1:
                        if row.low < sm['zone_low'] - 5:
                            sm['state'] = 2
                    elif sm['state'] == 2:
                        if row.high >= sm['zone_low']:
                            sm['state'] = 3
                    elif sm['state'] == 3:
                        prev_bar = list_of_dicts[i-1]
                        if row.high > prev_bar['high'] and row.close < row.open:
                            ll = min([d['low'] for d in list_of_dicts[max(0, i-40):i]])
                            sl = (row.close + (row.atr * 1.5))
                            tp = ll
                            pending_orders.append({
                                "Pattern": "Fibo", "TF": tf_str, "Time (BKK)": row.time_dt.strftime('%Y-%m-%d %H:%M'),
                                "Type": "SELL", "Limit_Price": row.close, "Entry": row.close, 
                                "SL": round(sl, 2), "TP": round(tp, 2),
                                "Magic": 5091, "Lot": 0.01,
                                "Bars_Waited": 0, "Wait_For_Price_Below": False,
                                "RSI": round(row.rsi, 2) if hasattr(row, 'rsi') else 0, 
                                "ATR": round(row.atr, 2) if hasattr(row, 'atr') else 0,
                                "dist_sma50": 0, "dist_sma200": 0, "body": row.body, "range": row.high - row.low
                            })
                            doji_state_machine.remove(sm)
            # ========================
