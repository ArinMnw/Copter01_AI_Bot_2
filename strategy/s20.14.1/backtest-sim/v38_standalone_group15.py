...
if __name__ == '__main__':
    ...
    group_id = 15
    target_orders = [20]
    target_tfs = {"Naiya": [], "Inst_Gap": [], "Fibo": ["H1", "M30"], "FVG": [], "GapSweep": [], "Doji": [], "MA12": [], "Div": [], "ATR": []}
                            ...
                            if b >= 5: # Need some bars to have passed
                                ...
                                if len(past_lows) > 0 and min(past_lows) <= krh3_down + 1.0:
                                    ...
                                    if abs(row.low - krh1_down) < 4.0:
                                        ...
                                        if not s11_sl_buy: s11_sl_buy = krh1_down - (b_bar['high'] - b_bar['low']) * 0.618
                                        if not s11_tp_buy: s11_tp_buy = krh1_down + (b_bar['high'] - b_bar['low']) * 5.0
                                        fibo_buy_krh3 = krh1_down
                            ...
                            if b >= 5:
                                ...
                                if len(past_highs) > 0 and max(past_highs) >= krh3_up - 1.0:
                                    ...
                                    if abs(row.high - krh1_up) < 4.0:
                                        ...
                                        if not s11_sl_sell: s11_sl_sell = krh1_up + (b_bar['high'] - b_bar['low']) * 0.618
                                        if not s11_tp_sell: s11_tp_sell = krh1_up - (b_bar['high'] - b_bar['low']) * 5.0
                                        fibo_sell_krh3 = krh1_up
