...
for i in range(50, len(rates)):
    ...
    s1_res = strategy1.strategy_1(current_rates, 'M30')
    if s1_res and s1_res.get('signal') in ['BUY', 'SELL']:
        strategy11.record_s1_pattern('M30', s1_res['signal'], current_rates, current_rates[-1]['time'])
        print(f"S1 Signal: {s1_res.get('signal')} at {df.iloc[i].time_dt}")
        
    s11_res = strategy11.strategy_11(current_rates, 'M30')
