...
if __name__ == '__main__':
    ...
    group_id = 24
    target_orders = [34]
    target_tfs = {"Naiya": [], "Inst_Gap": [], "Fibo": [], "FVG": ["M15"], "GapSweep": [], "Doji": [], "MA12": [], "Div": [], "ATR": []}
    ...
    def get_dynamic_tp(current_time, is_buy, tf_str, pattern_list):
        ...
        if target_tf == 'D1': days = 3
        elif target_tf == 'H12': days = 3
        elif target_tf == 'H4': days = 3
        elif target_tf == 'H1': days = 3
        elif target_tf == 'M30': days = 3
        else: days = 3
    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            for t in open_trades[:]:
                if t['Type'] == 'BUY':
                    ...
                    if (row.high + spread_price) >= t['SL']:
                        ...
                        # Trailing SL for Group34
                        if "Group34" in t.get('Pattern', '') and i >= 1:
                            prev_high = list_of_dicts[i-1]['high']
                            if row.high > prev_high:
                                if row.high < t['SL']:
                                    t['SL'] = row.high
                                    # print(f"Trailed SL for Group34 to {row.high} at {row.time_dt}")
            ...
            # Group 24 Pattern 34 (FVG H1 Group)
            if tf_str == "H1":
                if row.bear_fvg_raw and i >= 1 and list_of_dicts[i-1]['bear_fvg_raw']:
                    lookback_40 = list_of_dicts[max(0, i-40):max(0, i-1)]
                    if len(lookback_40) > 20:
                        sh2_dict = max(lookback_40, key=lambda x: x['high'])
                        idx_sh2 = lookback_40.index(sh2_dict)
                        if idx_sh2 > 0 and sh2_dict['high'] > lookback_40[idx_sh2-1]['high']:
                            lookback_sh1 = lookback_40[max(0, idx_sh2-20):idx_sh2]
                            if len(lookback_sh1) > 2:
                                sh1_dict = max(lookback_sh1, key=lambda x: x['high'])
                                idx_sh1 = lookback_sh1.index(sh1_dict)
                                if idx_sh1 > 0 and sh1_dict['high'] > lookback_sh1[idx_sh1-1]['high']:
                                    if sh2_dict['high'] > sh1_dict['high']:
                                        recent_5_low = min([r['low'] for r in list_of_dicts[max(0, i-5):i]])
                                        if row.close < recent_5_low:
                                            patterns_sell.append("Group34")
                                            group34_limit = row.high # Enter at bottom of FVG gap
                                            group34_sl = list_of_dicts[i-2]['high'] # High of the FVG sequence
            ...
            if patterns_sell:
                ...
                if "Fibo" in patterns_sell:
                    limit_price = fibo_sell_krh3
                    sl = s11_sl_sell if s11_sl_sell else sl
                    tp = s11_tp_sell if s11_tp_sell else tp
                elif "Group34" in patterns_sell:
                    limit_price = group34_limit
                    sl = group34_sl
                    
                dyn_tp = get_dynamic_tp(row.time_dt, False, tf_str, patterns_sell)
                if dyn_tp is not None and dyn_tp < limit_price:
                      if group_id == 9 or "Fibo" in patterns_sell or "Group34" in patterns_sell:
                          tp = dyn_tp
                      else:
