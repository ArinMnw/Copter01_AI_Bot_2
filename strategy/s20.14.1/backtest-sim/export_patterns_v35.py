...
def backtest_patterns_to_csv(days, compound):
    ...
    for tf_str in all_tfs:
        ...
        # S1 Tamni (BOS / ย้อนโครงสร้าง)
        df['local_high_5'] = df['high'].rolling(window=5, min_periods=1).max()
        df['is_local_high'] = (df['high'] == df['local_high_5'])
        df['s1_tamni_sell'] = df['is_local_high'].shift(1) & df['is_red'] & (df['low'] < df['low'].shift(1))

        df['local_low_5'] = df['low'].rolling(window=5, min_periods=1).min()
        df['is_local_low'] = (df['low'] == df['local_low_5'])
        df['s1_tamni_buy'] = df['is_local_low'].shift(1) & df['is_green'] & (df['high'] > df['high'].shift(1))
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            if tf_str in target_tfs["Doji"] and row.s1_tamni_buy:
            ...
            if tf_str in target_tfs["Doji"] and row.s1_tamni_sell:
