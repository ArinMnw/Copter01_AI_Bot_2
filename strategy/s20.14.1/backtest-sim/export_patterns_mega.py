...
def backtest_patterns_to_csv(days, compound):
        ...
        for row in valid.itertuples():
            ...
            for pat in patterns_buy:
                ...
                open_trades.append({
                    ...
                    "SMA50": round(row.sma50, 2), "SMA200": round(row.sma200, 2), "Close": round(row.close, 2)
                })
    ...
    for (pat, tf), group_df in grouped:
        ...
        prefix = f"{pat}_{tf}"
        cols = ["Time (BKK)", "CloseTime (BKK)", "TF", "Pattern", "Type", "RSI", "ATR", "SMA50", "SMA200", "Close", "Entry", "SL", "TP", "Lot", "P&L", "Balance", "Reason"]
    ...
    cols = ["Time (BKK)", "CloseTime (BKK)", "TF", "Pattern", "Type", "RSI", "ATR", "SMA50", "SMA200", "Close", "Entry", "SL", "TP", "Lot", "P&L", "Balance", "Reason"]
    df_trades[cols].to_csv(os.path.join(out_dir, "mega_trades.csv"), index=False)
