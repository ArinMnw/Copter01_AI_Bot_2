...
def backtest_patterns_to_csv(days, compound):
    ...
    for tf_str in all_tfs:
        ...
        for row in valid.itertuples():
            ...
            if tf_str in target_tfs["ATR"] and row.close < row.sma20 - 2.0 * row.atr and row.sma50 > row.sma200 and row.rsi > 35:
            ...
            if tf_str in target_tfs["Fibo"] and row.close <= fibo_38_b and row.close >= fibo_61_b:
                patterns_buy.append("Fibo")
            if tf_str in target_tfs["Doji"] and row.range > 0 and row.body < 0.4 * row.range and (row.close - row.low) > 0.5 * row.range and row.low <= row.recent_low + row.atr*0.5:
                patterns_buy.append("Doji")
            if tf_str in target_tfs["Div"] and row.low < row.recent_low and row.rsi > row.rsi_low:
                patterns_buy.append("Div")
            if tf_str in target_tfs["MA12"] and row.low <= row.sma12 and row.close > row.sma12:
            ...
            if tf_str in target_tfs["ATR"] and row.close > row.sma20 + 2.0 * row.atr and row.sma50 < row.sma200 and row.rsi < 65:
            ...
            if tf_str in target_tfs["Fibo"] and row.close >= fibo_38_s and row.close <= fibo_61_s:
                patterns_sell.append("Fibo")
            if tf_str in target_tfs["Doji"] and row.range > 0 and row.body < 0.4 * row.range and (row.high - row.close) > 0.5 * row.range and row.high >= row.recent_high - row.atr*0.5:
                patterns_sell.append("Doji")
            if tf_str in target_tfs["Div"] and row.high > row.recent_high and row.rsi < row.rsi_high:
                patterns_sell.append("Div")
            if tf_str in target_tfs["MA12"] and row.high >= row.sma12 and row.close < row.sma12:
