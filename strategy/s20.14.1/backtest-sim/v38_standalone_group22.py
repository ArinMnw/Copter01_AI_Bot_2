...
if __name__ == '__main__':
    ...
    group_id = 22
    target_orders = [30]
    target_tfs = {"Naiya": [], "Inst_Gap": [], "Fibo": [], "FVG": [], "GapSweep": ["H1"], "Doji": [], "MA12": [], "Div": [], "ATR": []}
    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            # -- Wick Sweep Trigger Logic (Group 22 Order 30) --
            if tf_str in target_tfs.get("GapSweep", []):
                
                # Global invalidation for State 1 and 2
                if s1_sweep_state in [1, 2]:
                    # If price goes completely above the S1 Zone (Close > S1 Zone High), invalidate
                    if row.close > s1_sweep_zone_high:
                        s1_sweep_state = 0
                        
                # Always check for a NEW pattern, even if in State 1 or 2. Overwrite if found.
                pattern_found = False
                if len(current_rates) >= 3:
                    ...
                    c2_cond = c2['close'] < c1['open'] and c2['low'] < c1['low']
                    c3_engulf = c3['open'] >= c2['close'] and c3['close'] <= c2['open']
                    
                    if c1_green and c2_red and c3_red and c2_cond and c3_engulf:
                        s1_sweep_zone_high = c1['high']
                        s1_sweep_zone_low = c1['open']
                        s1_sweep_support = c2['low']
                        pattern_found = True
                        
                        print(f"✨ PATTERN FOUND at {row.time_dt} c1: {c1['time_dt']} c2: {c2['time_dt']} c3: {c3['time_dt']}")
                        
                        # If c3 already closed below support, skip State 1
                        if c3['close'] < s1_sweep_support:
                            s1_sweep_state = 2
                        else:
                            s1_sweep_state = 1
                        
                        # Find HL and LL on the left for TP
                        recent_data = valid.iloc[max(0, i-200):i]
                        if not recent_data.empty:
                            s1_sweep_ll = recent_data['low'].min()
                            s1_sweep_ll_idx = recent_data['low'].idxmin()
                            
                            after_ll = valid.loc[s1_sweep_ll_idx:]
                            if not after_ll.empty:
                                after_ll = after_ll.copy()
                                ...
                                if not swings.empty:
                                    ...
                                    s1_sweep_tp = swings.loc[hl_idx, 'low']
                                    pos_hl = valid.index.get_loc(hl_idx)
                                    for k in range(pos_hl + 2, min(len(valid), pos_hl + 20)):
                                        c1_high = valid.iloc[k-2]['high']
                                        c3_low = valid.iloc[k]['low']
                                        if c1_high < c3_low:
                                            s1_sweep_tp = c3_low
                                            break
                                else:
                                    s1_sweep_tp = after_ll['low'].min()
                            else:
                                s1_sweep_tp = s1_sweep_ll
                        else:
                            s1_sweep_tp = row.low
                
                # State 1: Wait for SELL BOS (Close < Support)
                if not pattern_found:
                    if s1_sweep_state == 1:
                        if row.close < s1_sweep_support:
                            s1_sweep_state = 2
                    
                    # State 2: Wait for Wick Sweep in S1 Zone
                    elif s1_sweep_state == 2:
                        if row.high > current_rates[-2]['high'] and row.close < current_rates[-2]['high']:
                            if s1_sweep_zone_low <= row.high <= s1_sweep_zone_high:
                                print(f"🔥 SWEEP TRIGGERED at {row.time_dt} Zone: {s1_sweep_zone_low}-{s1_sweep_zone_high}")
                                sl = s1_sweep_zone_high
                                tp = s1_sweep_tp if s1_sweep_tp > 0 else s1_sweep_ll
                                ...
                                if _tpsl_mode and _tpsl_target:
                                    ...
                                    if 'weekly_open' in str('round(row.close'): _entry_p = row.weekly_open
                                ...
                                open_trades.append({
                                    ...
                                    "Type": "SELL", "Limit_Price": round(row.close, 2), "Entry": round(row.close, 2), "SL": round(sl, 2), "TP": round(tp, 2),
                                    "be_trig": row.close - (row.atr * 6 * 0.4), "be_act": False,
