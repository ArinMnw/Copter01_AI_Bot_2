...
def backtest_patterns_to_csv(days, compound):
    ...
    for tf_str in all_tfs:
        ...
        df['vol_avg_5'] = df['tick_volume'].rolling(window=5).mean().shift(1)
        df['is_vol_spike'] = df['tick_volume'] > df['vol_avg_5']
        
        df['bull_fvg_raw'] = (df['high'].shift(2) < df['low']) & df['is_vol_spike'].shift(1)
        df['bear_fvg_raw'] = (df['low'].shift(2) > df['high']) & df['is_vol_spike'].shift(1)
