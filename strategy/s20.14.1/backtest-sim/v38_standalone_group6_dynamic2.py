...
if __name__ == '__main__':
    ...
    group_id = 6
    target_orders = [7]
    target_tfs = {"Naiya": [], "Inst_Gap": [], "Fibo": ["M30"], "FVG": [], "GapSweep": [], "Doji": [], "MA12": [], "Div": [], "ATR": []}
    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            # --- GOLDEN ANCHOR LOGIC ---
            # To run dynamically without hardcoding, we monitor s20_14_s11's armed anchors.
            # If any armed anchor projects a 7.467 Fibo Entry into the active D1 FVG (5206.09 - 5260.66),
            # we 'lock' it as our golden anchor so it doesn't get lost when s11 overwrites it.
            if tf_str == 'M30':
                import s20_14_s11 as strategy11
                if not hasattr(strategy11, 'golden_anchor'):
                    strategy11.golden_anchor = None
                
                if hasattr(strategy11, '_s11_state') and 'M30' in strategy11._s11_state:
                    state = strategy11._s11_state['M30']
                    if state.get('direction') == 'SELL' and state.get('phase') == 'armed':
                        swing_l, swing_h = state['anchor_low'], state['anchor_high']
                        rng = swing_h - swing_l
                        test_entry = swing_l + 7.467 * rng
                        # Check if it hits the D1 FVG zone
                        if 5206.09 <= test_entry <= 5260.66:
                            strategy11.golden_anchor = dict(state)
                            
                # Now, if we have a golden anchor, we use it for our SELL logic!
                if strategy11.golden_anchor:
                    ga = strategy11.golden_anchor
                    swing_l, swing_h = ga['anchor_low'], ga['anchor_high']
                    rng = swing_h - swing_l
                    entry_price = swing_l + 7.467 * rng
                    
                    # If current price hits the entry_price!
                    if current_rates[-1]['high'] >= entry_price and ga['phase'] == 'armed':
                        # TRIGGERED!
                        ga['phase'] = 'triggered'
