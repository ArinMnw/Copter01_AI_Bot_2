...
def backtest_patterns_to_csv(days, compound):
    ...
    for tf_str in all_tfs:
        ...
        for row in valid.itertuples():
            ...
            # BUY LOGIC
            patterns_buy = []
            if tf_str in target_tfs["FVG"] and row.bull_fvg_10 and row.sma50 > row.sma200:
                patterns_buy.append("FVG")
            if tf_str in target_tfs["ATR"] and row.close < row.sma20 - 2.0 * row.atr and row.sma50 > row.sma200:
            ...
            if tf_str in target_tfs["Fibo"] and row.close <= fibo_38_b and row.close >= fibo_61_b and row.sma50 > row.sma200:
                patterns_buy.append("Fibo")
            if tf_str in target_tfs["Doji"] and row.range > 0 and row.body < 0.4 * row.range and (row.close - row.low) > 0.5 * row.range and row.low <= row.recent_low + row.atr*0.5 and row.sma50 > row.sma200:
            ...
            if tf_str in target_tfs["MA12"] and row.low <= row.sma12 and row.close > row.sma12 and row.sma50 > row.sma200:
            ...
            # SELL LOGIC
            patterns_sell = []
            if tf_str in target_tfs["FVG"] and row.bear_fvg_10 and row.sma50 < row.sma200:
            ...
            if tf_str in target_tfs["Fibo"] and row.close >= fibo_38_s and row.close <= fibo_61_s and row.sma50 < row.sma200:
                patterns_sell.append("Fibo")
            if tf_str in target_tfs["Doji"] and row.range > 0 and row.body < 0.4 * row.range and (row.high - row.close) > 0.5 * row.range and row.high >= row.recent_high - row.atr*0.5 and row.sma50 < row.sma200:
            ...
            if tf_str in target_tfs["MA12"] and row.high >= row.sma12 and row.close < row.sma12 and row.sma50 < row.sma200:
    ...
    for (pat, tf), group_df in grouped:
        ...
        d = group_df.groupby('Date').agg(Trades=('P&L', 'count'), Win=('Reason', lambda x: (x=='TP').sum()), Loss=('Reason', lambda x: (x=='SL').sum()), Net_PnL=('P&L', 'sum')).reset_index()
        ...
        m = group_df.groupby('Month').agg(Trades=('P&L', 'count'), Win=('Reason', lambda x: (x=='TP').sum()), Loss=('Reason', lambda x: (x=='SL').sum()), Net_PnL=('P&L', 'sum')).reset_index()
        m['Cumulative_PnL'] = m['Net_PnL'].cumsum()
        m.to_csv(os.path.join(out_dir, f"{prefix}_monthly.csv"), index=False)
        
        # 4. _compare.csv
        c = group_df.groupby(['Pattern', 'TF']).agg(Trades=('P&L', 'count'), Win=('Reason', lambda x: (x=='TP').sum()), Loss=('Reason', lambda x: (x=='SL').sum()), Net_PnL=('P&L', 'sum')).reset_index()
        ...
        print(f"Saved: {prefix} set (Trades: {total} | W: {wins} | L: {losses} | WR: {wr:.1f}% | P&L: {pnl:.2f})")
    ...
    print(f"Saved: trades.csv (Total Trades: {total} | W: {wins} | L: {losses} | WR: {wr:.1f}% | P&L: {pnl:.2f})")
