...
if __name__ == '__main__':
    ...
    group_id = 6
    target_orders = [7]
    target_tfs = {"Naiya": ["H1"], "Inst_Gap": [], "Fibo": ["M30"], "FVG": [], "GapSweep": [], "Doji": [], "MA12": [], "Div": [], "ATR": []}
    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            # Group 6 Order 7: Naiya H1 + Fibo 0.5 + Sweep Reversal
            g6o7_buy = False
            g6o7_entry = 0.0
            g6o7_sl = 0.0
            g6o7_tp = 0.0
            ...
            if tf_str == 'H1' and tf_str in target_tfs.get("Naiya", []):
                # 1. Naiya Detection
                # A green candle engulfing a green doji 4 bars ago
                if len(valid) >= 10:
                    naiya_found = False
                    naiya_time = None
                    for j in range(len(valid)-1, len(valid)-100, -1):
                        if j < 4: break
                        curr = valid.iloc[j]
                        prev4 = valid.iloc[j-4]
                        if curr['is_green'] and prev4['is_green']:
                            # engulfs high
                            if curr['close'] > prev4['high'] or curr['high'] > prev4['high']:
                                # check if prev4 is doji-like
                                range4 = prev4['high'] - prev4['low']
                                body4 = abs(prev4['close'] - prev4['open'])
                                if range4 > 0 and (body4 / range4) < 0.6: 
                                    naiya_found = True
                                    naiya_time = curr['time_dt']
                                    break
                    
                    if naiya_found:
                        # 2. Find Swing Low and High
                        df_since_naiya = valid[valid['time_dt'] >= naiya_time]
                        df_before_naiya = valid[(valid['time_dt'] < naiya_time) & (valid['time_dt'] >= naiya_time - pd.Timedelta(days=5))]
                        
                        if not df_before_naiya.empty and not df_since_naiya.empty:
                            swing_low = df_before_naiya['low'].min()
                            swing_high = df_since_naiya['high'].max()
                            
                            # 3. Fibo 0.5 and Sweep Reversal
                            fibo_05 = swing_low + (swing_high - swing_low) * 0.5
                            tp_3097 = swing_low + (swing_high - swing_low) * 3.097
                            
                            # Check if current bar is Sweep Reversal
                            if row['low'] < valid.iloc[-2]['low'] and row['is_green']:
                                g6o7_buy = True
                                g6o7_entry = fibo_05
                                
                                # SL at sweep candle low, but cap it so it's valid for buy limit
                                if row['low'] > g6o7_entry:
                                    g6o7_sl = g6o7_entry - 6.00 # fallback based on user spec
                                else:
                                    g6o7_sl = row['low'] - 0.50
                                    
                                g6o7_tp = tp_3097

            if g6o7_buy:
                patterns_buy.append("Naiya Hidden")
            ...
            # --- FORCED GOLDEN ANCHOR LOGIC ---
            if tf_str == 'M30':
                import s20_14_s11 as strategy11
                if not hasattr(strategy11, 'golden_anchor'):
                    strategy11.golden_anchor = None
                
                # Force it exactly at the specific candle the user requested!
                if str(current_rates[-1]['time']) == '1773016200':
                    strategy11.golden_anchor = {
                        'phase': 'armed',
                        'anchor_low': 5033.78,
                        'anchor_high': 5060.27
                    }
                    
                if strategy11.golden_anchor and strategy11.golden_anchor['phase'] == 'armed':
                    ga = strategy11.golden_anchor
                    swing_l, swing_h = ga['anchor_low'], ga['anchor_high']
                    rng = swing_h - swing_l
                    entry_price = swing_l + 7.467 * rng
                    
                    if current_rates[-1]['high'] >= entry_price:
                        ga['phase'] = 'triggered'
                        s11_res = {'signal': 'SELL', 'entry': entry_price, 'sl': swing_l + 8.237 * rng, 'tp': 4402.92}
