...
if __name__ == '__main__':
    ...
    for t_name, t_val in [('D1', mt5.TIMEFRAME_D1), ('H12', mt5.TIMEFRAME_H12), ('H4', mt5.TIMEFRAME_H4), 
        ...
        for _ in range(3):
            if start_dt and end_dt:
                ...
                rates = mt5.copy_rates_range(symbol, t_val, start_pad.astimezone(pytz.utc).replace(tzinfo=None), end_dt.astimezone(pytz.utc).replace(tzinfo=None))
            else:
                mins = 1 if t_name == 'M1' else 5 if t_name == 'M5' else 15 if t_name == 'M15' else 30 if t_name == 'M30' else 60 if t_name == 'H1' else 240 if t_name == 'H4' else 720 if t_name == 'H12' else 1440
                bars = ((days * 24 * 60) // mins) + 1000
                if bars > 500000: bars = 500000
                rates = mt5.copy_rates_from_pos(symbol, t_val, 0, bars)
            if rates is not None and len(rates) > 0:
                break
            print(f"Retrying MT5 data fetch for {t_name}...")
            import time
            time.sleep(1)
        ...
        if rates is not None and len(rates) > 0:
            ...
            print(f"WARNING: Failed to fetch MT5 data for {t_name}!")
            
    # Config for SL Mode
    SL_MODE = 'ATR_TIGHT' # Options: 'H1_LIQ', 'ATR_TIGHT'
    ...
    print("Executing MTF strategy on LTFs: ['M30', 'M15', 'M5', 'M1']")
    df_h1 = global_dfs['H1'].copy()
    df_h1['low_roll'] = df_h1['low'].rolling(3, center=True).min()
    df_h1['high_roll'] = df_h1['high'].rolling(3, center=True).max()
    df_h1['is_red'] = df_h1['close'] < df_h1['open']
    
    # Precompute H1 Liquidity Sweep Lows
    h1_liq_lows = []
    h1_swings = df_h1[df_h1['low'] == df_h1['low_roll']]
    for s_idx in h1_swings.index:
        loc = df_h1.index.get_loc(s_idx)
        if loc > 0:
            prev_c = df_h1.iloc[loc-1]
            curr_c = df_h1.iloc[loc]
            if curr_c['low'] < prev_c['low'] and prev_c['is_red']:
                h1_liq_lows.append(curr_c)
    
    for ltf_str in ['M30', 'M15', 'M5', 'M1']:
        if ltf_str not in global_dfs: continue
        
        df_ltf = global_dfs[ltf_str]
        
        if 'atr' not in df_ltf.columns:
            import strategy20_13_24
            rates_dict = df_ltf.to_dict('records')
            
            # Use smaller chunk if it's too large to prevent MemoryError inside strategy
            if len(rates_dict) > 100000:
                print(f"[{ltf_str}] Slicing to 100k bars to avoid OOM...")
                rates_dict = rates_dict[-100000:]
                
            df_ltf = strategy20_13_24.compute_indicators_df(rates_dict)
            import pandas as pd
            if not isinstance(df_ltf, pd.DataFrame):
                df_ltf = pd.DataFrame(df_ltf)
            from datetime import timedelta
            if 'time_dt' not in df_ltf.columns:
                df_ltf['time_dt'] = pd.to_datetime(df_ltf['time'], unit='s') + timedelta(hours=1)
                
            # Update global_dfs with computed indicators so we don't recalculate
            global_dfs[ltf_str] = df_ltf
                
        df_ltf['recent_low_35'] = df_ltf['low'].rolling(window=35).min().shift(1)
        valid_ltf = df_ltf.dropna(subset=['atr', 'rsi', 'recent_low_35']).copy()
        valid_ltf['low_roll'] = valid_ltf['low'].rolling(3, center=True).min()
        ltf_swings_idx = set(valid_ltf[valid_ltf['low'] == valid_ltf['low_roll']].index)
        
        if start_dt and end_dt:
            valid_ltf = valid_ltf[(valid_ltf['time_dt'] >= start_dt.replace(tzinfo=None)) & (valid_ltf['time_dt'] <= end_dt.replace(tzinfo=None))]
        else:
            cutoff = valid_ltf['time_dt'].max() - timedelta(days=days)
            valid_ltf = valid_ltf[valid_ltf['time_dt'] >= cutoff]
            
        print(f"[{ltf_str}] valid len (after cutoff): {len(valid_ltf)}")
        
        open_trade = None
        pending_trade = None
        for i, (idx, row) in enumerate(valid_ltf.iterrows()):
            if pending_trade is not None and row.time_dt >= pending_trade['exec_time']:
                open_trade = pending_trade['trade']
                pending_trade = None
            if open_trade is not None:
                if row.low <= open_trade['SL']:
                    open_trade['CloseTime (BKK)'] = row.time_dt.strftime('%Y-%m-%d %H:%M')
                    open_trade['Close'] = open_trade['SL']
                    open_trade['P&L'] = (open_trade['Close'] - open_trade['Entry']) * 100 * open_trade['Lot']
            ...
            # If current bar is a Respect Low in LTF
            if idx in ltf_swings_idx:
                # Find the most recent H1 Liq Low within 5 days
                recent_liq = [l for l in h1_liq_lows if l['time_dt'] < row.time_dt and (row.time_dt - l['time_dt']).total_seconds() < 86400 * 5]
                if not recent_liq: continue
                
                h1_liq = recent_liq[-1] # The most recent H1 Sweep Low
                
                # Check Respect Low condition (LTF low > H1 liq low)
                if row.low > h1_liq['low']:
                    print(f"???? MTF BUY at {row.time_dt} on {ltf_str}")
                    
                    if i + 2 >= len(valid_ltf): continue
                    exec_row = valid_ltf.iloc[i+2]
                    entry_price = exec_row.open
                    
                    # Stop Loss logic based on SL_MODE toggle
                    sl_price = 0
                    if SL_MODE == 'ATR_TIGHT':
                        sl_price = entry_price - (exec_row.atr * 1.5)
                    else:
                        sl_price = h1_liq['low'] - (exec_row.atr * 0.5)
                        
                    # Find dynamic TP on H1
                    tp_price = entry_price + (exec_row.atr * 2) # Default TP if no valid liquidity is found
                    h1_idx_end = df_h1['time_dt'].searchsorted(row.time_dt)
                    h1_idx_start = max(0, h1_idx_end - 150)
                    recent_h1 = df_h1.iloc[h1_idx_start:h1_idx_end]
                    ...
                    for h_idx in highs.index:
                        ...
                        if loc > 0:
                            ...
                            if not prev_highs.empty:
                                h1_pk = prev_highs.iloc[-1]
                                if h2['high'] < h1_pk['high']: 
                                    loc1 = recent_h1.index.get_loc(h1_pk.name)
                                    if loc1 > 0:
                                        h1_prev = recent_h1.iloc[loc1-1]
                                        if h1_pk['close'] < h1_pk['open'] and h1_pk['high'] > h1_prev['high']:
                                            respect_highs.append(h2['high'])
                                            
                    valid_tps = [h for h in respect_highs if h > entry_price + (exec_row.atr * 1)]
                    if valid_tps:
                        tp_price = valid_tps[-1]
                    else:
                        # Fallback to recent H1 max if valid
                        h1_max = recent_h1['high'].max()
                        if h1_max > entry_price + (exec_row.atr * 1):
                            tp_price = h1_max
                        
                    pending_trade = {
                        'exec_time': exec_row.time_dt,
                        'trade': {
                            'Time (BKK)': exec_row.time_dt.strftime('%Y-%m-%d %H:%M'),
                            'TF': ltf_str,
                            'Pattern': 'Naiya_MTF',
                            'Type': 'BUY',
                            'Entry': entry_price,
                            ...
                            'TP': tp_price,
                            'Lot': compound,
                            'RSI': round(exec_row.rsi, 2) if 'rsi' in exec_row else 0,
                            'ATR': round(exec_row.atr, 2) if 'atr' in exec_row else 0,
                            'ADX': round(exec_row.adx, 2) if 'adx' in exec_row else 0,
                            'Hour': exec_row.time_dt.hour,
                            'dist_sma50': 0,
                            'dist_sma200': 0,
                            'body': abs(exec_row.open - exec_row.close),
                            'range': exec_row.high - exec_row.low
                        }
                    }
    ...
    out_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', f'excel_group{group_id}_live'))
    ...
    for (pat, tf), group_df in grouped:
        ...
        cols = ["Time (BKK)", "CloseTime (BKK)", "TF", "Pattern", "Type", "RSI", "ATR", "ADX", "Hour", "dist_sma50", "dist_sma200", "body", "range", "Entry", "SL", "TP", "Lot", "P&L", "Balance", "Reason"]
