...
def backtest_patterns_to_csv(days, compound):
    ...
    for tf_str in all_tfs:
        ...
        for row in valid.itertuples():
            ...
            if tf_str in target_tfs["ATR"] and row.close < row.sma20 - 2.0 * row.atr and row.rsi < 45:
            ...
            if tf_str in target_tfs["Fibo"] and row.close <= fibo_38_b and row.close >= fibo_61_b and row.rsi < 45:
                patterns_buy.append("Fibo")
            if tf_str in target_tfs["Doji"] and row.range > 0 and row.body < 0.4 * row.range and (row.close - row.low) > 0.5 * row.range and row.low <= row.recent_low + row.atr*0.5 and row.rsi < 45:
            ...
            if tf_str in target_tfs["MA12"] and row.low <= row.sma12 and row.close > row.sma12 and row.rsi < 55:
            ...
            if tf_str in target_tfs["ATR"] and row.close > row.sma20 + 2.0 * row.atr and row.rsi > 55:
            ...
            if tf_str in target_tfs["Fibo"] and row.close >= fibo_38_s and row.close <= fibo_61_s and row.rsi > 40:
                patterns_sell.append("Fibo")
            if tf_str in target_tfs["Doji"] and row.range > 0 and row.body < 0.4 * row.range and (row.high - row.close) > 0.5 * row.range and row.high >= row.recent_high - row.atr*0.5 and row.rsi > 55:
            ...
            if tf_str in target_tfs["MA12"] and row.high >= row.sma12 and row.close < row.sma12 and row.rsi > 55:
