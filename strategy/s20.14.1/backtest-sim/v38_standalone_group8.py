...
if __name__ == '__main__':
    ...
    group_id = 8
    target_orders = [9]
    target_tfs = {"Naiya": ["H1"], "Inst_Gap": [], "Fibo": [], "FVG": [], "GapSweep": ["H1"], "Doji": ["H1"], "MA12": [], "Div": [], "ATR": []}
    ...
    for tf_str in all_tfs:
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            # Group 8 Order 9: Naiya Sweep Far
            g8o9_buy = False
            g8o9_entry = 0.0
            g8o9_sl = 0.0
            g8o9_tp = 0.0
            ...
            if tf_str == 'H1' and tf_str in target_tfs.get("Naiya", []):
                # 1. Naiya Detection
                # A green candle engulfing a red doji or small red body 4-5 bars ago? 
                # Actually, in our data for 14-04 19:00 MT5, it was a Green Doji. Let's just find ANY doji engulfed
                if len(valid) >= 40:
                    naiya_found = False
                    naiya_doji_body_50 = 0.0
                    naiya_low = 99999.0
                    
                    # Search backwards up to 50 bars for the Naiya Doji
                    for j in range(len(valid)-1, len(valid)-50, -1):
                        ...
                        if curr['is_green']:
                            ...
                            if rng > 0 and (body / rng) < 0.3:
                                # It's a Doji. Does current engulf it?
                                if curr['close'] > prev1['high']:
                                    naiya_found = True
                                    naiya_doji_body_50 = (prev1['open'] + prev1['close']) / 2
                                    # SL is the low around this Naiya (say, 10 bars before and after)
                                    start_idx = max(0, j-10)
                                    end_idx = min(len(valid), j+5)
                                    naiya_low = valid.iloc[start_idx:end_idx]['low'].min()
                                    break
                    
                    if naiya_found:
                        # 2. Sweep Condition
                        # Check if current bar sweeps the previous bar's low
                        if row['low'] < valid.iloc[-2]['low'] and row['close'] > row['low']:
                            g8o9_buy = True
                            g8o9_entry = naiya_doji_body_50
                            g8o9_sl = naiya_low
                            tp_high = valid[valid['time_dt'] >= valid.iloc[-50]['time_dt']]['high'].max()
                            g8o9_tp = tp_high if tp_high > g8o9_entry + 10 else g8o9_entry + 50.0

            if g8o9_buy:
                patterns_buy.append("Naiya_Sweep_Far")
