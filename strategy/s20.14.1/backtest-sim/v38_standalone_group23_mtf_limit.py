...
if __name__ == '__main__':
    ...
    for ltf_str in ['M30', 'M15', 'M5', 'M1']:
        ...
        for i, (idx, row) in enumerate(valid_ltf.iterrows()):
            # Check if pending limit is filled
            if pending_limit is not None:
                if row.time_dt >= pending_limit['exec_time']:
                    # It's active. Check if price hits the limit entry
                    if row.low <= pending_limit['trade']['Entry']:
                        open_trade = pending_limit['trade']
                        open_trade['Time (BKK)'] = row.time_dt.strftime('%Y-%m-%d %H:%M') # Update time to actual fill time
                        pending_limit = None
            ...
            if idx in ltf_swings_idx:
                ...
                if row.low > h1_liq['low']:
                    ...
                    if SL_MODE == 'ATR_TIGHT':
                        sl_price = entry_price - (row.atr * 1.5)
                    else:
                        sl_price = h1_liq['low'] - (row.atr * 0.5)
                        
                    # Find dynamic TP on H1
                    tp_price = entry_price + (row.atr * 2) # Default TP if no valid liquidity is found
                    ...
                    valid_tps = [h for h in respect_highs if h > entry_price + (row.atr * 1)]
                    if valid_tps:
                        tp_price = valid_tps[-1]
                    else:
                        # Fallback to recent H1 max if valid
                        h1_max = recent_h1['high'].max()
                        if h1_max > entry_price + (row.atr * 1):
                            tp_price = h1_max
                    ...
                    pending_limit = {
                        ...
                        'TP': tp_price,
                        'Lot': compound,
                        'RSI': round(row.rsi, 2) if 'rsi' in row else 0,
                        'ATR': round(row.atr, 2) if 'atr' in row else 0,
                        'ADX': round(row.adx, 2) if 'adx' in row else 0,
                        'Hour': row.time_dt.hour,
                        'dist_sma50': 0,
                        'dist_sma200': 0,
    ...
    out_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', f'excel_group{group_id}_limit'))
