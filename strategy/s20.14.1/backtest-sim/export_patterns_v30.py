    ...
    for tf_str in all_tfs:
        ...
        for row in valid.itertuples():
            ...
            if patterns_buy:
                ...
                # 🎯 True Logic: SL = Swing Low - 1.0 ATR | TP = 1.618 Fib Extension
                sl = row.recent_low - (1.0 * row.atr)
                swing_size = row.recent_high - row.recent_low
                tp = actual_entry + (swing_size * 1.618)
                be_trig = actual_entry + (swing_size * 1.618 * 0.4) # BE at 40% of TP distance
            ...
            if patterns_sell:
                ...
                # 🎯 True Logic: SL = Swing High + 1.0 ATR | TP = 1.618 Fib Extension
                sl = row.recent_high + (1.0 * row.atr)
                swing_size = row.recent_high - row.recent_low
                tp = actual_entry - (swing_size * 1.618)
                be_trig = actual_entry - (swing_size * 1.618 * 0.4) # BE at 40% of TP distance
