...
def backtest_patterns_to_csv(days, compound):
    ...
    for tf_str in all_tfs:
        ...
        # Institutional Gap Fill: Daily Gap Down -> buy the dip, Daily Gap Up -> sell the rip
        df['inst_gap_buy'] = (df['time_diff'].dt.total_seconds() > 3600) & (df['open'] < df['close'].shift(1))
        df['inst_gap_sell'] = (df['time_diff'].dt.total_seconds() > 3600) & (df['open'] > df['close'].shift(1))
        ...
        for i, (idx, row) in enumerate(valid.iterrows()):
            ...
            if tf_str in target_tfs.get("Inst_Gap", ["H1", "M30", "M15"]):
                if getattr(row, 'inst_gap_buy', False):
                    # Gap down, wait for reversal. Buy limit at Open - ATR*1.75
                    limit_p = row.open - (row.atr * 1.75)
                    sl = limit_p - (row.atr * 1.5)
                    tp = limit_p + (limit_p - sl) * 5.0
                    pending_orders.append({
                ...
                if getattr(row, 'inst_gap_sell', False):
                    # Gap up, wait for reversal. Sell limit at Open + ATR*1.75
                    limit_p = row.open + (row.atr * 1.75)
                    sl = limit_p + (row.atr * 1.5)
                    tp = limit_p - (sl - limit_p) * 5.0
                    pending_orders.append({
            ...
            if patterns_buy:
                ...
                sl = limit_price - (2.5 * row.atr) # ขยายเกราะเป็น 2.5 ATR กันสะบัด
