# -*- coding: utf-8 -*-
"""run_s20_302_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.302 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.302 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Non-Conflicting Multi-Session Architecture", flush=True)
    print("=" * 115, flush=True)
    ...
    print("=" * 115, flush=True)
    print(" S20.302 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "S20_302_monthly.csv")
    df_m.to_csv(csv_path, index=False)
    print(f"Monthly breakdown saved to S20_302_monthly.csv", flush=True)
