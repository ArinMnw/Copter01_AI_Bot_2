# -*- coding: utf-8 -*-
"""s20_50_research.py
Explore Multi-Strategy Confluence Expansion to beat S20.49 (+$26,180.81):
Candidate Integrations:
1. Strategy 15 / Multi-Session Liquidity Pool: NY PM Session (17:00 - 21:00 UTC, active >= 21:00 UTC)
2. Strategy 4 / Micro Market Structure Shift (MSS Confirmation on Reversal)
3. Strategy 6 / S6i Institutional Displacement (Strong Momentum Body)
4. Quattuordeca-Stage Quantum Ratchet Lock Engine (14 Stages, TP 6.75R - 7.00R)
...
def compute_indicators(rates, use_london=True, use_ny=True, use_ny_pm=True):
    ...
    df['fvg_bull_zone'] = pd.Series(fvg_b1).combine_first(pd.Series(fvg_b2))
    ...
    df['fvg_bear_zone'] = pd.Series(fvg_s1).combine_first(pd.Series(fvg_s2))
...
def extract_setups(df, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, sl_mult=0.20, hours=(0, 22),
                   use_pyh=True, use_ldn=True, use_ny=True, use_ny_pm=True, use_fvg=True, use_mss=False):
    setups = []
    h_start, h_end = hours
    ...
    for idx in range(45, len(records)):
        ...
        # Cross-Breed: Strategy 1 & 2 Fair Value Gap (FVG) Imbalance Confluence
        if use_fvg:
        ...
        # Strategy 4: Micro Market Structure Shift (MSS) option
        mss_buy_ok = True
        mss_sell_ok = True
        if use_mss:
            # Reversal candle closes above the high of previous bar for BUY, or below previous low for SELL
            mss_buy_ok = cur['close'] >= prev['high']
            mss_sell_ok = cur['close'] <= prev['low']

        sig = None
        if has_wick_buy and closed_high and rsi_ok_buy and swept_low and mss_buy_ok:
            sig = "BUY"
        elif has_wick_sell and closed_low and rsi_ok_sell and swept_high and mss_sell_ok:
            sig = "SELL"
...
def run_simulation(setups, m5_gold, m5_times, m5_len,
                   ...
                   lock12_trig=6.0, lock12_amt=5.8, lock13_trig=6.15, lock13_amt=5.95,
                   lock14_trig=6.35, lock14_amt=6.15, tp_r=6.80):
    point_val = 1.0
    busy_until = 0
    trades = 0
    wins = 0
    ...
    for s in setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if lock14_trig and b['high'] >= l14_target:
                    l14_active = True
                ...
                if lock14_trig and b['low'] <= l14_target:
                    l14_active = True
...
def main():
    ...
    print(" S20.50 ADVANCED CONFLUENCE RESEARCH (TARGET > $26,180.81 ON STRICT LOT 0.01)", flush=True)
    print(" Benchmark S20.49: +$26,180.81 | MaxDD $12.96 | WR 87.4% | Trades 3,112", flush=True)
    print("=" * 115, flush=True)

    # Test 1: Add New York PM Session Pool (17:00 - 21:00 UTC active >= 21:00 UTC)
    print("\n--- 1. Testing NY PM Liquidity Pool ---", flush=True)
    s_list_pm = []
    for tf_label, df_tf in base_tfs:
        s_list_pm.extend(extract_setups(df_tf, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True, use_ny_pm=True, use_fvg=True))
    comb_pm = sorted(s_list_pm, key=lambda x: x['time'])
    print(f"Total candidate setups with NY PM: {len(comb_pm)}", flush=True)
    res_pm = run_simulation(comb_pm, m5_gold, m5_times, m5_len, tp_r=6.70)
    print(f"NY PM + Base Trailing | Trades: {res_pm['trades']} | W/BE/L: {res_pm['wins']}/{res_pm['bes']}/{res_pm['losses']} | WR: {res_pm['wr']:.1f}% | Net: ${res_pm['pnl']:.2f} | DD: ${res_pm['max_dd']:.2f}", flush=True)

    # Test 2: Quattuordeca-Stage Trailing (Lock 14 @ +6.35R -> +6.15R, TP 6.75R - 6.95R)
    print("\n--- 2. Testing Quattuordeca-Stage Trailing on S20.50 Candidate ---", flush=True)
    for tp in [6.70, 6.75, 6.80, 6.85, 6.90]:
        for l14_t, l14_a in [(None, None), (6.35, 6.15), (6.40, 6.20)]:
            res_tp = run_simulation(comb_pm, m5_gold, m5_times, m5_len,
                                    lock14_trig=l14_t, lock14_amt=l14_a, tp_r=tp)
            l14_s = f"L14={l14_t}->{l14_a}" if l14_t else "No L14"
            print(f"TP={tp:.2f}R | {l14_s:<15} | Trades: {res_tp['trades']} | W/BE/L: {res_tp['wins']}/{res_tp['bes']}/{res_tp['losses']} | WR: {res_tp['wr']:.1f}% | Net: ${res_tp['pnl']:.2f} | DD: ${res_tp['max_dd']:.2f} | Mos: {res_tp['pos_m']}/{res_tp['tot_m']}", flush=True)
