# -*- coding: utf-8 -*-
"""strategy20_36.py — S20.36 Omni-Horizon Quantum Sovereign Matrix:
True multi-strategy confluence working together on the exact same setup:
1. Macro Liquidity Sweep:
   - Asian Range High/Low (00:00 - 08:00 server time)
   - Previous Day High/Low (PDH / PDL, shifted by 1 day)
   - Previous Week High/Low (PWH / PWL, shifted by 1 week)
   - Structural Swing 12 High/Low
...
5. Ultra-Precision Retest Entry (18.0% Retest Depth of the Wick, SL behind wick tip)
6. Global Institutional Session Window (07:00 - 21:00 server time)
7. Quad-Horizon Coverage (H4 Macro + H1 Trend + M30 Structural + M15 Intraday, 15-min dedup)
8. Quad-Stage Ratchet Profit Lock Engine:
   - Stage 1: Move SL to Breakeven at +0.8R
   - Stage 2: Lock +1.0R Profit at +1.5R
   - Stage 3: Lock +2.0R Profit at +2.4R
   - Stage 4: Lock +2.8R Profit at +3.0R
   - Final Target: 3.5R
...
- ZERO magic fill (realistic M5 limit retest fill within 2 hours).
- ZERO repaint.
"""
...
def compute_indicators_df(rates):
    ...
    # Previous Week High / Low (shifted by 1 week)
...
def evaluate_setup(cur, tf_label="M30", min_vol=1.30, min_wick=0.38, retest_depth=0.180, hours=(7, 21)):
    """Evaluate confluence setup on single bar dictionary."""
    atr = cur['atr']
