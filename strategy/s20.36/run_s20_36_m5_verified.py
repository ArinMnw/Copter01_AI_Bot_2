# -*- coding: utf-8 -*-
"""run_s20_36_m5_verified.py — 100% Verifiable M5 Sequential Execution:
...
5. Quad-Stage Ratchet Profit Lock: BE at +0.8R, Lock +1.0R at +1.5R, Lock +2.0R at +2.4R, Lock +2.8R at +3.0R, Target 3.5R.
"""
...
def main():
    ...
    df_h4 = strategy20_36.compute_indicators_df(h4_gold)
    df_h1 = strategy20_36.compute_indicators_df(h1_gold)
    df_m30 = strategy20_36.compute_indicators_df(m30_gold)
    df_m15 = strategy20_36.compute_indicators_df(m15_gold)

    m5_times = [int(r['time']) for r in m5_gold]
    m5_len = len(m5_gold)

    setups_h4 = [strategy20_36.evaluate_setup(r, "H4") for r in df_h4.to_dict('records')[45:]]
    setups_h1 = [strategy20_36.evaluate_setup(r, "H1") for r in df_h1.to_dict('records')[45:]]
    setups_m30 = [strategy20_36.evaluate_setup(r, "M30") for r in df_m30.to_dict('records')[45:]]
    setups_m15 = [strategy20_36.evaluate_setup(r, "M15") for r in df_m15.to_dict('records')[45:]]
    ...
    print("=" * 105)
    print(" S20.36 100% VERIFIABLE M5 INTRABAR SEQUENTIAL EXECUTION (STRICT LOT 0.01)")
    print(" Omni-Horizon Quantum Sovereign | Precision Retest 0.18 | SL-First on Every Bar")
    print("=" * 105)
    ...
    for s in combined_setups:
        ...
        l3_target = round(entry + (l3_trig * risk), 2) if sig == "BUY" else round(entry - (l3_trig * risk), 2)
        l3_sl = round(entry + (l3_amt * risk), 2) if sig == "BUY" else round(entry - (l3_amt * risk), 2)
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if not l3_active and b['high'] >= l3_target:
                    l3_active = True
                    current_sl = l3_sl
                ...
                if not l3_active and b['low'] <= l3_target:
                    l3_active = True
                    current_sl = l3_sl
    ...
    print(f"S20.36 Sovereign Result | Trades: {trades:3d} | W/BE/L: {wins:3d}/{bes:3d}/{losses:2d} | WR: {wr:4.1f}% | NonLoss: {non_loss:4.1f}% | NetPnL: ${pnl:8.2f} | PF: {pf:6.2f} | MaxDD: ${max_dd:5.2f} | Mos: {pos_m}/{tot_m}")
