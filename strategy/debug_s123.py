# -*- coding: utf-8 -*-
"""debug_s123.py
Find search space parameters to exceed S20.122 ($33,949.36) and bridge to S20.130
"""
...
def main():
    ...
    stages_32 = stages_31 + [(9.10, 8.90)]
    stages_33 = stages_32 + [(9.25, 9.05)]
    stages_34 = stages_33 + [(9.40, 9.20)]
    stages_35 = stages_34 + [(9.55, 9.35)]

    # Explore vsa=1.11, 1.12, 1.13 | wicks 0.07, 0.08, 0.09, 0.10, 0.11, 0.12 | TP 9.20 to 9.80
    for stg in [stages_33, stages_34, stages_35]:
        for vsa in [1.12, 1.11, 1.13]:
            for wick in [0.08, 0.09, 0.10, 0.11, 0.12]:
                for depth in [0.124, 0.125, 0.126]:
                    ...
                    for tp in [9.30, 9.40, 9.50, 9.60, 9.70]:
                        ...
                        if res['pnl'] > cur_champion_pnl and res['wr'] >= 83.0 and res['max_dd'] <= 22.0:
                            print(f"FOUND: stg_len={len(stg)} vsa={vsa} wick={wick} depth={depth} TP={tp} -> PnL: ${res['pnl']:,.2f} Trades: {res['trades']} WR: {res['wr']:.1f}% DD: ${res['max_dd']:.2f}", flush=True)
