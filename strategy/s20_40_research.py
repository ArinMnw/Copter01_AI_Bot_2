# -*- coding: utf-8 -*-
"""s20_40_research.py
Explore quantum breakthroughs to beat S20.39 (+$10,930.75) on STRICT LOT 0.01:
Hard Constraints:
...
1. Octa-Stage Quantum Ratchet Lock (TP 4.8R, 5.0R, 5.2R).
2. Addition of H2 Timeframe (2-Hour Intermediate Horizon).
3. Asian Peak Liquidity Window: 03:00 to 22:00 UTC (10:00 to 05:00 BKK).
4. Equal Highs / Equal Lows (EQH/EQL) Sweep Confluence.
...
def extract_setups(df, tf_label, min_vol=1.20, min_wick=0.38, retest_depth=0.130, sl_mult=0.20, hours=(4, 22), use_pqh=True):
    setups = []
    h_start, h_end = hours
    ...
    for s in setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if lock7_trig > 0 and not l7_active and b['high'] >= l7_target:
                    l7_active = True
                    current_sl = l7_sl
                ...
                if lock7_trig > 0 and not l7_active and b['low'] <= l7_target:
                    l7_active = True
                    current_sl = l7_sl
...
def main():
    ...
    print(" S20.40 QUANT RESEARCH (TARGET > $10,930.75 ON STRICT LOT 0.01)", flush=True)
    print(" Benchmark S20.39: +$10,930.75 | MaxDD $13.77 | WR 87.5%", flush=True)
    print("=" * 115, flush=True)

    # Test 1: Test with and without H2 Timeframe
    print("--- 1. Testing Multi-Horizon (Without vs With H2 Horizon) ---", flush=True)
    for use_h2 in [False, True]:
        ...
        h2_s = extract_setups(df_h2, "H2", min_vol=1.20, retest_depth=0.130, sl_mult=0.20, hours=(4, 22), use_pqh=True) if use_h2 else []
        comb = sorted(s0 + s1 + s2 + s3 + h2_s, key=lambda x: x['time'])
        ...
        res = run_simulation(comb, m5_gold, m5_times, m5_len,
                             ...
                             lock6_trig=4.2, lock6_amt=3.9, lock7_trig=0.0, lock7_amt=0.0,
                             tp_r=4.6)
        print(f"Use H2={use_h2}: Trades: {res['trades']:4d} | W/BE/L: {res['wins']:3d}/{res['bes']:3d}/{res['losses']:2d} | WR: {res['wr']:4.1f}% | Net: ${res['pnl']:8.2f} | PF: {res['pf']:5.2f} | MaxDD: ${res['max_dd']:5.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)

    # Test 2: Test Session Window 03:00 vs 04:00
    print("\n--- 2. Testing Session Window (03:00 vs 04:00) ---", flush=True)
    for win in [(4, 22), (3, 22)]:
    ...
    # Test 3: Octa-Stage Quantum Ratchet Lock (TP 4.8R, 5.0R, 5.2R)
    print("\n--- 3. Testing Octa-Stage Ratchet Trailing Lock ---", flush=True)
    s0 = extract_setups(df_h4, "H4", min_vol=1.20, retest_depth=0.130, sl_mult=0.20, hours=(4, 22), use_pqh=True)
    s1 = extract_setups(df_h1, "H1", min_vol=1.20, retest_depth=0.130, sl_mult=0.20, hours=(4, 22), use_pqh=True)
    s2 = extract_setups(df_m30, "M30", min_vol=1.20, retest_depth=0.130, sl_mult=0.20, hours=(4, 22), use_pqh=True)
    s3 = extract_setups(df_m15, "M15", min_vol=1.20, retest_depth=0.130, sl_mult=0.20, hours=(4, 22), use_pqh=True)
    ...
    octa_configs = [
        ("Base Septa TP 4.6R", 0.8, 1.5, 1.0, 2.4, 2.0, 3.0, 2.8, 3.5, 3.3, 3.8, 3.5, 4.2, 3.9, 0.0, 0.0, 4.6),
