# -*- coding: utf-8 -*-
"""run_s20_301_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.301 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.301 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Multi-Timeframe Institutional Synergy & Sniper Extension", flush=True)
    print("=" * 115, flush=True)
    ...
    for tf, df_tf in dfs.items():
        for cur in df_tf.to_dict('records'):
            ...
            if sig:
                ...
                sl_mult = ((0.188 - 0.002 if is_overlap else 0.188) if is_hc else (0.190 if is_comp else 0.195)) + 0.0
                sl_buf = max(sl_mult * cur['atr'], 0.22)
                active_depth = (0.121 if is_hc else (0.124 if is_comp else 0.125)) + 0.001
    ...
    res = run_simulation(m5_gold, m5_times, setups, tp_r=13.43, stages=stages)

    print("=" * 115, flush=True)
    print(" S20.301 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    print(f"Monthly breakdown saved to S20_301_monthly.csv", flush=True)
