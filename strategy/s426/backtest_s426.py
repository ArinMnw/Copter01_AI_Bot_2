# -*- coding: utf-8 -*-
"""Standalone backtest runner สำหรับ S426 Pivot Reversal Stop-Breakout —
โครงสร้างเหมือน backtest_s424.py/backtest_s425.py (precompute signal ทั้งชุด
ครั้งเดียว, reversal-only ไม่มี SL/TP ตาม default) ต่างตรงที่นี่เป็น STOP order
(ไม่ใช่ market fill ที่ open ของแท่งถัดไปทันที) — armed stop level ที่คำนวณ
เสร็จตอนปิดแท่ง i จะถูกเช็ค trigger ด้วย high/low ของแท่ง i+1 เป็นต้นไป ถ้าราคา
ทะลุระดับ stop จะ fill ที่ max/min(open, stop_price) เพื่อจำลอง gap-through

Usage:
    python backtest_s426.py --start 2026-06-01 --end 2026-08-11 --tf M15
    python backtest_s426.py --days 60 --tf M15
    python backtest_s426.py --days 60 --tf all   (M1/M5/M15/M30/H1)
"""
...
from strategy426 import DEFAULT_CFG, _pivot_state_series
...
def backtest(tf_name, spread, lot, start_bkk, end_bkk, cfg=None,
             pct_equity=None, commission_pct=None, starting_balance=100000.0,
             reverse=False):
    """pct_equity/commission_pct = None -> โหมดเดิม (fixed lot + flat spread,
    ตรงกับ convention S420-S425 ของโปรเจกต์). ถ้าใส่ค่า -> โหมด "TV mode"
    (percent_of_equity sizing + percent commission ตรงตาม Pine ต้นฉบับเป๊ะ
    default_qty_type=percent_of_equity/25, commission_type=percent/0.1 —
    ใช้เทียบ exact-match กับผล backtest จาก TradingView โดยตรง)

    reverse=True -> สลับทิศทุกสัญญาณ (BUY<->SELL ที่ราคา fill เดิม) ผลคือ
    sl/tp ก็สลับกันโดยอัตโนมัติ (เพราะสูตร sl/tp คำนวณจาก side_wanted ที่กลับ
    ด้านแล้ว) ใช้เช็คว่า strategy ที่ PF<1 มี edge กลับด้านซ่อนอยู่หรือไม่"""
    detector_cfg = dict(DEFAULT_CFG)
    detector_cfg.update(cfg or {})
    left_bars = int(detector_cfg["LEFT_BARS"])
    right_bars = int(detector_cfg["RIGHT_BARS"])
    mintick = float(detector_cfg["MINTICK"])
    ...
    long_stop_series, short_stop_series = _pivot_state_series(bars_norm, left_bars, right_bars)
    ...
    def open_position(index, side_wanted, fill_price):
        ...
        qty = (equity * pct_equity / 100.0 / fill_price) if tv_mode else None
        position = {
            "side": side_wanted, "entry": fill_price, "sl": sl, "tp": tp, "qty": qty,
            "entry_time": bar_time_to_bkk(bars_norm[index]["time"]),
            "pattern": f"S426 {signal_name} Pivot Reversal Stop",
            "reason": f"pivot breakout ATR={atr_str}",
        }

    # armed stop levels สังเกตตอนปิดแท่ง i-1 -> เช็ค trigger ด้วยแท่ง i
    for index in range(max(start_index, 1), len(bars_norm)):
        ...
        if position is not None and use_atr_stop:
            side = position["side"]
            if side > 0 and (bar["high"] >= position["tp"] or bar["low"] <= position["sl"]):
                hit_tp = bar["high"] >= position["tp"]
                close_position(index, position["tp"] if hit_tp else position["sl"], "TP" if hit_tp else "SL")
                position = None
            elif side < 0 and (bar["low"] <= position["tp"] or bar["high"] >= position["sl"]):
                hit_tp = bar["low"] <= position["tp"]
                close_position(index, position["tp"] if hit_tp else position["sl"], "TP" if hit_tp else "SL")
                position = None

        long_armed = long_stop_series[index - 1]
        short_armed = short_stop_series[index - 1]
        long_level = round(long_armed + mintick, 2) if long_armed is not None else None
        short_level = round(short_armed - mintick, 2) if short_armed is not None else None

        if long_level is None and short_level is None:
            continue

        # จำลอง intrabar path แบบ synthetic tick (ไม่มี tick จริงให้ใช้) —
        # แท่งขาขึ้น (close>=open): open->low->high->close, แท่งขาลง:
        # open->high->low->close (convention เดียวกับที่ TradingView ใช้ตอน
        # replay ข้อมูลที่ไม่มี tick จริง — ทำให้เช็ค whipsaw/หลาย fill ภายใน
        # แท่งเดียวได้ ใกล้เคียง calc_on_every_tick=true ของต้นฉบับ Pine มากขึ้น)
        o, h, l, c = bar["open"], bar["high"], bar["low"], bar["close"]
        path = [o, l, h, c] if c >= o else [o, h, l, c]

        long_active = long_level is not None
        short_active = short_level is not None
        for seg_start, seg_end in zip(path, path[1:]):
            if not long_active and not short_active:
                break
            lo, hi = (seg_start, seg_end) if seg_start <= seg_end else (seg_end, seg_start)
            rising = seg_end >= seg_start
            hits = []
            if long_active and lo <= long_level <= hi:
                hits.append(("BUY", long_level))
            if short_active and lo <= short_level <= hi:
                hits.append(("SELL", short_level))
            if not hits:
                continue
            hits.sort(key=lambda t: t[1], reverse=not rising)

            for signal_name, level in hits:
                side_wanted = 1 if signal_name == "BUY" else -1
                if reverse:
                    side_wanted = -side_wanted
                if signal_name == "BUY":
                    long_active = False
                else:
                    short_active = False
...
def main():
    ...
    parser.add_argument("--cfg-json", default="{}",
                         help="JSON object ส่งเข้า cfg (เช่น LEFT_BARS/RIGHT_BARS/SL_ATR_MULT)")
    parser.add_argument("--use-atr-stop", action="store_true",
                         help="เปิด ATR-based SL/TP (default ปิด — reversal-only ตามต้นฉบับ)")
    parser.add_argument("--tv-mode", action="store_true",
                         help="เทียบ exact-match กับ TradingView: percent_of_equity sizing "
                              "+ percent commission ตาม Pine ต้นฉบับเป๊ะ (แทน fixed lot + flat spread)")
    parser.add_argument("--pct-equity", type=float, default=25.0,
                         help="%% ของ equity ต่อไม้ (เฉพาะ --tv-mode, default 25 ตาม Pine)")
    parser.add_argument("--commission-pct", type=float, default=0.1,
                         help="%% commission ต่อ order (เฉพาะ --tv-mode, default 0.1 ตาม Pine)")
    parser.add_argument("--tv-balance", type=float, default=100000.0,
                         help="ยอดเงินตั้งต้น (เฉพาะ --tv-mode, default 100000 ตาม TV)")
    parser.add_argument("--reverse", action="store_true",
                         help="สลับทิศทุกสัญญาณ (BUY<->SELL, sl<->tp อัตโนมัติ) เช็คว่ามี edge กลับด้านไหม")
    parser.add_argument("--csv", default=os.path.join(_HERE, "s426_trades.csv"),
                         help="path ไฟล์ CSV ผลลัพธ์ (default = s426_trades.csv ในโฟลเดอร์นี้)")
    ...
    parser.add_argument("--compare-comment-prefix", default="S426",
                         help="กรอง deal ที่มี comment ขึ้นต้น/มีคำนี้เท่านั้น (default S426)")
    ...
    for tf_name in tf_list:
        ...
        try:
            summary, trades = backtest(tf_name, args.spread, lot, start_bkk, end_bkk, detector_cfg,
                                        ...
                                        starting_balance=args.tv_balance, reverse=args.reverse)
