...
def strategy_20_13_2(rates, tf="H1"):
    ...
    # IDEA 2: Engulfing bar body size filter
    cur_range = current_bar['high'] - current_bar['low']
    cur_body = abs(current_bar['open'] - current_bar['close'])
    cur_body_pct = cur_body / cur_range if cur_range > 0 else 0
    strong_body = cur_body_pct >= 0.5
    ...
    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        if not strong_body:
            return {"signal": "WAIT", "reason": "Weak Engulfing Body"}
        if is_london_open:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.2 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | Strong Body | TP {tp:.2f}"
        }
    ...
    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.2 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | Strong Body | TP {tp:.2f}"
        }
