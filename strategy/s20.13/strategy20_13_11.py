...
def strategy_20_13_11(rates, tf="H1"):
    ...
    # IDEA 11: Minimum Distance from EMA 50 (Mean Reversion Stretched Filter)
    dist_ema50 = abs(current_bar['close'] - current_bar['ema_50'])
    is_stretched = dist_ema50 >= (0.8 * current_bar['atr'])
    ...
    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        if not is_stretched:
            return {"signal": "WAIT", "reason": "Too close to EMA 50 (Not stretched enough)"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.11 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | EMA50 Dist OK | TP {tp:.2f}"
        }
    ...
    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.11 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | EMA50 Dist OK | TP {tp:.2f}"
        }
