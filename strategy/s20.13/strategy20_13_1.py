...
def strategy_20_13_1(rates, tf="H1"):
    ...
    # Use a 10-bar lookback for local structural swing (prior to the sweep zone)
    ...
    # Check Time of Day (Server Time)
    # Server Time 8 and 9 corresponds to BKK 15:00 and 16:00 (London Open)
    ...
    # IDEA 1: Wick Filter. Reversal bar must have a prominent wick on the sweep side.
    def check_lower_wick(row, min_pct=0.3):
        rng = row['high'] - row['low']
        if rng == 0: return False
        lower_wick = min(row['open'], row['close']) - row['low']
        return (lower_wick / rng) >= min_pct

    def check_upper_wick(row, min_pct=0.3):
        rng = row['high'] - row['low']
        if rng == 0: return False
        upper_wick = row['high'] - max(row['open'], row['close'])
        return (upper_wick / rng) >= min_pct
    ...
    has_wick_buy = False
    if sweep_buy or instant_sweep_buy:
        # Find the bar with the lowest low in recent 3 + current
        lowest_bar_idx = df.iloc[-4:]['low'].idxmin()
        lowest_bar = df.loc[lowest_bar_idx]
        has_wick_buy = check_lower_wick(lowest_bar, min_pct=0.3)

    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        if not has_wick_buy:
            return {"signal": "WAIT", "reason": "No Prominent Lower Wick"}
        if is_london_open:
            return {"signal": "WAIT", "reason": "London Open Trap"}
        if is_uptrend:
            return {"signal": "WAIT", "reason": "Avoid Buy in Uptrend"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.1 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | Wick | TP {tp:.2f}"
        }
    ...
    has_wick_sell = False
    if sweep_sell or instant_sweep_sell:
        highest_bar_idx = df.iloc[-4:]['high'].idxmax()
        highest_bar = df.loc[highest_bar_idx]
        has_wick_sell = check_upper_wick(highest_bar, min_pct=0.3)

    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        if not has_wick_sell:
            return {"signal": "WAIT", "reason": "No Prominent Upper Wick"}
        if is_london_open:
            return {"signal": "WAIT", "reason": "London Open Trap"}
        if is_downtrend:
            return {"signal": "WAIT", "reason": "Avoid Sell in Downtrend"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.1 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | Wick | TP {tp:.2f}"
        }
