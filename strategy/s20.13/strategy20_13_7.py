...
def strategy_20_13_7(rates, tf="H1"):
    ...
    # IDEA 7: Minimum Swing Size (Volatility Context)
    swing_size = local_high - local_low
    is_valid_swing = swing_size > (current_bar['atr'] * 2.0)
    ...
    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        if not is_valid_swing:
            return {"signal": "WAIT", "reason": "Swing Too Small"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.7 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | Valid Swing | TP {tp:.2f}"
        }
    ...
    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.7 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | Valid Swing | TP {tp:.2f}"
        }
