...
def optimize_200k(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    # Let's test different unlock aggression modes
    modes = [
        {"name": "Mode 1 (Precision)", "drift_z": 1.0, "tz_adx": 50, "rsi_l_adx": 35, "rsi_h_vol": 1.5, "sell_adx": 20},
        {"name": "Mode 2 (Balanced 200k)", "drift_z": 0.5, "tz_adx": 55, "rsi_l_adx": 45, "rsi_h_vol": 1.3, "sell_adx": 25},
        {"name": "Mode 3 (Aggressive 200k)", "drift_z": 0.0, "tz_adx": 60, "rsi_l_adx": 55, "rsi_h_vol": 1.1, "sell_adx": 30},
        {"name": "Mode 4 (Ultra Unlocked)", "drift_z": -0.5, "tz_adx": 70, "rsi_l_adx": 65, "rsi_h_vol": 0.9, "sell_adx": 40},
    ]
    ...
    for m in modes:
        ...
        for t in candidate_trades:
            ...
            if t['v23_pass']:
                ...
                # V24 Unlock Rules based on mode
                if t['sig'] == "BUY" and t['category'] == "BUY Low Volatility Drift Block":
                    if t['di_diff'] >= 2.0 and t['vol_ratio'] >= 1.0 and t['z_score'] >= m['drift_z'] and t['adx'] >= 15.0 and t['hour'] not in [6, 10]:
                        final_signal = "BUY"
                elif t['sig'] == "BUY" and t['category'] == "BUY Trend/Z Block":
                    if t['di_diff'] >= -5.0 and t['adx'] <= m['tz_adx'] and t['vol_ratio'] >= 1.2 and t['z_score'] >= 0.0 and t['hour'] not in [10, 15]:
                        final_signal = "BUY"
                elif t['sig'] == "BUY" and t['category'] == "RSI too low":
                    if t['adx'] <= m['rsi_l_adx'] and t['di_diff'] >= -25.0 and t['vol_ratio'] >= 0.8 and t['hour'] not in [15, 16, 18]:
                        final_signal = "BUY"
                elif t['sig'] == "SELL" and t['category'] == "RSI too high":
                    if t['vol_ratio'] >= m['rsi_h_vol'] and t['atr_pct'] >= 0.25 and t['hour'] not in [6, 15]:
                        final_signal = "SELL"
                elif t['sig'] == "SELL" and t['category'] in ["SELL Low ADX Above EMA50 Block", "SELL Momentum Conflict Block", "SELL Low Volatility Upper BB Block"]:
                    if t['vol_ratio'] >= 1.0 and t['adx'] <= m['sell_adx']:
                        final_signal = "SELL"
        ...
        print(f"Win Rate (W/L): {wr_wl:.2f}% | Net Profit: ${pnl:,.2f}")
        print(f"Master Dates: {all(master_dates.values())}\n")
