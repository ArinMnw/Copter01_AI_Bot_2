...
def test_wrapper(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    def evaluate_v24_test(df, idx, tf="H1", rule_mode=1):
        ...
        if rule_mode == 1:
            # Let's test blocking the remaining SELL losses:
            if sig == "SELL":
                # Check if we can block any of the 5 SELL losses cleanly without touching wins!
                if cur['dist_ema200'] < -65.0:
                    return {"signal": "WAIT", "reason": f"v24 SELL Deep Below EMA200 Block (DistEMA200={cur['dist_ema200']:.1f})"}
                if cur['hour'] == 5 and cur['vol_ratio'] < 1.10:
                    return {"signal": "WAIT", "reason": f"v24 SELL Early Asian Low Volume Trap (Hour={cur['hour']}, VolRatio={cur['vol_ratio']:.2f})"}
                if cur['rsi_7'] > 64.0 and cur['z_score'] > 0.75:
                    return {"signal": "WAIT", "reason": f"v24 SELL High RSI7 & Positive Z Trap (RSI7={cur['rsi_7']:.1f}, Z={cur['z_score']:.2f})"}
        elif rule_mode == 2:
            if sig == "SELL":
                if cur['dist_ema200'] < -65.0: return {"signal": "WAIT"}
    ...
    for mode in [0, 1, 2]:
        ...
        for i in range(100, len(rates) - 10):
            res = evaluate_v24_test(df_master, i, tf="H1", rule_mode=mode) if mode > 0 else evaluate_bar(df_master, i, tf="H1")
