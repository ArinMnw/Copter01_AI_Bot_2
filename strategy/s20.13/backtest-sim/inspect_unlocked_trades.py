...
def inspect_unlocked(days=700, symbol="XAUUSD.iux", compound=1.5):
        ...
        if sig:
            ...
            if closed_type in ["WIN", "LOSS"]:
                records.append({
                    ...
                    "dist_ema50": round(current_bar['dist_ema50'], 1), "body_pct": round(current_bar['body_pct'], 2),
                    "hour": hour
                })

    df = pd.DataFrame(records)
    target_cats = ["BUY Low Volatility Drift Block", "BUY Trend/Z Block", "RSI too low", "RSI too high", "BUY Trend Exhaustion Block", "BUY Extreme Low Volume Breakout Block"]
    
    for cat in target_cats:
        print(f"\n=========================================================================================")
        print(f"CATEGORY: {cat}")
        print(f"=========================================================================================")
        sub = df[df['category'] == cat].sort_values("outcome", ascending=False)
        if len(sub) == 0: continue
        print(sub[['time', 'outcome', 'rsi', 'adx', 'di_diff', 'vol_ratio', 'z_score', 'atr_pct', 'dist_ema50', 'hour']].to_string(index=False))
