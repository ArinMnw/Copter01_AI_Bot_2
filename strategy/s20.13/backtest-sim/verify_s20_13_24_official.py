...
def run_official_verify(days=700, symbol="XAUUSD.iux"):
    ...
    for i in range(100, len(rates) - 10):
        res = strategy20_13_24.evaluate_bar(df_master, i, tf="H1")
        ...
        if closed_type == "WIN":
            wins += 1; trades += 1; pnl += ((tp - entry) * 10 * 2.0) if sig == "BUY" else ((entry - tp) * 10 * 2.0)
            if sig == "BUY": sl_buy_count = 0
            else: sl_sell_count = 0
        elif closed_type == "LOSS":
            losses += 1; trades += 1; pnl -= ((entry - sl) * 10 * 2.0) if sig == "BUY" else ((sl - entry) * 10 * 2.0)
    ...
    print("=========================================================")
    print(f"OFFICIAL STRATEGY S20.13.24 VALIDATION (700 DAYS):")
