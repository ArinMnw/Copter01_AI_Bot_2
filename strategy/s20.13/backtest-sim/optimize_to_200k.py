...
def optimize_to_200k(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    # Let's test combinations of filter relaxations to maximize profit towards 200k!
    # First, let's see how much profit we get if we modify evaluate_bar logic directly in a custom evaluator function:
    
    def eval_bar_custom(df, idx, relax_mode=1):
        if idx < 20 or idx >= len(df): return {"signal": "WAIT", "reason": "Not enough data"}
        ...
        if (sweep_buy and engulf_buy) or instant_sweep_buy:
            if not is_strong_range or is_ny_pre_open or is_sydney_open or is_tokyo_buy or is_late_ny_buy or is_midnight_buy or is_london_open or is_london_fake:
                return {"signal": "WAIT"}
            if current_bar['rsi'] < 35: return {"signal": "WAIT"}
            if current_bar['z_score'] > 0.0 and current_bar['adx'] > 30.0:
                if relax_mode not in [2, 3]: return {"signal": "WAIT"} # test relaxing Trend/Z block
            if current_bar['adx'] > 52.0: return {"signal": "WAIT"}
            
            # v18 Low Volatility Drift Block
            if current_bar['atr_pct'] <= 0.41 and current_bar['adx'] < 50.0:
                if relax_mode == 1:
                    return {"signal": "WAIT"} # baseline
                elif relax_mode in [2, 3]:
                    # ONLY block if bearish DI or weak RSI
                    if current_bar['di_diff'] <= 0.0 or current_bar['rsi_7'] <= 52.0:
                        return {"signal": "WAIT"}
            
            if current_bar['vol_ratio'] < 0.55: return {"signal": "WAIT"}
            if current_bar['rsi'] > 40.0 and current_bar['dist_ema50'] < -20.0: return {"signal": "WAIT"}
            if current_bar['rsi'] < 48.0 and current_bar['di_diff'] > 0.0: return {"signal": "WAIT"}
            if current_bar['rsi'] < 40.0 and current_bar['adx'] > 42.0: return {"signal": "WAIT"}
        ...
        if (sweep_sell and engulf_sell) or instant_sweep_sell:
            if not is_strong_range or is_ny_pre_open or is_sydney_open or is_london_open or is_london_fake:
                return {"signal": "WAIT"}
            if current_bar['rsi'] > 60: return {"signal": "WAIT"}
            if (current_bar['z_score'] < 0.0 and current_bar['adx'] > 50.0) or (current_bar['close'] - current_bar['ema_50'] > 100.0): return {"signal": "WAIT"}
            if current_bar['rsi_7'] > 50.5 and 0.35 <= current_bar['body_pct'] <= 0.615: return {"signal": "WAIT"}
            if current_bar['rsi_7'] > 50.5 and 0.35 <= current_bar['body_pct'] <= 0.65 and current_bar['di_diff'] < 4.0: return {"signal": "WAIT"}
            if current_bar['vol_ratio'] < 0.88 and current_bar['di_diff'] > 1.0: return {"signal": "WAIT"}
            if current_bar['body_pct'] < 0.145: return {"signal": "WAIT"}
            if current_bar['rsi'] < 37.0 and current_bar['z_score'] > -1.80: return {"signal": "WAIT"}
            if current_bar['adx'] < 17.5 and current_bar['vol_ratio'] < 0.90: return {"signal": "WAIT"}
            if current_bar['rsi'] > 58.5 and current_bar['rsi_7'] < 41.5: return {"signal": "WAIT"}
            if current_bar['rsi_7'] < 41.5 and current_bar['di_diff'] > 0.5: return {"signal": "WAIT"}
            if current_bar['adx'] < 20.2 and (current_bar['close'] - current_bar['ema_50']) > -12.0: return {"signal": "WAIT"}
            if current_bar['vol_ratio'] < 1.90 and current_bar['rsi'] < 39.0: return {"signal": "WAIT"}
            if current_bar['z_score'] > 0.20 and current_bar['atr_pct'] < 0.31: return {"signal": "WAIT"}
            if current_bar['rsi'] > 53.0 and current_bar['rsi_7'] < 37.0: return {"signal": "WAIT"}
            if current_bar['dist_ema50'] > 20.0 and current_bar['upper_wick_pct'] > 0.15: return {"signal": "WAIT"}
            if hour == 10: return {"signal": "WAIT"}
            if current_bar['adx'] > 20.0 and current_bar['vol_ratio'] > 2.50: return {"signal": "WAIT"}
            if current_bar['adx'] < 25.0 and current_bar['lower_wick_pct'] > 0.40: return {"signal": "WAIT"}
            if current_bar['di_diff'] < 0.0 and current_bar['dist_ema50'] > 20.0: return {"signal": "WAIT"}
            if current_bar['dist_ema50'] < 10.0 and current_bar['dist_ema200'] > 50.0: return {"signal": "WAIT"}
            if current_bar['rsi'] > 55.0 and current_bar['di_diff'] < -3.0: return {"signal": "WAIT"}
            if current_bar['rsi'] > 58.0 and current_bar['di_diff'] > 10.0: return {"signal": "WAIT"}
            if current_bar['rsi'] < 50.0 and current_bar['dist_ema200'] > 80.0: return {"signal": "WAIT"}
            if current_bar['z_score'] < -1.50 and current_bar['upper_wick_pct'] > 0.30: return {"signal": "WAIT"}
            if current_bar['body_pct'] > 0.70 and current_bar['atr_pct'] < 0.30: return {"signal": "WAIT"}
            if current_bar['rsi'] < 52.0 and current_bar['z_score'] > 0.0: return {"signal": "WAIT"}
            if current_bar['rsi_7'] < 55.0 and current_bar['body_pct'] > 0.90: return {"signal": "WAIT"}
            
            # v24 precision SELL blocks for the remaining 5 losses:
            if relax_mode == 3:
                # Let's test blocking the 5 remaining losses!
                # 1) 2024-10-21 -> rsi7=30.7, dist_ema50=6.0, uWick%=0.19, body%=0.48
                # 2) 2024-11-15 -> dist_ema200=-69.1
                # 3) 2025-04-09 -> dist_ema200=-70.2
                if current_bar['dist_ema200'] < -65.0: return {"signal": "WAIT"}
                # 4) 2025-05-19 -> rsi7=66.3, z_score=0.86, hour=1
                # 5) 2025-06-04 -> hour=5, z_score=-0.68, vol_ratio=1.01
                if current_bar['hour'] == 5 and current_bar['vol_ratio'] < 1.10: return {"signal": "WAIT"}
                if current_bar['rsi_7'] > 65.0 and current_bar['z_score'] > 0.80: return {"signal": "WAIT"}
    ...
    for mode in [1, 2, 3]:
        ...
        for i in range(100, len(rates) - 10):
            res = eval_bar_custom(df_master, i, relax_mode=mode)
