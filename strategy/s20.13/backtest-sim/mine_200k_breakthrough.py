...
def test_breakthrough(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    def eval_v24_candidate(df, idx, tf="H1", test_mode=1):
        ...
        if (sweep_buy and engulf_buy) or instant_sweep_buy:
            ...
            # Trend/Z Block relaxation
            if current_bar['z_score'] > 0.0 and current_bar['adx'] > 30.0:
                if test_mode in [1, 2, 3]:
                    # Allow high momentum breakout climbs
                    if not (current_bar['rsi'] > 55.0 and current_bar['vol_ratio'] > 1.20 and current_bar['di_diff'] > 5.0):
                        return {"signal": "WAIT"}
                else:
                    return {"signal": "WAIT"}
            ...
            if current_bar['adx'] > 52.0:
                if test_mode in [2, 3]:
                    if not (current_bar['vol_ratio'] > 1.50 and current_bar['rsi_7'] > 60.0):
                        return {"signal": "WAIT"}
                else:
                    return {"signal": "WAIT"}
            
            # v18 Low Volatility Drift Block relaxation
            if current_bar['atr_pct'] <= 0.41 and current_bar['adx'] < 50.0:
                if test_mode == 0:
                    return {"signal": "WAIT"}
                elif test_mode in [1, 2, 3]:
                    # Unlock clean bullish drifts!
                    is_clean_drift = (current_bar['di_diff'] > 0.0 and current_bar['rsi_7'] > 52.0 and current_bar['dist_ema50'] > 5.0)
                    if not is_clean_drift:
                        return {"signal": "WAIT"}
        ...
        if (sweep_sell and engulf_sell) or instant_sweep_sell:
            ...
            # v24 surgical SELL blocks:
            if test_mode in [1, 2, 3]:
                if current_bar['dist_ema50'] > 5.0 and current_bar['upper_wick_pct'] < 0.22 and current_bar['body_pct'] < 0.50:
                    return {"signal": "WAIT"}
                if current_bar['hour'] == 5 and current_bar['vol_ratio'] < 1.10:
                    return {"signal": "WAIT"}
                if current_bar['rsi_7'] > 64.0 and current_bar['z_score'] > 0.75:
                    return {"signal": "WAIT"}
    ...
    for mode in [0, 1, 2, 3]:
        ...
        for i in range(100, len(rates) - 10):
            res = eval_v24_candidate(df_master, i, tf="H1", test_mode=mode)
