...
def sim_v21(days=365, symbol="XAUUSD.iux", compound=1.5):
    ...
    wins21, losses21, be21, trades21, pnl21 = 0, 0, 0, 0, 0.0
    ...
    for i in range(100, len(rates) - 10):
        ...
        if res and res.get("signal") in ["BUY", "SELL"]:
            ...
            # v21 Precision Filters
            if signal == "SELL":
                if cur['dist_ema50'] > 20.0 and cur['upper_wick_pct'] > 0.15: continue
                if cur['hour'] == 10: continue
                if cur['adx'] > 20.0 and cur['vol_ratio'] > 2.50: continue
                if cur['adx'] < 25.0 and cur['lower_wick_pct'] > 0.40: continue
                if cur['di_diff'] < 0.0 and cur['dist_ema50'] > 20.0: continue
            elif signal == "BUY":
                if cur['rsi'] > 40.0 and cur['dist_ema50'] < -20.0: continue
            ...
            for f_bar in future_rates:
                if signal == "BUY":
                    if f_bar['low'] <= sl:
                        if be_act:
                            be21 += 1; all_trades_log.append({"time": dt_str, "type": signal, "pnl": 0.0, "reason": "BE"})
                        else:
                            losses21 += 1; pnl21 -= ((entry - sl) * 10 * compound)
                            sl_buy_count += 1; last_buy_loss = entry
                            all_trades_log.append({"time": dt_str, "type": signal, "pnl": -((entry - sl) * 10 * compound), "reason": "SL"})
                        trades21 += 1; closed = True; break
                    elif f_bar['high'] >= tp:
                        wins21 += 1; trades21 += 1; sl_buy_count = 0
                        pnl21 += ((tp - entry) * 10 * compound)
    ...
    win_rate_all = (wins21 / trades21 * 100) if trades21 > 0 else 0
    win_rate_wl = (wins21 / (wins21 + losses21) * 100) if (wins21 + losses21) > 0 else 0
    
    print("\n================== S20.13.21 SIMULATION RESULTS (365 DAYS) ==================")
    print(f"Total Trades : {trades21}")
    print(f"Wins (TP)    : {wins21}")
    print(f"Losses (SL)  : {losses21}")
    print(f"Break Even   : {be21}")
    print(f"Win/Loss Rate: {win_rate_wl:.2f}% (Target: >80%)")
    print(f"All Trade WR : {win_rate_all:.2f}%")
    print(f"Net P&L ($)  : ${pnl21:,.2f} (Target: >$100k)")
