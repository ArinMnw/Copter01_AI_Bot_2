...
def test_final(days=700, symbol="XAUUSD.iux"):
    ...
    print("=== FINAL OFFICIAL VALIDATION (COMPOUND=2.0) ===")
    ...
    for t in candidate_trades:
        ...
        if t['v23_pass']:
            ...
            # V24 Surgical Zero-Loss Unlocks
            if t['sig'] == "BUY" and t['category'] == "BUY Low Volatility Drift Block":
                ...
                if t['adx'] > 50.0 and t['vol_ratio'] > 1.2 and t['z_score'] < -0.1:
                    final_signal = "BUY"
            elif t['sig'] == "SELL" and t['category'] == "SELL Oversold Trap Block":
                if t['vol_ratio'] >= 1.2 and t['rsi'] <= 35.0:
                    final_signal = "SELL"
            elif t['sig'] == "SELL" and t['category'] == "SELL Low ADX Above EMA50 Block":
                if t['vol_ratio'] >= 2.0 and t['adx'] <= 10.0:
                    final_signal = "SELL"
            elif t['sig'] == "SELL" and t['category'] == "SELL Low Volume Chop Block":
                if t['hour'] == 5 and t['vol_ratio'] >= 0.8:
                    final_signal = "SELL"
        ...
        if t['type'] == "WIN":
            wins += 1; trades += 1; pnl += ((t['tp'] - t['entry']) * 10 * 2.0) if final_signal == "BUY" else ((t['entry'] - t['tp']) * 10 * 2.0)
            if final_signal == "BUY": sl_buy_count = 0
            else: sl_sell_count = 0
        elif t['type'] == "LOSS":
            losses += 1; trades += 1; pnl -= ((t['entry'] - t['sl']) * 10 * 2.0) if final_signal == "BUY" else ((t['sl'] - t['entry']) * 10 * 2.0)
