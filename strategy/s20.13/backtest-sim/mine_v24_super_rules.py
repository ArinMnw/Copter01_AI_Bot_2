...
def mine_super_rules(days=700, symbol="XAUUSD.iux"):
        ...
        if sig:
            ...
            if closed_type in ["WIN", "LOSS"]:
                records.append({
                    ...
                    "category": clean_reason, "sig": sig, "outcome": 1 if closed_type == "WIN" else 0,
                    ...
                    "dist_ema50": current_bar['dist_ema50'], "body_pct": current_bar['body_pct'],
                    "hour": hour
                })

    df = pd.DataFrame(records)
    features = ['rsi', 'adx', 'di_diff', 'vol_ratio', 'z_score', 'atr_pct', 'dist_ema50', 'body_pct', 'hour']
    
    for sig_type in ["BUY", "SELL"]:
        sub = df[df['sig'] == sig_type]
        print(f"\n===================================================================")
        print(f"MINING SUPER RULES FOR {sig_type} (Wins: {sum(sub['outcome'] == 1)}, Losses: {sum(sub['outcome'] == 0)})")
        print(f"===================================================================")
        ...
        imp = pd.DataFrame({'feature': features, 'importance': tree.feature_importances_}).sort_values('importance', ascending=False)
        print("\nFeature Importances:")
