...
def map_blocked_wins(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    for i in range(100, len(rates) - 10):
        ...
        if sig:
            ...
            if res and res.get("signal") == "WAIT":
                ...
                if closed_type == "WIN":
                    blocked_wins.append({"clean_reason": clean_reason, "type": sig, "rsi": current_bar['rsi'], "adx": current_bar['adx'], "di_diff": current_bar['di_diff'], "vol_ratio": current_bar['vol_ratio']})
                elif closed_type == "LOSS":
                    blocked_losses.append({"clean_reason": clean_reason, "type": sig, "rsi": current_bar['rsi'], "adx": current_bar['adx'], "di_diff": current_bar['di_diff'], "vol_ratio": current_bar['vol_ratio']})

    df_bw = pd.DataFrame(blocked_wins)
    df_bl = pd.DataFrame(blocked_losses)
    print("=== ALL BLOCKED WIN CATEGORIES IN v23 ===")
    if len(df_bw) > 0:
        print(df_bw['clean_reason'].value_counts())
    print("\n=== ALL BLOCKED LOSS CATEGORIES IN v23 ===")
    if len(df_bl) > 0:
        print(df_bl['clean_reason'].value_counts())
