...
def test_all_combos(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    print("=== PRINTING COMBINATION TEST RESULTS ===")
    
    # Let's test various candidate sets of rules
    configs = [
        {"name": "Combo A: Drift + Trend/Z + Exhaustion (Our Base)", "rsi_l": False, "rsi_h": False, "sell_mods": False},
        {"name": "Combo B: Add Strict RSI Low (DI>0, Vol>1.2)", "rsi_l": True, "rsi_l_rule": 1, "rsi_h": False, "sell_mods": False},
        {"name": "Combo C: Add Strict RSI High (Vol>1.5, ATR%>0.4)", "rsi_l": False, "rsi_h": True, "rsi_h_rule": 1, "sell_mods": False},
        {"name": "Combo D: Add Both Strict RSI Low & High", "rsi_l": True, "rsi_l_rule": 1, "rsi_h": True, "rsi_h_rule": 1, "sell_mods": False},
        {"name": "Combo E: Add SELL Categories Strict (ADX<=20, Vol>1.2)", "rsi_l": False, "rsi_h": False, "sell_mods": True, "sell_rule": 1},
        {"name": "Combo F: ALL STRICT UNLOCKS COMBINED", "rsi_l": True, "rsi_l_rule": 1, "rsi_h": True, "rsi_h_rule": 1, "sell_mods": True, "sell_rule": 1},
        {"name": "Combo G: ALL MODERATE UNLOCKS COMBINED", "rsi_l": True, "rsi_l_rule": 2, "rsi_h": True, "rsi_h_rule": 2, "sell_mods": True, "sell_rule": 2},
    ]
    ...
    for cfg in configs:
            ...
            if t['v23_pass']:
                ...
                if t['sig'] == "BUY" and t['category'] == "BUY Low Volatility Drift Block":
                    ...
                    if t['adx'] > 50.0 and t['vol_ratio'] > 1.2 and t['z_score'] < -0.1:
                        final_signal = "BUY"
                elif cfg.get("rsi_l") and t['sig'] == "BUY" and t['category'] == "RSI too low":
                    if cfg["rsi_l_rule"] == 1:
                        if t['di_diff'] >= 0.0 and t['vol_ratio'] >= 1.2 and t['adx'] <= 35.0: final_signal = "BUY"
                    else:
                        if t['di_diff'] >= -5.0 and t['vol_ratio'] >= 1.0 and t['adx'] <= 42.0: final_signal = "BUY"
                elif cfg.get("rsi_h") and t['sig'] == "SELL" and t['category'] == "RSI too high":
                    if cfg["rsi_h_rule"] == 1:
                        if t['vol_ratio'] >= 1.5 and t['atr_pct'] >= 0.40 and t['z_score'] >= 0.8: final_signal = "SELL"
                    else:
                        if t['vol_ratio'] >= 1.3 and t['atr_pct'] >= 0.30 and t['z_score'] >= 0.5: final_signal = "SELL"
                elif cfg.get("sell_mods") and t['sig'] == "SELL" and t['category'] in ["SELL Low ADX Above EMA50 Block", "SELL Momentum Conflict Block", "SELL Low Volatility Upper BB Block"]:
                    if cfg["sell_rule"] == 1:
                        if t['vol_ratio'] >= 1.2 and t['adx'] <= 20.0 and t['z_score'] >= 0.5: final_signal = "SELL"
                    else:
                        if t['vol_ratio'] >= 1.0 and t['adx'] <= 28.0: final_signal = "SELL"
        ...
        print(f"[{cfg['name']}]")
        print(f"  Wins: {wins} | Losses: {losses} | BE: {be} | WR: {wr_wl:.2f}% | PnL: ${pnl:,.2f} | Master: {all(master_dates.values())}\n")
