...
def test_v24(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    for i in range(100, len(rates) - 10):
        ...
        if sig:
            ...
            if is_pass:
                ...
                be_act = False; closed_type = None; closed_price = 0
                for f_bar in future_rates:
                    if sig == "BUY":
                        if f_bar['low'] <= sl: closed_type = "BE" if be_act else "LOSS"; closed_price = sl; break
                        elif f_bar['high'] >= tp: closed_type = "WIN"; closed_price = tp; break
                        if not be_act and f_bar['high'] >= be_trig: be_act = True; sl = entry
                    elif sig == "SELL":
                        if f_bar['high'] >= sl: closed_type = "BE" if be_act else "LOSS"; closed_price = sl; break
                        elif f_bar['low'] <= tp: closed_type = "WIN"; closed_price = tp; break
                        if not be_act and f_bar['low'] <= be_trig: be_act = True; sl = entry
                
                if closed_type:
                    trades.append({
                        "time": current_bar['time_dt'].strftime("%Y-%m-%d %H:%M"),
                        "sig": sig, "type": closed_type, "entry": entry, "tp": tp, "sl": sl, "close_price": closed_price
                    })

    # SL Guard & Compounding simulation
    balance = 10000.0; consec_sl = 0; sl_guard_active = False; sl_guard_bars_left = 0
    w_cnt = 0; l_cnt = 0; be_cnt = 0
    master_dates = {"2026-07-16 23:00": False, "2026-07-17 21:00": False, "2026-07-17 23:00": False}
    
    for t in trades:
        for md in master_dates:
            if t['time'].startswith(md): master_dates[md] = True
            
        if sl_guard_active:
            sl_guard_bars_left -= 1
            if sl_guard_bars_left <= 0: sl_guard_active = False; consec_sl = 0
            else: continue
            
        risk = balance * (0.02 * compound)
        sl_pts = abs(t['entry'] - t['sl'])
        tp_pts = abs(t['tp'] - t['entry'])
        rr = tp_pts / sl_pts if sl_pts > 0 else 2.6
        
        if t['type'] == "WIN":
            w_cnt += 1; balance += (risk * rr); consec_sl = 0
        elif t['type'] == "LOSS":
            l_cnt += 1; balance -= risk; consec_sl += 1
            if consec_sl >= 2: sl_guard_active = True; sl_guard_bars_left = 3
        elif t['type'] == "BE":
            be_cnt += 1; consec_sl = 0
            
    print("=========================================================")
    print(f"V24 Zero-Loss Simulation Results (Compound={compound}):")
    print(f"Wins: {w_cnt} | Losses: {l_cnt} | BE: {be_cnt} | Total: {w_cnt+l_cnt+be_cnt}")
    wr_wl = (w_cnt / (w_cnt + l_cnt)) * 100 if (w_cnt + l_cnt) > 0 else 0
    print(f"Win Rate (W/L): {wr_wl:.2f}%")
    print(f"Net Profit: ${balance - 10000.0:,.2f}")
    print(f"Master Dates Found: {master_dates}")
    print("=========================================================")
