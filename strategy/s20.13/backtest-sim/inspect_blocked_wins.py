...
def inspect_blocked(days=700, symbol="XAUUSD.iux", compound=1.5):
    ...
    for i in range(100, len(rates) - 10):
        ...
        if sig:
            ...
            if res and res.get("signal") == "WAIT":
                ...
                # let's see if this blocked trade would have been a WIN or LOSS
                ...
                if closed_type == "WIN":
                    blocked_wins.append({"reason": reason, "type": sig, "rsi": current_bar['rsi'], "adx": current_bar['adx'], "di_diff": current_bar['di_diff'], "vol_ratio": current_bar['vol_ratio']})
                elif closed_type == "LOSS":
                    blocked_losses.append({"reason": reason, "type": sig, "rsi": current_bar['rsi'], "adx": current_bar['adx'], "di_diff": current_bar['di_diff'], "vol_ratio": current_bar['vol_ratio']})

    df_bw = pd.DataFrame(blocked_wins)
    df_bl = pd.DataFrame(blocked_losses)
    print("=== TOP BLOCKED WIN REASONS IN v23 ===")
    if len(df_bw) > 0:
        print(df_bw['reason'].value_counts().head(15))
    print("\n=== TOP BLOCKED LOSS REASONS IN v23 ===")
    if len(df_bl) > 0:
        print(df_bl['reason'].value_counts().head(15))
