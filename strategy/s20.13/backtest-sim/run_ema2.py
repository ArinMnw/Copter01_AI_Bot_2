...
ema_calc = '''    cur_range = current_bar['high'] - current_bar['low']
    is_strong_range = cur_range >= (0.8 * current_bar['atr'])
    ema_50 = df['close'].ewm(span=50, adjust=False).mean().iloc[-2]'''
text = text.replace("    cur_range = current_bar['high'] - current_bar['low']\n    is_strong_range = cur_range >= (0.8 * current_bar['atr'])", ema_calc)
...
buy_replace = '''        if current_bar['rsi'] < 35:
        ...
        if current_bar['close'] < ema_50:
            return {"signal": "WAIT", "reason": "Counter Trend EMA50 (BUY)"}'''
...
sell_replace = '''        if current_bar['rsi'] > 60:
            return {"signal": "WAIT", "reason": f"RSI too high ({current_bar['rsi']:.1f})"}
        if current_bar['close'] > ema_50:
            return {"signal": "WAIT", "reason": "Counter Trend EMA50 (SELL)"}'''
