...
def mine_all_pure(days=700, symbol="XAUUSD.iux"):
    ...
    print("=== MINING PURE RULES ACROSS ALL CATEGORIES ===")
    for cat, group in df.groupby('category'):
        w_tot = len(group[group['outcome']=='WIN']); l_tot = len(group[group['outcome']=='LOSS'])
        if w_tot == 0: continue
        print(f"\n[{cat}] (Total: {w_tot} Wins, {l_tot} Losses):")
        
        best_w = 0; best_l = 999; best_rule = "None"
        for adx_th in [0, 15, 20, 25, 30, 40, 50]:
            for vol_th in [0.5, 0.8, 1.0, 1.2, 1.5]:
                for z_th in [-1.0, 0.0, 0.5, 1.0]:
                    sub = group[(group['adx'] >= adx_th if group['sig'].iloc[0]=='BUY' else group['adx'] <= 50) & 
                                (group['vol_ratio'] >= vol_th) & 
                                (group['z_score'] >= z_th)]
                    w = len(sub[sub['outcome']=='WIN']); l = len(sub[sub['outcome']=='LOSS'])
                    if w > best_w and l <= 1:
                        best_w = w; best_l = l; best_rule = f"vol>={vol_th}, z>={z_th}, adx>={adx_th if group['sig'].iloc[0]=='BUY' else 'any'}"
        if best_w > 0:
            print(f"  -> Best Pure Rule: {best_w} Wins | {best_l} Losses ({best_rule})")
        else:
            print("  -> No simple pure rule found with <= 1 loss.")
