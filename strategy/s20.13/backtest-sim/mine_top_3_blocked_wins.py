...
def mine_blocked_categories(days=700, symbol="XAUUSD.iux", compound=1.5):
        ...
        if sig:
            ...
            if res and res.get("signal") == "WAIT":
                ...
                if closed_type in ["WIN", "LOSS"]:
                    records.append({
                        "category": clean_reason, "type": sig, "outcome": closed_type,
    ...
    target_cats = ["BUY Low Volatility Drift Block", "RSI too low", "BUY Trend/Z Block", "RSI too high"]
    ...
    for cat in target_cats:
        ...
        for col in ["rsi", "adx", "di_diff", "vol_ratio", "z_score", "atr_pct", "dist_ema50", "body_pct"]:
            ...
            print(f"  {col:<10} -> WIN mean: {w_mean:6.2f} (med: {w_med:6.2f}) | LOSS mean: {l_mean:6.2f} (med: {l_med:6.2f})")
            
        print("  Top Hours for WINS:", wins['hour'].value_counts().head(3).to_dict() if len(wins)>0 else {})
        print("  Top Hours for LOSSES:", losses['hour'].value_counts().head(3).to_dict() if len(losses)>0 else {})

    mt5.shutdown()

if __name__ == "__main__":
    mine_blocked_categories()
