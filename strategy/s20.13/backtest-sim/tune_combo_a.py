...
def tune_combo(days=700, symbol="XAUUSD.iux"):
    ...
    print("=== TUNING COMBO A & TESTING COMPOUND 2.0 / 2.1 ===")
    
    for c_val in [1.5, 2.0, 2.1]:
        for d_z in [0.8, 1.0]:
            for tz_v in [1.3, 1.5]:
                ...
                for t in candidate_trades:
                    ...
                    if t['v23_pass']:
                        ...
                        if t['sig'] == "BUY" and t['category'] == "BUY Low Volatility Drift Block":
                            if t['di_diff'] >= 4.0 and t['vol_ratio'] >= 1.0 and t['z_score'] >= d_z and t['adx'] >= 15.0:
                                final_signal = "BUY"
                        elif t['sig'] == "BUY" and t['category'] == "BUY Trend/Z Block":
                            if t['di_diff'] >= 0.0 and t['adx'] <= 50.0 and t['vol_ratio'] >= tz_v and t['z_score'] >= 0.0:
                                final_signal = "BUY"
                    ...
                    if t['type'] == "WIN":
                        wins += 1; trades += 1; pnl += ((t['tp'] - t['entry']) * 10 * c_val) if final_signal == "BUY" else ((t['entry'] - t['tp']) * 10 * c_val)
                        if final_signal == "BUY": sl_buy_count = 0
                        else: sl_sell_count = 0
                    elif t['type'] == "LOSS":
                        losses += 1; trades += 1; pnl -= ((t['entry'] - t['sl']) * 10 * c_val) if final_signal == "BUY" else ((t['sl'] - t['entry']) * 10 * c_val)
                ...
                if wr_wl >= 90.0 and pnl >= 190000:
                    print(f"Compound={c_val}, Drift_Z={d_z}, TZ_Vol={tz_v} -> Wins: {wins}, Losses: {losses}, BE: {be} | WR: {wr_wl:.2f}% | Net Profit: ${pnl:,.2f} | Master: {all(master_dates.values())}")
