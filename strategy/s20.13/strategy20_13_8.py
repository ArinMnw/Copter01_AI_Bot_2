...
def strategy_20_13_8(rates, tf="H1"):
    ...
    if pd.isna(current_bar['atr']) or pd.isna(current_bar['rsi']) or pd.isna(current_bar['bb_lower']):
    ...
    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        ...
        if sweep_bottom > current_bar['bb_lower']:
            return {"signal": "WAIT", "reason": "No BB Pierce"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.8 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | BB Pierce | TP {tp:.2f}"
        }
    ...
    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        ...
        # Bollinger Band Pierce
        if sweep_top < current_bar['bb_upper']:
            return {"signal": "WAIT", "reason": "No BB Pierce"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.8 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | BB Pierce | TP {tp:.2f}"
        }
