...
def strategy_20_13_5(rates, tf="H1"):
    ...
    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        # IDEA 5: Max Sweep Depth
        sweep_bottom = min(recent_3['low'].min(), current_bar['low'])
        sweep_depth = local_low - sweep_bottom
        if sweep_depth > (current_bar['atr'] * 1.5):
            return {"signal": "WAIT", "reason": "Sweep Too Deep"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.5 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | Depth Filter | TP {tp:.2f}"
        }
    ...
    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.5 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | Depth Filter | TP {tp:.2f}"
        }
