...
def strategy_20_13_6(rates, tf="H1"):
    ...
    # IDEA 6: Close Quality (Opposite Wick Filter)
    # The engulfing bar should not have a long wick on the opposite side of the trade
    ...
    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        if upper_wick_pct > 0.40:
            return {"signal": "WAIT", "reason": "Rejection from above (Upper Wick too long)"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.6 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | Close Quality | TP {tp:.2f}"
        }
    ...
    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        if lower_wick_pct > 0.40:
            return {"signal": "WAIT", "reason": "Rejection from below (Lower Wick too long)"}
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13.6 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | Close Quality | TP {tp:.2f}"
        }
