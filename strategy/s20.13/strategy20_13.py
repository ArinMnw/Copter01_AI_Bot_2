...
def get_fuel_multiplier(current_tf, target_tf):
    tf_minutes = {
        ...
        "H1": 60, "H4": 240, "H12": 720, "D1": 1320  # 22 hours for gold
    }
    cur_mins = tf_minutes.get(current_tf, 60)
    tgt_mins = tf_minutes.get(target_tf, 720) # Default H12
    # Dynamic calculation based on Volatility Time-Scaling (Square Root of Time)
...
def strategy_20_13(rates, tf="H1"):
    if rates is None or len(rates) < 20:
        return {"signal": "WAIT", "reason": "Not enough data"}
    ...
    # -------------------------------------------------------------
    # Check if a sweep happened in the last 3 bars
    recent_3 = df.iloc[-4:-1]
    sweep_buy = recent_3['low'].min() < local_low
    # Engulfs previous bar high
    ...
    if (sweep_buy and engulf_buy) or instant_sweep_buy:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13 PA Confirmed BUY",
            "reason": f"Sweep {local_low:.2f} | Engulf | TP {tp:.2f}"
        }
    ...
    if (sweep_sell and engulf_sell) or instant_sweep_sell:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": "S20.13 PA Confirmed SELL",
            "reason": f"Sweep {local_high:.2f} | Engulf | TP {tp:.2f}"
        }
