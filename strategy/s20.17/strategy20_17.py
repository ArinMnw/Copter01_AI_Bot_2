...
def evaluate_bar(df, idx, tf="H1"):
    ...
    active_mode = getattr(config, "S20_17_ACTIVE_MODE", 2.6)
    target_tf_buy = getattr(config, "S20_17_TARGET_TF_BUY", "H12")
    target_tf_sell = getattr(config, "S20_17_TARGET_TF_SELL", "D1")
    tpsl_mode = getattr(config, "S20_17_TPSL_MODE", "DYNAMIC")
    ...
    # -------------------------------------------------------------
    # BUY PA CONFIRMATION (3 Reds followed by 1 Green)
    # -------------------------------------------------------------
    red_1 = df.iloc[idx-1]['close'] < df.iloc[idx-1]['open']
    red_2 = df.iloc[idx-2]['close'] < df.iloc[idx-2]['open']
    red_3 = df.iloc[idx-3]['close'] < df.iloc[idx-3]['open']
    green_curr = current_bar['close'] > current_bar['open']
    
    buy_condition = red_1 and red_2 and red_3 and green_curr
    ...
    if buy_condition:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": f"S20.17 YiawDam Reversal BUY (Prob {yd['prob']}%)",
            "order_mode": "limit",
            "reason": f"YiawDam Base {yd['base']:.2f} | TP {tp:.2f}"
        }
        
    # -------------------------------------------------------------
    # SELL PA CONFIRMATION (3 Greens followed by 1 Red)
    # -------------------------------------------------------------
    green_1 = df.iloc[idx-1]['close'] > df.iloc[idx-1]['open']
    green_2 = df.iloc[idx-2]['close'] > df.iloc[idx-2]['open']
    green_3 = df.iloc[idx-3]['close'] > df.iloc[idx-3]['open']
    red_curr = current_bar['close'] < current_bar['open']
    
    sell_condition = green_1 and green_2 and green_3 and red_curr
    ...
    if sell_condition:
        ...
        return {
            ...
            "sl": sl,
            "tp": tp,
            "pattern": f"S20.17 YiawDam Reversal SELL (Prob {yd['prob']}%)",
            "order_mode": "limit",
            "reason": f"YiawDam Base {yd['base']:.2f} | TP {tp:.2f}"
        }
