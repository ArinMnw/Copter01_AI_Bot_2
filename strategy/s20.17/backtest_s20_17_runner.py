...
def run_backtest(days=365, tf_list=["H1"], compound=1.0, symbol="XAUUSD.iux"):
    ...
    print(f"Running S20.17 backtest for {days} days, TFs: {tf_list}, Compound: {compound}")
    ...
    for tf_name in tf_list:
        ...
        for tpsl_mode in ["S20.13.23", "S20.14.23"]:
            config.S20_17_TPSL_MODE = tpsl_mode
            ...
            for i in range(100, len(rates) - 10):
                res = strategy20_17.evaluate_bar(df_master, i, tf=tf_name)
    ...
    if results:
        ...
        csv_path = os.path.join(os.path.dirname(__file__), "S20_17_backtest_summary.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Backtest Runner for S20.17')
