# -*- coding: utf-8 -*-
"""run_s20_249_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.249 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.249 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Sovereign Apex Matrix v213", flush=True)
    print("=" * 115, flush=True)
    ...
    print("=" * 115, flush=True)
    print(" S20.249 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    pd.DataFrame(monthly_rows).to_csv(os.path.join(os.path.dirname(__file__), "S20_249_monthly.csv"), index=False)
    print(f"Monthly breakdown saved to S20_249_monthly.csv", flush=True)
