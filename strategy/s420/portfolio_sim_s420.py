# -*- coding: utf-8 -*-
"""จำลอง S420 แบบ "พอร์ตจริง" — รันทั้ง 5 TF (M1/M5/M15/M30/H1) พร้อมกันบน
ไทม์ไลน์เดียว พร้อมจำลอง MAX_POS_PER_LEG stacking (leg เดียวเปิดซ้อนได้สูงสุด
กี่ไม้ ตรงกับ config.DEMO_PORTFOLIO_MAX_POS_PER_LEG) — backtest_s420.py ปกติ
รันทีละ TF แยกกัน ไม่มีทางเห็นความเสี่ยงที่ทำให้บัญชี 3586 พังไปเมื่อ
2026-08-14 (หลาย TF ยิงทิศเดียวกันพร้อมกัน + leg เดียวซ้อนได้ถึง 3 ไม้) —
ไฟล์นี้สร้างมาเพื่อเห็นความเสี่ยงนั้นโดยเฉพาะ

⚠️ simplification จากของจริง: ใช้ lot คงที่ (ไม่ใช้ balance-based volume แบบ
live), ไม่จำลอง margin/leverage/stop-out จริงของโบรกเกอร์ (แค่รายงาน
"จำนวนไม้เปิดพร้อมกันสูงสุด" + "equity curve/drawdown แบบ lot คงที่" เป็น
ตัวชี้วัดความเสี่ยงเชิงโครงสร้างแทน) — ไม่ได้ใช้ demo_portfolio.py's
_process_pending_limit_order/cooldown logic ตรงๆ แต่ประมาณด้วยกฎเดียวกัน
(pending เดียวต่อครั้งต่อ leg, arm ใหม่ได้เมื่อ open_count < MAX_POS_PER_LEG)

Usage:
    python portfolio_sim_s420.py --days 5
"""
...
from backtest_s420 import (
    BKK, LOOKBACK_BARS, PINE_TARGET1_CFG, TF_MAP,
    bar_time_to_bkk, bkk_to_query, compute_signals_parallel, fmt_bkk, parse_bkk, prepare_rates,
)

import config as _config

MAX_POS_PER_LEG = _config.demo_portfolio_max_pos_per_leg("S420-J")  # 2026-08-14: ตอนนี้ = 1 (แก้ config.py แล้ว)
...
def main():
    ...
    parser.add_argument("--max-pos-per-leg", type=int, default=None,
                         help="override cap เดียวกันทุก TF (เช่น 999 เพื่อจำลอง 'ไม่มี cap') "
                              "ไม่ใส่ = default อ่าน cap ต่อ TF จาก config.demo_portfolio_max_pos_per_leg() "
                              "ตรงๆ ต่อ leg (รองรับ cap แยกตาม TF เช่น M1/M30=1, M5/M15/H1=2)")
    parser.add_argument("--csv", default=os.path.join(_HERE, "s420_portfolio_trades.csv"))
    args = parser.parse_args()

    end_bkk = parse_bkk(args.end)
    start_bkk = parse_bkk(args.start) if args.start else end_bkk - timedelta(days=args.days)
    cfg = {"TP_RATE": 1.618}  # ตรงกับ strategy420.DEFAULT_CFG ที่ live ใช้จริง
    ...
    for tf_name in TF_LIST:
        ...
        try:
            ...
            continue
        leg_id = f"S420-J_{tf_name}"
        leg_cap = args.max_pos_per_leg if args.max_pos_per_leg is not None else _config.demo_portfolio_max_pos_per_leg(leg_id)
        print(f"  cap({tf_name}) = {leg_cap}")
        trades, events = simulate_leg(tf_name, bars, start_index, cfg, args.spread, args.lot,
                                       max_pos_per_leg=leg_cap)
        for t in trades:
            t["leg"] = leg_id
    ...
    cap_desc = f"uniform={args.max_pos_per_leg}" if args.max_pos_per_leg is not None else "per-TF จาก config.py"
    print(f"\n\n===== สรุปพอร์ต S420 ทั้ง 5 TF พร้อมกัน (cap={cap_desc}, "
          f"lot={args.lot} ต่อไม้) =====")
