# -*- coding: utf-8 -*-
"""run_s20_134_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.134 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.134 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Paradigm 4: Balanced Price Range BPR & Breakaway Void Confluence", flush=True)
    print("=" * 115, flush=True)
    ...
    for tf, df_tf in dfs.items():
        for cur in df_tf.to_dict('records'):
            ...
            sig = "BUY" if ((swept_low or ob_mitigated_bull or bpr_tap_bull) and has_wick_buy and closed_high) else                   ("SELL" if ((swept_high or ob_mitigated_bear or bpr_tap_bear) and has_wick_sell and closed_low) else None)
    ...
    res = run_simulation(m5_gold, m5_times, setups, tp_r=11.55, stages=stages)

    print("=" * 115, flush=True)
    print(" S20.134 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    pd.DataFrame(monthly_rows).to_csv(os.path.join(os.path.dirname(__file__), "S20_134_monthly.csv"), index=False)
    print(f"Monthly breakdown saved to S20_134_monthly.csv", flush=True)
