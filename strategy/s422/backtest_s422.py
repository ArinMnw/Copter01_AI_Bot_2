# -*- coding: utf-8 -*-
"""Standalone backtest runner สำหรับ S422 FTSMA Strategy โดยเฉพาะ (ไม่ใช้
sim_strategy_backtest.py ร่วมกับ strategy อื่น) — ต่างจาก backtest_s420.py
ตรงที่ไม่ต้อง recompute state ทุกบาร์ในลูป: strategy422.compute_series()
คำนวณ Fourier-smoothed MA ทั้งช่วงราคาด้วย numpy ในครั้งเดียว (ไม่มี state
สะสมข้ามบาร์แบบ zigzag ของ S420) ทำให้ backtest เร็วกว่ามาก — ลูปหลักแค่เดิน
ผ่านค่าที่คำนวณไว้แล้วเพื่อจำลอง entry/exit (ATR-based SL/TP)

Usage:
    python backtest_s422.py --start 2026-06-01 --end 2026-08-11 --tf M15
    python backtest_s422.py --days 60 --tf M15
    python backtest_s422.py --days 60 --tf all
"""
...
from strategy422 import DEFAULT_CFG, compute_series
...
# จำนวนแท่งย้อนหลังพอสำหรับ warm-up ของ SLOW_MA (default 200) + FT_LOOKBACK
# (default 64) — ให้เผื่อเยอะกว่าค่า default หลายเท่าเพราะ cfg อาจปรับ
# SLOW_MA ให้ยาวขึ้นได้ผ่าน --cfg-json
LOOKBACK_BARS = 2000
...
def prepare_rates(tf_name, start_bkk, end_bkk, lookback_bars):
    ...
    if start_index < lookback_bars:
        print(f"หมายเหตุ: ประวัติก่อน --start มีแค่ {start_index} แท่ง (น้อยกว่า LOOKBACK_BARS={lookback_bars} "
              f"ที่ตั้งใจไว้) — ใช้เท่าที่มีแทน SLOW_MA/FT_LOOKBACK warm-up อาจแม่นน้อยลงช่วงต้น backtest")
    return bars, start_index


def backtest(tf_name, spread, lot, start_bkk, end_bkk, cfg=None, atr_period=14):
    """คำนวณ FT/MA series ทั้งช่วงครั้งเดียว (compute_series) แล้วเดินลูป
    จำลอง entry/exit ด้วย ATR-based SL/TP (regime-flip = สัญญาณเข้าไม้ ดู
    docstring ของ strategy422.py สำหรับรายละเอียดที่ต่างจาก Pine ต้นฉบับ)."""
    detector_cfg = dict(DEFAULT_CFG)
    detector_cfg.update(cfg or {})
    lookback_bars = max(LOOKBACK_BARS,
                         int(detector_cfg["FT_LOOKBACK"]) + int(detector_cfg["SLOW_MA"]) + 50)
    ...
    bars_tail = series["bars_tail"]  # bars_tail[j] สอดคล้อง close/slow/med/fast[j]
    close, slow, med, fast = series["close"], series["slow"], series["med"], series["fast"]
    offset = len(bars_raw) - len(bars_tail)  # bars_raw[offset+j] == bars_tail[j]
    tail_start_index = max(0, start_index - offset)
    ...
    def close_position(j, exit_price, outcome):
        side = position["side"]
    ...
    total_bars = len(bars_tail) - 1 - tail_start_index
    progress_step = max(1, total_bars // 20)
    progress_start = _time.time()

    for j in range(tail_start_index, len(bars_tail) - 1):
        done = j - tail_start_index
        ...
        event = bars_tail[j]
        cur_long = fast[j] > med[j] and close[j] > slow[j]
        cur_short = fast[j] < med[j] and close[j] < slow[j]
        prev_long = fast[j - 1] > med[j - 1] and close[j - 1] > slow[j - 1] if j > 0 else False
        prev_short = fast[j - 1] < med[j - 1] and close[j - 1] < slow[j - 1] if j > 0 else False
        ...
        if position is not None:
            ...
            if side > 0 and (event["high"] >= position["tp"] or event["low"] <= position["sl"]):
                ...
                close_position(j, position["tp"] if hit_tp else position["sl"], "TP" if hit_tp else "SL")
                position = None
        ...
        if not (signal_long or signal_short):
            continue
        atr_end = offset + j + 1
        atr = float(_atr(bars_raw[max(0, atr_end - atr_period - 1):atr_end], atr_period))
        if atr <= 0.0:
            continue
        ...
        if signal_long:
            if position is not None and position["side"] < 0:
                close_position(j, fill_price, "REV")
                position = None
            if position is None:
                signals += 1
                sl, tp = fill_price - risk, fill_price + risk * tp_rr
                position = {
                    "side": 1, "entry": fill_price, "sl": sl, "tp": tp,
                    "entry_time": entry_time, "pattern": "S422 BUY FTSMA regime flip",
                    "reason": f"fast={fast[j]:.2f} med={med[j]:.2f} slow={slow[j]:.2f} ATR={atr:.2f}",
                }
        elif signal_short:
            if position is not None and position["side"] > 0:
                close_position(j, fill_price, "REV")
            ...
            if position is None:
                ...
                position = {
                    "side": -1, "entry": fill_price, "sl": sl, "tp": tp,
                    "entry_time": entry_time, "pattern": "S422 SELL FTSMA regime flip",
                    "reason": f"fast={fast[j]:.2f} med={med[j]:.2f} slow={slow[j]:.2f} ATR={atr:.2f}",
                }
...
def main():
    ...
    parser.add_argument("--cfg-json", default="{}",
                         help="JSON object ส่งเข้า cfg ของ detect_s422 (เช่น SLOW_MA/MA_TYPE/SL_ATR_MULT/TP_RR)")
    parser.add_argument("--csv", default=os.path.join(_HERE, "s422_trades.csv"),
                         help="path ไฟล์ CSV ผลลัพธ์ (default = s422_trades.csv ในโฟลเดอร์นี้ ไม่ใช่ CWD)")
    ...
    parser.add_argument("--compare-comment-prefix", default="S422",
                         help="กรอง deal ที่มี comment ขึ้นต้น/มีคำนี้เท่านั้น (default S422)")
