# -*- coding: utf-8 -*-
"""strategy20_43.py
Strategy S20.43: Omni-Horizon Sovereign Apex Matrix + M20/M12 Dual Cycle & Deca-Stage Ratchet Trailing
Core Innovations:
1. Multi-Strategy Single-Candle Confluence (Working together on the EXACT SAME BAR):
   - SMC Macro Liquidity Sweep: Asian Range H/L + PDH/PDL + PWH/PWL + PMH/PML + PQH/PQL + PYH/PYL (52-Week Yearly H/L) + Swing 12
...
2. Omni-Horizon Timeframe Matrix: H4 + H3 + H2 + H1 + M30 + M20 + M15 + M12 (8 institutional timeframes)
   - M12 (12-Minute Harmonic Bars): Divides each hour into 5 auction segments (:00, :12, :24, :36, :48) capturing institutional sweep rejections before M15 closes.
3. Sovereign Session Ignition: 01:00 to 22:00 UTC (08:00 to 05:00 BKK - capturing Tokyo Open ignition sweep through NY close before rollover)
4. Precision Retest Limit Entry: 12.2% into rejection wick with 0.20 ATR SL buffer
5. Deca-Stage Quantum Ratchet Lock Engine:
...
def extract_setups(df, tf_label, min_vol=1.20, min_wick=0.38, retest_depth=0.122, sl_mult=0.20, hours=(1, 22), use_pyh=True):
    setups = []
    h_start, h_end = hours
