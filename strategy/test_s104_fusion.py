# -*- coding: utf-8 -*-
"""test_s104_fusion.py
Testing Fusion of Paradigms (CVD + Judas + Volatility + Dynamic Horizon)
to beat S20.103 (+$31,907.57) and reach towards S20.130
"""
...
def main():
    ...
    target_pnl = 31907.57  # S20.103 benchmark

    # Test Matrix:
    # Synergistic Combination:
    # 1. Volatility Regime Expansion + Judas Alignment
    # 2. Dual-Depth: In Judas window or Vol Expansion -> Retest depth 0.120 / 0.125
    # 3. CVD Absorption Confirmation for Non-Judas bars
    print("\n--- Exploring S20.104+ Candidates (Target: > $31,907.57) ---")

    for vol_gate in [False, True]:
        for judas_bonus in [True, False]:
            for cvd_mode in ['none', 'soft_gate']:
                for wick in [0.10, 0.11, 0.12, 0.14]:
                    for depth in [0.124, 0.125]:
                        for tp in [8.75, 8.85, 8.95]:
                            setups = []
                            for tf, df_tf in dfs.items():
                                ...
                                for idx in range(45, len(recs)):
                                    ...
                                    if cvd_mode == 'soft_gate':
                                        # Only require CVD confirmation if not in Judas window
                                    ...
                                    if sig:
                                        # Dynamic SL multiplier based on Volatility Regime
                                        sl_mult = 0.19 if (vol_gate and is_expanding) else 0.20
                                        sl_buf = max(sl_mult * cur['atr'], 0.22)
                                        active_depth = 0.120 if (judas_bonus and is_judas) else depth
                            ...
                            if res['pnl'] > target_pnl:
                                print(f"MATCH: vol_gate={vol_gate} judas={judas_bonus} cvd={cvd_mode} wick={wick} depth={depth} TP={tp} -> PnL: ${res['pnl']:,.2f} Trades: {res['trades']} WR: {res['wr']:.1f}% DD: ${res['max_dd']:.2f}")
