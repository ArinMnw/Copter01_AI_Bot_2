# -*- coding: utf-8 -*-
"""run_s20_39_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.39 on Gold (XAUUSD.iux)
...
5. Target: Verifiably beat S20.38 (+$10,483.46) with record-shattering performance!
"""
...
from strategy20_39 import compute_indicators, extract_setups
...
def main():
    ...
    print(" S20.39 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    ...
    # Simulation parameters (Septa-Stage Ratchet Trailing)
    be_trigger = 0.8
    lock1_trig = 1.5
    lock1_amt = 1.0
    lock2_trig = 2.4
    ...
    for s in all_setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if not l6_active and b['high'] >= l6_target:
                    l6_active = True
                    current_sl = l6_sl
                ...
                if not l6_active and b['low'] <= l6_target:
                    l6_active = True
                    current_sl = l6_sl
    ...
    print("\n" + "=" * 115, flush=True)
    print(" 🏆 FINAL VERIFIED PERFORMANCE — STRATEGY S20.39 (STRICT LOT 0.01)", flush=True)
    print("=" * 115, flush=True)
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "S20_39_monthly.csv")
