# -*- coding: utf-8 -*-
"""Standalone backtest runner สำหรับ S429 UT Bot Alerts Strategy — fill ที่
close ของแท่งสัญญาณเอง (ตรงกับ process_orders_on_close=true ของต้นฉบับ Pine
ต่างจาก S420-428 ที่ fill ที่ open ของแท่งถัดไป) รองรับ SL/TP/breakeven-trail
เต็มรูปแบบ แต่ตัดทอน partial-take-profit ออก (ปิดเต็มไม้ที่ TP) และเปลี่ยน
"ปิดไม้ก่อนกำหนดถ้าสัญญาณตรงข้ามระหว่างกำไร" ของต้นฉบับ เป็น reversal เต็มรูป
ทันทีที่มีสัญญาณตรงข้าม (ตรงกับ convention reversal ของ S424-427 ในโปรเจกต์
นี้ และ Pine เองก็ netting account อยู่แล้วซึ่งให้ผลใกล้เคียงกัน) — ดู
docstring บนสุดของ strategy429.py สำหรับรายละเอียด/deviation ทั้งหมด

Usage:
    python backtest_s429.py --start 2026-06-01 --end 2026-08-13 --tf M15
    python backtest_s429.py --days 60 --tf all --tv-mode
"""
...
from strategy429 import DEFAULT_CFG, _atr_series, _ema_series, _ut_bot_series
...
def backtest(tf_name, spread, lot, start_bkk, end_bkk, cfg=None,
             risk_pct=None, commission_pct=None, starting_balance=100000.0):
    """risk_pct=None -> โหมดเดิม (fixed lot + flat spread, ตรง convention
    S420-S428 ของโปรเจกต์). ถ้าใส่ค่า -> โหมด "TV mode" (risk-based sizing:
    qty_oz = equity*risk%/100 / stop_distance_price, ตรงเจตนาของสูตร money
    management ในต้นฉบับ Pine — risk=2.5% default ตาม Pine, commission 0.03%
    ตาม commission_value ที่ระบุไว้ในต้นฉบับ)"""
    detector_cfg = dict(DEFAULT_CFG)
    detector_cfg.update(cfg or {})
    key_value = float(detector_cfg["KEY_VALUE"])
    ema_len = int(detector_cfg["EMA_TREND_LEN"])
    atr_period = int(detector_cfg["ATR_SL_PERIOD"])
    sl_mult = float(detector_cfg["ATR_SL_MULT"])
    rr_be = float(detector_cfg["RR_BE"])
    rr_tp = float(detector_cfg["RR_TP"])
    allow_buy = bool(detector_cfg["ALLOW_BUY"])
    allow_sell = bool(detector_cfg["ALLOW_SELL"])
    tv_mode = risk_pct is not None
    commission_pct = commission_pct or 0.0

    bars_raw, start_index = prepare_rates(tf_name, start_bkk, end_bkk)
    bars_norm = _bars(bars_raw)
    closes = [b["close"] for b in bars_norm]
    ema200 = _ema_series(closes, ema_len)
    atr_sl = _atr_series(bars_norm, atr_period)
    _, above, below = _ut_bot_series(bars_norm, key_value)
    ...
    total_bars = len(bars_norm) - start_index
    ...
    def close_position(exit_index, exit_price, outcome):
        ...
        if tv_mode:
            qty = position["qty"]
            gross = side * (exit_price - position["entry"]) * qty
            commission = (position["entry"] * qty + exit_price * qty) * (commission_pct / 100.0)
        ...
        trades.append({
            ...
            "exit_time": fmt_bkk(bar_time_to_bkk(bars_norm[exit_index]["time"])),
    ...
    def open_position(index, side_wanted, fill_price):
        nonlocal signals, position
        signals += 1
        signal_name = "BUY" if side_wanted > 0 else "SELL"
        distance = atr_sl[index] * sl_mult
        sl = fill_price - distance if side_wanted > 0 else fill_price + distance
        tp = fill_price + distance * rr_tp if side_wanted > 0 else fill_price - distance * rr_tp
        be_level = fill_price + distance * rr_be if side_wanted > 0 else fill_price - distance * rr_be
        qty = (equity * risk_pct / 100.0 / distance) if tv_mode else None
        position = {
            "side": side_wanted, "entry": fill_price, "sl": sl, "tp": tp, "qty": qty,
            "be_level": be_level, "be_hit": False,
            "entry_time": bar_time_to_bkk(bars_norm[index]["time"]),
            "pattern": f"S429 {signal_name} UT Bot Alerts",
            "reason": f"UT Bot cross + EMA{ema_len} trend filter",
        }

    for index in range(start_index, len(bars_norm)):
        ...
        if ema200[index] is None or atr_sl[index] is None:
            continue
        bar = bars_norm[index]

        if position is not None:
            side = position["side"]
            if side > 0 and bar["low"] <= position["sl"]:
                close_position(index, position["sl"], "SL")
                position = None
            elif side > 0 and bar["high"] >= position["tp"]:
                close_position(index, position["tp"], "TP")
                position = None
            elif side < 0 and bar["high"] >= position["sl"]:
                close_position(index, position["sl"], "SL")
                position = None
            elif side < 0 and bar["low"] <= position["tp"]:
                close_position(index, position["tp"], "TP")
                position = None
            elif not position["be_hit"]:
                if side > 0 and bar["high"] >= position["be_level"]:
                    position["sl"] = position["entry"]
                    position["be_hit"] = True
                elif side < 0 and bar["low"] <= position["be_level"]:
                    position["sl"] = position["entry"]
                    position["be_hit"] = True

        bullish = closes[index] > ema200[index]
        bearish = closes[index] < ema200[index]
        buy_trigger = above[index] and bullish and allow_buy
        sell_trigger = below[index] and bearish and allow_sell
        if not buy_trigger and not sell_trigger:
            continue

        side_wanted = 1 if buy_trigger else -1
        ...
        if position is not None:
            ...
            position = None
        open_position(index, side_wanted, fill_price)
...
def main():
    ...
    parser.add_argument("--cfg-json", default="{}",
                         help='JSON object ส่งเข้า cfg (เช่น {"KEY_VALUE":2,"ATR_SL_MULT":2.0})')
    parser.add_argument("--tv-mode", action="store_true",
                         help="เทียบ exact-intent กับ TradingView: risk-based sizing ตาม Pine defaults")
    parser.add_argument("--risk-pct", type=float, default=2.5,
                         help="%% ของ equity เสี่ยงต่อไม้ (เฉพาะ --tv-mode, default 2.5 ตาม Pine risk_c)")
    parser.add_argument("--commission-pct", type=float, default=0.03,
                         help="%% commission ต่อ order (เฉพาะ --tv-mode, default 0.03 ตาม Pine commission_value)")
    parser.add_argument("--tv-balance", type=float, default=100000.0,
                         help="ยอดเงินตั้งต้น (เฉพาะ --tv-mode, default 100000 ตาม initial_capital ของ Pine)")
    parser.add_argument("--csv", default=os.path.join(_HERE, "s429_trades.csv"),
                         help="path ไฟล์ CSV ผลลัพธ์ (default = s429_trades.csv ในโฟลเดอร์นี้)")
    ...
    parser.add_argument("--compare-comment-prefix", default="S429",
                         help="กรอง deal ที่มี comment ขึ้นต้น/มีคำนี้เท่านั้น (default S429)")
    ...
    risk_pct = args.risk_pct if args.tv_mode else None
    if args.tv_mode:
        print(f"TV mode เปิด: risk_pct={args.risk_pct:g}% commission={args.commission_pct:g}% "
              f"balance เริ่มต้น={args.tv_balance:g}")
    ...
    for tf_name in tf_list:
        ...
        try:
            summary, trades = backtest(tf_name, args.spread, lot, start_bkk, end_bkk, detector_cfg,
                                        risk_pct=risk_pct, commission_pct=args.commission_pct,
                                        starting_balance=args.tv_balance)
        ...
        print("-" * 40)
        if summary["tv_mode"]:
            print(f"P&L        : {summary['net_profit']:.2f} USD ({summary['net_profit_pct']:.2f}%)")
        else:
