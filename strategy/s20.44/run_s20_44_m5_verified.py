# -*- coding: utf-8 -*-
"""run_s20_44_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.44 on Gold (XAUUSD.iux)
...
5. Target: Verifiably beat S20.43 (+$18,834.66) and achieve over $19,000+ on strict 0.01 lot!
"""
...
from strategy20_44 import compute_indicators, extract_setups
...
def main():
    ...
    print(" S20.44 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    ...
    print("Extracting omni-horizon confluences (H4, H3, H2, H1, M30, M20, M15, M12 + London Sweeps)...", flush=True)
    retest_depth = 0.121
    s0 = extract_setups(df_h4, "H4", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    s_h3 = extract_setups(df_h3, "H3", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    s_h2 = extract_setups(df_h2, "H2", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    s1 = extract_setups(df_h1, "H1", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    s2 = extract_setups(df_m30, "M30", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    s_m20 = extract_setups(df_m20, "M20", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    s3 = extract_setups(df_m15, "M15", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    s_m12 = extract_setups(df_m12, "M12", min_vol=1.20, retest_depth=retest_depth, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True)
    ...
    print("\n" + "=" * 115, flush=True)
    print(" 🏆 FINAL VERIFIED PERFORMANCE — STRATEGY S20.44 (STRICT LOT 0.01)", flush=True)
    print("=" * 115, flush=True)
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "S20_44_monthly.csv")
