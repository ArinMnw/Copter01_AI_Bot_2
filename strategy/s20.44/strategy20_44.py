# -*- coding: utf-8 -*-
"""strategy20_44.py
Strategy S20.44: Omni-Horizon Sovereign Apex Matrix + London Liquidity Pool Sweep & Deca-Stage Ratchet Trailing
Core Innovations:
1. Multi-Strategy Single-Candle Confluence (Working together on the EXACT SAME BAR):
   - SMC Macro Liquidity Sweep: Asian Range H/L + London Session H/L (PLH/PLL for NY session) + PDH/PDL + PWH/PWL + PMH/PML + PQH/PQL + PYH/PYL (52-Week Yearly H/L) + Swing 12
...
3. London Session (08:00 - 13:00 UTC) Liquidity Pool Integration:
   - When New York session opens (>= 13:00 UTC), institutional algorithms sweep London High or London Low to absorb stop orders before embarking on the true New York trend.
4. Sovereign Session Ignition: 01:00 to 22:00 UTC (08:00 to 05:00 BKK - Tokyo Open through NY close before rollover)
5. Precision Retest Limit Entry: 12.1% into rejection wick with 0.20 ATR SL buffer
6. Deca-Stage Quantum Ratchet Lock Engine:
...
def extract_setups(df, tf_label, min_vol=1.20, min_wick=0.38, retest_depth=0.121, sl_mult=0.20, hours=(1, 22), use_pyh=True, use_ldn=True):
    setups = []
    h_start, h_end = hours
