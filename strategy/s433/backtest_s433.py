# -*- coding: utf-8 -*-
"""Standalone backtest runner สำหรับ S433 AI Gold Star 123 (Coppock Curve)
โดยเฉพาะ — โครง prepare_rates()/CLI เหมือน backtest_s432.py เพิ่มสรุป PnL
รายเดือนแยกตาม TF

Usage:
    python backtest_s433.py --days 365 --tf all
    python backtest_s433.py --days 365 --tf M15
"""
...
import config
from strategy433 import DEFAULT_CFG, cfg_for_tf, run_backtest
...
def main():
    ...
    parser.add_argument("--csv", default=os.path.join(_HERE, "s433_trades.csv"))
    ...
    for tf_name in tf_list:
        ...
        print(f"Entries    : {summary['entries']} (flips={summary['flips']})")
        print("-" * 40)
        print("รายเดือน:")
