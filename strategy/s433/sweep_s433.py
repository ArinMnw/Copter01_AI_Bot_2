# -*- coding: utf-8 -*-
"""Sweep SL/TP multiplier (ATR_MULT, TP3_R) สำหรับ S433 บน H4/H12/D1 (365 วัน)
ดึงข้อมูลราคาครั้งเดียวต่อ TF แล้ววน cfg เพื่อประหยัดเวลา MT5 fetch

Usage:
    python sweep_s433.py
"""
...
from backtest_s433 import (DEFAULT_DAYS, parse_bkk, prepare_rates, _lookback_bars,
                            fmt_bkk, bar_time_to_bkk)
from strategy433 import DEFAULT_CFG, run_backtest

TF_LIST = tuple(sys.argv[1:]) or ("H4", "H12", "D1")
ATR_MULTS = (1.0, 1.5, 2.0, 2.5, 3.0)
TP3_RS = (1.5, 2.0, 2.5, 3.0, 4.0, 5.0)
    ...
    for tf_name in TF_LIST:
        ...
        for atr_mult in ATR_MULTS:
            for tp3_r in TP3_RS:
                ...
                summary.update({"atr_mult": atr_mult, "tp3_r": tp3_r,
                                 "entries": extra.get("entries"), "flips": extra.get("flips")})
        ...
        print(f"  Top 10 by PF (ATR_MULT default=1.5, TP3_R default=3.0):")
        print(f"  {'ATR_MULT':>9}{'TP3_R':>7}{'Net':>12}{'PF':>8}{'WinRate':>9}{'Closed':>8}{'MaxDD':>10}")
        for s in tf_results[:10]:
            ...
            print(f"  {s['atr_mult']:>9}{s['tp3_r']:>7}{s['net_profit']:>12.2f}{pf_str:>8}{wr:>9}{s['closed']:>8}{s['max_drawdown']:>10.2f}")

    out_name = f"sweep_s433_result_{'_'.join(TF_LIST)}.json"
