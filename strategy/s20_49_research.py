# -*- coding: utf-8 -*-
"""s20_49_research.py
Explore Multi-Strategy Confluence Expansion to beat S20.48 (+$25,944.61):
Candidate Integrations:
1. Multi-Bar Fair Value Gap (FVG) Memory (lookback 5 bars for unmitigated FVG)
2. Candle Range Theory (CRT) True Day Open Bias (S10)
3. Dynamic RSI Structural Exhaustion / Divergence (S9)
4. Institutional Displacement / Engulfing Expansion (S6)
5. Trideca-Stage Ratchet Trailing (TP 6.40R - 6.60R)
...
def compute_indicators(rates, use_london=True, use_ny=True):
    ...
    # Bullish FVG: Low of candle [i-1] > High of candle [i-3] or [i-2] > [i-4]
...
def extract_setups(df, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, sl_mult=0.20, hours=(0, 22),
                   use_pyh=True, use_ldn=True, use_ny=True, use_fvg=True, use_day_open=False):
    setups = []
    h_start, h_end = hours
    ...
    for idx in range(45, len(records)):
        ...
        # Strategy 10 True Day Open Alignment (Optional)
        if use_day_open:
            d_open = cur.get('day_open', np.nan)
            if not pd.isna(d_open):
                if cur['close'] > d_open: # Above day open -> prefer SELL sweeps
                    swept_low = False
                elif cur['close'] < d_open: # Below day open -> prefer BUY sweeps
                    swept_high = False
...
def run_simulation(setups, m5_gold, m5_times, m5_len,
                   ...
                   lock12_trig=6.0, lock12_amt=5.8, lock13_trig=6.2, lock13_amt=6.0, tp_r=6.45):
    point_val = 1.0
    busy_until = 0
    trades = 0
    wins = 0
    ...
    for s in setups:
        ...
        for k in range(fill_m5 + 1, min(fill_m5 + 288, m5_len)):
            ...
            if sig == "BUY":
                ...
                if lock13_trig and b['high'] >= l13_target:
                    l13_active = True
                ...
                if lock13_trig and b['low'] <= l13_target:
                    l13_active = True
...
def main():
    ...
    print(" S20.49 MULTI-STRATEGY CONFLUENCE RESEARCH (TARGET > $25,944.61 ON STRICT LOT 0.01)", flush=True)
    print(" Benchmark S20.48: +$25,944.61 | MaxDD $12.96 | WR 87.5% | Trades 3,079", flush=True)
    print("=" * 115, flush=True)

    # Test 1: Multi-Bar FVG Memory vs Single FVG
    print("\n--- 1. Testing Multi-Bar FVG Memory ---", flush=True)
    s_list_fvg = []
    for tf_label, df_tf in base_tfs:
        s_list_fvg.extend(extract_setups(df_tf, tf_label, min_vol=1.17, min_wick=0.34, retest_depth=0.122, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True, use_fvg=True, use_day_open=False))
    comb_fvg = sorted(s_list_fvg, key=lambda x: x['time'])
    print(f"Total setups detected with Multi-Bar FVG: {len(comb_fvg)}", flush=True)

    # Test 2: Trideca-Stage Trailing Lock (+6.0R -> +6.2R -> TP 6.40R - 6.60R)
    print("\n--- 2. Testing Trideca-Stage Trailing on Multi-Bar FVG ---", flush=True)
    for tp in [6.35, 6.40, 6.45, 6.50, 6.55]:
        for l13_t, l13_a in [(None, None), (6.2, 6.0)]:
            res = run_simulation(comb_fvg, m5_gold, m5_times, m5_len,
                                 lock13_trig=l13_t, lock13_amt=l13_a, tp_r=tp)
            l13_s = f"L13={l13_t}->{l13_a}" if l13_t else "No L13"
            print(f"TP={tp:.2f}R | {l13_s:<15} | Trades: {res['trades']} | W/BE/L: {res['wins']}/{res['bes']}/{res['losses']} | WR: {res['wr']:.1f}% | Net: ${res['pnl']:.2f} | DD: ${res['max_dd']:.2f} | Mos: {res['pos_m']}/{res['tot_m']}", flush=True)
