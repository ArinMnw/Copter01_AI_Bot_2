# -*- coding: utf-8 -*-
"""run_s20_192_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.192 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.192 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Multi-Timeframe Equilibrium Re-balancing Protocol", flush=True)
    print("=" * 115, flush=True)
    ...
    res = run_simulation(m5_gold, m5_times, setups, tp_r=13.66, stages=stages)

    print("=" * 115, flush=True)
    print(" S20.192 VERIFIED PERFORMANCE SUMMARY (STRICT 0.01 LOT)", flush=True)
    print("=" * 115, flush=True)
    ...
    pd.DataFrame(monthly_rows).to_csv(os.path.join(os.path.dirname(__file__), "S20_192_monthly.csv"), index=False)
    print(f"Monthly breakdown saved to S20_192_monthly.csv", flush=True)
