...
# Exact setup extraction that produced S20.127:
# hrs=(0,23), vsa=1.12, wick=0.14, depth=0.125, vol_g=True, judas_b=True, cvd_m='none'
...
for tf, df_tf in dfs.items():
    for cur in df_tf.to_dict('records'):
        if pd.isna(cur['atr']) or cur['atr'] <= 0.05 or cur['range'] < 0.45 * cur['atr'] or cur['vol_ratio'] < 1.12:
            continue
        ...
        if sig:
            ...
            active_depth = 0.120 if is_judas else 0.125
...
for stg, label in [(stg34, "stg34"), (stg35, "stg35"), (stg36, "stg36"), (stg37, "stg37")]:
    for tp in [11.25, 11.30, 11.35, 11.40, 11.45, 11.50, 11.55, 11.60]:
        res = run_simulation(m5_gold, m5_times, setups, tp_r=tp, stages=stg)
        print(f"[{label}] TP={tp} -> PnL: ${res['pnl']:,.2f} Trades: {res['trades']} WR: {res['wr']:.1f}% DD: ${res['max_dd']:.2f}")
