# -*- coding: utf-8 -*-
"""run_s20_141_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.141 on Gold (XAUUSD.iux)
"""
...
def main():
    ...
    print(" S20.141 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print(f" Paradigm: Paradigm 11: Multi-Session Overlap Momentum & Dynamic Volatility Buffer", flush=True)
    print("=" * 115, flush=True)
    ...
    for tf, df_tf in dfs.items():
        for cur in df_tf.to_dict('records'):
            ...
            sig = "BUY" if ((swept_low or ob_mitigated_bull or bpr_tap_bull or is_abs_buy or ifvg_buy) and has_wick_buy and closed_high) else                   ("SELL" if ((swept_high or ob_mitigated_bear or bpr_tap_bear or is_abs_sell or ifvg_sell) and has_wick_sell and closed_low) else None)

            if sig:
                is_hc = (ob_mitigated_bull if sig == "BUY" else ob_mitigated_bear) or                         (bpr_tap_bull if sig == "BUY" else bpr_tap_bear) or                         (is_abs_buy if sig == "BUY" else is_abs_sell) or                         (ifvg_buy if sig == "BUY" else ifvg_sell)
    ...
    pd.DataFrame(monthly_rows).to_csv(os.path.join(os.path.dirname(__file__), "S20_141_monthly.csv"), index=False)
    print(f"Monthly breakdown saved to S20_141_monthly.csv", flush=True)
