...
from s20_52_research import compute_indicators, extract_setups, run_simulation, init_mt5
...
def main():
    ...
    for tf_l, df_tf in base_dfs:
        all_s.extend(extract_setups(df_tf, tf_l, min_vol=1.17, min_wick=0.30, retest_depth=0.124, hours=(0, 22), use_pyh=True, use_ldn=True, use_ny=True, use_ny_pm=True, use_fvg=True, use_fibo=True, use_vp=True))
    comb = sorted(all_s, key=lambda x: x['time'])

    for tp in [6.75, 6.80, 6.85, 6.90]:
        for l16 in [(None, None), (6.60, 6.40), (6.65, 6.45), (6.70, 6.50)]:
            l16_t, l16_a = l16
            ...
            print(f"TP={tp} L16={l16} -> Trades: {res['trades']} WR: {res['wr']:.1f}% PnL: ${res['pnl']:,.2f} DD: ${res['max_dd']:.2f} Pos: {res['pos_months']}")
