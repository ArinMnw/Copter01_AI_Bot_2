# -*- coding: utf-8 -*-
"""run_s20_40_m5_verified.py
Official 100% Verifiable Backtest Runner for Strategy S20.40 on Gold (XAUUSD.iux)
...
5. Target: Verifiably beat S20.39 (+$10,930.75) and shatter the $11,700+ milestone!
"""
...
from strategy20_40 import compute_indicators, extract_setups
...
def main():
    ...
    print(" S20.40 VERIFIABLE M5 SL-FIRST SIMULATION RUNNER (STRICT LOT 0.01)", flush=True)
    print(f" Asset: {symbol_gold} | Period: 365 Days | Execution: Pessimistic Intrabar Sequential M5", flush=True)
    print("=" * 115, flush=True)

    print("Fetching multi-timeframe rates from MT5 (H4, H2, H1, M30, M15)...", flush=True)
    ...
    print("Extracting omni-horizon confluences (H4, H2, H1, M30, M15)...", flush=True)
    ...
    all_setups = sorted(s0 + s_h2 + s1 + s2 + s3, key=lambda x: x['time'])
    ...
    print("\n" + "=" * 115, flush=True)
    print(" 🏆 FINAL VERIFIED PERFORMANCE — STRATEGY S20.40 (STRICT LOT 0.01)", flush=True)
    print("=" * 115, flush=True)
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "S20_40_monthly.csv")
