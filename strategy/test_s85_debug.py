...
def main():
    ...
    stages_19 = list(stages_18) + [(7.15, 6.95)]
    stages_20 = list(stages_19) + [(7.30, 7.10)]

    target = 31573.16
    print(f"Target to beat for S20.85: ${target:,.2f}")

    for h_end in [22, 23]:
        for depth in [0.124, 0.125, 0.126]:
            for vsa in [1.17, 1.16, 1.15]:
                for wick in [0.15, 0.14, 0.12, 0.10, 0.20]:
                    all_s = []
                    for tf_l, df_tf in base_dfs:
                        all_s.extend(extract_setups(df_tf, tf_l, min_vol=vsa, min_wick=wick, retest_depth=depth, sl_mult=0.20, hours=(0, h_end), use_pyh=True, use_ldn=True, use_ny=True, use_ny_pm=True, use_fvg=True, use_fibo=True, use_vp=True))
                    comb = sorted(all_s, key=lambda x: x['time'])

                    for stg in [stages_18, stages_19, stages_20]:
                        for tp in [7.10, 7.15, 7.20, 7.25, 7.30, 7.40, 7.50]:
                            if tp <= stg[-1][0]: continue
                            res = run_simulation(m5_gold, m5_times, comb, tp_r=tp, stages=stg)
                            if res['pnl'] > target and res['wr'] >= 83.0 and res['max_dd'] <= 25.0:
                                print(f"MATCH: hours=(0,{h_end}) wick={wick} depth={depth} vsa={vsa} stg={len(stg)} TP={tp} -> PnL: ${res['pnl']:,.2f} Trades: {res['trades']} WR: {res['wr']:.1f}% DD: ${res['max_dd']:.2f}")
