# -*- coding: utf-8 -*-
"""backtest_s20_26_runner.py — 365-Day Backtest & Confluence Analysis for S20.26 Apex Matrix:
Evaluates whether combining:
1. Baseline (Sweep + Volume + VWAP)
2. SMT Intermarket Divergence (Gold vs Silver)
3. Session Liquidity Pools (Asian H/L + PDH/PDL)
4. All Confluence Quad-Engine
strictly enhances win rate, accuracy, and profit factor!
"""
...
def simulate_s20_26(gold_rates, df, mode="all_confluence", total_lot=0.02, tf_name="M30"):
    contract_size = 100.0
    ...
    while i < n - 5:
        res = strategy20_26.evaluate_s20_26_bar(df, i, tf=tf_name, mode=mode)
        if not res:
            i += 1
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Run S20.26 Confluence Backtest")
    ...
    print(f"\n================================================================================")
    print(f" S20.26 APEX CONFLUENCE MATRIX — 365-DAY INTERMARKET SYNERGY ANALYSIS")
    ...
    print("Computing indicators & intermarket alignments...")
    df = strategy20_26.compute_indicators_df(gold_rates, silver_rates)

    modes = [
        ("baseline", "S20.26 Mode 1: Sweep + Volume Climax + VWAP"),
        ("smt_only", "S20.26 Mode 2: Sweep + Intermarket SMT Divergence"),
        ("session_pool", "S20.26 Mode 3: Session Pool (Asia/PDH/PDL) Sweep"),
        ("all_confluence", "S20.26 Mode 4: Quad-Engine Apex Confluence")
    ]
    ...
    for m_key, m_desc in modes:
        res = simulate_s20_26(gold_rates, df, mode=m_key, total_lot=0.02, tf_name=args.tf)
        results.append(res)
        print(f"\n>>> {m_desc}")
    ...
    out_csv = os.path.join(current_dir, "S20_26_matrix_comparison.csv")
