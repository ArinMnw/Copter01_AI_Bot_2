...
if args.disp_atr is not None:
    cfg_s104["DISP_BODY_ATR"] = args.disp_atr
if args.disp_atr_stacked is not None:
    cfg_s104["DISP_BODY_ATR_STACKED"] = args.disp_atr_stacked
if args.tp_rr is not None:
    cfg_s104["TP_RR"] = args.tp_rr
if args.retrace is not None:
    cfg_s104["ENTRY_RETRACE"] = args.retrace
if args.no_trail:
    cfg_s104["TRAIL_ENABLED"] = False
if args.trail_atr_mult is not None:
    cfg_s104["TRAIL_ATR_MULT"] = args.trail_atr_mult
if args.trail_be_rr is not None:
    cfg_s104["TRAIL_BE_RR"] = args.trail_be_rr
if args.tp_rr_max is not None:
    cfg_s104["TP_RR_MAX"] = args.tp_rr_max
if args.no_pdh:
    cfg_s104["USE_PDH_PDL"] = False
if args.no_eq:
    cfg_s104["USE_EQ_CLUSTERS"] = False
if args.no_time_filter:
    cfg_s104["TIME_FILTER_ENABLED"] = False
...
for i in range(LOOKBACK, len(all_bars) - 2):
    ...
    sig = strategy104.detect_s104(rates_slice, tf=TF, dt_bkk=dt_bkk, cfg=cfg_s104)
