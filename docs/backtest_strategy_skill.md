...
- Near-match pattern diagnostics show the largest backtest-only cluster is `NOENGULF` M1 (`ENTRY_TOO_FAR` and `NEAREST_ALREADY_MATCHED_OR_GREEDY`), while live-only rows are mostly live comment codes `G/R`; next pass should compare runtime `check_s3_maru_pending()` guards (`last_traded_per_tf`, adjacent active order, sweep/trend/PD decision order) against replay pending placement.
