...
- Backtest ต้องใช้ log แยกเท่านั้น: `logs/backtest_bot.log`, `logs/backtest_system.log`, `logs/backtest_error.log`; ห้ามอ่าน/เขียน `logs/bot.log`, `logs/system/system.log`, `logs/error.log` หรือ archive live logs ใน default backtest path
...
- [x] แยก backtest logs ออกจาก live logs: default `backtest_auto_trade.py` ใช้ `backtest_bot.log`, สร้าง `backtest_bot/system/error.log` ถ้ายังไม่มี และ reject live log names ผ่าน `--log-files`
...
- S3: all-TF compare โหลด MT5 history ได้ปกติแล้ว (`deals=10014`, `orders=12669`, live S3 rows `264`) และยืนยันว่า M15-only `BT_AFTER_LAST_FILTERED_LIVE_FILL` เป็น filter-specific symptom. Step ถัดไปคือไล่ `LIVE_ONLY` ที่ `nearest_raw_replay_sid=3` ก่อน: ล่าสุดมี 38 rows โดย 28 rows เป็น `nearest_raw_replay_close_type=PD_FAIL` / `PD Fibo Plus round1 fail`; `PD_FAIL` ใน replay หมายถึง pending/pre-create cancel ก่อน fill ไม่ใช่ fill timing; ให้แยก current-config PD reject ออกจาก entry/adjacent bug ก่อน.
- S3 report bucket cleanup done: `CLOSE_LIFECYCLE_PD` เดิมปน live-PD กับ BT-PD และ `PNL_DIFF_SAME_BUCKET` เดิมปน loose matching; report ล่าสุดแยกเป็น `LIVE_CLOSE_PD_BT_NON_PD`, `BT_CLOSE_PD_LIVE_NON_PD`, `LOOSE_MATCH_PNL_DIFF`, `CLOSE_PRICE_DIFF_SAME_BUCKET`, และ `CLOSE_LIFECYCLE_PD` จริงแล้ว. Step ถัดไปให้ไล่ loose matching/entry drift, live/BT PD asymmetry, `ENTRY_TOO_FAR`, และ `REPLAY_PD_REJECTED`.
...
- Compare enrichment now prefixes live-only `gap_reason` with `REPLAY_PD_PRECREATE_REJECTED:` when the nearest raw replay candidate was cancelled by `PD FIBO PLUS` before pending placement, and exports `nearest_raw_replay_pd_stage=pending_round1`; old reports used the broader `REPLAY_PD_REJECTED:` label. This keeps current-config pre-create PD rejects visible directly in the compare summary instead of being hidden inside nearest raw columns.
...
- Compare summary now uses `REPLAY_PD_PRECREATE_REJECTED:` on S3 live-only rows whose nearest raw replay candidate was cancelled by `PD FIBO PLUS` before pending placement; old reports used `REPLAY_PD_REJECTED:`. Latest old S3 live gap includes `REPLAY_PD_REJECTED:TIME_TOO_FAR+ENTRY_TOO_FAR BUY count=4`, `REPLAY_PD_REJECTED:TIME_TOO_FAR SELL count=1`, and one combined `REPLAY_PD_REJECTED:LIVE_SIGNAL_PD_CLOSED_NO_REPLAY...` row.
...
- Compare report now exports backtest PD diagnostic columns (`bt_pd_h/l`, `bt_pd_fib_382/618`, `bt_pd_fill_h/l`, `bt_pd_round2_*`, `bt_pd_fallback_used`, `bt_pd_outside_range`) for matched and backtest-only rows. Latest S3 all-TF report has PD meta on all 299 backtest rows; the old `CLOSE_LIFECYCLE_PD=60` bucket is now split into `LIVE_CLOSE_PD_BT_NON_PD=31`, `BT_CLOSE_PD_LIVE_NON_PD=19`, and true `CLOSE_LIFECYCLE_PD=10`.
- S3 all-TF largest remaining mismatch groups: `LOOSE_MATCH_PNL_DIFF BUY count=38 pnl=+91.39`, `LIVE_CLOSE_PD_BT_NON_PD BUY count=31 pnl=-88.33`, `BT_CLOSE_PD_LIVE_NON_PD SELL count=19 pnl=+35.61`, `CLOSE_BUCKET_DIFF BUY count=17 pnl=+16.59`, `LIVE_HISTORICAL_TREND_SKIP_DRIFT BUY count=13 pnl=+10.56`, `CLOSE_LIFECYCLE_PD BUY count=10 pnl=-10.20`, `CLOSE_PRICE_DIFF_SAME_BUCKET SELL count=7 pnl=+27.47`, `PNL_DIFF_SAME_BUCKET` now has only 1 residual row.
- S3 near-match diagnostic (`--max-match-quality near`) confirms the old loose matches are mostly signal timing/entry drift, not pure P&L math: matched total drops `146 -> 43`, mismatches `143 -> 41`, live-only `118 -> 221`, backtest-only `153 -> 256`, and `LOOSE_MATCH_PNL_DIFF` disappears by design. Use report `excel_reports/backtest_compare/s3/compare_s3_ALL_20260528_0800_20260608_1000_near.csv` to inspect true signal gaps; keep loose baseline for continuity.
- S3 near report now includes `live_s3_pattern_code`, `bt_s3_pattern_code`, `bt_detect_ts`, `bt_source_candle_ts`, and `bt_marubozu_source` diagnostic columns. Latest near rerun wrote CSV/XLSX successfully and shows `BACKTEST_ONLY=256` with `bt_s3_pattern_code`: `NOENGULF=133`, `G=48`, `R=35`, `MARUBOZU=18`, `R_DOJI=17`, `G_DOJI=5`; `LIVE_ONLY=221` with `live_s3_pattern_code`: `G=113`, `R=77`, `G_DOJI=16`, `R_DOJI=15`.
- S3 near timing diagnostic: for backtest-only rows with source candle metadata, `bt_detect_ts - bt_source_candle_ts` is mostly one bar (`1min=113`, `5min=24`, `15min=8`, `30min=3`, plus single higher-TF cases). This points the next S3 pass toward Marubozu/NoEngulf pending confirmation, same-candle/last-traded dedup, and shared active-state timing before P&L math.
- S3 context-strategy diagnostic: `backtest_auto_trade.py` now supports `--context-strategies` so replay can run extra strategies for shared state while report/compare remains filtered to `--strategies`. Context extras obey restored `active_strategies`; in the latest probe, requested `1,2,4,5,8` became active context `[1,2,4]` and skipped `[5,8]` because they were OFF in restored config.
- Latest S3 near + context run after restore fix (valid bot_state/config restore): live rows `264`, backtest rows `236`, matched total `36` (`MATCH=1`, `MISMATCH=35`), live-only `228`, backtest-only `200`, matched P&L live `+55.49` | BT `+42.63` | diff `+12.86`, match quality `EXACT=2 | NEAR=34 | LOOSE=0`; report `excel_reports/backtest_compare/s3/compare_s3_ALL_20260528_0800_20260608_1000_near_ctx.csv` and `.xlsx`, regenerated successfully on 2026-06-24 in `09:50`.
- S3 nearest-candidate diagnostics added to compare exports: `nearest_bt_*` / `nearest_live_*` now include close time/price, side, TF, pattern, S3 C1 code, Marubozu source, entry, PnL, reason, time diff, and entry diff. Regenerated the near+context report above; counts stayed unchanged (`MATCH=1`, `MISMATCH=35`, `LIVE_ONLY=228`, `BACKTEST_ONLY=200`), confirming this was report-only diagnostics.
- S3 placement-context diagnostics added: replay now exports `bt_s3_prev_sid_time_raw`, `bt_s3_prev_sid_gap_sec`, `bt_s3_prev_sid_adjacent`, `bt_s3_last_traded_time_raw`, `bt_s3_last_traded_matches_source`, `bt_s3_pending_same_sid_tf`, `bt_s3_open_same_sid_tf`, and `bt_s3_active_same_sid_tf` for matched/backtest-only rows. Latest near+context rerun stayed unchanged (`MATCH=1`, `MISMATCH=35`, `LIVE_ONLY=228`, `BACKTEST_ONLY=200`) and shows BACKTEST_ONLY state: active same-S3 TF `True=72`, `False=124`, blank `4`; previous SID adjacent is almost never true (`True=1`), so the remaining overproduction is not mainly a simple adjacent-bar block miss.
- S3 raw-replay diagnostics were expanded for live-only rows: `nearest_raw_replay_sid`, `nearest_raw_replay_side`, `nearest_raw_replay_s3_pattern_code`, `nearest_raw_replay_marubozu_source`, and `nearest_raw_replay_source_candle_ts` now show whether the nearest cancelled/open raw candidate is really S3 or only a context strategy candidate. Valid regenerated near+context report shows live-only nearest raw replay sid distribution: S1=`135`, S2=`49`, S3=`38`, S4=`6`; therefore many live-only rows are nearest to context replay candidates and should not be interpreted as direct S3 setup failures without checking `nearest_raw_replay_sid`.
- S3 live-only rows with true nearest S3 candidate (`nearest_raw_replay_sid=3`) now isolate the next actionable gap: 38 rows total; `nearest_raw_replay_close_type` is `PD_FAIL=28` and `CANCEL=10`; cancel reasons are mostly `PD Fibo Plus round1 fail=28`, then `Sweep Filter scan block=5`, with single rows from SL Guard, Limit Guard, and adjacent same-sid. Pattern/source split: C1 `G=17`, `R=15`, `R_DOJI=4`, `G_DOJI=2`; Marubozu source `noengulf=22`, normal blank `15`, `marubozu=1`. Because `PD_FAIL` is the replay pending/pre-create cancel close type, this is pre-create/pending round1 reject under current config; compare output now tags these as `REPLAY_PD_PRECREATE_REJECTED` with `nearest_raw_replay_pd_stage=pending_round1` before touching adjacent-state logic.
- `backtest_auto_trade.py` now refuses to run by default when `config.restore_runtime_state()` fails, because real-bot parity requires restored `bot_state.json` / config. Diagnostic-only override is `--allow-restore-fail`. Restore blocker found on 2026-06-24 (`name 'config' is not defined` while restoring `S20_7_ENABLED`) was fixed in `config.py` by adding `S20_7_ENABLED` to the restore globals and using `S20_7_ENABLED` as the default instead of `getattr(config, ...)`. Verified restore result: `{'restored': True, 'saved_at': '2026-06-24 17:01:13', 'positions': 0, 'orders': 9, ...}`.
- S10 smoke after restore fix: `python backtest_auto_trade.py --start "2026-06-05 08:00" --end "2026-06-05 10:00" --tf H1 --strategies 10 --exclude-cancelled --symbol XAUUSD.iux --compare-mt5-history --compare-csv excel_reports/backtest_compare/s10/smoke_restore_guard.csv --match-minutes 180 --match-entry-points 5` completed in `04:37`, loaded MT5 history (`deals=3186`, `orders=6599`), live rows `2`, backtest rows `1`, matched `0`, live-only `2`, backtest-only `1`; CSV and summary were written. This proves restore guard no longer blocks real-bot-config backtests, but S10 signal timing still differs in that smoke window (`ENTRY_TOO_FAR`).
- S3 context diagnostic reduced backtest-only rows versus S3-only near (`256 -> 200`), but matched count also dropped (`43 -> 36`) and live-only increased (`221 -> 228`). Treat it as diagnostic evidence that shared active strategy state matters, not as the new baseline.
- S3 Marubozu/NoEngulf metadata parity: `sim_s3_backtest._pending_from_maru()` now stores order pattern with `[C1:G/R/G_DOJI/R_DOJI]` like runtime `open_order()` comments, while the source remains separate in `bt_marubozu_source`. Latest context report shows backtest-only `bt_s3_pattern_code`: `G=86`, `R=80`, `R_DOJI=18`, `G_DOJI=16`; `bt_marubozu_source`: `noengulf=110`, `marubozu=3`, blank normal rows `87`.
- Tested and reverted S3 source-time adjacent-state probe: changing unified replay `last_sid_tf` to store S3 Maru source candle time instead of confirm-bar time made near+context worse (`matched 36 -> 35`, live-only `228 -> 229`, backtest-only `200 -> 205`, replay S3 rows `236 -> 240`). Do not re-apply that change without new evidence.
- S3 all-TF largest live-only groups from the old report: `REPLAY_PD_REJECTED:LIVE_SIGNAL_PD_CLOSED_NO_REPLAY:ENTRY_TOO_FAR BUY count=23 pnl=-49.64`, `LIVE_SIGNAL_PD_CLOSED_NO_REPLAY:ENTRY_TOO_FAR SELL count=21 pnl=-43.92`, `ENTRY_TOO_FAR BUY count=18 pnl=+36.51`, `REPLAY_PD_REJECTED:ENTRY_TOO_FAR SELL count=17 pnl=-7.87`. New report labels the pre-create subset as `REPLAY_PD_PRECREATE_REJECTED`.
...
- Historical/current PD rows are now identified directly in `gap_reason` via `LIVE_SIGNAL_PD_CLOSED_NO_REPLAY`, `REPLAY_PD_PRECREATE_REJECTED` (new), or `REPLAY_PD_REJECTED` (old reports), so remaining non-drift S3 work should focus on PD close lifecycle parity, entry drift, adjacent/sweep/global-state timing, high backtest-only count, and shared Trail/Guard state.
- Near-match diagnostic shows S3 has a large signal-discovery/timing gap hidden by loose matching; next S3 implementation pass should prioritize why live/replay entries are offset by 30-180 minutes or only exist on one side before tuning P&L or trail math.
- Near-match pattern diagnostics now separate C1 pattern from Marubozu/NoEngulf source. The remaining backtest-only cluster is not a C1-code parser issue; it is mostly `bt_marubozu_source=noengulf` with `ENTRY_TOO_FAR` / `NEAREST_ALREADY_MATCHED_OR_GREEDY`, so next pass should compare runtime `check_s3_maru_pending()` guards (`last_traded_per_tf`, adjacent active order, sweep/trend/PD decision order) against replay pending placement.
- Context replay shows active S1/S2/S4 shared state can suppress some S3 overproduction, but exact cross-strategy pending/open state, order-limit state, fill/trail timing, and global lifecycle ordering still need work before S3 is audit-grade.
