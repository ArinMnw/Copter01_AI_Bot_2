# -*- coding: utf-8 -*-
"""Exact cached payoff/BE sweep for S300 market signals."""
...
RR_VALUES = tuple(round(7.0 + 0.5 * index, 1) for index in range(93))
...
def main():
    ...
    base_cfg = {
        "ALLOW_BUY": True,
        "ALLOW_SELL": False,
        # Keep the cache parity check on a simple quoted 10R/BE1 baseline.
        "TP_RR": 10.0,
        "BE_RR": 1.0,
    }
    cached = {}
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s300(window, "M5", dt_bkk, base_cfg)
