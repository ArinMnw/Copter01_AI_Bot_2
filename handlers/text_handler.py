...
async def _handle_custom_input(update, context, text, awaiting):
    ...
    if awaiting == "s20_entry_buffer":
    ...
    elif awaiting == "s17_risk_pct":
        config.S17_RISK_PCT = val
        config.save_runtime_state()
        await update.effective_message.reply_text(
            f"✅ ตั้งค่า **S17 Risk %** เป็น {val}% เรียบร้อย",
            parse_mode="Markdown"
        )
        from handlers.callback_handler import _show_strategy_detail
        ...
        await _show_strategy_detail(MockQueryS17(update.effective_message), 17)

    elif awaiting == "s20_12_risk_pct":
        config.S20_12_RISK_PCT = val
        config.save_runtime_state()
        await update.effective_message.reply_text(
            f"✅ ตั้งค่า **S20.12 Risk %** เป็น {val}% เรียบร้อย",
            parse_mode="Markdown"
        )
        from handlers.callback_handler import _show_strategy_detail
        # Mocking query-like object since it's a message update
    ...
    elif awaiting == "s20_13_24_compound":
        config.S20_13_24_COMPOUND = val
        config.save_runtime_state()
        await update.effective_message.reply_text(
            f"✅ ตั้งค่า **S20.13.24 Compound** เป็น {val} เรียบร้อย",
            parse_mode="Markdown"
        )
        from handlers.callback_handler import _show_strategy_detail
        class MockQuery:
            ...
            async def edit_message_text(self, *args, **kwargs):
                await self.message.reply_text(*args, **kwargs)
        await _show_strategy_detail(MockQuery(update.effective_message), 20.1324)
