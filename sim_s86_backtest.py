"""
sim_s86_backtest.py - Backtest S86 Fibo 50-60 RUN Decision.
...
from strategy86 import S86_DEFAULTS, _detect_closed, _in_session
...
def run_backtest(cfg, days, spread, label, verbose=True):
    if not config.mt5_initialize(mt5):
        ...
        return None
    entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg["ENTRY_TF"], days, extra_bars=700)
    mt5.shutdown()
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s86_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = [
        "timestamp", "label", "entry_tf", "lookback", "impulse_min_atr",
        "zone_low", "zone_high", "zone_tol_atr", "confirm_body_atr",
        "confirm_body_ratio", "require_trend", "trend_lookback",
        "trend_min_atr", "sl_mode", "sl_atr_mult", "tp_mode", "tp_rr",
        "trades", "wr", "avg_per_day_span", "max_dd_pct", "profit_factor",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "impulse_min_atr": cfg["IMPULSE_MIN_ATR"],
            "zone_low": cfg["ZONE_LOW"],
            "zone_high": cfg["ZONE_HIGH"],
            "zone_tol_atr": cfg["ZONE_TOL_ATR"],
...
def main():
    ...
    ap.add_argument("--impulse", type=float)
    ap.add_argument("--ztol", type=float)
    ap.add_argument("--body", type=float)
    ...
    ap.add_argument("--slmode", choices=["swing", "zone"])
    ap.add_argument("--slmult", type=float)
    ap.add_argument("--tpmode", choices=["old", "rr"])
    ...
    if args.impulse is not None:
        cfg["IMPULSE_MIN_ATR"] = args.impulse
    if args.ztol is not None:
        cfg["ZONE_TOL_ATR"] = args.ztol
    if args.body is not None:
        cfg["CONFIRM_BODY_ATR"] = args.body
    if args.ratio is not None:
        cfg["CONFIRM_BODY_RATIO"] = args.ratio
    if args.trend:
        cfg["REQUIRE_TREND"] = args.trend == "on"
