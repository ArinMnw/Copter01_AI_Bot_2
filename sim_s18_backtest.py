"""
sim_s18_backtest.py — Backtest S18 TJR/ICT Full-Confluence จากข้อมูล MT5 จริง
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
เรียก strategy18.detect_s18() ตรง (pure function — logic เดียวกับ runtime)
inject HTF rates ที่ slice ตามเวลาแท่ง → กัน look-ahead bias ของ bias/MSS

กัน look-ahead bias (เหมือน sim_s17):
  - แท่ง "กำลังวิ่ง" (rates[-1]) ใช้ close = open ของแท่งถัดจาก signal
  - entry = LIMIT รอ fill ภายใน S18_LIMIT_CANCEL_BARS แท่ง
  - TP/SL เดินไปข้างหน้าทีละแท่ง; แท่งเดียวกันแตะทั้ง TP+SL → นับ SL (conservative)
  - spread หักจาก P/L ทุกไม้ (default 0.20 USD)

ตัวอย่าง:
  python sim_s18_backtest.py --days 30 --tf M1,M5
  python sim_s18_backtest.py --days 60 --tf M5 --csv
"""
...
def replay_tf(bars, htf_bars, tf_name, spread):
    ...
    for j in range(lookback + 2, n - 1):
        ...
        res = detect_s18(window, tf=tf_name, dt_bkk=dt_bkk, htf_rates=htf_slice)
        sig = res.get("signal")
        if sig not in ("BUY", "SELL"):
            continue

        # dedup level cooldown (logic เดียวกับ strategy_18 wrapper)
...
def main():
    ...
    ap.add_argument("--csv", action="store_true", help="เซฟ trades ลง excel_reports/backtest_compare/s18/")
    args = ap.parse_args()
    ...
    if args.csv and all_trades:
        out_dir = os.path.join("excel_reports", "backtest_compare", "s18")
        os.makedirs(out_dir, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M")
        path = os.path.join(out_dir, f"sim_s18_{stamp}.csv")
