...
ACCOUNT_LABELS = {
    ...
    "demo-iux-2101183586": "🟢 Demo • IUX 3586 — S420",
...
def read_account_status(acc_dir):
    ...
    if not d.get("ts", "").isdigit():
        return {"state": "OFF", "label": "⚪ ไม่ได้รัน", "age": None, "pid": None}
    age = int(time.time()) - int(d["ts"])
    if os.path.exists(marker_path):
        try:
            if os.path.getmtime(marker_path) >= os.path.getmtime(hb_path):
                return {"state": "OFF", "label": "⚪ ปิดแล้ว", "age": age, "pid": d.get("pid")}
        except Exception:
            pass
    if age < 30:
        return {"state": "LIVE", "label": "🟢 ทำงานอยู่", "age": age, "pid": d.get("pid")}
    elif age < 90:
        return {"state": "LAGGING", "label": "🟡 ค้าง/ช้า", "age": age, "pid": d.get("pid")}
    else:
        return {"state": "STALE", "label": "🔴 ไม่ตอบสนอง", "age": age, "pid": d.get("pid")}
...
def load_live_terminal_data(acc_key, acc_status_state, mt5_path):
    if acc_status_state not in ("LIVE", "LAGGING"):
...
def load_fleet_overview(_accounts):
    ...
    for a in _accounts:
        ...
        rows.append({
            ...
            "Auto": "✅ ON" if state.get("auto_active") else "❌ OFF",
...
with st.sidebar:
    ...
    with col_ar2:
        refresh_sec = st.selectbox("Interval", options=[15, 30, 60], index=1, label_visibility="collapsed")
...
if news_active:
    st.markdown(
        ...
        f"border-radius:12px;padding:12px 18px;margin-bottom:12px;display:flex;align-items:center;gap:12px;'>"
    ...
    st.markdown(
        ...
        f"border-radius:12px;padding:10px 18px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;'>"
...
for col, s in zip(sess_cols, sessions):
    with col:
        ...
        if s["is_open"]:
            ...
            """
        st.markdown(
            f"<div style='background:{st_['bg']};border:1px solid {st_['border']};border-radius:12px;padding:8px 12px;text-align:center;'>"
...
with st.spinner("กำลังเชื่อมต่อและโหลดข้อมูล MT5..."):
    ...
    live_acc, live_positions, live_orders = load_live_terminal_data(sel_key, acc_status["state"], sel_mt5_path)
...
with tab_overview:
    ...
    with col4:
        ...
        st.metric("Daily P/L", f"${daily_realized:,.2f}", f"{trades_cnt} trades", delta_color="normal")
...
with tab_live:
    ...
    if not live_acc:
        ...
        la1, la2, la3, la4, la5, la6 = st.columns(6)
...
with tab_fleet:
    ...
    st.caption("สรุปพอร์ตโฟลิโอทั้งหมดจากทุกบัญชี (Real & Demo) โดยอ่านจากไฟล์ heartbeat และ state รวดเร็วและไม่รบกวน Terminal")
    ...
    for r in fleet_sorted:
        ...
        status_badge = f"<span class='badge-buy'>{r['Status']}</span>" if "🟢" in r["Status"] else f"<span class='badge-sell'>{r['Status']}</span>"
        auto_badge = "<span style='color:#34d399;font-weight:700;'>ON</span>" if "ON" in r["Auto"] else "<span style='color:#94a3b8;'>OFF</span>"
        rows_html.append(
            f"<tr>"
    ...
    st.markdown(
        ...
        f"<table class='pro-table'>"
        f"<thead><tr>"
        f"<th>บัญชี (Account & Profile)</th><th>สถานะบอท</th><th>Auto Trade</th><th>Daily P/L</th><th>จำนวนไม้</th>"
        f"</tr></thead>"
...
with tab_strat:
    ...
    strat_subtab1, strat_subtab_playbook, strat_subtab2, strat_subtab3 = st.tabs([
