# -*- coding: utf-8 -*-
"""S224 - Rollover opening-range breakout (anchored range), 10R.

Designed from the campaign's sharpest finding: the rollover edge needs BOTH the
clock AND a structural-break confirmation. Removing either kills it — S212
dropped the clock and lost, S223 dropped the break confirmation (same 04-06
window, plain conviction bar) and lost -35/-24 while S206 made +412/+137.

So this keeps both ingredients and varies only the *form* of the structure.
S206 breaks a rolling 8-bar micro-range, which slides along with price. S224
breaks an **anchored** range: the high/low established by the session's own
first bars. That is the classic opening-range breakout, and it is structurally
distinct — a rolling range can drift with a slow trend and print a "break" that
is really just continuation, whereas an anchored range only breaks when price
leaves the zone the session itself defined.
"""
...
def detect_s224(rates, tf="", dt_bkk=None, cfg=None, **kwargs):
    ...
    return {
        ...
        "tp": tp,
        "order_type": "market",
        "pattern": f"S224 {signal} Rollover ORB {rr:g}R",
        "reason": (f"First close beyond the {or_size:.2f} anchored opening "
                   f"range ({or_bars} bars) of the rollover session"),
        "be_rr": float(c["BE_RR"]),
