...
import sim_s39_backtest as sim

mt5.initialize()
days = 60
cfg0 = dict(S39_DEFAULTS)
entry_bars = s30sim.fetch_bars(config.SYMBOL, cfg0["ENTRY_TF"], days, extra_bars=cfg0["MAX_ZONE_AGE_BARS"] + 100)
...
combos = list(itertools.product(
    [2, 3],                # BASE_BARS
    [1.0, 1.5, 2.0],        # BASE_ATR_MULT
    [0.5, 0.8],             # IMPULSE_ATR_MULT
    [30, 40],               # MAX_ZONE_AGE_BARS
    [0.8, 1.0],             # SL_ATR_MULT
    [1.0, 1.5],              # TP_RR
))
...
t0 = time.time()
results = []
for i, (bn, ba, ia, ma, sl, rr) in enumerate(combos):
    cfg = dict(S39_DEFAULTS)
    cfg.update(BASE_BARS=bn, BASE_ATR_MULT=ba, IMPULSE_ATR_MULT=ia, MAX_ZONE_AGE_BARS=ma, SL_ATR_MULT=sl, TP_RR=rr)
    ...
    results.append((bn, ba, ia, ma, sl, rr, s, c))
print(f"done in {time.time()-t0:.1f}s, {len(results)} valid")
mt5.shutdown()

results.sort(key=lambda r: r[7]["sharpe_like"], reverse=True)
print(f"\n{'bn':>3} {'ba':>4} {'ia':>4} {'ma':>4} {'sl':>4} {'rr':>4} {'n':>4} {'WR%':>5} "
      f"{'$/mo':>8} {'DD%':>6} {'PF':>5} {'posDay%':>8} {'streak':>7} {'sharpe':>7}")
for bn, ba, ia, ma, sl, rr, s, c in results[:15]:
    print(f"{bn:>3} {ba:>4} {ia:>4} {ma:>4} {sl:>4} {rr:>4} {s['trades']:>4} {s['wr']:>5.1f} "
