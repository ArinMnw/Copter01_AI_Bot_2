...
parser = argparse.ArgumentParser(description="Backtest S99 Sweep+Displacement+Retrace")
...
parser.add_argument("--out-prefix", type=str, default="s99")
...
if args.disp_atr is not None:
    cfg_s99["DISP_BODY_ATR"] = args.disp_atr
if args.tp_rr is not None:
    cfg_s99["TP_RR"] = args.tp_rr
if args.retrace is not None:
    cfg_s99["ENTRY_RETRACE"] = args.retrace
if args.sl_buf is not None:
    cfg_s99["SL_BUF_ATR"] = args.sl_buf
if args.no_pd:
    cfg_s99["PD_FILTER_ENABLED"] = False
if args.require_fvg:
    cfg_s99["REQUIRE_FVG"] = True
if args.no_time_filter:
    cfg_s99["TIME_FILTER_ENABLED"] = False
...
for i in range(LOOKBACK, len(all_bars) - 2):
    ...
    sig = strategy99.detect_s99(rates_slice, tf=TF, dt_bkk=dt_bkk, cfg=cfg_s99)
    ...
    # --- Phase 1: wait for limit fill (ราคาต้องทะลุ entry เกิน SPREAD — กติกาโลกจริง) ---
    fill_idx = None
    ...
    if fill_idx is None:
        cancelled += 1
        last_trade_idx = i  # still consume cooldown to mimic live pending occupancy
        continue
