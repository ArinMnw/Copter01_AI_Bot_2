"""
sim_s38_backtest.py — Backtest S38 (Fibonacci Premium/Discount OTE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ RESEARCH / BACKTEST-ONLY — ไม่แก้ S1-S37, ไม่ wire เข้า live
"""
...
def _cfg_get(cfg, key):
    return cfg[key] if (cfg and key in cfg) else S38_DEFAULTS[key]


def replay38(bars, htf_series, spread, cfg):
    conf_type = cfg["CONFIRMATION_TYPE"]
    min_gap_bars = int(_cfg_get(cfg, "MIN_GAP_BARS"))
    lb = int(_cfg_get(cfg, "SWING_LOOKBACK_BARS"))
    win_size = lb + 30
    ...
    for j in range(start_j, n - 1):
        ...
        res = detect_s38(window, tf=cfg["ENTRY_TF"], dt_bkk=dt_bkk, cfg=cfg, htf_ctx=htf_ctx)
        sig = res.get("signal")
...
def run_single(entry_bars, htf_bars, cfg, days, spread):
    ...
    return replay38(entry_bars, htf_series, spread, cfg)
...
def run_backtest(cfg, days, spread, label, verbose=True):
    ...
    lb = int(cfg["SWING_LOOKBACK_BARS"])
...
def append_summary_csv(label, s, cfg):
    path = os.path.join(os.path.dirname(__file__), "s38_backtest_summary.csv")
    is_new = not os.path.exists(path)
    fields = ["timestamp", "label", "entry_tf", "swing_lookback_bars", "min_swing_atr",
              "ote_low", "ote_high", "max_retrace_age_bars", "sl_atr_mult", "tp_rr", "risk_pct",
    ...
    with open(path, "a", newline="", encoding="utf-8") as f:
        ...
        w.writerow({
            ...
            "entry_tf": cfg["ENTRY_TF"], "swing_lookback_bars": cfg["SWING_LOOKBACK_BARS"],
            "min_swing_atr": cfg["MIN_SWING_ATR"], "ote_low": cfg["OTE_LOW"], "ote_high": cfg["OTE_HIGH"],
            "max_retrace_age_bars": cfg["MAX_RETRACE_AGE_BARS"], "sl_atr_mult": cfg["SL_ATR_MULT"],
...
def main():
    ...
    ap.add_argument("--risk", type=float, default=S38_DEFAULTS["RISK_PCT"])
    ap.add_argument("--lookback", type=int, default=None)
    ap.add_argument("--minswing", type=float, default=None)
    ap.add_argument("--slmult", type=float, default=None)
    ap.add_argument("--rr", type=float, default=None)
    ap.add_argument("--label", default="baseline")
    args = ap.parse_args()

    cfg = dict(S38_DEFAULTS)
    cfg["RISK_PCT"] = args.risk
    if args.lookback is not None:
        cfg["SWING_LOOKBACK_BARS"] = args.lookback
    if args.minswing is not None:
        cfg["MIN_SWING_ATR"] = args.minswing
    if args.slmult is not None:
        cfg["SL_ATR_MULT"] = args.slmult
    if args.rr is not None:
        cfg["TP_RR"] = args.rr

    print(f"S38 backtest | days={args.days} | risk={cfg['RISK_PCT']}% | lookback={cfg['SWING_LOOKBACK_BARS']} "
          f"minswing={cfg['MIN_SWING_ATR']} sl={cfg['SL_ATR_MULT']} rr={cfg['TP_RR']}")
