# -*- coding: utf-8 -*-
"""Overlap, risk-distance, and incremental portfolio audit for S373."""
...
def main():
    by_sid = {sid: [] for sid in (*BASE_IDS, 373)}
    h1_trades = {}
    risks = []
    for name, months, end_text in WINDOWS:
        end = parse_bkk(end_text)
        prepared = prepare_rates(months, "M5", end, 300)
        for sid in (*BASE_IDS, 373):
            _, trades = backtest(
                sid,
                months,
                "M5",
    ...
    target_times = {trade["signal_time"] for trade in h1_trades[373]}
    overlap = {
        sid: sum(
