# -*- coding: utf-8 -*-
"""Fast exact payoff and breakeven sweep for S178."""
...
from sim_strategy_backtest import (BKK, backtest, parse_bkk, prepare_rates,
                                   validate_signal)
from strategy178 import detect_s178
from sweep_s161_payoff import _summary


CASES = ((7.0, 1.0), (10.0, 1.0), (10.0, 1.10),
         (10.0, 1.12), (10.0, 1.14), (10.0, 1.16),
         (10.0, 1.18), (10.0, 1.20), (10.2, 1.16))
...
def main():
    ...
    for index in range(start_index, len(bars) - 1):
        ...
        signal = detect_s178(window, "M5", dt_bkk, base_cfg)
        validate_signal(signal, 178)
        cached[index] = signal
    rows = [_summary(rr, be, bars, start_index, cached) for rr, be in CASES]

    if not args.skip_guard:
        official, _ = backtest(178, args.months, "M5", 0.20, 0.01, end, lookback,
